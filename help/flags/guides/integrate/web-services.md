---
title: Web サービス
description: JavaまたはNode.js SDKを使用して、Flagsをバックエンド web サービスに統合する方法を説明します。
hide: true
exl-id: 2e73fc30-93d4-46dc-8855-950dd1b15064
source-git-commit: fea4d9e87ad8417de9d820ee3556796fba112dc1
workflow-type: tm+mt
source-wordcount: '79'
ht-degree: 10%

---

# Web サービス {#web-services}

バックエンドサービスは、サーバーサイドのSDKを使用してFlagsと統合されます。 テクノロジースタックに合わせて、適切なSDKを選択しましょう。

## 利用可能なSDK {#available-sdks}

>[!NOTE]
>
>サーバーサイド SDKのドキュメントは現在準備中であり、近日中に公開される予定です。 早期アクセスガイダンスについては、Adobe担当者にお問い合わせください。

## その他の言語 {#other-languages}

特定のスタックとの統合に関するガイダンスについては、フラグのサポートにお問い合わせください。

## 詳しくは、 {#see-also}

* [SDK](sdks.md)
* [統合ステップ](integration-steps.md)
* [スタートアップガイド](startup-guide.md)

<!-- -->
