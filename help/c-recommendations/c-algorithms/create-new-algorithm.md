...
* **Analytics**：[!DNL Adobe Analytics] を行動データソースとして使用する条件を作成した場合は、選択されているレポートスイートとルックバックウィンドウが他の条件でも使用されているかどうかに応じて、条件が使用可能になる時間が異なります。

   * **初期遅延**:最初の遅延は、2~7日です。この初期遅延は1回のみ発生し、以前に使用されていないレポートスイートで条件が設定された場合、またはより長いルックバックウィンドウで使用されます。
   * **遅延待ち時間**:レポートスイートが前にルックバックウィンドウで使用されていて、選択したルックバックウィンドウが少なくとも長すぎる場合、使用される行動データの量とカタログのサイズに応じて、新しい条件と既存の基準の待ち時間が12時間未満になります。
   例えば、「表示された親和性」のレコメンデーションの場合、ユーザーが製品を表示すると、製品ビュートラッキングの呼び出しがAnalyticsにリアルタイムで送信されます。Analyticsデータは、翌日の早い段階でTargetにプッシュされ、Targetは12時間未満のアルゴリズムになります。
