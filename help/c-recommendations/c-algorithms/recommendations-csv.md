...
カスタム条件の更新は、デフォルトでは「累積的」です。既存のキー値ペアが、CSV アップロードファイルで指定した新しいキー値ペアで上書きされます。CSVアップロードで指定されたキーを持たない既存のキーと値のペアは引き続き配信可能で、CSVファイルの一部として最後にアップロードされた時点から31日で有効期限が切れます。
