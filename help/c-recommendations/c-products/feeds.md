...
| Feed Imported Successfully at *time* | Targetはフィードファイルをコンテンツ配信システムにインポートしました。項目属性の変更は、コンテンツ配信システムで行われたものであり、配信された推奨に近日中に反映されます。期待される変更が表示されない場合は、しばらく待ってから、レコメンデーションを含むページを更新してください。<br>*注1:* 品目の属性に対する変更がレコメンデーションから除外される品目の場合、除外はすぐに反映されます。If an item is newly added, or changes to attributes result in an item being *no longer* excluded from recommendations, it will not be reflected until the next algorithm update, which will occur within 24 hours.<br>*注2:* このステータスを表示すると、カタログ検索ユーザーインターフェイスに更新がまだ反映されていない可能性があります。カタログ検索には、検索可能なカタログが最後に更新された日時を示す別のステータスが表示されます。 |
