...
多くの小売業者が商品を Google にアップロードしているので、訪問者が Google による製品検索を使用すると、商品が表示されます。[!DNL Recommendations] では、Google の仕様に厳密に従ったエンティティフィードを使用します。エンティティフィードは [!DNL Recommendations] 、送信 [!DNL .xml]、 [!DNL .txt]または [!DNL .tsv]、Googleで定義されている [属性を使用して送信](https://support.google.com/merchants/answer/188494?hl=en&topic=2473824&ctx=topic#US)できます。結果は [Google のショッピングページ](https://www.google.com/prdhp)で検索できます。
...
## フィードの作成 {#task_C6CD9EA905744C2CA0BB8259BB74C867}
...
1. 「**[!UICONTROL 次へ]**」矢印をクリックして、「[!UICONTROL スケジュール]」オプションを表示します。
   ...
   ![ステップ結果](assets/CreateFeedSchedule.png)
...
1. 「**[!UICONTROL 次へ]**」矢印をクリックして「[!UICONTROL マッピング]」オプションを表示してから、データを [!DNL Target] 定義にマッピングする方法を指定します。
   ...
   ![ステップ結果](assets/CreatFeedMapping.png)
...
### フィードステータス {#section_5DDC2DECF70A42FDAFF2235E91371537}
...
| 同期に失敗しました | フィード設定の詳細を、Targetに保存できませんでした。再試行してください。 |
| フィードが実行されていません | フィードを作成しましたが、スケジュールされていません（頻度がなしに設定されている）。 |
| *スケジュールされた日時* | フィードは実行されていませんが、特定の日時に実行するようスケジュールされています。 |
| ダウンロードを待機中 | Targetはフィードファイルをダウンロードする準備中です。 |
| フィードファイルのダウンロード | Targetはフィードファイルをダウンロード中です。 |
| 項目の読み込み | Targetは、フィードファイルから項目をインポートしています。注意:この手順が完了し、「検索インデックスの更新準備」が表示されると、項目属性への変更が中央システムにインポートされ、地理的エッジノードから返された配信されたレコメンデーションコンテンツに60分以内に反映されます。 |
| 検索インデックスの更新の準備 | Targetは、カタログ検索のインデックスを更新する準備をしています。注意:このステータスが表示されている場合、項目属性への変更は既に行われており、配信されたレコメンデーションに反映されますが、まだカタログ検索に反映されていません。 |
| 検索インデックスの更新 | ターゲットはカタログ検索のインデックスを更新中です。注意:このステータスが表示されている場合、項目属性への変更は既に実行されており、配信されたレコメンデーションには近日中に反映されますが、まだカタログ検索に反映されていない可能性があります。 |
| 完了したアップデート | Targetは、フィードファイルに関連付けられているすべてのアップデートを完了しました。 |
| インデックスに失敗しました | インデックス操作に失敗しました。再試行してください。 |
...
>[!IMPORTANT]
>
>アップロードされたエンティティは61日後に期限切れになります。つまり、レコメンデーションアクティビティを中断しないように、フィードファイルを60日ごとに少なくとも60日アップロードする必要があります。フィードファイル（またはその他のエンティティ更新方法）に少なくとも60日ごとに品目が含まれていない場合、Adobe Targetはその項目を関連付けなくなり、カタログから削除します。
