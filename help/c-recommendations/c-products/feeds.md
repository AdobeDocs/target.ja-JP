...
| 項目の読み込み | Target は、フィードファイルから項目をインポートしています。注：この手順が完了し、「検索インデックスの更新準備」が表示されると、項目属性への変更が中央システムにインポートされ、地理的エッジノードから返された配信されたレコメンデーションコンテンツに 60 分以内に反映されます。 |
| 検索インデックスの更新の準備 | Target は、カタログ検索のインデックスを更新する準備をしています。注：このステータスが表示されている場合、項目属性への変更は既に行われており、配信されたレコメンデーションに反映されますが、まだカタログ検索に反映されていません。 |
| 検索インデックスの更新 | ターゲットはカタログ検索のインデックスを更新中です。注：このステータスが表示されている場合、項目属性への変更は既に実行されており、配信されたレコメンデーションには近日中に反映されますが、まだカタログ検索に反映されていない可能性があります。 |
| 完了したアップデート | Target は、フィードファイルに関連付けられているすべてのアップデートを完了しました。 |
