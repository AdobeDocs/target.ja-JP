...
## カタログ内の項目の更新について、サイトに反映されるまでの時間はどのくらいか。

フィードファイルを読み込んだ後、またはAPIまたはmbox経由でエンティティの更新を受け取った後、次の変更が60分以内に反映されます。

* デザインテンプレートで返されるアイテム属性。
* グローバル除外ルールで使用される品目属性。返されたレコメンデーションに品目が含まれないようにします。
* 品目属性は、返されたレコメンデーションに品目が含まれているか除外されているかに影響する条件内のインクルージョンルールで使用されます。

次のアルゴリズムが実行されるまでは、次の変更が反映されません（12~24時間以内）。

* アクティビティに使用されるコレクションルールで使用される品目属性。
* アクティビティに関連付けられた属性またはコレクションに基づくプロモーションで使用される品目属性。
* トップセラーまたは最も頻繁に閲覧されたアルゴリズムの「現在のカテゴリ」または「お気に入りのカテゴリ」の項目が表示される品目カテゴリ。
* 属性が変更されたときに推奨品目のランクが、アルゴリズムのカスタムキーとして使用されるカスタム属性になります。
* レコメンデーションロジックが「類似の属性を持つ品目」である場合、または「コンテンツの類似性」の重み付け要因が使用されている場合、または「属性の重み付け」の要素が使用されている場合の、変更された属性に基づくレコメンデーション品目のランク付け。

>[!NOTE]
>
>フィードファイルは、ステータスが「項目の読み込み」から「検索インデックスの更新の準備」に変わったときにインポートされます。カタログ検索ユーザインターフェイスに反映されるまでに60分以上かかる場合があります。フィードステータスが「完了したアップデート」に変更された場合、カタログ検索は最新です。カタログ検索がまだ最新ではない場合でも、サイトは上記の時間枠の更新を反映します。カタログ検索ページに、最新のカタログ検索インデックスの更新時間が表示されます。
...
クエリ文字列では、レコメンデーションから除外するエンティティのエンティティIDを渡すことができます。例えば、既に買い物かごにある品目は除外した方がいいでしょう。

除外機能を有効にするには、`excludedIds` mbox パラメーターを使用します。このパラメーターは、コンマ区切りのエンティティIDのリストを示します。例： `mboxCreate(..., "excludedIds=1,2,3,4,5")`値は、レコメンデーションのリクエスト時に送信されます。

>[!NOTE]
>
>除外されているエンティティが多すぎる場合、レコメンデーションテンプレートを満たすのに十分なエンティティがないかのように動作します。

除外 `entityIds`するには、オファーコンテンツURLに `&excludes=${mbox.excludedIds}` トークンを追加します。コンテンツ URL が除外されると、必要なパラメーターが、現在の mbox リクエストパラメーターを使用して代用されます。

デフォルトでは、この機能は新しく作成されたレコメンデーションで有効になります。既存のレコメンデーションを保存して、動的に除外されるエンティティをサポートする必要があります。
