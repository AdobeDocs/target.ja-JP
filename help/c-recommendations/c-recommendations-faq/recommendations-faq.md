...
description: Adobe Target Recommendationsアクティビティに関するよくある質問(FAQ)のリストです。
...
リクエストされたアルゴリズムとキーの組み合わせでレコメンデーションが使用できない場合、NO_CONTENTが返されます。 一般的に、これは、アルゴリズムのバックアップが無効になっていて、次の1つ以上が真の場合に発生します。

* 結果はまだ準備ができていません。

   これは通常、新しく作成されたアクティビティを最初に保存する場合、またはアクティビティで使用されるコレクション、条件またはプロモーションに設定の変更が行われた後に発生します。

* 要求されたアルゴリズム/キーの組み合わせに対して、結果は準備はできていますが、最も近いエッジサーバーではキャッシュされていません。

   今行われた要求はキャッシュ操作を開始するので、数ページのリロードや数分経過後に解決されます。

* 結果は準備ができていますが、指定されたキー値に対しては使用できません。

   これは、通常、最新のアルゴリズムの実行後にカタログに追加された品目に対してレコメンデーションをリクエストし、次のアルゴリズムの実行後に解決される場合に発生します。

* テンプレートの部分レンダリングが無効になっているので、テンプレートを埋めるのに十分な結果が得られません。

   これは通常、可能な結果から多くの項目を積極的にフィルターする動的な包含ルールがある場合に発生します。 これを防ぐには、バックアップを有効にし、バックアップにインクルージョンルールを適用しないでください。または、あまり積極的にフィルターされない条件で条件を順に使用してください。
