...
| ターゲットUIまたはAPIを介してカタログから削除された品目 | <ul><li>Recommendationsは15分以内に更新されます。</li><li>既存のレコメンデーションと品目の属性は、更新が利用可能になるまで表示されます。</li><li>カタログ検索は、カタログインデックス（3 ～ 8時間）の後に更新されます。</li></ul> |
...
* 現在閲覧、購入されている製品/コンテンツは、ユーザーが受け取るレコメンデーションに影響を与え、同じページビュー/ターゲットコンテンツリクエストに影響を与えます。
* 「最後に閲覧された商品」、「最も多く閲覧された商品」、全体的な視聴/購入履歴などの過去のユーザー行動がそのリクエストで更新され、次のページビュー/ターゲットコンテンツリクエストでユーザーが受け取るレコメンデーションに影響します。 例えば、「最近表示した品目」や「お勧めの品目」アルゴリズムは、商品の表示や購入ごとに更新され、以降のコンテンツリクエストに反映されます。
