...
除外は、現在のターゲット呼び出しに対してのみ実行されます。以降のターゲット呼び出しでは、 `excludedIds` 値が再度渡されない限り、項目は除外されません。 すべてのページのレコメンデーションから買い物かご内の品目を除外するには、引き続き各ページで `excludedIds` 値を渡します。
...
リクエストされたアルゴリズムとキーの組み合わせでレコメンデーションが使用できない場合、NO_CONTENTが返されます。 一般的に、これは、アルゴリズムでバックアップが無効になっていて、次の1つ以上が真の場合に発生します。

* 結果はまだ準備ができていません。

   これは、通常、新しく作成したアクティビティを最初に保存するとき、またはアクティビティで使用されているコレクション、条件、プロモーションに設定が変更された後に発生します。

* 要求されたアルゴリズム/キーの組み合わせの結果は準備完了ですが、最も近いエッジサーバーではまだキャッシュされていません。

   今行われた要求はキャッシュ操作を開始するので、数ページリロードや数分経過した後に自動的に解決されます。

* 結果の準備はできましたが、指定されたキー値に対しては使用できません。

   これは、通常、最新のアルゴリズムの実行後にカタログに追加された品目のレコメンデーションをリクエストし、次のアルゴリズムの実行後に解決される場合に発生します。
