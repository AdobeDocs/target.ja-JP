...
「マーケティングボックス」、コンテンツが表示され、訪問者の行動が記録されるWebページ上の場所。中には、何も表示しない mbox もあります。そのような mbox は、Web サイト上で訪問者が実行する操作の進行状況の追跡や、訪問者が特定の成功指標を達成したかどうか（買い物かごにアイテムを追加したか、購入を完了したかなど）の追跡に使用されます。このような種類の mbox の 1 つとして「*コンバージョン mbox*」があります。
