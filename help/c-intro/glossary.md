...
アクティビティの成功の測定に使用されるパラメーター。成功指標には、Targetアクティビティで特定のエクスペリエンスまたはオファーの成功を判定するための主要ビジネス計測値が含まれます。例えば、新しいオファーが訪問者あたりの売上高を増加するか、買い物かごに品目を追加するかを判断できます。成功指標は、登録、注文、購入の各ファネルだけでなく、訪問者や顧客の関与に関する問題を発見するのに役立ちます。
...
Target Standardでは、サイトコンテキストでパーソナライズされたエクスペリエンスとオファーを簡単に作成およびテストできるWYSIWYGユーザーインターフェイスです。Webページ（オファー）またはモバイルWebページのレイアウトとコンテンツをドラッグ&amp;ドロップ、スワップおよび変更して、Targetアクティビティのエクスペリエンスおよびオファーを作成できます。
