...
Adobe Targetは、2つのJavaScriptライブラリのいずれかによってWebサイトと統合されます。at. jsまたはmbox. js
...
Target では、ページ上の各要素は、ページ全体の単一のエクスペリエンスの一部になります。各エクスペリエンスには、ページに複数の要素を含めることができます。
...
自動ターゲットは、高度な機械学習を使用して、複数のマーケティング担当者に定義されたエクスペリエンスから選択し、個々の顧客プロファイルおよび類似のプロファイルを持つ以前の訪問者の行動を、コンテンツをパーソナライズしてコンバージョンを推進するための類似のプロファイルに基づいて提供します。
...
応答時間を短縮するために、Edge環境では、アクティビティのロジックとキャッシュされたプロファイルおよびオファー情報のみをホストします。アクティビティおよびコンテンツのデータベース、[!DNL Analytics] データ、API、およびマーケティング担当者向けのユーザーインターフェイスは、アドビの中央データ環境に格納されます。その後、最新情報が Edge ノードに送信されます。中央環境とEdgeノードは自動的に同期され、キャッシュされたアクティビティデータが継続的に更新されます。また、1:1モデリングは各エッジに保存されるので、より複雑なリクエストをEdge上で処理することもできます。
...
ユーザーのブラウザーが定義されたタイムアウト期間内に接続できない場合、そのページに変更は加えられません。15秒）。このタイムアウトのしきい値に達した場合、デフォルトの位置のコンテンツが表示されます。
...
Googleでは、「これらのガイドラインに従うと、テスト結果がサイトの検索結果にほとんどまたはまったく影響しないことがあります」。
...
* Adobe Audience Manager（AAM）セグメントを検索します（該当する場合）
* Recommendations、自動ターゲット、自動パーソナライゼーション、自動配分アクティビティのために、モデリングおよびパーソナライズされたコンテンツをモデリングで使用する
* レポート用のアクティビティ訪問のログ
