...
&zwsp;1. 訪問者が、サーバー上のページをリクエストしてブラウザーに表示します。
&zwsp;2. ファーストパーティ Cookie が、訪問者行動を保存するために訪問者のブラウザー内に設定されます。
&zwsp;3. ページから、ターゲット設定システムが呼び出されます。
&zwsp;4. キャンペーンのルールに基づいてコンテンツが表示されます。
