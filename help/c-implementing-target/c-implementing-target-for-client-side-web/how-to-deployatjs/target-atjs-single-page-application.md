---
keywords: シングルページアプリケーションの実装；シングルページアプリケーションの実装；spa;at.js 2.x;at.js；シングルページアプリケーション；シングルページアプリ；spa;SPA
description: シングルページアプリケーション(SPA)でAdobe [!DNL Target] at.js 2.x to implement [!DNL Target] を使用する方法を説明します。
title: シングルページアプリケーション(SPA)に [!DNL Target] を実装できますか。
feature: サーバー側の実装
role: Developer
...
1. at.js 2.x をインストールします。
   ...
   [!UICONTROL 管理/実装]にあるAdobe Target UIを使用して、at.js 2.xをダウンロードします。 at.js 2.xは、[!DNL Adobe Experience Platform]のタグを使用してもデプロイできます。 ただし、[!DNL Adobe Target]拡張機能は現在最新ではなく、サポートされていません。
...
以下の例ではJavaScriptコードを使用していますが、[Adobe Experience Platform](/help/c-implementing-target/c-implementing-target-for-client-side-web/how-to-deployatjs/cmp-implementing-target-using-adobe-launch.md)のタグなど、タグマネージャーを使用する場合は、これらすべてを簡略化できます。

上記の手順に従う場合は、SPA 用の堅牢な A4T ソリューションが必要です。

## 実装のベストプラクティス {#bp}

at.js 2.x APIを使用すると、様々な方法で[!DNL Target]実装をカスタマイズできますが、このプロセスの間、正しい操作順に従うことが重要です。

次の情報は、ブラウザーで初めてシングルページアプリケーションを読み込む際に従う必要がある操作の順序と、その後に発生するビューの変更について説明します。

### 最初のページ読み込みの操作の順序

| 手順 | アクション | 詳細 |
| --- | --- | --- |
| 3 | VisitorAPI JSの読み込み | このライブラリは、訪問者にECIDを割り当てます。 このIDは、後でWebページ上の他の[!DNL Adobe]ソリューションで使用されます。 |
| 2 | at.js 2.xの読み込み | at.js 2.xは、[!DNL Target]リクエストと表示の実装に使用する必要なAPIをすべて読み込みます。 |
| 3 | [!DNL Target]リクエストの実行 | データレイヤーがある場合、[!DNL Target]リクエストを実行する前に[!DNL Target]に送信する必要がある重要なデータを読み込むことをお勧めします。 これにより、`targetPageParams`を使用して、ターゲティングに使用する任意のデータを送信できます。 このAPI呼び出しで、 execute > pageLoadおよびprefetch >ビューを必ずリクエストする必要があります。 `pageLoadEnabled`と`viewsEnabled`を設定した場合は、手順2で、execute > pageLoadとprefetch >ビューの両方が自動的に実行されます。それ以外の場合は、`getOffers()` APIを使用してこのリクエストをおこなう必要があります。 |
| 4 | 呼び出し `triggerView()` | 手順3で開始した[!DNL Target]リクエストは、ページ読み込みの実行とビューの両方のエクスペリエンスを返す可能性があるので、[!DNL Target]リクエストが返された後に`triggerView()`を呼び出し、キャッシュへのオファーの適用を完了します。 この手順は、ビューごとに1回だけ実行する必要があります。 |
| 5 | [!DNL Analytics]ページビュービーコンを呼び出します | このビーコンは、データステッチの手順3と4に関連付けられたSDIDを[!DNL Analytics]に送信します。 |
| 6 | 追加の`triggerView({"page": false})`を呼び出す | これは、ビューの変更をおこなわずに、ページ上の特定のコンポーネントを再レンダリングする可能性があるSPAフレームワークのオプションの手順です。 このような場合は、このAPIを呼び出して、SPAフレームワークがコンポーネントを再レンダリングした後で[!DNL Target]エクスペリエンスが再適用されるようにすることが重要です。 この手順は、SPAビュー内で[!DNL Target]エクスペリエンスが確実に保持されるように、何回でも実行できます。 |

### SPAビューの変更に対する操作の順序（完全なページの再読み込みなし）

| 手順 | アクション | 詳細 |
| --- | --- | --- |
| 3 | 呼び出し `visitor.resetState()` | このAPIは、読み込み時に新しいビュー用にSDIDを再生成します。 |
| 2 | `getOffers()` APIを呼び出してキャッシュを更新する | これは、このビューの変更により、現在の訪問者を他の[!DNL Target]アクティビティの対象として認定したり、アクティビティの対象から除外したりする可能性がある場合におこなうオプションの手順です。 この時点で、追加のデータを[!DNL Target]に送信して、さらにターゲティング機能を有効にすることもできます。 |
| 3 | 呼び出し `triggerView()` | 手順2を実行した場合は、この手順を実行する前に[!DNL Target]リクエストを待機し、オファーをキャッシュに適用する必要があります。 この手順は、ビューごとに1回だけ実行する必要があります。 |
| 4 | 呼び出し `triggerView()` | 手順2を実行していない場合は、手順1を完了したらすぐにこの手順を実行できます。 手順2と手順3を実行した場合は、この手順をスキップする必要があります。 この手順は、ビューごとに1回だけ実行する必要があります。 |
| 5 | [!DNL Analytics]ページビュービーコンを呼び出します | このビーコンは、データを結び付けるために、手順2、3、4に関連付けられたSDIDを[!DNL Analytics]に送信します。 |
| 6 | 追加の`triggerView({"page": false})`を呼び出す | これは、ビューの変更をおこなわずに、ページ上の特定のコンポーネントを再レンダリングする可能性があるSPAフレームワークのオプションの手順です。 このような場合は、このAPIを呼び出して、SPAフレームワークがコンポーネントを再レンダリングした後で[!DNL Target]エクスペリエンスが再適用されるようにすることが重要です。 この手順は、SPAビュー内で[!DNL Target]エクスペリエンスが確実に保持されるように、何回でも実行できます。 |
...
### at.js 2.x の仕組みについて![概要バッジ](/help/assets/overview.png)

>[!VIDEO](https://video.tv.adobe.com/v/26250)

詳しくは、[at.js 2.xの仕組みの理解](https://helpx.adobe.com/target/kt/using/atjs20-diagram-technical-video-understand.html)を参照してください。

### SPAの![チュートリアルバッジ](/help/assets/tutorial.png)にat.js 2.xを実装します。

>[!VIDEO](https://video.tv.adobe.com/v/26248)

詳しくは、「シングルページアプリケーション(SPA)](https://helpx.adobe.com/target/kt/using/atjs2-single-page-application-technical-video-implement.html)でのat.js 2.xを使用したAdobe Targetの実装」を参照してください。[

### Adobe TargetでのSPA向けVECの使用![チュートリアルバッジ](/help/assets/tutorial.png)
