...
1. at.js 2.x をインストールします。
   ...
   Download at.js 2.x via the Adobe Target UI located in [!UICONTROL Administration > Implementation]. at.js 2.x は、Adobe Launch を使用してもデプロイできます。ただし、Adobe Target Extensions は現在更新されてなく、サポートされていません。
...
at.js 2.x APIを使用すると、様々な方法で [!DNL Target] 実装をカスタマイズできますが、このプロセスでは、正しい操作順序に従うことが重要です。

次の情報では、単一ページアプリの読み込み時に実行する必要がある操作の順序と、その後発生する表示の変更について説明します。

### 初期ページ読み込みの操作の順序

| 手順 | アクション | 詳細 |
| --- | --- | --- |
| 1 | VisitorAPI JSを読み込む | このライブラリは、訪問者にECIDを割り当てます。 このIDは、後でWebページの他の [!DNL Adobe] ソリューションで使用されます。 |
| 2 | at.js 2.xを読み込む | at.js 2.xは、リクエストと表示を実装するために使用する必要なすべてのAPIを読み込み [!DNL Target] ます。 |
| 3 | 要求の実行 [!DNL Target] | データレイヤーがある場合は、リクエストを実行する [!DNL Target] 前に送信する必要がある重要なデータを読み込むことをお勧めし [!DNL Target] ます。 これにより、を使用して、ターゲティング `targetPageParams` に使用する任意のデータを送信できます。 このAPI呼び出しでは、必ずexecute > pageLoadおよびprefetch >表示をリクエストする必要があります。 とを設定した場合 `pageLoadEnabled``viewsEnabled`、実行/pageLoadとプリフェッチの両方を選択すると、手順2で表示が自動的に発生します。 それ以外の場合は、 `getOffers()` APIを使用してこのリクエストを行う必要があります。 |
| 4 | 呼び出し `triggerView()` | 手順3で開始した [!DNL Target]`triggerView()`[!DNL Target] リクエストは、ページ読み込みの実行と表示の両方のエクスペリエンスを返す可能性があるので、リクエストが返され、オファーのキャッシュへの適用が完了した後に必ずが呼び出されるようにします。 この手順は、表示ごとに1回だけ実行する必要があります。 |
| 5 | ページ [!DNL Analytics] 表示ビーコンを呼び出す | このビーコンは、データのステッチのために、手順3と4に関連付けられたSDID [!DNL Analytics] をに送信します。 |
| 6 | 追加の `triggerView({"page": false})` | これは、表示の変更を行わずにページ上の特定のコンポーネントを再レンダリングする可能性がある、SPAフレームワークのオプションの手順です。 そのような場合、このAPIを呼び出して、SPAフレームワークがコンポーネントを再レンダリングした後でエクスペリエンスが再適用されるようにすることが重要です。 [!DNL Target] この手順は、SPA表示内でエクスペリエンスが維持されるように、必要な回数だけ実行でき [!DNL Target] ます。 |

### SPA表示の変更に対する操作の順序（フルページのリロードなし）

| 手順 | アクション | 詳細 |
| --- | --- | --- |
| 1 | 呼び出し `visitor.resetState()` | このAPIは、読み込み時に新しい表示用にSDIDが再生成されるようにします。 |
| 2 | APIを呼び出してキャッシュを更新する `getOffers()` | これは、この表示の変更により、現在の訪問者をより多くのアクティビティに絞り込んだり、アクティビティから [!DNL Target] の資格を取り下げたりする可能性がある場合に、必要に応じて行う手順です。 この時点で、追加のデータをに送信して、さらなるターゲティング機能を有効にす [!DNL Target] ることもできます。 |
| 3 | 呼び出し `triggerView()` | 手順2を実行した場合は、 [!DNL Target] 要求を待ってオファーをキャッシュに適用してから、この手順を実行する必要があります。 この手順は、表示ごとに1回だけ実行する必要があります。 |
| 4 | 呼び出し `triggerView()` | 手順2を実行していない場合は、手順1を完了するとすぐにこの手順を実行できます。 手順2と手順3を実行した場合は、この手順をスキップする必要があります。 この手順は、表示ごとに1回だけ実行する必要があります。 |
| 5 | ページ [!DNL Analytics] 表示ビーコンを呼び出す | このビーコンは、データのステッチのために、ステップ2、3、4に関連付けられたSDID [!DNL Analytics] をに送信します。 |
| 6 | 追加の `triggerView({"page": false})` | これは、表示の変更を行わずにページ上の特定のコンポーネントを再レンダリングする可能性がある、SPAフレームワークのオプションの手順です。 そのような場合、このAPIを呼び出して、SPAフレームワークがコンポーネントを再レンダリングした後でエクスペリエンスが再適用されるようにすることが重要です。 [!DNL Target] この手順は、SPA表示内でエクスペリエンスが維持されるように、必要な回数だけ実行でき [!DNL Target] ます。 |
...
### at.js 2.xをSPA ![チュートリアルバッジに実装する](/help/assets/tutorial.png)

>[!VIDEO](https://video.tv.adobe.com/v/26248)

詳しくは、 [Adobe Targetのat.js 2.xのシングルページアプリケーション(SPA)での実装を参照してください](https://helpx.adobe.com/target/kt/using/atjs2-single-page-application-technical-video-implement.html) 。
