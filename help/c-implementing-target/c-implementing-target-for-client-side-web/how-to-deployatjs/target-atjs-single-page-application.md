...
| 6 | 追加の `triggerView({"page": false})` | これは、表示の変更を行わずに、ページ上の特定のコンポーネントを再レンダリングする可能性があるSPAフレームワークのオプションの手順です。 そのような場合、SPAフレームワークがコンポーネントを再レンダリングした後でエクスペリエンスが再適用されるように、このAPIを呼び出すことが重要 [!DNL Target] です。 この手順は、SPA表示内でエクスペリエンスが維持されるように必要な回数だけ実行でき [!DNL Target] ます。 |

### SPA表示の変更に伴う操作の順序（ページ全体のリロードなし）
...
詳しくは、「Adobe Targetのat.js 2.xをシングルページアプリ(SPA)に [実装する」を参照してください](https://helpx.adobe.com/target/kt/using/atjs2-single-page-application-technical-video-implement.html) 。
