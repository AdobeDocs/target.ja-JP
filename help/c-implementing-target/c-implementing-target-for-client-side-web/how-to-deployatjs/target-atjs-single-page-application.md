...
| 4 | 呼び出し `triggerView()` | 手順3で開 [!DNL Target] 始したリクエストは、ページ読み込みの実行と表示の両方のエクスペリエンスを返す可能性があるので、リクエストが返され、キャッシュ内のオファーの適用が完了した後に必ず `triggerView()`[!DNL Target] が呼び出されるようにします。 この手順は、1回の実行で1回だけ実行する必要があります。表示 |
...
| 2 | APIを呼び出してキャッシュを更新 `getOffer()` する | この表示の変更により、現在の訪問者をより多くのアクティビティに絞り込んだり、アクティビティから絞り込みを取り下げたりする可能性がある場合は、 [!DNL Target] この手順を選択する必要があります。 この時点で、追加のデータをに送信して、さらにターゲティング機能を有効にす [!DNL Target] ることも選択できます。 |
