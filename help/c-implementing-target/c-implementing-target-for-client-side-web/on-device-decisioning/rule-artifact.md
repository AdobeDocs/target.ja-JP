---
keywords: 実装；javascriptライブラリ；js;atjs;on-device decisioning;on device decisioning；ルールアーティファクト
description: On-Device Decisioningで使用されるJSONルールのアーティファクトについて説明します。
title: On-Device DecisioningのJSONルールアーティファクトには何がありますか？
feature: at.js
role: Developer
...
# オンデバイス判定ルールのアーティファクト

ルールのアーティファクトは、[!DNL Adobe Target]オンデバイス判定アクティビティのJSON表現です。 ルールアーティファクトは[!DNL Target]によって生成され、Akamai CDNに伝播され、訪問者にできる限り近い場所で使用できるルールアーティファクトが存在することを確認します。 ルールアーティファクトには、イベントの正確な実行と配信を保証するメタデータが含まれています。また、アクティビティの追跡を介したリアルタイム分析も可能です。

以下は、JSONルールのアーティファクトの例です。
