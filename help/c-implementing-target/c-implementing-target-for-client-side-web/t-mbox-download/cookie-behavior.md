...
seo-description: Cookie の動作は、その Cookie がファーストパーティ Cookie であるか、ファーストパーティ Cookie を伴うサードパーティ Cookie であるか、サードパーティ Cookie のみであるかによって異なります。
seo-title: mbox.js の Cookie
solution: 'Target '
subtopic: 導入
...
[Target の Cookie の削除](https://marketing.adobe.com/resources/help/en_US/target/target/t_cookie_deleting.html)も参照してください。
...
1. 1 つのドメインの場合。
   ...
   すべてのテストは、1 つのトップレベルドメイン（[!DNL `www.domain.com`]、[!DNL store.domain.com]、[!DNL anysub.domain.com] など）で実装します。
...
1. サードパーティサイトで adbox または flashbox のみを使用している場合。
   ...
   * サードパーティ Cookie のみを有効にする。
      ...
      この方法は、adbox の実装がオンサイトのターゲット化なしで使用されている場合にのみ適していますが、これはまれな場合です。
...
## ファーストパーティ Cookie を伴うサードパーティ Cookie の動作 {#section_F0C9AD8BFDF8457A999C4A07A0F7A981}
