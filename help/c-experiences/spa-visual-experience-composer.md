...
アクションをクリックすると、このアクションが適用されるサイトの要素がハイライトされます。ビューの下に作成される各 VEC アクションには、以下に示す通り、情報、編集、「ページ読み込み」へ移動、削除の 4 つのアイコンがあります。

![変更パネル](/help/c-experiences/assets/modifications.png)
...
| 「ページ読み込み」へ移動 | ページ読み込みのイベントへアクションを移動します。これらのアクションは通常、ページ間で使用される SPA の静的 HTML（ヘッダーやフッターなど）に適用されます。 |
| 削除 | アクションを削除します。 |
