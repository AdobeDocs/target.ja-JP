...
オーディエンスURLのターゲット設定では、完全に一致するURLを探します。 URLが一致する場合、ターゲットはそれ以上のロジックを考慮しません。 上記のURLでは、アクティビティが`www.example.com`で起動するように設定されている場合、オーディエンスURLのターゲット設定はクエリに依存しないので、URLは次のURLと一致します。
