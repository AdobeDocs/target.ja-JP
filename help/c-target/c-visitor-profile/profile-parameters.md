...
* プロファイルスクリプトの複雑さを制限するための推奨事項。

   プロファイルスクリプトで実行できる命令の数は限られています。
   ...
   * 再帰を避けます。
   * プロファイルスクリプトは、Targetに追加する前に、パフォーマンステストを行う必要があります。 すべてのプロファイルスクリプトは、すべてのmboxリクエストで実行されます。 プロファイルスクリプトが正しく実行されない場合、mboxリクエストの実行に時間がかかり、トラフィックとコンバージョンに影響が及ぶ可能性があります。
