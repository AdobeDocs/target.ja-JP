...
description: Adobe [!DNL Target] のカテゴリ親和性について説明します。この親和性は、ターゲット設定とセグメント化が可能なように、ユーザーが訪問するカテゴリを自動的にキャプチャし、そのカテゴリに対するユーザーの親和性を計算します。
title: カテゴリ親和性とは
feature: オーディエンス
...
[!DNL Adobe Target]のカテゴリ親和性機能では、ターゲット設定とセグメント化のために、ユーザーが訪問するカテゴリを自動的にキャプチャし、そのカテゴリに対するユーザーの親和性を計算します。 カテゴリ親和性は、その情報に基づいて行動する可能性が最も高い訪問者をコンテンツのターゲットにするのに役立ちます。

## カテゴリ親和性情報のへの引き渡し [!DNL Target] {#section_B0C8E46EEBAC4549AD90352A47787D04}

ユーザーがサイトを訪問したときには常に、訪問者に固有のプロファイルパラメーターが [!DNL Target] データベースに記録されます。データはユーザーの Cookie に結び付けられます。有効なパラメーターの1つは`user.categoryId`です。製品ページに割り当てられたmboxパラメーターです。 訪問者が閲覧を続けるか、別のセッションに戻る場合に、特定のユーザーが閲覧する製品のカテゴリを記録することができます。カテゴリ情報の記録は、任意の mbox（ネストされた mbox を含む）の mbox パラメーター `user.categoryId` または URL パラメーター `user.categoryId` として、またはグローバル mbox を使用した Target ページのパラメーターで渡すことによっても実行できます。詳しくはアカウント担当者にお問い合わせください。
...
>[!IMPORTANT]
>
>カテゴリ親和性のアルゴリズムに使用される`user.categoryId`属性は、[!DNL Adobe Target Recommendations]&#39;製品およびコンテンツのレコメンデーションに使用される`entity.categoryId`属性とは異なります。 ユーザーのお気に入りのカテゴリを追跡するには、`user.categoryId` が必要です。現在のページのカテゴリまたは現在の項目のカテゴリに関するレコメンデーションをベースにするには、`entity.categoryId` が必要です。両方の機能を使用したい場合、両方の値を Adobe Target に渡します。
...
* カテゴリが既にクリック（表示）されている場合、そのカテゴリを再度クリックしても、他のすべてのカテゴリから1が減算されることはありません
...
以下の節では、アクティビティでターゲティングのためのカテゴリ親和性オーディエンスを使用するのに役立つ情報を示します。
...
カテゴリ親和性オーディエンスは、どのアクティビティでも使用できます。 3ステップのガイドによるワークフローの[!UICONTROL Target]ステップで、目的のオーディエンスを選択します。
