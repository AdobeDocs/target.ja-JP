...
description: 国、市区町村、郵便番号など、ターゲットユーザーの地理的な場所に基づいてAdobeオーディエンス [!DNL Target] を作成する方法について説明します。
title: 訪問者ベースの場所を [!DNL Target] できますか。
feature: オーディエンス
...
[!DNL Adobe Target]のオーディエンスを使用して、国、都道府県、市区町村、郵便番号、DMA、携帯電話会社など、地理的な場所に基づいてターゲットを行います。

地域パラメーターを使用することで、訪問者の地域情報に基づいてアクティビティとエクスペリエンスをターゲット設定することができます。国、都道府県、市区町村、郵便番号、緯度、経度、DMA または携帯電話会社に基づいて訪問者を選択または除外することができます。このデータは[!DNL Target]リクエストごとに送信され、訪問者のIPアドレスに基づきます。 他のターゲット値と同様に、これらのパラメーターを選択します。

## 地域ターゲティング{#section_49CBFFAAC8694C4AAD3DE4B2DB7B05DE}を使用したオーディエンスの作成
...
ほとんどのモバイルデバイスユーザーはWiFi経由でコンテンツにアクセスします。つまり、[!DNL Target]のIPベースのGeoTargetingはデスクトップと同様に正確です。 携帯電話基地局ベースの接続では、信号が補足される基地局に基づいて訪問者の IP アドレスが決まるので、正確性に劣る場合があります。モバイルでの地域ターゲティングの問題には、[Geolocation API](https://developer.mozilla.org/en-US/docs/Web/API/Geolocation_API)を使用して解決できるものもあります。

**AOL 経由の訪問者は地域ターゲット設定ではどのように処理されますか?**

AOLがトラフィックをプロキシする方法により、[!DNL Target]は国レベルでのみターゲットできます。 例えば、フランスをターゲットにしたキャンペーンは、フランスのAOLユーザーを正常にターゲットします。 パリをターゲットにしたキャンペーンは、パリのAOLユーザーをターゲットできません。 AOL ユーザーだけをターゲットにする場合は、地域フィールドに「aol」を設定できます。実際、米国内の AOL ユーザーをターゲットにする場合は、国と地域の 2 つのターゲット条件がそれぞれ「米国」と「aol」に正確に一致するように指定します。
...
* **at.js 1.*x***:IPアドレスを別の場所のIPアドレスで上書きし、 `mboxOverride.browserIp url` パラメーターを使用できます。会社が英国にあり、ニュージーランドのオークランドのグローバルキャンペーンターゲット訪問者の場合は、オークランドのIPアドレスが`60.234.0.39`であると仮定して、次の形式のURLを使用します。

   `https://www.mycompany.com?mboxOverride.browserIp=60.234.0.39`

   アクティビティのテストを行う前に、Cookieをクリアします。

   >[!NOTE]
   >
   >`mboxOverride.browserIp` はat.js 1でサポートされています。*x* のみで使用できます。この機能は、at.js 2ではサポートされていません。*x*.

* **at.js 2.*x***:IPアドレスをat.js 2で上書きする場合。*x*、ブラウザーの拡張機能/プラグイン（ChromeまたはFirefoxのX-Forwarded-For Headerなど）をインストールします。この拡張機能を使用すると、x-forwarded-forヘッダーをページリクエストに渡すことができます。
...
いいえ。[!DNL Target]は、セッション中のみ地域データを使用し、データは破棄されます。
