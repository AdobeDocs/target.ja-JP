...
[!UICONTROL Auto-Target]の[!DNL Adobe Target]のアクティビティでは、高度な機械学習を使用して、複数のパフォーマンスの高いマーケター定義エクスペリエンスから選択し、コンテンツをパーソナライズしてコンバージョンを促進します。 [!UICONTROL Auto-Target]は、個々の顧客プロファイルと、類似したプロファイルを持つ以前の訪問者の行動に基づいて、各訪問者に最もカスタマイズされたエクスペリエンスを提供します。
...
* アクティビティがドラフトとして保存された後、トラフィックの割り当て（従来の[!UICONTROL Manual]）を[!UICONTROL A/B Test]に[!UICONTROL Auto-Target]から切り替えることはできません。
...
[!UICONTROL Control]の割合を調整するには、[!UICONTROL Experiences] ペインの[!UICONTROL Traffic Allocation]をクリックし、必要に応じて割合を調整します。 コントロールグループの割合を 10％未満にすることはできません。
...
## [!UICONTROL Auto-Target]よりも[!UICONTROL Automated Personalization]を選択するタイミング {#section_BBC4871C87944DD7A8B925811A30C633}

[!UICONTROL Auto-Target]よりも[!UICONTROL Automated Personalization]を使用するシナリオがいくつかあります。

* エクスペリエンスを形成するために自動的に組み合わされる個々のオファーではなく、エクスペリエンス全体を定義する場合。
* [!UICONTROL Visual Experience Composer]でサポートされていない[!UICONTROL Auto Personalization] （VEC）機能の完全なセットを使用する場合：カスタムコードエディター、複数のエクスペリエンスオーディエンスなど。
...
* [と[!UICONTROL Auto-Target]のデータのアップロードについて詳しくは、[!UICONTROL Automated Personalization]](/help/main/c-activities/t-automated-personalization/uploading-data-for-the-target-personalization-algorithms.md)を参照してください。
...
*または* モデルを構築するために必要なエクスペリエンスごとのトラフィック [!UICONTROL Auto-Target]数[!UICONTROL Auto Personalization]は同じですが、通常、[!UICONTROL Automated Personalization] アクティビティには[!UICONTROL Auto-Target] アクティビティよりも多くのエクスペリエンスがあります。
