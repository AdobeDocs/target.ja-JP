...
* 少なくとも[!UICONTROL Editor]権限（または[!UICONTROL Approver]）
...
1. 「デバッガーツール」セクションで、「**[!UICONTROL Generate New Authentication Token]**」をクリックします。
...
出力には、コンテンツに関する詳細情報が表示されます。 mboxTraceは、キャンペーンまたはアクティビティとプロファイルに関する詳細を表示します。 また、実行前のプロファイルのスナップショットと、実行後に変更された内容のスナップショットも提供されます。 さらに、各場所で評価されたキャンペーンやアクティビティも表示します。
...
クエリパラメーターに`=console`、`=json`または`=window`を含める必要はありません。 mboxTraceの詳細が完了したら、`=disable`を追加して&#x200B;**[!UICONTROL Enter]**&#x200B;を押すと、通常の表示モードに戻ります。
...
| `mboxOverride.browserIp=<Insert IP address>` | 地域ターゲティングのテスト<br>地域ターゲティングをこの URL パラメーターでテストします。 この属性の値に IP アドレスを入力すると、Test&amp;Target のジオターゲティングがこの IP アドレスを評価し、キャンペーン内のジオターゲティングまたはセグメント化セットに一致させます。 |
...
Adobe Experience Cloud デバッガーを使用すると、Target の実装をすばやく簡単に理解できます。 ライブラリ設定をすばやく表示し、リクエストを検証してカスタムパラメーターが正しく渡されたことを確認し、コンソールのログを有効にして、Target のすべてのリクエストを無効にすることができます。 Experience Cloud への認証を行うと、強力な MboxTrace ツールを使用して、アクティビティ、オーディエンス選定、訪問者プロファイルを調査できます。
...
DOM 要素が Adobe Experience Manager（AEM）パーソナライゼーションのターゲティングと Target アクティビティの両方に含まれていると、Target コンテンツがちらついたり表示されなかったりする場合があります。
...
[!UICONTROL Activity URL]は、アクティビティに対する訪問者を認定し、アクティビティエクスペリエンスをユーザーにレンダリングするページを決定します。 アクティビティの作成中にプロンプトが表示された場合、特にクエリ文字列パラメーターを含む URL の場合、完全な URL を入力してもコンテンツがそのサイトページに配信されるとは限りません。
...
VECが開いた後に別のページを表示するには、**[!UICONTROL Configure gear icon]** > **[!UICONTROL Page Delivery]**&#x200B;を選択>をクリックし、[!UICONTROL Activity URL] フィールドに目的のURLを指定します。
...
### オプション 1: URLをレプリケートし、「contains」オプションを使用してテンプレートルールに保持します。
...
### オプション 2：クエリ文字列のみを使用して、URLの「contains」条件を制限します。
...
このシナリオでは、URLは`https://shopping.mycart.com?type=Summers%20Offers`で、追加のテンプレートルールは[!UICONTROL Query]を[!UICONTROL type] > [!UICONTROL is (case sensitive)] > type=Summers%20Offersで指定し、OR演算子で区切ります。

![URL の特定の部分を活用するテンプレートルール](assets/option3.png)

## [!DNL Target] プロファイル属性値の二重引用符のエスケープが期待どおりに機能しません。 {#escape}

[!DNL Target] プロファイル属性に二重引用符を含む値を送信する場合は、次に示すように、二重引用符をエスケープする必要があります。
