...
description: Adobeの目標と設定ページの使用方法を説明します [!DNL Target] をクリックして、エクスペリエンスターゲット設定 (XT) アクティビティの目標に関する情報を指定します。
...
# エクスペリエンスのターゲット設定 (XT) アクティビティの目標と設定
...
![アクティビティ設定ページ](/help/main/c-activities/t-experience-target/t-xt-create/assets/ab_settings-new.png)
...
このオプションが「管理」で有効（デフォルト）になっていない場合は、優先度を指定します。「低」、「中」、「高」のいずれか。
