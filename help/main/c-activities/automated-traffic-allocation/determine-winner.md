...
description: 上昇率や信頼性などの主要指標に焦点を当てて、[!UICONTROL 自動配分]のA/B アクティビティの結果を解釈する方法について説明します。
...
>[!NOTE]
>
>[!UICONTROL 自動割り当て] アクティビティは、すべてのオプションの中から最適なエクスペリエンスを見つけるように設計されており、制御とペア比較を行うだけではありません。
...
[!UICONTROL 自動配分] アクティビティの[!UICONTROL Confidence]列には、エクスペリエンスが勝者になる確率が1%の許容誤差で表示されます。 このアルゴリズムでは、最適なコンバージョン率と2番目に高いコンバージョン率の間に、1%の最小検出可能な効果を使用します。 アルゴリズムは[Bernstein Inequality](https://en.wikipedia.org/wiki/Bernstein_inequalities_%28probability_theory%29)を使用して、この確率を計算します。
