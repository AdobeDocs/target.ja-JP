...
description: ドラフトフォームでのアクティビティの保存など、既存のアクティビティを編集する様々な方法について説明します。
...
変更をドラフトとして保存する方法など、[!DNL Adobe Target] で既存のアクティビティを編集する方法について説明します。 この記事では、[!DNL Target] インターフェイスで使用できる、アクティビティを変更するための様々な方法について説明します。 エクスペリエンスの更新、ターゲティングルールの調整、目標の設定を行う場合でも、Target は、アクティブ化の前に変更が安全に保存されるようにします。
...
1. 新しいアクティビティを作成するか、ドラフトフォームの既存のアクティビティを編集します。
...
* 同じワークスペース内でアクティビティをコピーすると、新しくコピーしたアクティビティの作成フローの最初の手順が編集モードで開きます。
