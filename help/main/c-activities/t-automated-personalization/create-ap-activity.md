...
1. （条件付き）「**[!UICONTROL オファー]**」をクリックして、コンテンツを選択してレポートグループに割り当てたり、ターゲット設定によって特定のオファーの閲覧を特定の訪問者のみに許可したりします。
   ...
   詳しくは、「[自動パーソナライゼーションのオファーレポートグループ](/help/main/c-reports/personalization-reports/offer-reporting-groups-in-automated-personalization.md)」を参照してください。
...
1. **ターゲット設定**&#x200B;のステップは、Target の他のタイプのアクティビティと同様です。ここでは、オーディエンスを選択してから、**[!UICONTROL カスタム配分]**&#x200B;ドロップダウンリストをクリックして、コントロールエクスペリエンスを表示する訪問者の割合を指定し、「**次へ**」をクリックできます。
   ...
   * **パーソナライゼーショントラフィックの最大(90/10)化：** 「常時稼動」アクティビティを作成することを目的としている場合は、時間の経過と共に学習を続行するのに十分なデータをアルゴリズムに確保するために、訪問者の 10%を制御に入れます。 この場合は、トラフィックの大部分をパーソナライズ対象に回す代わりに、推定上昇率の精度が落ちるというデメリットがあります。これは、目標にかかわらず、特定のエクスペリエンスをコントロールとして使用する場合のお勧めのトラフィック分割です。
...
詳しくは、「[自動パーソナライゼーションレポート](/help/main/c-reports/personalization-reports/reports-ap.md)」を参照してください。
