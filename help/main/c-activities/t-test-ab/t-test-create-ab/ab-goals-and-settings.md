...
| [!UICONTROL 期間] | アクティビティは、承認されたときに開始させたり、特定の日時を設定したりできます。 同様に、非アクティブ化されたときに終了させたり、日時を設定したりできます。 タイムピッカーは 24 時間の時計を使用します。00:00 は真夜中です。 タイムゾーンはブラウザーで設定されたタイムゾーンに設定されます。 別のタイムゾーンを使用するには、ブラウザーのタイムゾーンを変更してからブラウザーを再起動します。 |
...
| [!UICONTROL 統計手法] | 実験結果の分析方法を選択します。 この選択により、このアクティビティの[!UICONTROL  コンバージョン ]目標指標に使用される統計フレームワーク、結果指標、決定条件が決まります。<ul><li>**[!UICONTROL ベイズ]**: データの到達時に信念を更新し、継続的な監視とサンプルサイズの縮小をサポートする確率ベースの意思決定。 結果は、あるバリアントが別のバリアントよりも優れている確率、例えば「バリアント Bがコントロールに勝つ確率92%」として表されます。 [!UICONTROL  ベイズ ]は、サンプルサイズが小さい、継続的な監視、確率を好む関係者、過去のテストから適用する事前知識がある場合に最適です。 厳格な規制やコンプライアンスの承認が必要な場合や、従来のp値が必要な場合は理想的ではありません。 レポートには、[!UICONTROL  ビートコントロールの可能性]と、フリークエンティストの[!UICONTROL 信頼度]列の代わりに信頼できる間隔が表示されます。</li><li>**[!UICONTROL ウェルチのt テスト （フリークエンティスト）]**：既存の従来の仮説テスト手法。 p値を使用し、固定サンプルサイズが必要です。 レポートには、[!UICONTROL 信頼性]の割合と信頼区間が表示されます。</li></ul>この設定は、主な目標指標が[!UICONTROL  コンバージョン ]のアクティビティでのみ使用できます。主な目標が[!UICONTROL 収益]または[!UICONTROL  エンゲージメント ]のアクティビティでは、常に[!UICONTROL  ウェルチのt-test]を使用します。 各手法の計算方法とレポート方法について詳しくは、[A/Bn テストでの統計計算](/help/main/c-reports/statistical-methodology/statistical-calculations.md)を参照してください。 |
| [!UICONTROL  ベイズ設定] | [!UICONTROL  ベイズ ]が[!UICONTROL 統計手法]として選択された場合にのみ表示されます。 次のオプションを設定します。<ul><li>[!UICONTROL 信頼区間]：真の効果が想定される範囲を設定します。</li><li>[!UICONTROL 決定しきい値]: バリアントが勝者として宣言される前に、[!UICONTROL Control]に対して到達しなければならない最小確率を設定します。 </li><li>[!UICONTROL 過去のテストの事前知識を使用しますか？]：このオプションをオンに切り替えて、過去のパフォーマンスの事前知識を提供し、[!UICONTROL  ベイズ ]計算に反映させます。</li></ul> |
