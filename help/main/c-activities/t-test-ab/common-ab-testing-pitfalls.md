...
description: Adobeで A/B テストを実行する際に企業が発生する最も一般的な落とし穴やミスを回避する方法を説明します [!DNL Target] および他のテストソリューション
...
での A/B テスト [!DNL Adobe Target] は、最も多くのデジタルマーケティング最適化プログラムの中心となり、マーケターが訪問者や顧客に対して最適化され、ターゲット設定されたエクスペリエンスを提供するのを支援します。 この節では、A/B テストを実施する際に企業が陥りがちな最も重要な落とし穴の 10 件について説明します。 また、回避方法についても説明し、会社がテストの取り組みを通じてより優れた ROI を達成し、レポートされた A/B テストの結果の信頼性が高まります。
...
## 手順 4:片側検定を使用する {#section_8BB136D1DD6341FA9772F4C31E9AA37C}
...
## 落とし穴 7 :テスト期間中のトラフィック配分の変更 {#allocation}

テスト期間中はトラフィック配分の割合を変更しないことをお勧めします。変更すると、データが正規化されるまでテスト結果がゆがむ可能性があるからです。
例えば、A/B テストアクティビティで、トラフィックの 80%がエクスペリエンス A（コントロール）に、トラフィックの 20%がエクスペリエンス B に割り当てられているとします。テスト期間中に、各エクスペリエンスの配分を 50%に変更します。 数日後、トラフィックの配分をエクスペリエンス B に 100%変更します。

このシナリオでは、ユーザーはどのようにエクスペリエンスに割り当てられますか。

エクスペリエンス B の配分分割を手動で 100%に変更した場合、最初にエクスペリエンス A（コントロール）に配分された訪問者は、最初に割り当てられたエクスペリエンス（エクスペリエンス A）にとどまります。 トラフィック配分の変更は、新規参加者にのみ影響します。

割合を変更したり、各エクスペリエンスへの訪問者のフローに大きく影響を与えたりする場合は、新しいアクティビティを作成するか、アクティビティをコピーしてから、トラフィック配分の割合を編集することをお勧めします。

テスト期間中に様々なエクスペリエンスの割合を変更した場合、特に多くの購入者が再訪問者の場合に、データが標準化されるまで数日かかります。
もう 1 つの例として、A/B テストのトラフィックの配分が50/50に分割され、分割を80/20に変更した場合、変更後の最初の数日間、結果が偏って見える場合があります。 コンバージョンまでの平均時間が長い場合、つまり、購入に数時間または数日かかる場合、これらのコンバージョンの遅れはレポートに影響を与える可能性があります。 つまり、最初のエクスペリエンスでは、訪問者数が 50%から 80%になり、コンバージョンまでの平均時間が 2 日間です。テストの最初の日は、訪問者の 50%からの訪問者のみがコンバージョンします。 これにより、コンバージョン率が急落したように見えますが、コンバージョンに 2 日かかった訪問者の 80%がこのような状況になった後で、再度標準化されます。
...
## 落とし穴 10:ビジネス目標を反映していない指標の使用 {#section_F0CD6DC7993B4A6F9BEEBB31CD1D9BEE}
...
## まとめ：A/B テストを通じて、落とし穴を特定し、回避することで成功 {#section_54D33248163A481EBD4421A786FE2B15}
