...
description: アクティビティや他の要素に影響する文字制限やその他の制限のリストを [!DNL Adobe Target].
title: の様々な文字、サイズ、その他の制限 [!DNL Adobe Target]?
...
* **制限**: 1 件につき 50 個の mbox [!DNL Target] コンテンツ配信のバッチ mbox リクエスト。

  1 回につき 50 個を超える mbox [!DNL Target] コンテンツ配信のバッチ mbox リクエストが応答エラーコードに結果を返す `HTTP 400` エラーメッセージ付き `size must be between 0 and 50`.

  バッチ mbox リクエストは順番に処理され、反復のたびに全体的な応答時間が増加します。 バッチリクエストの mbox が多いほど、応答の待ち時間が長くなるので、タイムアウトが発生する可能性があります。 この高遅延のバッチ要求でエクスペリエンスのレンダリングがブロックされると、ユーザーがエクスペリエンスのレンダリングを待つと、ユーザーエクスペリエンスが低下する可能性があります。

* **制限**:60 MB HTTPPOSTの本文サイズ [!DNL Target] コンテンツ配信リクエスト。

  の HTTPPOST本文サイズが 60 MB を超えている [!DNL Target] コンテンツ配信リクエストが応答エラーコードを返す `HTTP 413 Request Entity Too Large`.

* **推奨制限**:50 通の通知 [!DNL Target] 配信バッチリクエスト。

  次の通知の数が 50 件を超えています： [!DNL Target] 配信バッチリクエストを実行すると、応答の遅延とタイムアウトが増加する可能性があります。

  バッチ通知リクエストは順番に処理され、反復のたびに全体的な応答時間が増加します。 バッチリクエストの通知が多いほど、応答待ち時間が長くなるので、タイムアウトが発生する可能性があります。 バッチ通知リクエストに対する追加の遅延は、一部のお客様に許容される場合がありますが、タイムアウトとそれ以降の再試行では、より多くの遅延が生じる可能性があることに注意してください。
...
* **上限**：250 文字。
  ...
  の場合 [!DNL Delivery API] (at.js 2.*x*)、mbox V2 のバッチ、および [!DNL Adobe Experience Platform Web SDK] (alloy.js) 統合、mbox 名 *can* には、英数字 (A ～ Z、a ～ z、0 ～ 9) と、次の文字のいずれかを含めます。
