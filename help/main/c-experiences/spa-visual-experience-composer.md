...
次に示すように、[!UICONTROL 変更] パネルは、特定のビューに対して作成されたアクションをキャプチャします。 ビューのすべてのアクションが、ビューの下にグループ化されています。
...
これらの目標を実行するには、VECで「[!UICONTROL 作成]」をクリックし、ホームビューに変更を適用します。
...
1. 「[!UICONTROL さらに読み込む]」を1回クリックすると、2行目の商品にアクセスできます。
