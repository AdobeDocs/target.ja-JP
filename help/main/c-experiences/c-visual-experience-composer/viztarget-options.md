...
次のセクションでは、[!UICONTROL A/B Test]アクティビティの更新されたVECで使用できるオプションについて説明します。 このオプションは、アクティビティタイプによって異なります。

### [!UICONTROL Experiences]のレール
...
* **エクスペリエンスを追加**: **[!UICONTROL Add]**&#x200B;アイコン（ ![追加アイコン](/help/main/assets/icons/Add.svg) ）をクリックして、新しいエクスペリエンスを追加します。 必要に応じて、新しいエクスペリエンスを設定します。
* **エクスペリエンス名の変更**: **[!UICONTROL Rename]**&#x200B;アイコン（ ![アイコン名の変更](/help/main/assets/icons/Rename.svg) ）をクリックして、[!UICONTROL Rename Experience]ダイアログボックスを表示します。 新しい名前を指定し、**[!UICONTROL Save]**&#x200B;をクリックします。
* **エクスペリエンスを複製、削除、またはリダイレクトする**: **[!UICONTROL More Actions]**&#x200B;アイコン（ ![その他のアクションアイコン](/help/main/assets/icons/MoreSmall.svg) ）をクリックし、**[!UICONTROL Duplicate]**、**[!UICONTROL Delete]**、または&#x200B;**[!UICONTROL Redirect to URL]**&#x200B;を選択します。

### アクティビティ設定/構成 {#settings}
...
* **[!UICONTROL Page Delivery]**:サイト上の類似ページに同じエクスペリエンスを含めます。 ページテンプレートを使用してページに構造を設定するか、ページに類似のエレメントが含まれている場合に、類似の構造のページエレメントまたはドメイン全体でのバリエーションをテストします。 詳細については、[類似したページに同じエクスペリエンスを含める](/help/main/c-experiences/c-visual-experience-composer/temtest.md)を参照してください。
* **[!UICONTROL Site Preferences]**: [!DNL Target]がCSSセレクターを生成する方法を指定するように、サイトの環境設定を構成します。 詳細については、__&#x200B;の[CSSセレクター[!UICONTROL Visual Experience Composer]](/help/main/administrating-target/visual-experience-composer-set-up.md)を参照してください。
* **追加のページの追加**:アクティビティに追加のページを追加して、複数ページのアクティビティを作成します。各ページに固有のデザインを使用して、複数ページにわたってストーリーを作成できます。 詳しくは、[ 複数ページアクティビティ ](/help/main/c-experiences/c-visual-experience-composer/multipage-activity.md) を参照してください。
...
>[!NOTE]
>
>使用できるオプションは、作成または編集しているアクティビティタイプと要素によって異なります。 [!UICONTROL A/B Test]アクティビティでの画像やオファーの編集について詳しくは、以下の[[!UICONTROL Design]キャンバスを使用した要素の編集](#design)を参照してください。

### [!UICONTROL Properties]のレール
...
レールの上部にあるアイコンをクリックして、HTMLコードの編集、エレメントの削除、複製、非表示を行います。 変更が [!UICONTROL Modifications] パネルに表示されます。

[!UICONTROL Properties]レールは、右側のレールで折りたたみ可能です。これにより、デザインカンバスを非表示にしたり、デザインカンバスを拡大したりできます。 レールの右側にある[!UICONTROL Show/Hide Properties]アイコン（![プロパティアイコン](/help/main/assets/icons/Propertie.svg)）をクリックして、[!UICONTROL Properties]レールを折りたたむか表示します。

## [!UICONTROL Design]キャンバスを使用して要素を編集する {#design}

次のセクションでは、[!UICONTROL Design]キャンバスの画像とテキストを編集する方法について説明します。 デザインカンバスは、コンポーネント、変更、およびプロパティレールとともに、アクティビティのエクスペリエンスを簡単に作成できる強力なツールを提供します。
...
* 基本（ディバイダー、HTML、画像）。
* テキスト（見出し、段落、リンク）。
* 動的([おすすめ](/help/main/c-recommendations/recommendations-as-an-offer.md)、[エクスペリエンスフラグメント](/help/main/c-experiences/c-manage-content/aem-experience-fragments.md)、HTMLオファー)

画像の上部にあるメニューでは、次の操作を実行できます。

* リンクを挿入します（![リンクアイコンの挿入](/help/main/assets/icons/Link.svg)）。
* 画像を変更します（![画像アイコンを選択](/help/main/assets/icons/Images.svg) ）。
* 個人用設定を追加します（![個人用設定アイコンの追加](/help/main/assets/icons/PersonalizationField.svg) ）。
...
* ページ上の画像の位置を設定します（絶対、固定、相対、静的、または注釈）。
* マージン、パディングを含めたエレメントの間隔を設定します。
* エレメントの効果（不透明度）を設定します。 [!UICONTROL Show Advanced Options]をクリックして、画像のセピア、グレースケール、コントラスト、明るさおよびぼかしの値を設定します。 画像を反転または回転することもできます。
* 画像のインラインスタイルを設定します。
...
* HTMLを編集します（![HTMLを挿入アイコン ](/help/main/assets/icons/Code.svg)）。 詳細については、以下の「[HTMLの編集](#html)」を参照してください。
* テキストを複製します（![アイコンの複製](/help/main/assets/icons/Code.svg)）。
...
* 境界線のカラーを選択します。
* テキストボックスのサイズ（高さと幅）を設定します。 [!UICONTROL Show Advanced Options]をクリックして、テキストボックスの最小および最大サイズ、幅、高さ、オーバーフロー、およびオブジェクトのサイズを構成します。
* ページ上でのテキストボックスの位置（絶対、固定、相対、静的、または固定）を設定し、上、右、下、および左からのピクセル数を設定します。
* マージン、パディングを含めたエレメントの間隔を設定します。
