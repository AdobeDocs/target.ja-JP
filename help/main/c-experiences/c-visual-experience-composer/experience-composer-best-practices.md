...
以下のベストプラクティスは、エクスペリエンスを期待どおりに動作させるために役立ちます。[!DNL Adobe Target] で [!UICONTROL Visual Experience Composer] （VEC）を使用する際に注意しておく必要があるその他のヒントと制限事項もあります。
...
### 拡張 Experience Composer は、アカウントレベル（アカウントで作成されたすべてのアクティビティで有効）または個々のアクティビティレベルで有効にできます。
...
### 拡張 Visual Experience Composer がサイト上のセキュリティで保護されたページに読み込まれない場合は、特定の IP アドレスを許可リストに加えるできます。
...
VEC は、リンクを更新したプロキシサーバーを使用して、バックグラウンドで web サイトを操作します。 base タグを追加すると、このプロキシサーバーが使用する URL が、ブラウザーによって再度解決され、壊れているように表示されます。
...
### VEC の customCode アクションは、Internet Explorer 8 では機能しません。
