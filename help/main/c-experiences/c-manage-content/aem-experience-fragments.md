...
で作成したエクスペリエンスフラグメント (XF) を使用する [!DNL Adobe Experience Manager] (AEM) [!DNL Target] アクティビティを活用して、最適化やパーソナライゼーションを支援します。

>[!NOTE]
>
>でAEMエクスペリエンスフラグメントを使用する際は、次の点を考慮してください。 [!DNL Target]:
> 
...
![エクスペリエンスフラグメントタイプでフィルター：Target UI でのHTMLまたは JSON](/help/main/c-experiences/c-manage-content/assets/fragment-types.png)

リスト内のエクスペリエンスフラグメントの上にマウスポインターを置いて、 [!UICONTROL 表示] アイコン ![表示アイコン](assets/icon_info.png) をクリックすると、エクスペリエンスフラグメントの名前、タイプ、オファー ID、オファーパス、最後の変更情報など、エクスペリエンスフラグメントに関する追加情報が表示されます。 「オファー使用状況」タブをクリックして、このオファーを参照するアクティビティを表示します。

![エクスペリエンスフラグメント情報ポップアップ](/help/main/c-experiences/c-manage-content/assets/xf-info-popup.png)
...
>
>エクスペリエンスフラグメントは、 [!DNL Recommendations] アクティビティ。 ただし、レコメンデーションでエクスペリエンスフラグメントを使用するには、 [!UICONTROL A/B テスト] アクティビティ ( [!UICONTROL 自動配分] および [!UICONTROL 自動ターゲット]) または [!UICONTROL エクスペリエンスのターゲット設定] (XT) アクティビティと [レコメンデーションをオファーとして含める](/help/main/c-recommendations/recommendations-as-an-offer.md).
