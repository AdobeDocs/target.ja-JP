...
description: Adobe Target MCP サーバーが公開する23個の読み取り専用ツールすべてのパラメーターの完全なリファレンスです。
...
* **Observer**&#x200B;以上の役割：23個の読み取り専用ツールすべてにアクセス

>[!NOTE]
>
>書き込みツール（作成、更新、アクティブ化、非アクティブ化）は、パブリック Betaのパブリック MCP カタログを介して公開されません。 このページに記載されている23のツールはすべて読み取り専用です。 書き込みアクセスは、今後のリリースで利用できるようになります。
...
| オーディエンス | 2 | `list_target_audiences`, `get_target_audience` |
| mbox | 3 | `list_target_mboxes`, `get_target_mbox`, `list_target_mbox_profile_attributes` |
| プロパティ | 1 | `list_target_properties` |
| レポート | 6 | `get_ab_performance_report`, `get_ab_orders_report`, `get_xt_performance_report`, `get_xt_orders_report`, `get_activity_report_by_name`, `get_a4t_report` |
