...
| **A/B テストを作成** |「ホームページヒーロー画像テスト」と呼ばれるA/B テストを作成し、ホームページヒーローのmboxを対象とする「Control」と「Variant」の2つのエクスペリエンスを使用します。」 |
| **オファーを作成** | 「すべての夏アイテムが20% オフのプロモーションバナー付きの「サマーセールバナー」というHTML オファーを作成します。」 |
| **オーディエンスを作成** | 「カリフォルニア州のモバイルユーザーをターゲットとする、「カリフォルニア州からのモバイル訪問者」というオーディエンスを作成します。」 |
| **XT アクティビティを作成** |「別の地域の訪問者に別のヒーローバナーを表示する、「Geo Personalization」というエクスペリエンスのターゲット設定アクティビティを作成します。」 |
| **アクティビティのスケジュール** |「アクティビティのスケジュールを更新して、6月1日に開始し、6月30日に終了12345るようにします。」 |
| **アクティブ化または一時停止** | 「アクティビティ 12345をアクティブ化」または「ホームページヒーローテストを一時停止」します。 |
| **トラフィックの分割を更新** | 「アクティビティ 12345のトラフィックの分割を70% コントロールと30% バリアント Aに変更します。」 |
| **バリエーションを追加** | 「オファー67890ータを使用して、A/B アクティビティ 12345に「ホリデーテーマ」という新しいバリエーションを追加します。」 |
