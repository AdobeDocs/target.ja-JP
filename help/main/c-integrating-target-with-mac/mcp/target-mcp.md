...
[!DNL Adobe Target] MCP サーバーは、アクティビティ、オーディエンス、オファー、および実装設定に対する読み取りアクセスと書き込みアクセスの両方を提供します。 統合を活用することで、次のことが可能になります。
...
はい。 MCP サーバーは、A/Bおよびエクスペリエンスのターゲティングアクティビティの作成、アクティビティ設定の更新、トラフィック分割の管理、バリエーションの追加または削除、アクティビティのアクティブ化、一時停止、非アクティブ化などの書き込み操作をサポートします。 書き込み操作には編集者の役割以上が必要です。アクティブ化と非アクティブ化には承認者の役割が必要です。
+++
...
書き込みツールには、安全注釈と確認ゲートが含まれているため、ユーザーの明示的な確認なしに状態を変更するアクションが実行されることはありません。 Adobeでは、本番環境で書き込みツールを使用する前にサンドボックス環境で統合をテストし、MCPで開始されたすべてのアクションを確認する前に慎重に確認することをお勧めします。
+++

+++[!DNL Adobe Target]でどのような権限が必要ですか？

**Observer**&#x200B;以上の役割は、すべての読み取り専用ツールへのアクセスを許可します。 書き込みツール （作成、更新）には、**編集者**&#x200B;以上の役割が必要です。 **承認者**&#x200B;の役割は、状態変更ツール （アクティブ化、非アクティブ化）に必要です。 現在のアクセス レベルが不明な場合は、[!DNL Adobe Target]管理者にお問い合わせください。
+++
