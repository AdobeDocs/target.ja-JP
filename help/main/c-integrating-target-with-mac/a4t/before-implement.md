...
>[!NOTE]
>
>この記事は、at.js実装にのみ適用されます。 [!DNL Adobe Experience Platform Web SDK]での[!UICONTROL Analytics for Target] （A4T）の実装について詳しくは、[Adobe Analytics for Target （A4T）のExperience Platform Web SDK](https://experienceleague.adobe.com/docs/target-dev/developer/a4t/overview-a4t.html?lang=ja){target=_blank}へのログインを参照してください。
...
[Marketing Cloud統合プロビジョニングフォーム &#x200B;](https://survey.adobe.com/jfe/form/SV_ekBHTLSoP5Zki2y){target=_blank}に入力する際は、「[!UICONTROL For which capabilities are you requesting provisioning]?」に記載されている「[!UICONTROL Shared Audiences]」オプションに関する次の重要な情報に注意してください。

![リクエストフォーム](/help/main/c-integrating-target-with-mac/a4t/assets/request-form.png)

[!UICONTROL Shared Audiences]をリクエストすると、[!UICONTROL Target]と[!UICONTROL Adobe Audience Manager] （AAM）が情報を共有できるようになります（この場合はオーディエンス）。
