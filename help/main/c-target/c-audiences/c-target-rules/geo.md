...
GeoTargeting の精度は、複数の要因に左右されます。WiFi 接続は、携帯電話ネットワークよりも正確です。訪問者がモバイルデータ通信を使用している場合、地域ルックアップの精度は、場所（プロバイダーのデータとの関係）の影響を受ける可能性があります [DeviceAtlas](https://deviceatlas.com/device-data/user-agent-tester)、およびその他の要因 携帯電話基地局ベースのネットワーク接続では、有線または WiFi 接続に正確性で劣る可能性があります。また、訪問者の IP アドレスは、訪問者の ISP の場所にマッピングされ、訪問者の実際の場所とは異なる可能性があります。 モバイルの位置情報に関するいくつかの問題は、 [位置情報 API](https://developer.mozilla.org/en-US/docs/Web/API/Geolocation_API).
...
+++詳細を見るほとんどのモバイルデバイスユーザーは WiFi 経由でコンテンツにアクセスします。つまり、 [!DNL Target]の IP ベースの GeoTargeting は、デスクトップと同様に正確です。 携帯電話基地局ベースの接続では、信号が補足される基地局に基づいて訪問者の IP アドレスが決まるので、正確性に劣る場合があります。モバイルの位置情報に関するいくつかの問題は、 [位置情報 API](https://developer.mozilla.org/en-US/docs/Web/API/Geolocation_API).

+++

### AOL 経由の訪問者は地域ターゲット設定ではどのように処理されますか?

+++詳細を見る AOL がトラフィックをプロキシする方法により、 [!DNL Target] は、国レベルでのみターゲットにできます。 例えば、フランスをターゲットにしたキャンペーンで、フランスの AOL ユーザーが正常にターゲットに設定されたとします。 しかし、パリをターゲットにしたキャンペーンでは、パリの AOL ユーザーをターゲットにすることはできません。 AOL ユーザーだけをターゲットにする場合は、地域フィールドに「aol」を設定できます。実際、米国内の AOL ユーザーをターゲットにする場合は、国と地域の 2 つのターゲット条件がそれぞれ「米国」と「aol」に正確に一致するように指定します。
...
+++詳細を見るプエルトリコ、香港、その他の地域は別々の「国」値として扱われます。

+++

### 実行 [!DNL Target] アクティビティが位置情報ターゲティング機能でターゲット設定される場合に、郵便番号などの情報をキャプチャ（および保存）しますか？

+++詳細を見るいいえ。 [!DNL Target] では、セッション中にのみ地域データを使用するので、データは破棄されます。
