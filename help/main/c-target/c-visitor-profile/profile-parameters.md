...
スクリプトプロファイルを効果的に使用するには、シンプルなJavaScript知識が必要です
パラメーター： このセクションは、この機能を使用してわずか数分で生産性を高めるためのクイックリファレンスの役割を担います。
...
| `//` | ソースに含まれているすべての要素がターゲットブール値に含まれているかどうかを確認します（配列ソース、配列ターゲット）。<br>`//` ターゲット（regexpに対応）から部分文字列を抽出し、`Array/*String*/ decode(String encoding, String regexp, String target)`をデコードします。<br>この機能では、定数文字列値、グループ化（`condition1 \|\| condition2) && condition3`）、正規表現（`/[^a-z]$/.test(landing.referring.url)`）の使用もサポートしています。 |
