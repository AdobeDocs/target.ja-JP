...
description: Adobe Recommendations アクティビティの内容を制御する条件を作成して、アクティビティに最も適した推奨事項を表示する方法について説明します。
title: レコメンデーションで基準を作成するにはどうすればよいですか？
...
1. **[!UICONTROL ルックバックウィンドウ]**&#x200B;を設定して、表示するレコメンデーションを決定する際に使用できる過去のユーザー行動データの時間範囲を決定します。 このオプションは、類似属性とカスタムアルゴリズムを持つ項目を除くすべてのアルゴリズムで使用できます。
...
>[!NOTE]
>
>アルゴリズムの種類とアルゴリズムとして、**[!UICONTROL アイテムベース]**/**[!UICONTROL 類似属性を持つメディア]**&#x200B;を選択した場合、コンテンツの類似性ルールを設定するオプションがあります。
...
![ContentSimilarity画像](assets/ContentSimilarity.png)
...
![包含ルール &#x200B;](/help/main/c-recommendations/c-algorithms/assets/inclusion-rules.png)
...
1. 特定の基準を満たす場合のみ品目を表示するよう、レコメンデーションを設定します。
   ...
   ![Recs_InclusionRules画像](assets/Recs_InclusionRules.png)
...
1. キーワードを入力して、ルール属性を完成させます。
   ...
   ![Recs_AttributeWeighting画像](assets/Recs_AttributeWeighting.png)
