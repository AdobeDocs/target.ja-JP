...
description: Adobe [!DNL Target] Recommendationsで、潜在的なアイテムのプールを、ユーザーが操作した特定のアイテムと比較して、動的にフィルタリングする方法について説明します。
...
潜在的なレコメンデーション項目のプールを、ユーザーが操作した特定の項目と比較することで、[!DNL Adobe Target] [!DNL Recommendations]で動的にフィルターを実行します。
...
これらのランディングページが機能するには、mboxの`entity.brand`を配信する必要があります。
