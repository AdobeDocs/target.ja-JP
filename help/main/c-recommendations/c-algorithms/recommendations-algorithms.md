...
* **項目ベースのアルゴリズム**:「この項目を閲覧/購入したユーザーがこれらの項目も閲覧/購入した」ロジックに従うアルゴリズムを含めます。 これらのアルゴリズムは、アイテムとアイテムの協調フィルタリングという包括的な用語の下にグループ化され、同様の属性を持つ[!UICONTROL  アイテム ] アルゴリズムもグループ化されます。

* **ユーザーベースのアルゴリズム**: [!UICONTROL 最近表示した]と[!UICONTROL おすすめの] アルゴリズムを含めます。
...
そのような類似性の例の 1 つは、項目間の共起です。両方の項目を購入したユーザーの数の単純な数。 直感的ですが、そのような指標は、人気のあるアイテムを推奨することに偏っているという点で素朴です。 例えば、食料品店でパンを購入する人が多い場合、パンはすべての商品との共生が多いですが、必ずしもそれがよいレコメンデーションではありません。 代わりに [!DNL Target] ログ尤度比（LLR）と呼ばれる、より洗練された類似性指標を使用します。 A と B の 2 つのアイテムが共起する確率と共起しない確率が大きく異なる場合、この量は大きくなります。 具体的には、[!UICONTROL People Who Viewed This, Bought That] algorithmのケースを考えてみましょう。 B が購入された確率が誰かが A を見たかどうかとは無関係に *ない* 場合、LLR の類似性は大きくなります。
...
レコメンデーションアルゴリズムの[!DNL Target] スイートへの最新の追加機能は、[!UICONTROL おすすめ]と一連の買い物かごベースのレコメンデーションアルゴリズムです。 どちらのタイプのアルゴリズムも、協調フィルタリング手法を使用して、個々の項目ベースのレコメンデーションを形成します。 その後、サービス時に、ユーザーの閲覧履歴（[!UICONTROL おすすめユーザー]の場合）またはユーザーの現在のカート（カート ベースのレコメンデーションの場合）に複数のアイテムを使用して、これらのアイテムベースのレコメンデーションを取得し、結合して最終リストを作成します。 パーソナライズされたレコメンデーションアルゴリズムには、様々な種類があります。 複数キーアルゴリズムを選択した場合は、訪問者の閲覧履歴が記録されるとレコメンデーションがすぐに使用可能になり、最新の訪問者行動に対応するようにレコメンデーションが更新されます。
...
* **モデルトレーニング**:
  ...
  * **アイテムの類似性モデルのトレーニング**: コア アイテムの類似性計算は、候補アイテム ベクターの作成方法が[!UICONTROL おすすめ]および買い物かごベースのアルゴリズムと異なります。 [!UICONTROL お勧め]の場合、項目ベクトルにはディメンション NUsersがあります。各エントリは、項目のユーザーの暗黙的な評価の合計を表します。項目の購入には、項目のビューの2倍の重みが与えられます。 買い物かごベースのレコメンデーションの場合、項目ベクトルにはバイナリエントリがあります。セッション内動作のみを考慮する場合は、すべてのセッションに新しいエントリがあります。 それ以外の場合は、すべての訪問者に対してこの項目ベクトルにエントリが存在します。
...
* [!UICONTROL  サイト全体で最も閲覧された項目]と[!UICONTROL  サイト全体で上位の販売者]は、これらの項目がそれぞれ閲覧または購入されたセッションの合計数で項目をランク付けします。 出力されるのは、推奨される項目の単一（キーレス）リストです。
...
* [!UICONTROL  プロファイル属性で最も閲覧された項目]および[!UICONTROL  プロファイル属性で売り手]も、集計表示または購入数で項目をランク付けしますが、グループ化キーは、項目プロパティの代わりに、国、地域、メンバーシップ層などの訪問者プロファイル属性です。 各セッションのカウントは、訪問者がインタラクション時に保持したプロファイル値に起因するもので、アイテムの属性には起因しません。 その結果、そのプロファイル属性の明確な値によってキーが設定された一連のレコメンデーションリストが作成されます。
...
カスタム条件を使用すると、顧客は [ 独自のレコメンデーションを  [!DNL Target]](/help/main/c-recommendations/c-algorithms/recommendations-csv.md) にアップロードでき、重要な柔軟性が得られ、「独自モデルの導入」機能が可能になります。 カスタム条件は、[!UICONTROL  アイテムベース ]のレコメンデーションの「オフライントレーニング」部分に代わるものですが、オンラインコンテンツ配信フェーズでは、1つのキーがレコメンデーションの取得に使用され、ビジネスルール/フィルターが適用されるという点で、アイテムベースのレコメンデーションアルゴリズムと同様に動作します。
