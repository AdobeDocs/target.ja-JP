...
モデルトレーニングは、レコメンデーションが [!DNL Adobe Target] ース学習アルゴリズムによってどのように生成されるかを示すプロセスです。 モデルサービングは、サイト訪問者にレコメンデーションを配信する方法 [!DNL Target] す（コンテンツ配信とも呼ばれます）。
...
* **入力データ**：これは、品目協調フィルタリング （CF） メソッドと同じです。 [!UICONTROL Both Recommended For You] および買い物かごベースのアルゴリズムは、行動データを、（Target を実装 [ または ](https://experienceleague.adobe.com/docs/target-dev/developer/recommendations.html){target=_blank}4}Adobe Analytics](/help/main/c-recommendations/c-algorithms/use-adobe-analytics-with-recommendations.md){target=_blank} から収集したユーザーの表示および購入の形式で使用します。[
...
* **モデルトレーニング**:
   ...
   * **項目類似性モデルのトレーニング**：コアアイテムの類似性計算は、候補アイテムのベクトルの作成方法が [!UICONTROL Recommended For You] ールと買い物かごベースのアルゴリズムで異なります。 [!UICONTROL Recommended For You] の場合、アイテム・ベクトルには次元 NUsers があり、各エントリはそのアイテムのユーザーに対する暗黙の評価の合計を表します。アイテムの購入には、アイテムのビューの 2 倍の重みが与えられます。 買い物かごベースのレコメンデーションの場合、項目ベクトルにはバイナリエントリがあります。セッション内動作のみを考慮する場合は、すべてのセッションに新しいエントリがあります。 それ以外の場合は、すべての訪問者に対してこの項目ベクトルにエントリが存在します。
...
カスタム条件を使用すると、顧客は [ 独自のレコメンデーションをにアップロード  [!DNL Target]](/help/main/c-recommendations/c-algorithms/recommendations-csv.md) でき、重要な柔軟性が得られ、「独自モデルの導入」機能が可能になります。 カスタム条件は、レコメンデーションの「オフライントレーニング」の部分を置き換え [!UICONTROL Item-Based] ものですが、オンラインコンテンツ配信フェーズでは項目ベースのレコメンデーションアルゴリズムと同様に動作します。つまり、レコメンデーションの取得に単一のキーを使用し、ビジネスルール/フィルターが適用されます。
