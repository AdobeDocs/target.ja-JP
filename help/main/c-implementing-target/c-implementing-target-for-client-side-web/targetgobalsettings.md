...
* **説明**：`<clientCode>.tt.omtrdc.net`ドメインまたは `mboxedge<clusterNumber>.tt.omtrdc.net` ドメインを使用する必要があるかどうかを示します。
   ...
   この値が true の場合、`mboxedge<clusterNumber>.tt.omtrdc.net` ドメインは Cookie に保存されます。現在、はを使用していません [CNAME](https://developer.adobe.com/target/before-implement/implement-cname-support-in-target/)at.js 1.8.2 および at.js 2.3.1 より前のバージョンの at.js を使用する場合は、{target=_blank} してください。これが問題となる場合は、次の点を考慮してください。 [at.js の更新](https://developer.adobe.com/target/implement/client-side/atjs/target-atjs-versions/){target=_blank} は、サポートされている新しいバージョンです。
