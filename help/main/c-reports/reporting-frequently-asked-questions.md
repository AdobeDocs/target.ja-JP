...
* 訪問者がサイトを訪問したのは、 [訪問者プロファイルの有効期間](/help/main/c-target/c-visitor-profile/visitor-profile-lifetime.md) は期限切れです。
...
**例 1**：これらの 2 つのセグメントが 1 つのアクティビティに適用されている場合、新規訪問者セグメントと再訪問者セグメントで合計訪問者数が合計されるとは限りません。
...
**例 2**：新規訪問者と再訪問者のカウントの相違も、アクティビティの [成功指標](/help/main/c-activities/r-success-metrics/success-metrics.md).
...
アクティビティの内容が訪問者に配信されたのにそのレポートにデータが含まれていない場合は、「選択したレポート設定で使用できるデータがありません」というエラーメッセージが表示されることがあります。

アクティビティレポートにデータが表示されない理由はいくつか考えられます。

* レポートの設定で正しい環境が選択されていません
* コントロールエクスペリエンスに割り当てられたトラフィックがありません
...
アクティビティのコンテンツがユーザーに配信されたのにそのレポートにデータがない場合は、コントロールエクスペリエンスにトラフィックが割り当てられていることを確認します。
...
>[!NOTE]
>
>を更新する方法の詳細については、 [!UICONTROL Automated Personalization] (AP) アクティビティを実行し、コントロールエクスペリエンスをトラフィックを受け取るエクスペリエンスに変更します。詳しくは、 [Automated Personalizationまたは自動ターゲットアクティビティ用のコントロールの選択](/help/main/c-activities/t-automated-personalization/experience-as-control.md).
...
* 次の場所にいる場合： [!DNL Target] または [!DNL Analytics] を使用している **[!UICONTROL 訪問回数]** 指標、 [!DNL Target] は訪問者ベースのシステムであり、A/B テストまたは MVT テストのトラフィック配分は訪問者レベルで割り当てられます。 したがって、 **[!UICONTROL 訪問回数]** 指標を使用すると、特定の訪問者が複数回訪問している可能性があるので、トラフィックの配分が不均等に表示される場合があります。 訪問者数は、アクティビティのパフォーマンスを評価する際の標準的な標準化指標です。
