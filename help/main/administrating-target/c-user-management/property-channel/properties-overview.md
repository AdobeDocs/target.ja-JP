...
Enterprise 権限を活用するには、ワークスペース ID を渡す必要があります。 [Target API](https://experienceleague.corp.adobe.com/docs/target-dev/developer/server-side/server-side-overview.html){target=_blank}.
...
1. の更新 [!DNL Target] 前の手順で取得した実装コードを使用した実装。
   ...
   * **targetPageParamsAll() 関数を使用：** 次のコードを `<head>` タグを使用します。
      ...
      at.js を使用してこれをおこなう方法について詳しくは、 [targetPageParamsAll](https://experienceleague.corp.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/targetpageparamsall.html){target=_blank}.
