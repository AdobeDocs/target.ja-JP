...
Enterprise ユーザーの権限 は、管理する企業全体のユーザーに対する正式なアクセス手段です。 [!DNL Adobe Target]. ユーザーの追加先 [!DNL Target]、役割に基づいて権限を割り当て、様々な部門、グローバルな場所、チャネル、その他の論理グループに基づいてチーム用にワークスペースを作成します。 ユーザーに [!UICONTROL 監視者], [!UICONTROL 編集者], [!UICONTROL 承認者]または [!UICONTROL 投稿者].
