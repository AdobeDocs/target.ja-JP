...
+++詳細を表示
* **更新された UI で、顧客が決定オファーを編集したり、特定のページ要素を選択したりできない問題を修正しました**：更新されたアクティビティ作成プロセスでは、顧客は Visual Experience Composer （VEC）で既存の決定オファーを編集したり、特定のページ要素を選択したりできませんでした。 決定オファーがHTML オファーとして誤って表示され、編集中に行われた変更は保存されませんでした。 さらに、日本サイトなどの特定の地域の URL が VEC に正しく読み込まれず、アクティビティの作成と更新がブロックされていました。 （TGT-53425）
* **保存後、[!UICONTROL Offer Decision] セレクターを変更できず、予期せず変更される問題を修正しました**：更新されたアクティビティ作成プロセスで、お客様が [!UICONTROL Offer Decision] セレクターを意図したとおりに変更できませんでした。 セレクターの変更は失敗し、保存後にセレクターが誤った値に戻りました。 その結果、決定オファーが Visual Experience Composer （VEC）から消え、それ以上の編集がブロックされていました。 （TGT-53433）
* **保存後、[!UICONTROL Offer Decisions] がアクティビティから消える問題を修正しました**：アクティビティ作成プロセス中に追加された [!UICONTROL Offer Decisions] は、アクティビティを保存して再度開いた後も保持されませんでした。 この問題は、動的コンテンツを編集し、特定のセレクターおよびプロパティを使用して [!UICONTROL Offer Decisions] を挿入する際に発生していました。 （TGT-53434）
...
* **新しいアクティビティ作成プロセスで、透明画像の fmt=png-alpha パラメーターが設定されなかった問題を修正しました**：更新された UI では、アクティビティ作成プロセス中に挿入された画像に `fmt=png-alpha` パラメーターがデフォルトで含まれなくなり、透明性が失われました。 この動作は、背景を透明に保ちながら画像 URL にパラメーターを自動的に追加した以前の UI とは異なります。 （TGT-52615）
