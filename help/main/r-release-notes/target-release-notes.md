...
+++詳細を表示
* **Recommendations UI で、カスタム条件の CSV ダウンロードが 404 エラーを返す問題を修正しました**：顧客がアクティビティ作成プロセスでカスタム条件の CSV をダウンロードできない問題を修正しました。 ダウンロードリンクが正しく機能し、顧客が期待どおりにカスタム条件を書き出せるようになりました。 （TGT-51966）
