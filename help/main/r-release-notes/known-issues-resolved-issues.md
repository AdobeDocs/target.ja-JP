...
* フィードまたは API を介した更新がなくなって 60 日が経過すると、期限切れのエンティティは正しく期限切れになります。ただし、有効期限切れのエンティティは、有効期限切れの後もカタログ検索インデックスから削除されません。 フィードまたは API で削除されたエンティティも、現在、カタログ検索インデックスから削除されません。 （IRI-857）
