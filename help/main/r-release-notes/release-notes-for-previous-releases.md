...
* **アクティビティ [!DNL A/B Test (Manual)]のベイズ統計** [!DNL A/B Test (Manual)] アクティビティが、Welchのt テストの代替としてベイズ統計をサポートするようになりました。 [!UICONTROL 目標と設定]の統計的手法を選択します。ベイズ活動のレポートには、[!UICONTROL  ビートコントロールの可能性]などの確率ベースの決定指標と、周波数論的な信頼区間の代わりに信頼性の高い区間が表示されます。

* [!DNL Auto-Allocate]件のアクティビティに関する&#x200B;**[!DNL Customer Journey Analytics]件のレポート。** [!DNL Auto-Allocate]のアクティビティで、[!DNL Target]および[!DNL Adobe Analytics] （A4T）に加えて、[!DNL Customer Journey Analytics]をレポートソースとして使用できるようになりました。
...
* **許可されたURLの正規表現の検証。** 適切な検証を行うことなく、**[!UICONTROL 許可URL]** フィールドに無効な正規表現を入力できる問題を修正しました。 （TGT-55095）
...
* [!UICONTROL  オーディエンスを編集] ダイアログの&#x200B;**ボタン。** **[!UICONTROL オーディエンスを編集]** ダイアログ内のボタンが正しくレンダリングされなくなった問題を修正しました。 （TGT-54638）
...
* **[!UICONTROL グラフ ビュー]は、テーブルとダウンロードと同期していません。** 一部の日付範囲の&#x200B;**[!UICONTROL グラフビュー]**&#x200B;でアクティビティレポートに欠落またはゼロの指標が表示される問題を修正しました。**[!UICONTROL テーブルビュー]**&#x200B;とダウンロードしたレポートに正しい値が表示されたままです。 （TGT-54998）
...
* **オーディエンス使用リストが完全にレンダリングされていません。** オーディエンスの詳細の「**[!UICONTROL 使用状況]**」セクションで、追加のアクティビティがそのオーディエンスに関連付けられている場合でも、マッピングされたアクティビティのサブセットのみを表示できる問題を修正しました。 （TGT-55094）
...
* **最後のオクテット IPの難読化に関する確認がより明確になります。** **[!UICONTROL 管理]** > **[!UICONTROL 実装]**&#x200B;で&#x200B;**[!UICONTROL 訪問者IP アドレスを難読化]**&#x200B;から&#x200B;**[!UICONTROL 最後のオクテット]**&#x200B;に変更すると、確認ダイアログで[!DNL Target]が訪問者IP アドレスの最後のオクテットを非表示にすることが説明されるようになりました。 （TGT-44821）
...
* **拡張Experience Composer （EEC）を使用した空白または不完全なページ。** **[!UICONTROL 拡張Experience Composer]**&#x200B;が有効になっている場合、[!UICONTROL Visual Experience Composer]がエディターでサイトを読み込めない問題を修正しました。 （TGT-54576）
...
* **レポートのオーディエンスフィルターでエラーが発生しました。** **[!UICONTROL 目標と設定]**&#x200B;内のオーディエンスフィルターを変更すると、[!DNL Target] ユーザーインターフェイスの「レポート」セクションでエラーが発生する問題を修正しました。 （TGT-55006）

* **アクティビティを優先度順に並べ替え。** **[!UICONTROL 優先度]**&#x200B;列ヘッダーを使用して、他の並べ替え可能な列と一致する昇順と降順で、アクティビティリストの優先度による並べ替えを追加しました。 （TGT-54948）

* **追加のアクティビティ プロパティは、保存後も保持されません。** アクティビティを保存して再度開いた後、特定の&#x200B;**[!UICONTROL プロパティ]**&#x200B;の選択が保持されない問題を修正しました。 （TGT-53889）
...
* [!UICONTROL  ページ配信] ルール演算子&#x200B;**の**&#x200B;日本語ラベル 日本語UIのページ配信ルール演算子ラベルの読み取り不可能または破損した文字列を修正しました。 （TGT-53097）
...
* HTMLを挿入を削除すると、**変更が削除されました。** **[!UICONTROL 挿入前]**&#x200B;または&#x200B;**[!UICONTROL 挿入後]**&#x200B;で追加された余分な&#x200B;**[!UICONTROL HTML]** ブロックを削除すると、CSS セレクターを持たないリンクされた変更も削除される問題を修正しました。 （TGT-54530）
...
* **一部のアクティビティに場所がありません。** 一部のアクティビティで&#x200B;**[!UICONTROL 場所]**&#x200B;が見つからない問題を修正しました。 （TGT-54951）

* **指標の列の順序を報告しています。** 更新された[!DNL Target] インターフェイスを使用すると、選択を完全にクリアして指標を順番に再追加することなく、レポート指標を並べ替えることができます。 以前は、ユーザーはすべての指標の選択を解除し、目的の順序で再度選択する必要がありました。これは、多くの指標が有効になっている場合や、列の配置を調整して水平スクロールを制限する場合に時間がかかりました。 （TGT-53044）
...
* **追加のビューに適用すると、カスタムコードが保持されます。** 同じ&#x200B;**[!UICONTROL Activity]**&#x200B;で別の&#x200B;**[!UICONTROL ビュー]**&#x200B;のカスタムコードを追加または保存する際に、1つの&#x200B;**[!UICONTROL ビュー]**&#x200B;に適用されたカスタムコードが削除される問題を修正しました。 （TGT-53933）
+++
...
* **アクティビティ ビューに表示されるオーディエンス属性。** **[!UICONTROL アクティビティ]**&#x200B;から表示されたオーディエンスルールの詳細で、**[!UICONTROL オーディエンス]** セクションから同じオーディエンスを開いたときに表示された特定の属性が表示されない問題を修正しました。 （TGT-54742）

* **アクティビティ リスト ページでCSVをエクスポートします。** 日常的な書き出しのAPIのみに依存することなく、フィルターが適用される場合を含め、ユーザーインターフェイスからアクティビティリストを書き出せるように、**[!UICONTROL CSVの書き出し]** アクションを追加しました。 （TGT-51466）

* **セレクターが見つからないときにエクスペリエンスの変更がフラグ付けされます。** エクスペリエンスの変更で、セレクターの存在チェックが実行されるようになりました。ページにセレクターが見つからない場合、変更は無効としてフラグ付けされます。 （TGT-54815）

* **[!UICONTROL 自動パーソナライゼーション ]のアクティビティ。** インターフェイスとアクティビティの読み込みの問題を修正し、ユーザーがAutomated Personalization アクティビティを確実に作成、編集、管理できなくなり、キャンペーンの設定がブロックされ、パーソナライゼーションのユースケースが遅れました。 （TGT-54421）
...
* アクティビティからオーディエンスを作成する際に&#x200B;**オーディエンス名と説明が表示されます。** **[!UICONTROL オーディエンス]**&#x200B;の直下にオーディエンスを作成する場合と比較して、アクティビティフローからオーディエンスを作成または編集する際に、オーディエンス **[!UICONTROL 名前]**&#x200B;および&#x200B;**[!UICONTROL 説明]** フィールドが明確に表示されない問題を修正しました。 （TGT-54837）
...
* **[!UICONTROL ライブアクティビティ ]はインサイトをカウントします。** インサイトダッシュボードの&#x200B;**[!UICONTROL ライブアクティビティ]**&#x200B;指標が、**[!UICONTROL すべてのアクティビティ]**&#x200B;でライブとして表示されたアクティビティの数よりも多い合計を報告する可能性がある問題を修正しました。 （TGT-54788）
...
* [!UICONTROL  グローバル除外].**の**&#x200B;長いID リスト **[!UICONTROL グローバル除外]**&#x200B;でIDの長いリストを貼り付けたり入力したりすると、更新されたインターフェイスでレガシーと比較して切り捨てられる可能性があり、不完全な除外リストが発生する問題を修正しました。 （TGT-54422）
...
* [!UICONTROL Visual Experience Composer]の&#x200B;**Enhanced Experience Composer （EEC） ステータスインジケーター。** EEC インジケーターは、拡張Experience Composerが有効かどうかを示します。 表示が修正され、インタラクティブではないステータス表示としてのみ機能するため、インタラクティブなトグルに似なくなりました。 （TGT-54828）

* **Visual Experience Composerの[!UICONTROL 折りたたみ可能な左パネル ].** アクティビティを編集用に開いている間に、左側のパネルを折りたたむことができるようになりました。 これにより、小さなディスプレイを含め、複数のオーディエンスとページを含むアクティビティの&#x200B;**[!UICONTROL コンポーネント]**&#x200B;と&#x200B;**[!UICONTROL プロパティ]**&#x200B;へのアクセスが改善されます。 （TGT-54269）
...
* オーディエンス インターフェイスで&#x200B;**オーディエンス ソース ラベルの正確性。** Adobe Experience PlatformのAdobe Target v2の宛先から来たオーディエンスが、**Adobe Experience Platform**&#x200B;ではなく&#x200B;**Adobe Experience Cloud**&#x200B;でソースとして表示される問題を修正しました。 このアップデートにより、オーディエンスをフィルタリングおよびレビューする際のソースラベルの一貫性が向上します。 （TGT-54802）
...
* **新しいVisual Experience Composer UIでの認証後の断続的なグレーの画面。** 新しいVisual Experience Composerにログインした後、ユーザーに一時的にグレーの画面が表示され、それ以上の編集が行われない問題を修正しました。 このアップデートにより、認証後の読み込み信頼性が向上します。 （TGT-54626）
...
* **Visual Experience Composer web エディターに表示されるデザイン/参照モードの選択。** この修正は、デザインモード、参照モード、プレビューモードの切り替えが正しく機能したが、アクティブモードがインターフェイスに明確に表示されない問題を解決します。 選択したモードがハイライト表示され、どのモードになっているかがわかります。 （TGT-54790）

* **Visual Experience Composer （Chrome）のプライベート ネットワーク URL。** Chromeでは、ローカルネットワークへのアクセスが許可されていない限り、プライベートネットワーク上のページをブロックできます。 **[!UICONTROL Visual Experience Composer]**&#x200B;は、Experience Cloudで使用するフレーミングの権限をリクエストするようになりました。これにより、Chromeはサイレントで失敗する代わりに許可プロンプトを表示できます。 アクセスを許可すると、内部URLは、お使いの環境でサポートされているときにVECに読み込まれる可能性があります。 （TGT-54346）
...
* **エクスペリエンスのダイレクトオファーの変更は保存されません。** この修正は、アクティビティエクスペリエンス内のダイレクトオファーに対して行われた変更が保存されない問題を解決します。 以前は、ユーザーがダイレクトオファーを開いて変更し、保存すると、変更内容は最初に反映されたように見えましたが、オファーを再度開いたときに失われていました。 この修正により、ダイレクトオファーへの変更が適切に保存され、オファーが再度開かれたときに保持されるようになります。 （TGT-54653）
...
* **実装画面のちらつき管理トグルを追加します。** フリッカー管理設定の有効化を制御するために、[!UICONTROL 実装]画面に新しい切り替えスイッチが追加されました。 この切り替えにより、管理者は実装画面から直接ちらつき管理を設定できます。 （TGT-52247）
...
* **概要ページでオーディエンスとエクスペリエンスのフルネームを表示します。** この機能強化により、[!UICONTROL 概要] ページが更新され、オーディエンスとエクスペリエンスの完全な名前が表示されます。 以前は、長い名前は切り捨てられ、完全に表示されていなかったため、ユーザーはトリプルクリックしてすべてのテキストを選択し、完全な名前を表示する必要がありました。 このアップデートにより、完全なオーディエンス名とエクスペリエンス名が表示されるようになり、ユーザーがアクティビティ設定を識別およびレビューしやすくなります。 （TGT-53323）
...
* **VECの変更が、Shadow DOM （Salesforce Lightning Web コンポーネント）を使用するサイトに反映されない。** この修正プログラムは、Lightning Web コンポーネント（LWC）を使用してAdobe Targetで行った変更（CTAのカラー変更など）が、Salesforce ベースのサイトのライブサイトに保存または反映されない問題を解決します。 CMSでは、Target アクティビティからの更新を受け付けていませんでした。この問題は、A/B テストやその他のアクティビティタイプで一貫して発生しました。 （TGT-54059）
...
* **追加のビューに適用すると、カスタムコードが失われます。** この修正プログラムは、追加のビューにカスタムコードを追加して適用する際に、1つのビューに適用されたカスタムコードが失われた問題を解決します。 以前は、ユーザーがアクティビティ内の複数のビューにカスタムコードを適用すると、以前に適用されたカスタムコードは消えていました。 この修正により、追加のビューに変更を適用する際に、すべてのビューでカスタムコードが適切に保持されるようになります。 （TGT-54230）
...
* **プロファイルパラメーター&#39;In Between&#39;規則が条件に保存されていません。** プロファイル パラメーターに&#x200B;**In Between**&#x200B;演算子を使用する条件ルールが正しく保存されなかった問題を解決します。 以前、ユーザーが&#x200B;**In Between**&#x200B;演算子を使用してルールを設定し、範囲値を入力した場合、条件は正常に保存されたように見えましたが、条件を再度開いたり編集したりすると、ルールが消えます。 この修正により、**In Between**&#x200B;規則が正しく保存され、条件が再度開かれたときに保持されるようになります。 （TGT-54498）
...
* **すべてのアクティビティの後にインサイトページの位置を変更します。** **インサイトページ**&#x200B;の位置が更新され、ナビゲーションの&#x200B;**すべてのアクティビティ** ページの後に表示されるようになりました。 すべてのアクティビティ ページが、Target アプリケーションのデフォルトページになりました。 （TGT-54672）
...
* **左側のナビゲーションを折りたたみ可能にします。** 左側のナビゲーションは、すべてのタブに対して折りたたみ可能で拡張可能になりました。 折りたたむと、ツールヒントと共にアイコンが表示されます。 展開すると、アイコンとテキストの両方が表示されます。 この機能強化により、ナビゲーションオプションへの迅速なアクセスを維持しながら、必要に応じてより多くの画面スペースが確保されます。 （TGT-54608）
...
* **バックエンドがnullを返す場合、デフォルトのexperienceEditorTypeを「visual」に変更します。** この修正は、一部のシナリオで`experienceEditorType`値がnullとして返され、エディタータイプが正しく初期化されなかったUIで意図しない動作が発生する問題を解決します。 （TGT-54690）
...
* **アクティビティに画像を追加できません – 不明なエラーです。** この修正プログラムは、アクティビティで画像を追加または置換しようとしたときに、ユーザーが「不明なエラー」に遭遇した問題を解決します。 この問題は、Scene7 URLの同期と認証エラーに関連していました。 この修正により、Visual Experience Composerで画像を正しく追加および置き換えることができるようになりました。 （TGT-54137）
...
* **削除後にプロパティトークンが自動的に再追加されません。** この修正は、ユーザーが選択を解除したプロパティトークンが、アクティビティの編集時に自動的に再追加される問題を解決します。 以前、ユーザーが自動検出されたプロパティを削除し、ステップ間を移動すると、システムはアラートやプロンプトを表示せずに削除されたプロパティを再導入し、予期しないプロパティでアクティビティが有効になりました。 このアップデートにより、プロパティトークンが削除されると、その後のすべての編集とナビゲーション中に削除されたままになります。 （TGT-54040）
...
Adobe Targetには新しいインサイトダッシュボードが追加され、テストやパーソナライゼーションのためにAdobe Targetをどのように使用しているのか、その全体像を把握できるようになりました。 ダッシュボードには、ライブアクティビティ、ライブアクティビティと変更アクティビティ、完了アクティビティ、公開アクティビティ、A/B テストアクティビティなどの主要指標が表示されます。 時間範囲セレクターを使用して、2025年の振り返りを含む、さまざまな期間のトレンドを調査します。 このダッシュボードは、導入とアクティビティを継続的に追跡する方法として、引き続き利用できます。

[Adobe Target インサイトダッシュボード ](/help/main/c-activities/insights-dashboard.md)を参照してください
+++
...
* **URL管理機能を許可リストに加えるしました。** 新しい「許可リストに加えるURL」管理機能が[!UICONTROL 管理] セクションに追加され、許可されたリモートオファーURL パターンを管理できるようになりました。 この機能には、許可リストに加えるされたURLを表に表示する機能、URLを検索してフィルタリングする機能、モーダルダイアログを使用して新しいURLを追加する機能、確認を行ってURLを削除する機能が含まれています。 この機能により、顧客はリモートオファーに使用できるURLを制御できるようになり、セキュリティとコンプライアンスが向上します。 （TGT-54486）
...
* **GraphQLのcreatedAtおよびmodifiedAtによるフィルタリングのサポート。** `createdAt`および`modifiedAt`日付フィールドによるアクティビティのフィルタリングのサポートが、GraphQL APIに追加されました。 この機能強化により、Target UIでの正確なフィルタリングが可能になり、クライアントサイドのデータ処理が削減され、フィルタリングされたアクティビティデータを取得するための複数のAPI呼び出しが不要になります。 （TGT-54499）
...
* **アクティビティをコピーできません – 無効なユーザー入力** アクティビティのコピー時にユーザーに「無効なユーザー入力」エラーが表示される問題を修正しました。 以前は、アクティビティが複製されると、そのワークスペース固有のプロパティの割り当てが保持されず、ABActivityではデフォルト以外のワークスペースに属するプロパティが少なくとも1つ必要なため、バックエンドが保存リクエストを拒否していました。 この不一致により、UIで一般的なエラーが発生し、ユーザーにガイダンスが与えられませんでした。 この修正により、コピー操作中にワークスペースの割り当てが正しく保持され、ユーザーは変更なしでコピーされたアクティビティを保存でき、誤解を招く検証エラーを防ぐことができます。 （TGT-54282）
* **Web エディターのオファーでワークスペース列を有効にします。** このアップデートは、Web エディター内の他のワークスペースに表示される[!UICONTROL  デフォルト Workspace]のオファーに起因するお客様の混乱に対処します。 このビヘイビアーは設計どおりに機能していますが、[!UICONTROL  デフォルトのWorkspace]のオファーは、すべてのワークスペースで意図的に表示されます。お客様は、特に「承認者」などのデフォルト以外のワークスペースでアクティビティを作成する場合、UIがワークスペースの出所を明確にしていないと報告しました。 わかりやすくするために、Web エディターのオファーリストで[!UICONTROL Workspace]列が有効になり、ユーザーは各オファーがどのワークスペースに属しているかを簡単に識別できるようになり、表示される追加オファーの誤解釈を防ぐことができます。 （TGT-54138）
* **target=&quot;_blank&quot;のリンクが新しいタブで開きます。** この修正は、[!UICONTROL 参照] モードでクリックすると、~target=&quot;_blank&quot;~のリンクを含む作成されたweb サイトが新しいブラウザータブで開き、エディター内のプレビューエクスペリエンスが中断される問題を解決します。 この動作は、アンカー要素が変換され、そのターゲットがエディター内でのナビゲーションを維持するために上書きされた従来のUIとは異なり、作成されたページのネイティブリンク属性が拡張機能の挿入されたJavaScriptによってインターセプトされなかったために発生しました。 このアップデートにより、~target=&quot;_blank&quot;~を使用するリンクがWeb エディター内で適切に処理されるようになり、オーサリング中に外部タブが開かなくなります。 （TGT-54134）
* **プロパティの選択を解除すると警告が表示されます。** このアップデートでは、アクティビティエディターで自動検出されたプロパティの選択を解除したときに、ユーザーに明確に通知する視覚的な警告が導入されました。 以前は、自動検出されたプロパティを削除しても、そのプロパティが完全に削除されることを示すものはありませんでした。そのため、c0はターゲティング設定が誤って失われる可能性があります。 この修正により、従来のUIの動作と一致する警告アイコンが追加され、プロパティの選択を解除するとアクティビティから削除されることをユーザーに通知できます。 （TGT-54121）
* **[!UICONTROL ワークスペース ] ドロップダウンは、[!UICONTROL  ユーザー] セクションの20に制限されています。** この修正により、ユーザーがもっと多くのワークスペースにアクセスできる場合でも、[!UICONTROL 管理] > [!UICONTROL  ユーザー] セクションの[!UICONTROL  ワークスペース ] ドロップダウンが20個のワークスペースのみを表示する問題が解決されました。 `licenseGroups`の基礎となるGraphQL呼び出しも20件に制限され、ユーザーが組織内のより多くのワークスペースにアクセスできたにもかかわらず、UIに不完全なリストが表示されました。 この更新により、このハード制限が削除され、使用可能なワークスペースの完全なセットが返され、正しく表示されるようになりました。 （TGT-53820）
* **オファーのモーダルでワークスペース列が表示されない問題を修正しました。** 更新されたUIでオファーのモーダルにワークスペース列が表示されない問題を修正しました。 これは、[!UICONTROL  デフォルトのWorkspace]からのオファーが、選択したワークスペースからのオファーと一緒に表示され、その出所が示されないため、お客様に混乱をもたらしました。 ワークスペース列が有効になり、顧客は各オファーがどのワークスペースに属しているかを明確に識別できます。 （TGT-52320）
...
* **既に削除されている場合、アクティビティ編集で自動検出されたプロパティを追加しないでください。** この修正は、アクティビティを編集すると、ユーザーが以前に削除した自動検出プロパティが自動的に再表示される問題を解決します。 編集のためにアクティビティを再開すると、削除されたプロパティがシステムによって誤って復元され、[!UICONTROL  プロパティリスト ]で動作が不安定になり、混乱が生じます。 このアップデートにより、自動検出されたプロパティが削除されると、その後のすべての編集中に削除され、ユーザーが明示的に追加しない限り再び表示されなくなります。 （TGT-54182）
* **既に削除されている場合は、自動検出されたプロパティを追加しないでください。** この修正により、ユーザーがアクティビティから自動検出されたプロパティを手動で削除すると、その後のアクティビティエディター内でのナビゲーション中にシステムが再導入されなくなります。 以前は、ユーザーが自動検出されたプロパティの選択を解除し、[!UICONTROL  ターゲティング ] ステップに移動してから[!UICONTROL  エクスペリエンス ]に戻った場合、エディターは、アクティビティエディターの状態スライスに保存されている自動検出リストに基づいて、削除されたプロパティを再入力していました。 更新されたロジックは、自動検出されたプロパティを~ActivityState~ スライスの現在のプロパティと比較し、ユーザーが既に削除した自動検出されたプロパティを再追加しないようにしました。 これにより、ステップをまたいで一貫性のある行動を実現し、ユーザーの意図を尊重できます。 （TGT-54181）
* **自動検出されたテキストをプロパティ リストに追加します。** この機能強化により、[!UICONTROL  プロパティリスト ]が更新され、システムによって自動的に検出されたプロパティに明確なラベルが付けられます。 自動検出されたプロパティがユーザーに表示される[!UICONTROL  プロパティ リスト ]にも存在する場合、~ActivityEditorSlice~状態に保存されている値を使用して、その名前の横に「（自動検出）」テキストが表示されるようになりました。 これは、従来のUIの動作を反映し、ユーザーが手動で選択したプロパティと自動的に識別されたプロパティを簡単に区別するのに役立ちます。 （TGT-54120）
* **自動検出された[!UICONTROL  プロパティ ]を状態に追加します。** この更新により、~ActivityEditorSlice.ExperienceEditor~状態で、Web エディターからアクティビティ [!UICONTROL  エクスペリエンス ] タブに渡されたすべての自動検出プロパティ IDの最新リストが常に維持されます。 ユーザーが「[!UICONTROL  エクスペリエンス ]」タブに移動するたびに、状態は新しく検出されたプロパティで更新され、重複を防ぎながら、正確な追跡と信頼性の高い下流の動作が保証されます。 （TGT-54119）
...
* **[!UICONTROL 環境] ドロップダウンには、100件の結果のみが表示されます。** この修正は、100を超える環境を持つ顧客が[!UICONTROL Recommendations]内の[!UICONTROL 環境] ドロップダウンの最初の100個のエントリしか表示できない制限に対処します。 基になるGraphQL クエリ （~getEnvironmentsV2~）は、ハードコードされたページサイズ 100でページ分割されたため、追加のページが使用可能な場合でも、UIに部分的なリストのみが表示されます。 環境が100を超えているお客様の場合、この問題により、オプションが欠落し、選択エクスペリエンスが不完全になりました。 このアップデートでは、すべての環境が返されて表示されるように制限が増え、環境数に関係なく完全に表示されます。 （TGT-53903）
...
* **[!UICONTROL  レポート ]の矢印が拡張可能な列を明確に示さない問題を修正しました。** 更新されたUIで追加の列を拡張できることがレポートテーブルに明確に表示されない問題を修正しました。 列ヘッダーの近くにある[!UICONTROL  レポート ]矢印に消えるツールチップが追加され、より多くの列が使用可能であることを顧客が理解しやすくなりました。 （TGT-53479）
...
* **ビューに適用された変更を削除できません。** この修正は、変更が最初に追加のビューに再適用されない限り、ユーザーがアクティビティ内の変更を削除できなかった問題を解決します。 アクティビティ（アクティビティ ID 302467など）を編集する場合、変更を削除しようとすると何も影響がなく、ユーザーが不要な変更を削除できないようにします。 しかし、「さらにビューに適用」を使用して変更を再適用し、`Page Load` イベントに割り当てると、削除が突然予想通りに機能しました。 （TGT-54088）
...
* 新しいVEC UI **で**[!UICONTROL  エクスペリエンスフラグメント ]名が切り捨てられました（TGT-54312）
* **収益]指標[!UICONTROL に[!UICONTROL 詳細設定]を使用できません。** この修正は、[!UICONTROL 目標と設定]で[!UICONTROL 収益]指標の[!UICONTROL 詳細設定]を設定する際に、ユーザーが403 「アクセス拒否」エラーを検出した問題を解決します。 プライマリ目標に関連付けられた依存関係の条件を追加する際に問題が発生しました。バックエンドでは、既にアクティビティの作成と編集に十分な権限を持っているユーザーであっても、誤ってエディター権限が必要でした。 その結果、有効な設定にもかかわらず、アクティビティの保存に失敗しました。 このアップデートでは、権限チェックが修正され、適切なアクセス権を持つユーザーが禁止リソースエラーをトリガーせずに収益メトリックの依存関係を正常に追加できるようになりました。 （TGT-54092）
* **選択した画像に追加ボタンが適用されない問題を修正しました。** アクティビティ作成プロセスで画像を選択または更新する際に、顧客が特定の画像を追加できない問題を修正しました。 例えば、顧客が特定のアセットを検索した際に「ipp」を検索したときに返された画像では、「[!UICONTROL 追加]」ボタンをクリックしても、選択した画像は適用されず、変更は作成されませんでした。 `Homepage-banner-1-moz.jpg`などの他の画像を選択しても、期待どおりに機能し続けました。 このアップデートにより、更新されたUIですべての有効な画像を一貫して適用できるようになります。 （TGT-53610）
* **URL条件を削除すると、目標指標の設定がリセットされる問題を修正しました。** [!UICONTROL 目標]指標で1つのURL条件を削除すると、更新されたUIで設定全体がリセットされる問題を修正しました。 お客様が[!UICONTROL  コンバージョン ] > [!UICONTROL  ページを表示]の下に保存されたURL条件を削除しようとすると、目標タイプが予期せず[!UICONTROL Mboxを表示]に切り替わり、以前に設定されたすべての設定が削除されました。 このアップデートにより、選択したURL条件のみが削除され、残りのすべての目標設定が維持されます。 （TGT-53271）
* **検索でサブフォルダーが検索されない問題を修正しました。** オファーを検索しても、更新されたUIのサブフォルダーから結果が返されない問題を修正しました。 顧客がオファーを見つけることができるのは、そのオファーが保存されているフォルダーに手動で移動した場合のみであり、検索動作がAPI機能と一致しません。 検索でフォルダーを繰り返し検索できるようになったため、顧客は各フォルダーを個別に開くことなく、オファーを見つけることができます。 （TGT-51954）
...
* **更新されたUIで非表示または無効なセレクターを使用して決定を提供できません。** 更新されたUIで、要素がVisual Experience Composer （VEC）に表示されない限り、非表示または無効なセレクターに関連付けられたオファーの決定を編集できない問題を解決しました。 編集はパネルから直接サポートされるようになりました。従来のUIで使用できる機能を復元し、セレクターの表示に関係なくオファーの決定を変更できるようになりました。 （TGT-53899）
...
* **アクティビティの条件を編集すると、ページがクラッシュしました。** 更新されたUIで、アクティビティ条件を編集するとページがクラッシュし、`useCrudActionsCtx`に関連するコンソールエラーが発生する問題を解決しました。 条件エディターが正しく読み込まれ、機能するようになり、アクティビティを中断することなく編集できるようになりました。 （TGT-53971）
* **[!UICONTROL メッセージ ]列が、更新されたUIに製品データを断続的に表示できませんでした。** 更新された[!UICONTROL Recommendations] UIで、[!UICONTROL  カタログ検索]の[!UICONTROL  メッセージ ]列が、フィードに値が存在していたにもかかわらず、製品データの表示に断続的に失敗する問題を解決しました。 列は、すべての製品で正しいメッセージ値を一貫して表示するようになり、列の再設定を手動で必要とせずに信頼性の高い可視性を確保します。 （TGT-52777）
* 更新されたUIにアクティビティを保存すると、**[!UICONTROL おすすめのデータのダウンロード ] ボタンが表示されません。** 更新されたUIで、再保存後でも、特定の保存済みアクティビティに対して「[!UICONTROL おすすめデータをダウンロード ]」ボタンが表示されない問題を解決しました。 このボタンは、あらゆるアクティビティで一貫して表示されるようになったため、ユーザーは回避策を必要とせずにレコメンデーションデータを確実に書き出すことができます。 （TGT-53802）
* **コレクションから特定の製品を開くと、「リクエストされたリソースが見つかりませんでした」と返され、モーダルには閉じるオプションがありませんでした。** 更新されたRecommendations UIで、コレクションから特定の製品を開くと、「リクエストされたリソースが見つかりませんでした」エラーがトリガーされ、閉じるオプションなしで空白モーダルが表示される問題を解決しました。 モーダルで製品の詳細が正しく読み込まれるようになり、クローズオプションが常に使用できるようになりました。 （TGT-53986）
...
* 更新されたUIで[!DNL Adobe Analytics]をレポートソースとして使用すると、**[!UICONTROL 目標と設定]のエラーメッセージが表示されます。** 更新された[!UICONTROL 概要] UIで、「目標」セクションに「問題が発生しました」というエラーが表示される問題を解決しました。 ご要望の処理は完了できません。 [!DNL Adobe Analytics] （A4T）がレポートソースとして選択されたときに問題が解決しない場合は、[!DNL Adobe Client Care]にお問い合わせください。 [!UICONTROL Adobe Analytics]指標で目標が正しく表示されるようになり、レポートソース全体で一貫した可視性が確保されるようになりました。 （TGT-54021）
...
* **更新されたUIで複数のレポートオーディエンスを選択できません。** 更新されたUIで、アクティビティの編集時に、ユーザーが新しく作成した複数のレポートオーディエンスを同時に選択できない問題を解決しました。 複数のオーディエンスを一度に割り当てることができるため、レポートの設定における柔軟性と効率性が向上しました。 （TGT-53253）
...
* **更新されたUIで決定オファーを編集または置き換えることができません。** 更新されたUIで、[!UICONTROL 修正] パネルで決定オファーを編集または置き換えることができず、オファー名が空白で表示される問題を解決しました。 決定オファーは完全にアクセス可能で編集可能になり、従来のUIとの整合性が回復され、顧客がアクティビティ内で直接オファーを管理できるようになります。 （TGT-53884）
...
* **韓国語と日本語のUIのいくつかのローカライゼーションエラーを修正しました。** （TGT-54003、TGT-54004、TGT-54006、TGT-54007、およびTGT-54018）
...
* **エンティティ属性の一致を持つ属性によるプロモーションは、アクティビティの保存後にレコメンデーションキーを読み込めませんでした。** アクティビティの保存後に編集した際に、ルールタイプ [!UICONTROL  エンティティ属性の一致]を持つタイプ [!UICONTROL 属性によるプロモーション ]のプロモーションで、レコメンデーションキーが読み込まれなかった問題を修正しました。 この問題は、`customKeyId`がGraphQLを通じてリクエストされていないことが原因で発生しました。 プロモーション編集中にレコメンデーションキーが正しく読み込まれるようになりました。 （TGT-53117）
* **ExpBからExpAに切り替えると、推奨事項が視覚的に保持されます。** エクスペリエンス Bにレコメンデーションを挿入し、エクスペリエンス Aに切り替えると、レコメンデーションオファーボックスが表示される問題を解決しました。 これは視覚的な不整合のみでした。エクスペリエンスを切り替える際に変更が正しくレンダリングされるようになり、正確なUI動作が確保されました。 （TGT-53911）
* **エンティティ属性]が一致する[!UICONTROL 属性によるプロモーション ]のレコメンデーションキーが読み込まれません。** [!UICONTROL アクティビティの保存後に編集した際に、ルールタイプ [!UICONTROL  エンティティ属性の一致]を持つタイプ [!UICONTROL 属性によるプロモーション ]のプロモーションで、レコメンデーションキーが読み込まれない問題を解決しました。 GraphQLを通じてレコメンデーションキーが正しく取得され、プロモーションが期待どおりに表示され、機能するようになりました。 （TGT-53917）
* **更新されたUIで非表示のHTML要素に関する推奨事項を編集できません。** 非表示のHTML要素に適用されたレコメンデーションアクティビティを編集できない[!UICONTROL 新規作成]およびVEC UIの問題を解決しました。 この機能が期待どおりに動作するようになり、従来のUIとのパリティを復元し、要素の表示に関係なくレコメンデーションを変更できるようになりました。 （TGT-53953）
* **更新されたUIで、非表示のHTML要素に対するレコメンデーションアクティビティを編集できません。** 更新されたUIで、非表示のHTML要素に適用されたレコメンデーションアクティビティを編集できない問題を解決しました。 この機能が期待どおりに動作するようになり、従来のUIとのパリティを復元し、要素の表示に関係なくレコメンデーションを変更できるようになりました。 （TGT-53951）
* **更新されたUIに属性値が断続的に欠落するレコメンデーションカタログ。** 更新された[!UICONTROL Recommendations] UIで、製品フィードに存在する場合でも、カタログ検索リストで特定の属性値（メッセージなど）を断続的に表示できない問題を解決しました。 属性値が、列の再構成を必要とせずに検索結果で一貫して読み込まれるようになり、カタログ管理の信頼性と効率性が向上しました。 （TGT-52769）
* 更新されたUIに[!DNL Recommendations]件のアクティビティに&#x200B;**[!UICONTROL ダウンロードの推奨事項] ボタンがありません。** 更新された[!DNL Recommendations] UIで、レコメンデーションを使用するA/B アクティビティに対して[!UICONTROL  レコメンデーションをダウンロード ] ボタンが表示されない問題を解決しました。 ボタンが正しく表示され、レコメンデーションデータを従来のUIの機能と一致した形式で書き出せるようになりました。 （TGT-53768）
* 更新された概要UIに&#x200B;**[!UICONTROL おすすめデータのダウンロード ] ボタンがありません。** 更新された[!UICONTROL 概要] UIで、レコメンデーションを含むアクティビティに「[!UICONTROL  レコメンデーションデータをダウンロード ]」ボタンが表示されない問題を解決しました。 レコメンデーションデータを直接書き出すことができるため、従来のUIに切り替える必要はありません。 （TGT-53772）
* **アクティビティ条件を編集すると、更新されたUIに空白の画面が表示されることがあります。** 更新されたUIで「[!UICONTROL  エクスペリエンスの条件を編集]」をクリックすると、特定のアクティビティに対して空白の画面が表示される問題を解決しました。 条件エディターがすべてのアクティビティで確実に読み込まれるようになり、ユーザーが中断することなく編集できるようになりました。 （TGT-53961）
* **更新されたUIでシーケンス条件を編集できません。** 更新されたUIで[!UICONTROL  シーケンス条件]を編集しようとすると、条件ポップアップが読み込み中に停止したままになり、空白の画面が表示される問題を解決しました。 条件エディターが正しく読み込まれるようになり、ユーザーは中断することなくシーケンス条件を編集および更新できるようになりました。 （TGT-53985）
...
* **[!UICONTROL 多変量テスト ] （MVT）の場所とグラフ レポートの問題により、レポートの生成が妨げられました。** MVT アクティビティがTarget UIで[!UICONTROL 場所の貢献度]およびグラフ レポートの生成に失敗し、「問題が発生しました」というエラーが表示される問題を解決しました。 お客様のリクエストを完了できません。」 レポートがUI内で正しく読み込まれるようになり、完全に表示されるようになりました。 （TGT-53654）
* [!UICONTROL 要素]の貢献度レポートエラーにより、**MVT レポートが読み込まれません。** Target UIでMVT アクティビティレポートの読み込みに失敗し、「要素貢献度レポートを取得できませんでした」というエラーが表示される問題を修正しました。 レポートが正しく表示され、要素の貢献度が完全に表示されるようになりました。 （TGT-53691）
* **エクスペリエンスのターゲット設定] （XT）アクティビティの注文の詳細をCSV問題にエクスポートします。** [!UICONTROL [!UICONTROL 注文詳細をCSV]に書き出すオプションが誤ってXT アクティビティに表示され、空のファイルが返される問題を修正しました。 このオプションは、AP アクティビティに対してのみ表示されるようになりました。これにより、正確な書き出し機能を確保し、混乱を防ぐことができます。 （TGT-53798）
...
* **[!UICONTROL 変更を削除] ボタンの問題により、アクティビティの変更を削除できませんでした。** [!DNL Target] UIの[!UICONTROL 変更を削除] ボタンが機能せず、ユーザーがアクティビティ内の変更を削除できない問題を解決しました。 ボタンが期待どおりに動作するようになり、変更を遅滞なく確実に削除できるようになりました。 （TGT-53728）
* **更新されたUIで優先セレクターが認識されません。** 更新されたUIで、`data-target-component-id`などの優先セレクターがVEC内のCSS セレクターリストに表示されない問題を解決しました。 ユーザーは、動的に生成されたクラス名ではなく、好みの属性を確実に選択できるようになり、SPA ページの更新全体で安定したターゲティングを実現できます。 （TGT-53908）
* **アクティビティの場所の調整が[!UICONTROL 編集] ページと[!UICONTROL 概要] ページで一致しません。** [!UICONTROL 概要] ページのアクティビティの場所の番号が、[!UICONTROL  エクスペリエンスの編集] ページで行われた更新と一致しない問題を解決しました。 これにより、両方のビューで位置の一貫性が維持され、正確な整列が確保され、位置の抜けや番号の間違いを防ぐことができます。 （TGT-53960 および TGT-53954）
* **更新されたVECで[!UICONTROL  デザイン ] モードに戻すことができません。** 更新されたVEC UIで、[!UICONTROL 参照] モードで新しいページに移動した後、ユーザーが[!UICONTROL  デザイン ] モードに戻れなかった問題を解決しました。 [!UICONTROL  デザイン ]切り替えが正しく機能するようになり、変更をページ間でシームレスに適用できるようになりました。 （TGT-53988 および TGT-53993）
* **クエリパラメーターがアクティビティの概要に表示されません。** 更新されたUIで、アクティビティの[!UICONTROL 概要] ページにクエリパラメーターが表示されず、[!UICONTROL 概要]とページ配信URLの間に矛盾が生じる問題を解決しました。 クエリパラメーターが正しく表示されるようになり、アクティビティの場所が完全に表示され、ビュー間で一貫性が保たれます。 （TGT-53701）

+++

### [!DNL Target Standard/Premium] 25.10.1 （2025年10月22日（PT））

このリリースには、次のアップデートと修正が含まれています。

**アクティビティ**

+++ 詳細を見る
* **更新されたUI**&#x200B;で操作性の問題を解決しました。 [!UICONTROL  オブザーバー]は、従来のUIと同様に、[!UICONTROL  アクティビティを表示] オプションを使用してアクティビティをプレビューできるようになりました。 （TGT-51741）
* **[!UICONTROL Observer]のユーザーは、更新されたUIでアクティビティコンテンツを表示できるようになりました。** 更新されたアクティビティ UIで、監視者ロールのユーザーの表示を復元しました。 以前は、オブザーバーは変更、オファー、コンテンツの変更を表示できませんでした。これは、従来のUIで使用できる機能です。 （TGT-53785）
* **[!UICONTROL 承認者]のユーザーが、編集者権限エラーなしでアクティビティ目標を編集できるようになりました。** アクティビティ作成UIで、承認者レベルのユーザーが変更を高度な目標設定に保存するのをブロックする権限の問題を解決しました。 影響を受けるユーザーは、十分なアクセス権があるにもかかわらず、エディター権限を必要とする`403 Forbidden.Resource` エラーを受け取りました。 （TGT-53819）
...
* **複数のオーディエンスの選択が「このアクティビティのみ」レポートに復元されました。** アクティビティ作成UIで、[!UICONTROL 目標と設定]の[!UICONTROL このアクティビティのみ] セクションで複数のオーディエンスを選択できない問題を解決しました。 （TGT-53283）
* **オーディエンスベースのレポートグラフで、コンバージョンデータが正しく表示されるようになりました。** デフォルト以外のオーディエンスを選択する際にグラフが失敗する問題を[!UICONTROL Reports] タブで解決しました。 データや信頼度の指標は利用可能でしたが、視覚的なグラフには直線しか表示されておらず、分析は困難でした。 （TGT-53769）
* **[!UICONTROL  ターゲティング ] UIで、除外されたオーディエンスルールが明確に示されるようになりました。** アクティビティ作成UIの[!UICONTROL  ターゲティング ] セクションで、[!UICONTROL 除外]に設定されたオーディエンスルールが明確に表示されない問題を解決しました。 そのため、ターゲティングロジックを確認する際に、特に特定のURLを除くオーディエンスに対して、混乱が生じました。 （TGT-53809）
* **オーディエンス定義の値が、[!UICONTROL  ターゲティング ] タブで選択およびコピーできるようになりました。** アクティビティ作成インターフェイスの問題を解決しました。ユーザーが「[!UICONTROL  ターゲット ]」タブでオーディエンスルール値を選択およびコピーできませんでした。 この機能は従来のUIでは利用できましたが、更新されたUIでは利用できませんでした。 （TGT-53856）
...
* **zh_CN ページエディターコンテキストの「quote」の誤訳を修正しました。** 「quote」という用語が「报价」として不正確に翻訳され、商用価格見積もりを意味するzh_CN ロケールの文脈的な翻訳エラーを修正しました。 タイポグラフィ/見出しスタイル/ブロック引用セクションでは、意図する意味は、価格設定ではなくフォーマット要素である引用ブロックを指します。 （TGT-53841）
* **zh_CN ページエディターコンテキストの「引用符が削除されました」の誤訳を修正しました。** 「quote removed」が「移除了报价」として不正確にレンダリングされるzh_CN ロケールの翻訳エラーを修正しました。これは、商用価格見積もりを意味します。 タイポグラフィ/見出しスタイル/ブロック引用セクションでは、この用語は、価格設定ではなく書式要素（引用ブロック）を指します。 （TGT-53843）
* **zh_CN ページエディターコンテキストの「quote rearranged」の誤訳を修正しました。** 「quote rearranged」が「重新排列了报价」として不正確に翻訳され、商用価格見積もりを意味するzh_CN ロケールの文脈的な翻訳エラーを修正しました。 タイポグラフィ/見出しスタイル/ブロック引用セクションでは、この用語は、価格設定ではなく書式要素（引用ブロック）を指します。 （TGT-53844）
* **zh_CN ページエディターのコンテキストで「quote changed」の誤訳を修正しました。** 「見積もりが変更されました」が「更改了报价」として不正確にレンダリングされるzh_CN ロケールの翻訳エラーを修正し、商用価格見積もりを示唆しました。 タイポグラフィ/見出しスタイル/ブロック引用セクションでは、この用語は、価格設定ではなく書式要素（引用ブロック）を指します。 （TGT-53845）
...
* レコメンデーションの&#x200B;**CSS セレクターの変更が正しく保存されるようになりました。** アクティビティ作成UIで、レコメンデーションのプレースメント用にCSS セレクターを更新できない問題を解決しました。 保存後に変更が元に戻され、ターゲティングコンテナの更新がブロックされます。 （TGT-53835）
* **ページ読み込みイベントの選択は、レコメンデーションの変更でも保持されるようになりました。** レコメンデーションのイベントタイプを[!UICONTROL 表示]から[!UICONTROL  ページ読み込み]に切り替える際に、ユーザーが変更を保存できないアクティビティ作成UIの問題を解決しました。 選択は正常に行われましたが、移動して元に戻り、アクティビティの公開がブロックされました。 （TGT-53957）
...
* **&quot;[!UICONTROL 注文の詳細をCSV]&quot;にエクスポートすると、完全なデータがダウンロードされるようになりました。** 更新された[!UICONTROL 概要] UIで、有効なレポートデータが存在する場合でも「[!UICONTROL 注文の詳細をCSV]に書き出し」オプションが空のファイルをダウンロードする問題を解決しました。 （TGT-53787）
...
* **IMS プリフィルターが追加され、組織をまたいだデータ露出からGQL エンドポイントを保護しました。** licenseGroupsおよびtargetProperties GraphQL エンドポイントに影響するAdmin タブのセキュリティの脆弱性を解決しました。 この問題は、組織間の製品データにアクセスするために悪用される可能性のある管理者クライアントトークンでJIL APIを使用したことに起因しています。 （TGT-53837）
...
* **アクティビティ作成UIでオーサリングの安定性が復元されました。** オーサリングが失敗し、リンクが予期せずクリック可能になり、ユーザーがページから離れるようにリダイレクトされるVEC UIの断続的な問題を解決しました。 （TGT-53153）
* **アクティビティ作成UIに保存されたアクティビティの編集が復元されました。** 変更を保存した後、ユーザーがアクティビティを編集できない問題を解決しました。 影響を受けるアクティビティは「[!UICONTROL 初期変更の適用]」で停止したままになり、さらに更新がブロックされ、[!UICONTROL  キャンセル ] ボタンが非表示になりました。 （TGT-53631）
* **VECは、「[!UICONTROL 初期修正の適用]」で停止しなくなりました。「**」 多数の変更を伴うエクスペリエンスを読み込む際に長い遅延が発生するVECのパフォーマンスの問題を解決しました。 影響を受けるユーザーは、特にExperience B シナリオで、UIが「[!UICONTROL 初期変更の適用]」で数分間停止していました。 （TGT-53727）
* **VECは、ルート要素なしで変更を読み込むようになりました。**
明確なルート要素を欠く変更を読み込むと、エクスペリエンスが停止する問題をVECで解決しました。 これらの変更により、UIが「A[!UICONTROL 最初の変更を適用する]」に無期限にハングしていました。 （TGT-53799）
* **アクティビティの変更の保存が正常に機能するようになりました。** 新しい作成UIで、アクティビティの目標と詳細設定を編集する際にユーザーが変更を保存できない権限に関連する問題を解決しました。 影響を受けるユーザーは、適切なアクセス権を持っているにもかかわらず、赤いエラーリボンと「Forbidden.Resource」メッセージが表示されました。 （TGT-53816）
* **VEC UIで、ビュー間でエクスペリエンスの変更が保持されるようになりました。** エクスペリエンス開発に影響を与えた更新されたVECの複数の問題を解決しました。 HTML オファーを使用する場合やビューを切り替える場合は、特に、変更内容が正しく保持されませんでした。 （TGT-53825）
* **変更が複数のエクスペリエンスにまたがる場合、すべてのビューが正しく表示されるようになりました。** 複数のビューに変更が適用された場合に1つのビューのみが表示されるアクティビティ作成UIの問題を解決しました。 修正が正しく適用されていても、ホバーツールチップで関連するビューをすべて一覧表示できませんでした。 （TGT-53827）
* **VECは、「[!UICONTROL 初期修正の適用]」で断続的に停止しなくなりました。「**」 エクスペリエンスの読み込みに失敗し、「[!UICONTROL 初期修正の適用]」で停止したままになるVECの断続的な問題を解決しました。 この動作は一貫性がなく、リダイレクトループまたは手動キャッシュのクリアが必要な場合がありました。 （TGT-53916）
* **VECの読み込み問題を再現できませんでした。** 既存のアクティビティを編集する際に、VECが「[!UICONTROL 初期変更の適用]」で停止したままになる問題を調査しました。 この振る舞いは、親エレメントを欠いたHTMLの内容に関連していると疑われた。 繰り返しを監視し続け、HTML オファーに構造化コンテナを使用して安定性を確保することをお勧めします。 （TGT-53972）
* VECの&#x200B;**[!UICONTROL デザイン ] モードは、[!UICONTROL 参照] モードと断続的に動作しなくなりました。** [!UICONTROL  デザイン ] モードが[!UICONTROL 参照] モードのように断続的に動作し、クリック可能な`<a>` リンクを許可し、要素の選択を妨げるUIの問題を解決しました。 これにより、ホバーボックスが消え、修正ワークフローがブロックされました。 （TGT-53136）
* **コンポーザー] モードの[!UICONTROL  ボタンのクリックにより、ページのリダイレクトがトリガーされなくなりました。** 更新されたVEC UIで、[!UICONTROL Composer] モードでボタンをクリックすると、ボタンのターゲット URLに予期しないリダイレクトが発生する問題を解決しました。 これにより、call-to-action（CTA）要素を編集できず、オーサリングワークフローが中断しました。 （TGT-53137）
* **自動パーソナライゼーションアクティビティのオファーのコード更新が、エラーなしで保存されるようになりました。** [!UICONTROL Automated Personalization] アクティビティでオファーコードを更新する際に「[!UICONTROL Invalid user input]」エラーが発生する新しい作成UIの問題を解決しました。 入力が有効な場合でも、エラーによりユーザーが変更を保存できなくなります。 （TGT-53586）
* VECの&#x200B;**[!UICONTROL デザイン ] モードで、編集可能なコンポーネントのリンク ナビゲーションがブロックされるようになりました。** 更新されたVECで、[!UICONTROL  デザイン ] モードでもボタンやリンクなどのクリック可能な要素がページリダイレクトをトリガーする問題を解決しました。 この動作は[!UICONTROL 参照] モードを模倣し、ユーザーが主要コンポーネントを変更することを妨げました。 （TGT-53696）
* **&quot;[!UICONTROL 要素をクリックしました]&quot;指標が、リダイレクトまたはエラーなしで機能するようになりました。** 新しい作成UIで、「[!UICONTROL 要素をクリック ]」コンバージョン指標を選択すると、予期しないリダイレクトと空白の画面が発生する問題を解決しました。 セットアップ中にボタンをクリックすると、要素を登録する代わりにナビゲーションがトリガーされ、「[!UICONTROL 要素が見つかりません]」エラーが発生しました。 （TGT-53817）
* **編集中に既存のアクティビティが無限の読み込みループで停止しなくなります。** 新しい作成UIで、VECで既存のアクティビティを編集すると、ページが無限の読み込みループで停止する問題を解決しました。 この問題は、新しく作成されたアクティビティには影響せず、ページ上の既存の変更によってトリガーされました。 （TGT-53913）
* **変更を加えた既存のアクティビティページが、VECで正しく読み込まれるようになりました。** 更新されたVECで、変更が保存された既存のアクティビティを編集する際に、ページが読み込み段階で停止する問題を解決しました。 新しいアクティビティは問題なく読み込まれましたが、以前に設定したアクティビティはレンダリングできませんでした。 （TGT-53967）

+++

### [!DNL Target Standard/Premium] 25.9.3 （2025年9月30日）

このリリースには、次の機能強化と修正が含まれています。

+++[!UICONTROL オーディエンス]

* **オーディエンス除外ルールが、[!DNL Target] UIに含まれているとして正しく表示されませんでした。** 除外ルールで設定されたオーディエンスは、アクティビティ内でターゲティングを編集する際に含まれるように表示されます。 除外ロジックは実行中に正しく適用されましたが、UIはルールを正確に反映できず、「除外」ラベルが省略されました。 [!DNL Target] UIで、オーディエンス設定とターゲティングワークフローの両方に除外ルールが正しく表示され、キャンペーン設定の明確性と一貫性が確保されるようになりました。 （TGT-53808）
* **「[!UICONTROL  ターゲティング ]」セクションでは、オーディエンスルールが除外に設定されていることを示していません。** 除外ロジックで設定されたオーディエンスが、アクティビティ作成UIの[!UICONTROL  ターゲティング ] セクションに含まれるように誤って表示されました。 バックエンドが除外ルールを正しく適用しましたが、UIは除外ルールを視覚的に表現できず、「除外」ラベルが省略され、キャンペーンの設定中に混乱が生じました。 「[!UICONTROL  ターゲティング ]」セクションに、除外ルールが明確に表示されるようになりました。これにより、オーディエンス設定とターゲティングのビジュアライゼーションの一貫性が確保されます。 （TGT-53809）

+++

+++ローカリゼーション

* **簡体字中国語の「詳細を表示」の用語の不整合を修正しました。**
以前は、「詳細」という用語は、簡体字中国語（zh_CN）ロケールで「详情」と誤って翻訳され、確立された用語ガイドラインに違反していました。 これは、用語ベースとの一貫性を確保するために、「详细信息」に修正されました。 （TGT-53741）

+++

+++[!UICONTROL レコメンデーション]

* **推奨ボックスが見つけにくく、VECで選択できませんでした。** （VEC）でレコメンデーションオファーを追加した後、左側のパネルで修正をクリックしても、ページ上の対応するレコメンデーションボックスがハイライト表示またはスクロールされませんでした。 これにより、特にセレクターの下に隠されている場合や最小限にスタイル設定されている場合は、オファーを見つけたり編集したりすることが困難になりました。 レコメンデーションの修正をクリックすると、関連する要素が正しくハイライト表示され、スクロールされるようになりました。これにより、更新されたアクティビティ作成プロセスでの使いやすさと編集効率が向上しました。 （TGT-52571）
* アクティビティの保存後、**レコメンデーションセレクターが誤って書き換えられました。** VEC内の要素にレコメンデーションを追加する際、セレクターは最初は正しかったのですが、アクティビティを保存して再度開くと、汎用セレクターに変更されました。 元のセレクターを手動で復元しようとすると、検証エラーが発生しました。 保存後もレコメンデーションセレクターが正確に保持され、更新されたアクティビティ作成プロセスで信頼性の高いターゲティングと編集性が確保されるようになりました。 （TGT-53709）
* **既存のアクティビティを変更する際に、条件コンテンツを編集できませんでした。** アクティビティを編集すると、[!UICONTROL 条件] コンテンツセクションが無効になり、ボタンがグレー表示され、応答が返されなくなりました。 この問題は、アクティビティの更新中に[!UICONTROL 条件]設定が完全に編集可能であることを確認することで解決されました。 お客様は、選択を切り替えたり、回避策を使用したりすることなく、[!UICONTROL 条件]のコンテンツを変更できるようになりました。これにより、更新されたアクティビティ作成プロセスの柔軟性と使いやすさが向上しました。 （TGT-53812）
* **アクティビティ内で条件を編集できませんでした。** アクティビティ内から条件にアクセスする場合、[!UICONTROL 条件を編集]および[!UICONTROL 条件を削除] オプションが無効になりました。 ただし、[!UICONTROL Recommendations] タブを使用して、同じ条件を正常に編集できます。 条件は、アクティビティ編集ワークフローと「[!UICONTROL Recommendations]」タブの両方から完全に編集可能になり、一貫性のある効率的な編集エクスペリエンスを実現します。 （TGT-53814）

+++

+++[!UICONTROL レポート]

* **A[!UICONTROL 自動Personalization] アクティビティでアドホックオファーを生成すると、レポートの不整合が発生しました。** [!UICONTROL Automated Personalization] （AP）アクティビティで「アドホックオファーを生成」機能を使用すると、レポートが不正確になりました。 具体的には、オファーIDが場所をまたいで再利用されたため、レポートデータが誤って帰属したり上書きされたりしていました。 現在、アドホックオファーは、場所ごとに異なるIDで生成され、設定されたあらゆるエクスペリエンスで正確なトラッキングとレポートを実現しています。 （TGT-53757）
* JavaScript エラーが発生したため、**アクティビティレポートを読み込めませんでした。** 特定のアクティビティの「[!UICONTROL  レポート ]」タブにアクセスすると、顧客に「問題が発生しました」というメッセージが表示されました。 エラーは、JavaScriptの例外が原因で発生しました：`getAnalyticsReportSummary` GraphQL呼び出し中にトリガーされた、未定義のプロパティを読み取れません（「indexOf」を読み取ります）。 レポートが正しく読み込まれるようになり、更新されたアクティビティ作成ワークフローで同様のエラーが発生するのを防ぐため、エラー処理が改善されました。 （TGT-53797）
* **スクロールバーを操作すると、レポートがクラッシュする。** 「[!UICONTROL  レポート ]」タブのスクロールバーをクリックすると、ページがクラッシュし、JavaScript エラーが発生しました。
  エラーやクラッシュを引き起こさずに、`SyntaxError: Failed to execute 'querySelector' on 'Element': '[data-key="a-currentcopy"hiretalent""]' is not a valid selector.`件のレポートの読み込みとスクロールが正しく行われるようになりました。 （TGT-53828）
* **レポートにプライマリ指標が表示されませんでした。** mboxを使用してコンバージョン指標として設定された主な指標がアクティビティレポートにありませんでした。 メトリック名またはmbox名で検索しても結果が得られず、主要なパフォーマンスデータが表示されません。 プライマリ指標が「[!UICONTROL  レポート ]」タブに正しく表示されるようになり、キャンペーンのパフォーマンスを正確に追跡および分析できるようになりました。 （TGT-53773）
* **水平スクロールバーを操作すると、更新されたUIの[!UICONTROL  レポート ] タブがクラッシュする。** 水平スクロールバーを使用してビュー外の指標にアクセスする際に、[!UICONTROL  レポート ] ビューが「問題が発生しました」エラーで断続的にクラッシュしました。 スクロールバーが確実に機能するようになり、ズームアウトやShift キーを押しながらスクロールするなどの回避策を必要とせずに、すべての指標を表示および分析できるようになりました。 （TGT-53824）

+++

+++[!UICONTROL Visual Experience Composer] （VEC）

* **VECでパンくずリストをクリックしても、編集メニューが表示されない。**
（VEC）のパンくずリストを使用してHTMLのエレメントを選択すると、編集メニューが断続的に表示されなかったり、すぐに表示されなくなったりして、エレメントの選択が信頼できないようになります。 パンくずリストを使用して移動する際に、編集メニューが一貫して表示されるようになり、更新されたアクティビティ作成プロセスでエレメント選択ワークフローが改善されました。 （TGT-52873）
* **コンテキスト メニューが断続的にVECに表示されませんでした。** 更新されたVEC UIのコンテキストメニューは、要素をクリックしても常に表示されないため、編集オプションにアクセスすることが困難です。 エレメントの選択時にコンテキストメニューが確実に表示されるようになり、更新されたアクティビティ作成プロセスでの編集ワークフローと全体的な使いやすさが向上しました。 （TGT-53015）
* **VEC内の特定の要素に対して、コンテキストメニューが表示されませんでした。** 更新されたVECで特定の要素を選択する際にコンテキストメニューが表示されず、修正を適用するのが困難でした。 サポートされているすべての要素に対してコンテキストメニューが一貫して表示されるようになり、更新されたアクティビティ作成ワークフローでの編集エクスペリエンスの信頼性と使いやすさが向上しました。 （TGT-53248）
* VECでパンくずリストを使用すると、最初のクリックで&#x200B;**コンテキストメニューが消えました。** VECのパンくずリストから親エレメントを選択すると、コンテキストメニューが一時的に表示され、その後表示されなくなり、編集オプションにアクセスすることが困難になりました。 パンくずリスト内でエレメントを移動する際に、コンテキストメニューが表示され、機能するようになり、更新されたアクティビティ作成プロセスでエレメント選択ワークフローの信頼性が向上しました。 （TGT-53424）
* **VECの最上位の要素にコンテキストメニューが表示されませんでした。** VECのパンくずリストで`<div>`や`<main>` タグなどのトップレベル要素を選択しても、コンテキストメニューがトリガーされず、それ以上の編集が行われませんでした。 トップレベルのコンテナを含め、サポートされているすべての要素にコンテキストメニューが一貫して表示されるようになり、アクティビティ作成ワークフローの柔軟性と使いやすさが向上しました。 （TGT-53770）
* 特定のページ上の&#x200B;**要素は、VECで編集できませんでした。** 更新されたVECで、ページ上の特定の要素を選択または編集できませんでした。 この問題はそのページに分離され、同じアカウント内の他のページには影響しませんでした。 ページ上のすべての要素が、必要に応じて選択および編集可能になり、アクティビティ作成ワークフローの機能を完全に復元できるようになりました。 （TGT-53353）
* **VECでの要素の選択中に子要素を表示する際のワークフローを改善しました。** アクティビティ作成時の操作性と精度を向上させるために、親HTML要素にカーソルを合わせたり選択したりすると、VECに子要素が表示されるようになりました。 この機能強化により、ユーザーはページの構造をより深く理解し、より正確な変更を行うことができ、更新されたUIでの編集ワークフローが合理化されます。 （TGT-53416）
* **既存のアクティビティの要素を修正バーを使用して編集できませんでした。** 以前に作成したアクティビティを編集する際に、ページ上の特定の要素に対して修正バーをアクティブ化できず、更新が妨げられました。 この問題は主に改変活動で観察され、新しく作成された活動で再現することが困難でした。 修正バーが一貫して表示され、サポートされているすべての要素を編集できるようになりました。これにより、更新されたアクティビティ作成ワークフローで信頼性と使いやすさが向上しました。 （TGT-53013）

+++

+++[!UICONTROL  ワークスペース ]

* **アクティビティを別のワークスペースに複製すると、「無効なユーザー入力」エラーが発生しました。** あるワークスペースから別のワークスペースにアクティビティを複製しようとすると、「InvalidProperty.Json - Unrecognized property name &#39;content&#39;」というエラーが発生しました。 この問題は、複製処理中にアクティビティメタデータが不適切に処理されていることが原因で発生しました。 検証エラーをトリガーすることなく、ワークスペース間でアクティビティを正常に複製できるようになり、アクティビティのデプロイメントワークフローがよりスムーズになりました。 （TGT-53731 および TGT-53736）
...
* **無効なオーディエンス IDのため、アクティビティをコピーできない問題を修正しました。** 更新されたアクティビティ作成プロセスでアクティビティをコピーしようとしているお客様で、無効なオーディエンス ID （–1752722444307など）が原因でエラーが発生しました。 このバックエンド検証の問題により、同じワークスペース内のアクティビティの重複が防止されました。 この問題は解決され、オーディエンス関連のエラーなしでアクティビティを正常にコピーできるようになりました。 （TGT-53717）
* **コンテンツの管理[!UICONTROL  モーダル ]の[!UICONTROL Automated Personalization] アクティビティで、アクティビティのみのオーディエンスに対して無効なユーザー入力エラーが表示される問題を修正しました。** AP アクティビティの[!UICONTROL  コンテンツを管理] モーダルでアクティビティのみのオーディエンスを設定する際に、無効なユーザー入力エラーが発生しました。 この問題は、以前はオーディエンスが正常に使用されていたにもかかわらず発生しました。 統合されたオーディエンス設定が、検証エラーをトリガーせずに正しく保存されるようになりました。 （TGT-53749）
...
* **最初のアクティビティ作成中に、オファー決定オプションが一貫して表示されるようになりました。** 更新されたUIで、A/B アクティビティの初回作成フロー中に[!UICONTROL  オファー決定] オプションが表示されない問題を解決しました。特に、オファー決定が有効なテナントでシークレットモードでアクセスした場合に発生します。 このオプションは、[!UICONTROL  ターゲティング ] ステップに移動して[!UICONTROL  エクスペリエンス ]に戻った後にのみ表示されます。 この修正により、[!UICONTROL  オファー決定] オプションが最初のセットアップ中にすぐに使用できるようになり、ユーザビリティが向上し、混乱が軽減されます。 （TGT-51888）
...
* **リダイレクトオファーが`includeContent=true`時にペイロードに`redirectOptions`を含めなかった問題を修正しました。** `includeContent=true `を含むリダイレクトオファーを取得する顧客に、応答ペイロードの`redirectOptions` フィールドがありませんでした。 この不整合は、オファーのコピーやアクティビティの作成などのワークフローに影響を与えます。 コンテンツが要求されたときに、リダイレクトオファーに`redirectOptions`が正しく含まれるようになりました。 （TGT-53737）
...
* **更新されたUIで作成された[!UICONTROL Recommendations] アクティビティに対するクリック追跡が復元されました。** 更新されたUIで作成された[!UICONTROL Recommendations] アクティビティがクリックトラッキングを登録できず、報告されたコンバージョンが0になる問題を解決しました。 従来のUIで構築されたアクティビティでは、クリック数が正しく追跡され、予想どおりにコンバージョンが報告されます。 この修正により、更新されたUIで作成されたRecommendations アクティビティに正しいトラッキング属性が含まれるようになり、コンバージョンレポートとA4T指標との整合性が復元されます。 （TGT-53287）
* **Recommendation アクティビティのクリック追跡が復元されました。** 更新されたUIで作成された[!UICONTROL Recommendations] アクティビティがクリックトラッキングを登録できず、報告されたコンバージョンが0になる問題を解決しました。 レガシーUIがトラッキング ID （`at-track-click`）を[!UICONTROL Recommendations] コンテンツに正しく適用しましたが、更新されたUIが誤ってプレースホルダー（`__recsClickTrackIdPlaceholder__`）を挿入したため、バックエンドのトラッキングが妨げられました。 この修正により、[!DNL Recommendations] コンテンツに正しいトラッキング IDが含まれるようになり、コンバージョンレポートとA4T指標との整合性が復元されます。 （TGT-53496）
* 更新されたUIで&#x200B;**コレクションエディターのクラッシュが解決されました。** 更新された[!UICONTROL Visual Experience Composer] （VEC） UIで、エディターパネルからコレクションを開くと、ページがTypeErrorでクラッシュする問題を修正しました：「未定義」のプロパティを読み取れません（「customLocale」を読み取ります）。 このエラーは、[!UICONTROL Recommendations]およびA/B テストを含む複数のアクティビティタイプで発生しました。 （TGT-53703）
* **VECで復元された選択したコレクションを削除するオプション。** ユーザーが[!UICONTROL Recommendations] アクティビティで選択したコレクションのみを置き換えることができるが、完全に削除できないVECの問題を修正しました。 この制限により、代替なしでコレクションをクリーンに削除する必要があるユースケースがブロックされました。 この修正により、選択したコレクションを削除する明確なオプションが導入され、アクティビティの設定と従来のUIの動作との整合性の柔軟性が向上しました。 （TGT-53652）
* **カスタム条件コレクションが[!UICONTROL Recommendations] UIに正しく表示されるようになりました。** Recommendations インターフェイスで、カスタム条件を使用して作成されたコレクションで製品結果が表示されない問題を修正しました。 標準の属性ベースのコレクションは期待どおりに機能しますが、カスタムフィルターを使用している場合、有効なカタログが一致しているにもかかわらず、「結果が見つかりません」が返されます。 この修正により、カスタム属性を使用したコレクションが[!UICONTROL 製品] タブに正しく入力され、パーソナライズされたレコメンデーションワークフローの機能を完全に復元できるようになりました。 （TGT-53653）
* **最初に[!UICONTROL 製品] ページを開いたときにコレクションに製品が表示されない問題を修正しました。** [!UICONTROL Recommendations] セクションのコレクションにアクセスする顧客は、[!UICONTROL 製品] ページを最初に開いたときに空の製品結果を経験しました。 この問題は、GraphQL クエリのバックエンドエラーが原因で、カスタム条件を持つコレクションの商品データを読み込めませんでした。 問題が解決され、環境の切り替えを必要とせずに製品が正しく表示されるようになりました。 （TGT-53694）
* **フォームベースの[!UICONTROL Recommendations] アクティビティでコレクションを削除できない問題を修正しました。** [!UICONTROL  フォームベースのExperience Composer]を使用して[!UICONTROL Recommendations] アクティビティを作成しているお客様は、以前に選択したコレクションの選択を解除できませんでした。 UIでは、保存する前にコレクションを選択する必要があり、ユーザーが「すべてのコレクション」に戻せませんでした。 この動作はVEC機能に合わせて更新され、お客様は選択したコレクションなしで保存でき、期待どおりに「すべてのコレクション」にデフォルト設定されています。 （TGT-53708）
* **コレクションまたはフィルタールール値が欠落しているため、属性でプロモーションを設定できない問題を修正しました。** アクティビティ作成プロセスの属性でプロモーションを設定しているお客様で、プロモーションにコレクション IDまたはフィルタールール値が欠落していることを示すエラーが発生しました。 設定が完了したように見えても、この検証の問題により進行がブロックされます。 属性で設定すると、プロモーションを正常に保存できるようになりました。 （TGT-53750）
* **エクスペリエンス名が重複しているため、アクティビティを保存できない問題を修正しました。** 条件とデザインの特定の組み合わせを含むアクティビティを保存する際に、無効なユーザー入力エラーが発生しました。 設定が有効と表示された場合でも、エクスペリエンス名が重複していることによってエラーがトリガーされました。 異なる設定を持つアクティビティを、名前の競合なしで保存できるようになりました。 （TGT-53805）
* **属性で設定されたプロモーションの検証が無効のままになる問題を修正しました。** すべての必須フィールドが正しく入力されている場合でも、アクティビティ作成プロセスで属性ごとにプロモーションを設定する際に、永続的な検証エラーが発生しました。 この問題は、[!UICONTROL Recommendations] ワークフローの検証ロジックが正しくないことが原因で発生しました。 属性ベースのプロモーションが検証され、期待どおりに保存されるようになりました。 （TGT-53811）
* **ライブ [!UICONTROL Recommendations] アクティビティにプロモーションを適用するとエラーが発生する問題を修正しました。** 有効な設定の詳細を指定した後でも、フロントプロモーションをライブ [!UICONTROL Recommendations] アクティビティに適用しようとすると、「プロモーションにコレクション IDまたはフィルタリングルール値が見つかりません」というエラーが発生しました。 検証エラーをトリガーすることなく、ライブアクティビティにプロモーションを正常に適用できるようになり、更新されたアクティビティ作成UIでよりスムーズなエクスペリエンスを実現できます。 （TGT-53738）
* **Recommendations] UI [!UICONTROL 内のコレクションに製品が表示されない問題を修正しました。** 単一テナントの顧客から、新しいUIの[!UICONTROL Recommendations] セクションで特定のコレクションを表示する際に、製品リストを読み込めなかったことが報告されました。 環境を切り替えることで一時的に問題が解決されましたが、この回避策ではユーザーエクスペリエンスが低下しました。 環境の切り替えを必要とせずに、製品エンティティが一貫して読み込まれるようになりました。 （TGT-53783）
* **アクティビティ作成UIの1行に基準とデザインが表示されない問題を修正しました。** 以前は、アクティビティ作成プロセスの基準とデザインが圧縮された形式で表示されていたため、顧客が個々のアイテムを表示および管理することが困難でした。 各基準とデザインが独自の行に表示され、更新されたUIでの読みやすさと使いやすさが向上しました。 （TGT-53818）
...
* **[!UICONTROL 合計収益]指標が、アクティビティレポートからのCSV書き出しに含まれるようになりました。** 更新された[!UICONTROL 概要] UIで、合計収益がアクティビティレポートビューに正しく表示されたが、CSV エクスポートから$0と表示される問題を解決しました。 この不一致により、ユーザーはオフライン分析とレポート用に書き出されたデータに頼ることができませんでした。 （TGT-53325）
* **[!UICONTROL 総売上]指標が、アクティビティレポートからのCSV書き出しに含まれるようになりました。** 更新されたUIで、アクティビティレポートビューに[!UICONTROL 総売上]指標が正しく表示されていたが、CSV書き出しから見つからなかった問題を解決しました。 この不一致により、ユーザーはダウンロードされたレポートの完全なパフォーマンスデータにアクセスできませんでした。 この修正により、[!UICONTROL 総売上]の値がCSV書き出しに正確に含まれるようになり、アプリ内レポートとオフライン分析の一貫性が復元されます。 （TGT-53330）
* **指標が有効になっていない[!UICONTROL  グラフ ビュー]のエラーメッセージが改善されました。** 関連する[!DNL Analytics] レポートスイートでリクエストされた指標が有効になっていない場合、[!UICONTROL  グラフビュー]が一般的な「問題が発生しました」メッセージを表示するVECの問題を修正しました。 この問題は、バックエンド GraphQLの応答で`not_enabled_metric` エラーがトリガーされました。 この修正により、あいまいなエラーは、ユーザーが[!DNL Analytics]の設定の問題を特定するのに役立つより有益なメッセージに置き換わり、混乱を減らし、不必要なサポートのエスカレーションを減らすことができます。 （TGT-53577）
* **レポート期間がサポートされている90日間の制限を超えた問題を修正しました。** 「[!UICONTROL  レポート ]」セクションの「[!UICONTROL 過去X日間]」フィルターを使用しているお客様は、90日を超える期間を選択できたため、パフォーマンスの問題やデータが不完全になる可能性があります。 フィルターが更新され、最大90日間が適用され、一貫性のある信頼できるレポートが作成されるようになりました。 （TGT-53795）
* **選択した環境ではなく、デフォルトの環境を使用してパフォーマンス CSV レポートが生成される問題を修正しました。** 以前、お客様がレポート設定の環境を変更し、パフォーマンスレポートを生成した場合、結果のCSVには、選択した環境ではなくデフォルト環境のデータが含まれていました。  UIが`environmentId` パラメーターを正しく渡すようになり、選択した環境がレポートに反映されるようになりました。 さらに、エラー処理が改善されました。 CSVの生成中にGraphQL エラーが発生した場合、空のCSV ファイルを生成する代わりに、UIに明確なエラーメッセージが表示されるようになりました。 （TGT-53064）
* **Analytics for Target （A4T）レポートで[!UICONTROL  グラフ ビュー]にデータを表示できなかった問題を修正しました。** A4T統合で[!DNL Target]を使用しているお客様が、A/B テストアクティビティの「レポート」タブでグラフ表示に切り替える際に、「問題が発生しました」エラーが発生しました。 [!UICONTROL  テーブル ビュー]は正しく読み込まれましたが、[!UICONTROL  グラフ ビュー]は、選択した時間範囲の指標の傾向をレンダリングできませんでした。 [!UICONTROL  グラフビュー]では、サポートされているすべての指標に対して期待どおりにパフォーマンスデータが表示されるようになりました。これにより、更新されたUIでの可視性とレポート精度が向上しました。 （TGT-53573）
...
* **エレメントのメタデータが、パンくずメニューのホバーに表示されるようになりました。** VECのパンくずメニューを改善して、項目にカーソルを合わせたときにID、クラス、名前などの追加の要素の詳細を表示できるようにしました。 この機能強化により、アクティビティの設定中に、より簡単に要素を識別および区別できるようになります。 （TGT-53409）
* **パンくずホバーで、VEC内の対応する要素がハイライト表示されるようになりました。** [!UICONTROL Visual Experience Composer]を改善して、パンくずメニューの項目にカーソルを合わせると、エディターで対応する要素がハイライト表示されるようになりました。 エレメントの種類（コンテナ、太字テキスト、ボタンなど）も表示されます。 このビヘイビアーは兄弟エレメントに適用され、検証リストに基づいて、SVGなどのサポートされていないタイプを除外します。 （TGT-53411）
* **VEC修正ワークフローで保存されていない変更アラートが復元されました。** カスタムコード変更の未保存の変更について、ユーザーに通知されなくなったVECの問題を修正しました。 従来のUIとは異なり、新しいエクスペリエンスでは、パーソナライゼーションエディターから移動したり、閉じたりする際にプロンプトが表示されないため、進行状況が誤って失われていました。 この修正により、カスタムコードを含むすべての変更タイプに対するアラートが復元され、保存せずに終了する前にユーザーに警告が表示されます。 （TGT-53435）。
* **VECでのページ更新中に、カスタムコードの変更が保持されるようになりました。** Web サイトの更新中にカスタムコードの変更が失われたVECの問題を修正しました。 この問題は、複数の再読み込みがあるページで発生し、エディターがリセットして保存されていない変更を元に戻しました。 この修正により、編集中にページが再読み込みされても、カスタムコードの編集が維持され、誤って進行状況が失われるのを防ぐことができます。 （TTGT-53501）
* **VEC内のサイト アクセスに関するログインの問題が解決されました。** ユーザーがVEC経由でサイトにアクセスする際にサイトにログインできない問題を修正しました。 ログインフローは、ユーザーをログインページに繰り返しリダイレクトし、アクティビティの設定とプレビューを妨げました。 この修正により、VECがログイン動作を妨害しなくなり、認証済みユーザーに期待されるアクセスが復元されます。 （TGT-53524）
* VEC Helper拡張機能で&#x200B;**Cookieの重複の問題が解決されました。** プレビューリンクを介したQA中に、[!UICONTROL Adobe Experience Cloud Visual Editing Helper]拡張機能が`at_qa_mode` Cookieを複製していた問題を修正しました。 プレビューインデックスを手動で切り替える場合、ドメイン間で値が競合する複数のCookieが作成され、テスターが確実にバリエーションを切り替えることができませんでした。 この動作は、Target UIの外部でも確認され、内部アカウントとクライアントアカウントの両方に影響を与えました。 この修正により、重複したエントリを防ぎ、ドメインスコープを調整することで、Cookieの処理が一貫性を確保し、Cookieのクリーンアップを手動で必要とせずにシームレスにバリアントを切り替えることができます。 （TGT-53579）
* **特定のホームページの要素をクリックするとメモリが漏れる問題を修正しました。** このホームページでアクティビティを作成する顧客は、ページ要素を操作する際にメモリリークを経験しました。 この問題は、過剰なコンソール警告と、サブフレームでのメモリ使用量の増加に関連しており、特に不正なsrcset属性に関連していました。 インタラクション中もメモリ使用量が安定したままになります。 （TGT-53761）
* **特定のアクティビティを読み込むと、VECがクラッシュし、空白の画面が表示される問題を修正しました。** 特定のテナントの顧客から、エディターが特定のアクティビティを読み込めなかったことが報告されました。 VECがクラッシュして空白の画面が表示される前に、「初期変更の適用」で停止したままになりました。 VECは、更新されたアクティビティ作成プロセスでエラーなく、影響を受けるアクティビティを読み込むようになりました。 （TGT-52932）
* **Automated Personalization] アクティビティの[!UICONTROL  コンテンツの管理] パネルに一貫性のない場所ラベルが表示される問題を修正しました。** [!UICONTROL 顧客から、[!UICONTROL  コンテンツを管理] パネルに、[!UICONTROL  エクスペリエンス ]と[!UICONTROL  オファー]のタブで位置番号が一致しないことが報告されました。 （例：エクスペリエンスの場所2と場所4と、オファーの場所1と場所2の場合） 2つの場所のみが設定されている場合でも、 場所ラベルが一貫性を持ち、変更に正確にマッピングされるようになり、アクティビティ作成プロセスにおける明瞭性と使いやすさが向上しました。 （TGT-52934）
* **保存後にVECの変更が失われる問題を修正しました。** VECで変更を保存すると、ページが更新され、以前のバージョンに変更が戻されます。 この問題により、アクティビティ全体がすぐに保存されない限り、最新の更新が失われます。 変更内容は保存後も正しく保持されるようになり、エディターが変更を予期せず元に戻すことがなくなり、アクティビティ作成ワークフローで信頼性の高いエクスペリエンスを確保できるようになりました。 （TGT-53500）
* **カーソルと選択範囲に要素の種類を表示することで、VECでの要素の選択を強化しました。** アクティビティの作成時の使いやすさを向上させるために、VECは、カーソルを合わせたときや選択したときに、HTML要素を種類で装飾するようになりました。 この機能強化により、顧客は適切な要素をより簡単に特定し、選択できるようになります。 選択した要素は明確な色でハイライト表示され、ホバーされた要素は青色で囲まれます。 エレメントタイプも表示され、編集中のコンテキストがより明確になります。 （TGT-53502）

+++

### データストリームの更新（2025年9月19日（PT））

データストリーム IDとサンドボックスの組み合わせは、[!DNL Adobe Target]宛先接続で一意である必要があります。

データストリーム IDとサンドボックス名の組み合わせがIMS組織内で一意である必要があることを強制するために、[!DNL Target]宛先接続の検証ロジックを更新しました。つまり次の式になります。

* 同じデータストリーム ID + サンドボックス名のペアを複数の[!DNL Target]宛先接続で再利用することはできません。
...
* **AEM エクスペリエンスフラグメント ]のオファー[!UICONTROL のユーザーアトリビューションが改善されました。** VECで、[!UICONTROL AEM エクスペリエンスフラグメント ] （XF）の「[!UICONTROL 最終更新日]」フィールドに、ユーザー名ではなくコード IDが誤って表示される問題を解決しました。 この不一致は、更新されたUIでのオファーの選択中に発生し、従来のUIとHTML オファーとの不整合が生じ、最後のエディターの名前が正しく表示されました。 この修正により、オファータイプ間で一貫したユーザーアトリビューションが保証され、透明性が向上し、期待される行動との整合性が高まります。 （TGT-52880 および TGT-52898）
...
* ODE オファーに対する&#x200B;**オファー決定エラーが解決されました。** [!DNL Target]を通じて挿入されたOffer Decision Engine （ODE） オファーで、形式メタデータが欠落しているため、400 エラーが返される問題を解決しました。 エラーメッセージがMIME タイプがnullであることを示したため、オファー決定の処理が正常に行われませんでした。 この修正により、オファーメタデータの適切な処理が保証され、パーソナライズされたコンテンツ配信の機能が復元され、マーケティングキャンペーンの中断のない実行が可能になります。 （TGT-53635）
* **ODS オファーのレンダリングの問題が解決されました。** 更新されたUIでVECを使用してアクティビティを構築した場合、[!DNL Adobe Journey Optimizer] （AJO）を介して作成された[!DNL Offer Decision Service] （ODS）オファーがレンダリングされない問題を解決しました。 従来のUIでも同じ設定が正しく機能するため、混乱が生じ、キャンペーンの実行がブロックされました。 この修正により、両方のバージョンで一貫したオファー配信が可能になり、AJO主導のパーソナライゼーションに期待される動作が復元されます。
...
* **更新されたレポートの概要UIの「レポート」セクションにダウンロードオプションが復元されました。** 新しい概要UIで、「[!UICONTROL  ダウンロード ]」ボタンがレポートセクションに表示されない問題を解決し、ユーザーが属性重要性スコアを書き出せなくなりました。 このアップデートにより、完全な書き出し機能が復元され、分析とレポート用にダウンロード可能なデータに合理的にアクセスできるようになります。 （TGT-53222）
* **[!UICONTROL 重要な属性] ビューで復元された完全なCSV レポート ] ボタンをダウンロードします。** [!UICONTROL 更新されたアクティビティ作成UIで、レポートビューの「[!UICONTROL 重要な属性]」セクションに「[!UICONTROL CSV レポート全体をダウンロード ]」ボタンが表示されない問題を解決しました。 この修正により、ダウンロード可能なインサイトへのアクセスが復元され、更新されたUIと従来のUIの両方で一貫した機能が保証されます。 （TGT-53238）
* **更新された概要UIの[!UICONTROL 自動ターゲット ]報告に影響するUIの問題を解決しました。** 更新された概要インターフェイスの複数のUIの問題を修正し、[!UICONTROL 自動ターゲット ] アクティビティレポートに影響を与えました。 これらの修正には、次のものが含まれます。

  * サマリーレポートに上昇率と信頼性の指標が欠落している
  * 「構築されたモデル」チェックボックスのカラーインジケーターが正しくない
  * [!DNL Analytics]のデータの相違にもかかわらず、非機能グラフレポート
  * [!UICONTROL 自動セグメント ]および[!UICONTROL 重要属性] レポートのダウンロードリンクがありません
  * 壊れた[!UICONTROL 自動セグメント ] レポート表示

  これらの修正により、期待されるレポート動作が復元され、更新されたUI全体で[!UICONTROL 自動ターゲット ]のパフォーマンスの可視性が向上しました。 （TGT-53484）
...
* 更新されたUIのパラメーターのプレゼンス条件に対する&#x200B;**フォームの検証が修正されました。** 更新されたUIで「[!UICONTROL  カスタム mbox パラメーター値が存在する]」または「[!UICONTROL  パラメーターが存在する]」を選択すると、お客様に値の入力が誤って必要になる問題を解決しました。 この動作は、値に関係なくパラメーターの存在を単純にチェックする意図されたロジックと競合しました。 この修正により、フォームの検証と期待される動作が一致し、よりスムーズなアクティビティの設定が可能になり、更新されたUIの完全な導入がサポートされます。 （TGT-53638）
* **ページ配信内のパラメーター存在ルールに対するフォームロジックが修正されました。&quot;** 更新されたUIで、「[!UICONTROL  パラメーターが存在する]」、「[!UICONTROL  パラメーターが存在しない]」、「[!UICONTROL  パラメーター値が存在する]」、「[!UICONTROL  パラメーター値が存在しない]」などのページ配信ルールを選択する際に、ユーザーが誤って追加パラメーター値を入力する必要があった問題を解決しました。 この動作は、従来のUIと矛盾しており、値を指定せずにパラメーターの存在を検出するという意図されたロジックと矛盾していました。 この修正により、期待されるルール設定の動作が復元され、アクティビティの設定が合理化され、ユーザビリティが向上します。 （TGT-53640）
* **更新されたUIのマルチページ ルール ビルダーの検証ロジックが改善されました。** 更新されたUI内のマルチページルールビルダーの複数の検証の問題を解決しました。 これらの修正には、次のものが含まれます。

  * mbox パラメーターが空の場合のルール作成の防止
  * 無効なルール状態に対する適切なエラーメッセージの表示
  * オペランド値を必要としない単項演算子およびパラメーターベースの演算子に対する検証ロジックの修正
  * 保存機能を復元して、単項演算子を使用したハッシュフラグメントルールを有効にする

  これらのアップデートにより、正確なルール設定が可能になり、複雑なページ配信シナリオ全体で使いやすさが向上します。 （TGT-53722）
* **A/BおよびMVT アクティビティで解決された場所の名前変更の問題。** [!UICONTROL A/B テスト ]または[!UICONTROL 多変量テスト ] （MVT）アクティビティ内の場所の名前を変更しても、場所リスト、ターゲティング、および戻りに移動した後に保持されない、更新されたUIのバグを修正しました。 このアップデートにより、場所の名前の変更が保存され、アクティビティワークフロー全体で一貫して反映されます。 （TGT-52367）
* **更新されたUIでMVTおよびAP アクティビティの場所の名前の永続性が修正されました。** 更新されたUIで、[!UICONTROL 多変量テスト ] （MVT）および[!UICONTROL Automated Personalization] （AP）アクティビティで編集された場所名が正しく保存されない問題を解決しました。 場所の名前を変更した後、[!UICONTROL  ターゲティング ]や[!UICONTROL  エクスペリエンス ]などのタブ間を移動すると、名前が以前の状態に戻りました。 この修正により、タブ間で一貫した場所の命名が保証され、アクティビティの設定中に信頼性が向上します。 （TGT-52927）
* **MVT アクティビティの管理エクスペリエンス パネルで場所のラベル付けが修正されました。** 更新されたUIで、[!UICONTROL 多変量テスト ] （MVT）アクティビティの[!UICONTROL  エクスペリエンスの管理] パネルに一貫性のない場所の番号付けが表示される問題を解決しました。 この修正により、位置情報ラベルが順次に行われ、ユーザー定義の変更と正しく整合することが保証され、エクスペリエンスのセットアップ時の明瞭性と使いやすさが向上します。 （TGT-52929）
...
* **お客様は、[!UICONTROL  アクティビティの概要]**&#x200B;からアクティビティ名またはドキュメント名をコピーできませんでした。以前は、更新されたアクティビティ作成プロセスで、アクティビティ名または関連するオファー/ドキュメントを[!UICONTROL  アクティビティの概要]から直接コピーできませんでした。 この制限は、特に小さな画面での操作性に影響を与えました。 顧客は、回避策なしで、アクティビティ名とドキュメント名の両方を簡単にコピーできるようになりました。 （TGT-51850）
* **アクティビティの作成中に厳選された[!DNL Target]の顧客データをプロアクティブに取り込む**: [!DNL Target]の顧客からのレポート、コンテンツ、スクリーンショットのプロアクティブなコレクションを有効にすることで、アクティビティ作成プロセスを改善しました。 この機能強化により、既存のユースケースで特定されたデータギャップに対処し、アクティビティや実験の設定中により正確なインサイトを得ることができます。 （TGT-52415）
* **AP アクティビティは、[!UICONTROL  レポート ] セクション**&#x200B;でモデル対応のデータを取得しませんでした。[!UICONTROL  レポート ] セクションでAutomated Personalization （AP） アクティビティを表示しているお客様は、レポートグループおよびオファーレベルでモデル対応の指標を確認できませんでした。 この問題は、モデル対応データがバックエンドから正しく取得されていなかったために発生しました。 機能が復元され、モデル対応データが期待どおりに表示されるようになりました。 （TGT-53600 および TGT-53601）
* **将来にスケジュールされたアクティビティが、[!UICONTROL  アクティビティ ]概要**&#x200B;に「[!UICONTROL  ライブ ]」ステータスを誤って表示しました。将来に開始するようにスケジュールされたアクティビティが、[!UICONTROL  アクティビティ ]概要で「[!UICONTROL  ライブ ]」として誤ってマークされていることが確認されたお客様が確認しました。 このステータスの不一致が解決され、スケジュールされたアクティビティは、ページの更新を必要とせずに「[!UICONTROL  スケジュール済み]」として正しく表示されるようになりました。 （TGT-52835）
...
* **商品リストが[!UICONTROL  コレクションを表示] ダイアログに表示されませんでした：**&#x200B;以前、お客様は[!UICONTROL おすすめ] タブでコレクションを表示する際に、商品リストを表示できませんでした。 [!UICONTROL  コレクションを表示] ダイアログに、関連する製品が正しく表示されるようになりました。更新された[!UICONTROL Recommendations] UIの透明性と使いやすさが向上しました。 （TGT-50531）
* **製品カタログ検索]高度な検索**&#x200B;で大文字と小文字を区別するフィルタリングが発生する問題を修正しました。[!UICONTROL 製品カタログ検索] ページの高度な検索フィルタリングで、バックエンドとGraphQL サービスの両方の動作に合わせて、大文字と小文字が区別されなくなりました。 [!UICONTROL このアップデートにより、テキストの大文字と小文字を問わず、顧客に対して一貫性のある正確な提案結果が提供されます。 （TGT-53585）
* 更新された[!UICONTROL 製品カタログ検索] UIの&#x200B;**詳細検索では、候補が表示されませんでした**：更新された[!UICONTROL 製品カタログ検索] UIの詳細検索機能を使用しているお客様は、候補が表示されないため、正確な値を入力する必要がありました。 そのため、商品を効率的に探すことが難しくなっていました。 高度な検索入力時に、候補が正常に表示されるようになりました。 （TGT-52008）
* **一部の承認者が[!UICONTROL 製品カタログ検索]**&#x200B;で製品を表示できませんでした：[!UICONTROL 承認者]権限を持つお客様は、同じ役割を持つ他のユーザーがアクセス権を持っているにもかかわらず、[!UICONTROL 製品カタログ検索]で製品を表示できませんでした。 この問題は、カタログの表示に影響を与える権限の不整合が原因で発生しました。 すべての承認者が、期待どおりに[!UICONTROL Recommendations] セクションで製品を表示できるようになりました。 （TGT-53617）
...
* **無効なオーディエンス名エラー**&#x200B;により、デスクトップオーディエンスのレポートの読み込みに失敗しました。お客様は、アクティビティ作成プロセスで1人のオーディエンスのレポートを表示しようとしたときに、GraphQL エラーが発生しました。 システムが「無効なオーディエンス名：XXXXX」メッセージを返し、レポート データへのアクセスが妨げられました。 デスクトップオーディエンスのレポートが正しく読み込まれるようになりました。 （TGT-53371）
* **レポートページでオーディエンスを切り替えると、ターゲット UI**&#x200B;でエラーが発生しました。お客様は、[!UICONTROL  レポート ] セクションで特定のオーディエンスを選択する際にエラーが発生しました。 この問題は、バックエンドのGraphQL呼び出しで無効なオーディエンス処理が発生し、予期しないエラーやデータ不足が発生したことが原因で発生しました。 この問題は解決され、データがない場合でも、デスクトップオーディエンスがエラーなしで読み込まれるようになりました。 （TGT-53370）
* [!UICONTROL  レポート ] セクションの&#x200B;**[!UICONTROL グラフビュー]で[!DNL Analytics]**&#x200B;の値が表示されませんでした：Re ポートセクションの[!UICONTROL  グラフビュー]にアクセスしているお客様で、すべての値が0として表示されるデータが見つかりませんでした。 この問題は、[!UICONTROL Analytics]からの誤ったデータ取得が原因で発生しました。 [!UICONTROL  グラフビュー]では、期待どおりに正確な値が表示されるようになりました。 （TGT-52792）
+++
...
* **機能が見つからなかったため、[!UICONTROL Enhanced Experience Composer] （EEC）を使用して「Cookieを受け入れる」をクリックできませんでした**：お客様は、EECを介してCookieを受け入れようとすると、コンソールエラーが発生したと報告しました：`handleclickAcceptAllButton is not defined`。 Cookie受け入れ機能が期待どおりに動作するようになり、更新されたUIでアクティビティ作成中にスムーズなエクスペリエンスが実現します。 （TGT-52794）
* **新しいEEC UIで、以前は従来のUIでサポートされていた特定のページを読み込めませんでした**：お客様から、新しいEECでは一部のページを読み込めないと報告されました。これらのページは、サイトにiframe バストコードが存在するにもかかわらず、従来のUIでアクセスできました。 更新されたアクティビティ作成プロセスでは、これらのページの読み込みがサポートされるようになりました。これにより、アクティビティ作成ワークフローの互換性が復元されます。 （TGT-53061）
* **VECは、エクスペリエンスを編集する際に空白の白い画面を表示しました**：特定のテナントのお客様は、更新されたVECでエクスペリエンスを編集しようとすると、VEC画面が空白になったと報告しました。 この問題は、新しく作成されたアクティビティと古いアクティビティの両方に影響を与え、ワークフローの継続性を妨げます。 VECが正しく読み込まれるようになり、顧客が中断することなくエクスペリエンスを編集できるようになりました。 （TGT-53547）
* **特定のアクティビティを読み込むと、VECがクラッシュし、空白の画面が表示されました**：特定のテナントの顧客から、VECが特定のアクティビティを読み込めなかったことが報告されました。 エクスペリエンスエディターがクラッシュして空白の画面が表示される前に、「初期変更の適用」で停止したままになりました。 コンソールエラーは、未定義のプロパティを読み取れなかったことを示しました。 エディターは、更新されたVECにエラーを含めずに影響を受けるアクティビティを読み込めるようになりました。 （TGT-53548）
* **Backspaceを使用してすべての日付値を消去すると、ページがクラッシュする**:Backspaceを使用して「[!UICONTROL 指定日時]」フィールドからすべての値を消去すると、[!UICONTROL 目標と設定]」セクションのスケジュール設定アクティビティがクラッシュする。 この問題は、日付処理ロジックのnull参照エラーが原因で発生しました。 ページがクラッシュすることなく、空の日付入力を適切に処理するようになりました。 （TGT-53624）
* **無効なペイロードが原因で[!UICONTROL 商品カタログ検索]に商品が表示されませんでした**:[!UICONTROL 商品カタログ検索]の[!UICONTROL Recommendations] セクションにアクセスしているお客様が、無効なGraphQL ペイロードによって生じた空の結果を検出しました。 このバックエンドエラーにより、製品データが正しく読み込まれませんでした。 更新されたUIで、製品が期待どおりに表示されるようになりました。 （TGT-53630）
* 更新されたVEC **では、**[!DNL Scene7]&#x200B;個の画像が低解像度で保存されました。更新されたVECでエクスペリエンスを編集しているお客様は、[!UICONTROL Scene7]個の画像URLが解像度パラメーターなしで保存され、低画質の画像が配信されたことに気づきました（意図した800×800ではなく400×）。 画像URLは、適切な解像度を確保するために正しいパラメーターを保持するようになりました。 （TGT-52631）
* **ライブ アクティビティはVEC**&#x200B;で引き続き編集できます。お客様は、更新されたVEC内のライブ アクティビティの編集オプションにアクセスできました。これにより、意図しない変更が発生する可能性があります。 この問題は、ライブアクティビティの編集機能を無効にすることで解決されました。 編集ボタンは、編集者のアクティビティリストと概要に非表示になり、承認者やその他の役割は影響を受けません。 （TGT-53055）
* **更新されたVEC**&#x200B;の[!UICONTROL 失敗]および[!UICONTROL  ドラフト ] アクティビティ セクションを廃止しました：[!UICONTROL 失敗]および[!UICONTROL  ドラフト ] アクティビティ オプションが更新されたVECから削除されました。 新しいUIでは、ドラフト状態がサポートされなくなり、失敗したキャンペーンはバックエンドに保存されません。 これらのオプションは、もはや関連性がありません。 関連するフィルターとバックエンドフィールド （例：`uiSyncFailed`、`errorMessage`）も削除され、アクティビティ管理が効率化されました。 （TGT-53150）
* **アクティビティのVECにログインできません**: VECを介してサイトにログインしようとしたお客様が、ログインページに繰り返しリダイレクトされ、アクティビティ編集へのアクセスが妨げられました。 この問題は内部的には再現可能ではなく、サイト側のセッション処理に関連している可能性があります。 ログインフローが安定し、リダイレクションエラーなしでVECにアクセスできるようになりました。 （TGT-53524）
* **[!UICONTROL 参照] モードで戻るボタンを2回押すと、VECがクラッシュしました**: VECで[!UICONTROL 参照] モードを進んでいるお客様は、ブラウザーの戻るボタンを2回押すとクラッシュしました。 この問題により、エディターがフリーズし、ページの更新が必要になりました。 エディターがクラッシュすることなく確実に戻るナビゲーションを処理するようになりました。 （GT-53568）
* **場所のマッピングが未定義であるため、アクティビティを編集できませんでした**：お客様がアクティビティを編集しようとしたときにエラーが発生しました。これは、`LocationMapping.behaviorTargetedActivity` ロジックの未定義の場所IDが原因です。 この問題により、400 エラーが発生し、アクティビティの更新がブロックされました。 場所に関連する検証エラーなしで、アクティビティを編集できるようになりました。 （TGT-53607）
* **アクティビティを保存すると、無効なユーザー入力エラーが発生しました**：更新されたVECで軽微な変更を行った後にアクティビティを保存しようとすると、無効なユーザー入力エラーが発生しました。 このエラーは、バックエンド検証ロジックの位置マッピングが一致していないことが原因で発生しました。 場所に関連するエラーをトリガーせずに、アクティビティを正常に保存できるようになりました。 （TGT-53603）
...
* **アクティビティを保存すると、形式が正しくないプロパティデータが原因で無効なユーザー入力エラーが発生する問題を修正しました**：既存のアクティビティを保存しようとすると、重大なエラーが発生しました。 エラーメッセージは無効なユーザー入力を示しており、特にJSON ペイロード内の認識できないプロパティ名コンテンツを参照しています。 注目すべきは、同じプロパティを使用する新しいアクティビティが正常に保存されたことです。これは、この問題が従来のアクティビティデータに分離されていることを示唆しています。 アクティビティ作成プロセスで、レガシープロパティの設定が正しく処理されるようになりました。これにより、無効な入力エラーを防ぎ、新規および既存のアクティビティ間で一貫した保存動作を確保できます。 （TGT-53507）
* **InvalidProperty.Json エラーが原因でアクティビティを保存できない問題を修正しました**：更新されたUIでアクティビティを保存しようとしたときに、変更を加えずにエラーが発生しました。 エラーメッセージは、無効なJSON構造を示しました：&quot;無効なJson。 プロパティ名「content」が認識されていません。 場所：行 – 1、列 – 432&quot;。 この問題は、リクエストペイロード内の認識できないプロパティが原因で発生したため、解決されました。 顧客は、このエラーが発生することなく、アクティビティを正常に保存できます。 （TGT-53513）
* **スケジュールされたアクティビティが、ページの更新**&#x200B;まで[!UICONTROL Live] ステータスを正しく表示しない問題を修正しました。お客様は、アクティビティをスケジュールして将来の日時に公開すると、ステータスが[!UICONTROL  スケジュール済み]ではなく[!UICONTROL Live]と表示されることに気づきました。 これにより、アクティビティがスケジュールされていることを確認メッセージが正しく示していたにもかかわらず、混乱が生じました。 ページを更新すると、ステータス表示が修正されました。 この問題は解決され、スケジュールされたアクティビティは、更新を必要とせずにスケジュールされたステータスを正しく表示するようになりました。 （TGT-52937）
...
* **お客様がアクティビティ作成プロセス中にレポートスイート名を入力できない問題を修正しました**: アクティビティ作成プロセス中に[!DNL Adobe Analytics]をレポートソースとして使用しているお客様が、特定のレポートスイートを検索するために[!UICONTROL  レポートスイート ] ドロップダウンリストに入力できませんでした。 これにより、レポートスイートが多数ある組織で、手動スクロールで設定が大幅に遅れるワークフローに影響が及びます。 ドロップダウンリストはアルファベット順ではなく、入力された入力に一貫して応答しなかったため、「Office + Store - Web – 製品」などのレポートスイートを見つけるのが困難でした。 この問題は解決され、顧客はレポートスイート名を入力して効率的に検索できるようになりました。 （TGT-53345）
...
* **期限切れの「時間枠」オーディエンスが削除後もアクティビティの保存をブロックする問題を修正しました**：期限切れの「時間枠」オーディエンスに関連するエラーにより、更新されたUIにアクティビティを保存できませんでした。 影響を受けるオーディエンスを削除した後も、「アクティビティに無効な時間枠を持つオーディエンスが含まれています」というエラーメッセージが残りました。 この問題は、使用されなくなった場合でも、システムがアクティビティのみのオーディエンスを検証し続けていたために発生しました。 この振る舞いは時間帯の不一致によってさらに複雑になった。 検証ロジックが修正され、このエラーが発生することなくアクティビティを保存できるようになりました。 （TGT-53517）

+++

**[!UICONTROL エクスペリエンスフラグメント ]s**

+++詳細を見る

* **ユーザーがUIで[!UICONTROL 前に挿入]または[!UICONTROL 後に挿入]を使用してエクスペリエンスフラグメントを挿入できない問題を修正しました** ユーザーが、更新されたUIの「前に挿入」または「後に挿入」オプションを使用してアクティビティに[!UICONTROL  エクスペリエンスフラグメント ]を挿入しようとしたときにエラーが発生しました。 表示されたエラーメッセージは、「オファーの内容には、1つのHTML要素が含まれている必要があります」です。 この問題は、更新されたUIに固有のもので、以前のバージョンでは発生しませんでした。 この問題は解決され、このエラーが発生することなく、顧客は[!UICONTROL  エクスペリエンスフラグメント ]を正常に挿入できます。 （TGT-53442）
...
* **更新されたUIで決定オファーを編集し、特定のページ要素を選択できない問題を修正しました**：更新されたアクティビティ作成プロセスで、お客様は既存の決定オファーを編集したり、Visual Experience Composer （VEC）で特定のページ要素を選択したりすることができませんでした。 決定オファーがHTML オファーとして誤って表示され、編集中に行われた変更は保存されませんでした。 さらに、日本のサイトなどの特定の地域のURLがVECで正しく読み込まれず、アクティビティの作成と更新がブロックされました。 決定オファーが正しく表示され、ページ要素が期待どおりに選択可能になり、地域URLがVECに正しく読み込まれ、完全な編集機能が復元されます。 （TGT-53425）
* **保存後に[!UICONTROL  オファー決定] セレクターを変更できず、予期せず変更される問題を修正しました**：更新されたアクティビティ作成プロセスで、顧客は[!UICONTROL  オファー決定] セレクターを意図したとおりに変更できませんでした。 セレクターの変更が失敗し、保存後にセレクターが誤った値に戻りました。 この動作により、決定オファーがVisual Experience Composer （VEC）から消え、さらに編集がブロックされました。 セレクターの変更が正しく保存され、保存後もVECで決定オファーが表示され、編集可能な状態が維持されるようになりました。（TGT-53433）
* **アクティビティの保存後に[!UICONTROL Offer Decisions]がアクティビティから消えた問題を修正しました**: アクティビティの作成中に追加された[!UICONTROL Offer Decisions]が、アクティビティの保存と再開後に保持されませんでした。 この問題は、動的コンテンツを編集し、特定のセレクターとプロパティを使用して[!UICONTROL  オファー決定]を挿入する際に発生しました。 [!UICONTROL  オファー決定]は、保存後も正しく保持され、セレクターは維持され、一貫したターゲティングと編集動作が保証されます。 （TGT-53434）
...
* **カスタム条件CSVのダウンロードで404 エラーが返される[!DNL Recommendations] UIの問題を修正しました**：顧客がアクティビティ作成プロセスでカスタム条件CSVをダウンロードできない問題を修正しました。 ダウンロードリンクが正しく機能し、顧客が期待どおりにカスタム条件を書き出せるようになりました。 （TGT-51966）
* **カタログ検索]**&#x200B;での画像の読み込みに一貫性がなかった問題を修正：[!UICONTROL  カタログ検索]のサムネールと画像が、アクティビティ作成プロセスで一貫して読み込まれなかった問題を修正しました。 [!UICONTROL 「サムネール URL」列が表示され、一部の製品画像がナビゲーションまたは検索アクションの後に部分的に読み込まれるか、まったく読み込まれない場合を除き、画像が表示されませんでした。 画像の読み込み動作が安定し、列の表示やナビゲーションアクションに関係なく、サムネールが確実に表示されるようになりました。 （TGT-52778）
* **重複したエクスペリエンスのレコメンデーションを編集すると、元のエクスペリエンスに影響が及ぶ問題を修正しました**：顧客は、重複したエクスペリエンスのレコメンデーションを変更すると、元のエクスペリエンスが意図せず変更されたと報告しました。 具体的には、アクティビティ作成プロセスでエクスペリエンス Bを複製し、そのデザインまたは基準を編集した後、同じ変更が別々のエンティティであるにもかかわらず、元のエクスペリエンス Bに反映されました。 エクスペリエンスの重複は個別の設定を維持するようになり、1つのエクスペリエンスの編集が元のエクスペリエンスに影響を与えなくなりました。 （TGT-53369）
* **重複したエクスペリエンスに対する変更が、アクティビティ内の元のエクスペリエンスに意図せず影響を与える問題を修正しました**：お客様は、アクティビティ内のエクスペリエンスを複製して新しいオーディエンスを割り当てる際に、複製されたエクスペリエンスのデザインや基準に加えられた変更も、元のエクスペリエンスに反映されたと報告しました。 この問題は、元のバージョンに直接編集を加えなかったにもかかわらず発生し、同じアクティビティ内で独立したバリエーションを作成する機能に影響を与えました。 アクティビティ作成プロセスで、重複したエクスペリエンスが正しく分離されるようになりました。これにより、1つのエクスペリエンスに加えられた編集が元のエクスペリエンスに影響を与えなくなります。 （TGT-53361）
* **レコメンデーションカタログ ]が完全な製品属性データを断続的に表示できなかった問題を修正しました**：更新された[!DNL Recommendations] UIで、顧客は、データがフィードに存在していたにもかかわらず、メッセージなどの特定の製品属性が[!UICONTROL  カタログ検索]結果に一貫して表示されない問題を経験しました。 [!UICONTROL この問題では、欠落している値を取得するために、列の表示を手動で再設定する必要がありました。 [!UICONTROL  カタログ検索]では、設定済みのすべての属性が確実に表示されるようになったため、列を手動でリセットする必要がなくなりました。 （TGT-52769）
* **ライブアクティビティで[!UICONTROL  フロントプロモーション ]を無効にできない問題を修正しました**: ライブアクティビティで[!UICONTROL  フロントプロモーション ]を無効にしようとしましたが、保存されませんでした。 「[!UICONTROL  プロモーションを変更]」を選択して無効にした後、アクティビティを再編集してもプロモーションはアクティブのままになり、レコメンデーション設定が更新されなくなります。 プロモーション設定が正しく保存されるようになりました。これにより、お客様はライブアクティビティのプロモーションを期待どおりに無効または変更できるようになりました。 （TGT-53231）
* **データを使用せずに[!DNL Recommendations] [!UICONTROL  プロモーション ]を有効にすると、不明確なエラーメッセージ**&#x200B;がトリガーされる問題を修正しました。必要な値を指定せずに[!DNL Recommendations] アクティビティで[!UICONTROL  フロント ]または[!UICONTROL  バック プロモーション ]を有効にすると、一般的な「無効な入力エラー」メッセージが表示されます。 根本的な問題は設定フィールドがありませんでしたが、エラーメッセージに原因が明確に示されていないため、トラブルシューティングが困難でした。 アクティビティ作成プロセスで、`collectionId`やルールなどの必須フィールドが見つからない場合に、明確で実用的なエラーメッセージが表示されるようになりました。これにより、お客様は設定の問題をすばやく特定して解決できます。 （TGT-52616）
* **お客様が「[!UICONTROL お勧め]」タブで[!UICONTROL  コレクション ]または[!UICONTROL 除外]を編集する際に、フィルター処理された製品リストを[!UICONTROL 編集] モーダルに表示できない問題を修正しました**。 リストは、適用されたルールに基づいてリアルタイムで更新されることを想定していましたが、意図したとおりに表示されませんでした。 この問題は解決され、ルールが変更されると、製品リストが正しく表示され、動的に更新されるようになりました。 （TGT-53481）
* **更新されたUIの詳細を表示ダイアログのレイアウトに関する問題を修正しました**：更新されたUIの詳細を表示モーダルのレイアウトが変更され、明瞭性と使いやすさが向上しました。 ダイアログには2つのタブが含まれるようになりました。
  * [!UICONTROL 詳細] タブ：選択した項目のすべての関連情報を表示します。
  * [!UICONTROL 在庫] タブ：現在のコレクションと除外ルールでフィルタリングされたすべての製品が表示されます。

  この機能強化により、顧客は、アクティビティ作成プロセス内で、項目固有のデータと在庫のコンテキストをより簡単に操作し、理解できるようになります。 （TGT-53503）

  * **レコメンデーションアクティビティの削除されたプロモーションが保存後に再度表示される問題を修正しました**：顧客は、[!UICONTROL front]または[!UICONTROL back]のプロモーションが[!DNL Recommendations]のアクティビティから削除され、アクティビティが保存されたときに、プロモーションが再オープン時に引き続き表示されることを報告しました。 この問題は、ステージング環境と実稼動環境の両方で発生し、更新されたアクティビティ作成プロセスに影響を与えました。 問題が解決されました。 アクティビティから削除されたプロモーションが、保存後も正しく保持されるようになりました。 （TGT-53490）
...
* **自動セグメント ] レポートにnull オーディエンス値が表示される問題を修正しました**：顧客は、アクティビティレポートの[!UICONTROL 自動セグメント ]がオーディエンスデータにnullを表示しており、セグメントパフォーマンスの正確な分析が妨げられていると報告しました。 [!UICONTROL この問題は、有効なオーディエンスデータが想定されていたにもかかわらず、[!UICONTROL  レポート ] セクションにアクセスし、[!UICONTROL 自動セグメント ]を選択したときに発生しました。 [!UICONTROL 自動セグメント ]は、オーディエンス値を正しく表示し、信頼性の高いレポートとセグメント化インサイトを確保できるようになりました。 （TGT-52793）
...
* **更新されたUI**&#x200B;で[!UICONTROL 参照]と[!UICONTROL  デザイン ] モードを切り替えるとSPAの状態がリセットされる問題を修正しました。更新されたUIで[!UICONTROL 参照]と[!UICONTROL  デザイン ] モードを切り替えると、web エディターがリロードされ、SPAの状態がリセットされると報告されました。 この問題により、ワークフローが中断され、顧客は情報を再入力する必要がありました。 問題が解決されました。 モードを切り替えると、SPA状態が保持されるようになり、アクティビティの作成中に、よりスムーズで一貫性のあるエクスペリエンスを実現できるようになりました。 （TGT-53074）
...
* **アクティビティ作成プロセスで、AP アクティビティの[!UICONTROL  ターゲティング ] ステップへの進行がブロックされる問題を修正しました**：お客様が[!UICONTROL Automated Personalization] （AP）アクティビティの[!UICONTROL  ターゲティング ] ステップに進めないアクティビティ作成プロセスの問題を修正しました。2つの場所が追加されていない限り。 この動作は、複数のオファーを含む単一の場所で十分だった以前のエクスペリエンスとは異なっていました。 お客様がAP ワークフローの一部として単一の場所の設定を引き続き使用できるようにするための要件が修正されました。 （TGT-53426）
* **新しいアクティビティ作成プロセスで、透明画像のfmt=png-alpha パラメーターが設定されない問題を修正しました**：更新されたUIで、アクティビティ作成プロセス中に挿入された画像に、デフォルトで`fmt=png-alpha` パラメーターが含まれなくなり、透明性が失われるようになりました。 この動作は、透明な背景を維持しながら、パラメーターを画像URLに自動的に追加する以前のUIとは異なりました。 透明が必要な場合に、アクティビティ作成プロセスが画像URLに`fmt=png-alpha` パラメーターを正しく適用し、透明なアセットの一貫したレンダリングを確保するようになりました。 （TGT-52615）
* **更新されたUIでレポートスイートを検索できない問題を修正しました**：更新されたUIで、[!UICONTROL 目標と設定] セクションの[!UICONTROL  レポートスイート ] ドロップダウンリストでは、顧客が入力と検索を行うことができず、特定のレポートスイートを見つけるのが困難でした。特に、エントリ数が多いテナントの場合は問題が発生します。 以前のUIとは異なり、リストは並べ替えられておらず、入力ベースのフィルタリングが不足していました。 この問題は解決されました。 お客様は、レポートスイートを検索およびフィルタリングして、従来のUIで使用できる機能を復元できるようになりました。 （TGT-53514）
...
* **単一のワークスペースに制限されているお客様が他のワークスペースからアクティビティを表示できる問題を修正しました**: アクセスが1つのワークスペースに制限されているお客様が、アクティビティ作成プロセスで[!UICONTROL すべてのワークスペース ]を選択すると、すべてのワークスペースでアクティビティを予期せず表示できるようになりました。 この可視性により、他のワークスペースでのライブアクティビティに意図しない変更が加わるリスクがあり、web サイトのパフォーマンスに影響を与える可能性がありました。 Workspaceのアクセス制御が強化され、割り当てられたワークスペース内のアクティビティのみを表示および操作できるようになりました。 （TGT-53101）
* **お客様がアクセス権を持たずに[!UICONTROL  デフォルトのWorkspace]からアクティビティを表示できる問題を修正しました：** ステージングワークスペースに制限されたアクセス権をお持ちのお客様は、アクティビティ作成プロセスを介して[!UICONTROL  デフォルトのWorkspace]からアクティビティを表示できました。 この動作は、バックエンドの設定とアクセス権が正しいにもかかわらず発生しました。 Workspace アクセス制御が強化され、割り当てられたワークスペース内でのみアクティビティを表示できるようになりました（TGT-53297）。
...
* **更新された[!DNL Target] UI**&#x200B;のアクティビティ読み込みの問題を修正：更新された[!DNL Target] UIで、特定のアクティビティが編集しようとしたときに読み込めなかった問題を修正しました。 この問題は、ユーザーが無期限の読み込み画面から離れる原因となりました。 この問題は複数のアクティビティに影響を与え、顧客間で一貫性のない問題が発生していることが報告されました。 この修正により、影響を受けるアクティビティが適切に読み込まれるようになり、シームレスな編集が可能になり、アクティビティワークフローの中断を減らすことができます。 （TGT-53209）
* **検証`optionLocalId`によるアクティビティ作成プロセスの保存エラーを修正**: バックエンドの検証エラーによる変更を保存できなかったアクティビティ作成プロセスの問題を修正しました：`OptionLocalIdReferentialIntegrity.ABActivity - Invalid optionLocalIds:` アクティビティ内の特定の要素を変更する際にこの問題が発生し、保存操作が失敗しました。 この修正により、`optionLocalId`参照が正しく検証され、このエラーが発生することなくアクティビティを保存できるようになりました。 （TGT-53293）
* **ページ切り替え時の無効なオプション参照によるアクティビティ作成プロセスのクラッシュを修正**: アクティビティ作成プロセスで、編集中にページを3回切り替えた後にUIがクラッシュする問題を修正しました。 クラッシュは無効なオプション参照によってトリガーされ、「LocalId &#39;7&#39;のオプションが見つかりません」などのコンソールエラーが発生しました。 今回のアップデートにより、顧客はシステムの障害や中断に遭遇することなく、ページを切り替えて変更を適用できるようになりました。 （TGT-53295）
* **エクスペリエンスの編集時に無効なユーザー入力が原因でアクティビティ作成プロセスの保存エラーが発生する問題を修正**：無効なユーザー入力エラーが原因で、顧客がアクティビティの変更を保存できないアクティビティ作成プロセスの問題を修正しました。 更新されたUIでエクスペリエンスを変更する際にエラーが発生し、更新がコミットされるのを防ぎます。 アクティビティを正常に保存できるようになり、顧客が中断することなく編集および公開できるようになりました。 （TGT-53267）
* **更新されたUIでの編集をブロックするアクティビティ作成プロセスの読み込み問題を修正しました**：継続的な読み込み画面のため、更新されたUIでアクティビティを編集できないアクティビティ作成プロセスの問題を修正しました。 読み込みエラーが発生することなく、アクティビティを開いて変更できるようになりました。 （TGT-53415）
* **更新されたUI**&#x200B;でのAP アクティビティのアクティビティ作成プロセスにおけるエクスペリエンス要件の問題を修正：[!UICONTROL Automated Personalization] （AP）アクティビティが更新されたUIで2つのエクスペリエンスではなく2つの場所を必要とするアクティビティ作成プロセスの問題を修正しました。 この動作により、顧客が複数のオファーを含む単一の場所を設定するユースケースがブロックされました。これは以前サポートされていました。 元の機能に合わせて要件が修正され、お客様は場所の数に関係なく2つのエクスペリエンスを使用してAP アクティビティを続行できるようになりました。 （TGT-53429）
* **クリックトラックモードでのトラッキング済み要素フィールドの動作を修正して、未保存の入力を防ぎ、明瞭性を向上しました**: アクティビティ作成プロセスで、[!DNL Click Track] モードの[!UICONTROL  トラッキング済み要素] フィールドが編集可能でしたが、入力された名前が保持されず、顧客の混乱が生じる問題を修正しました。 未保存の入力を防ぐためにフィールドが無効になり、選択した要素の名前が明確になり、目標設定とトラッキング精度が向上しました**（TGT-53458）。
* **追跡されたコンポーネントの名前付けを[!UICONTROL  クリックトラック ] モード**&#x200B;でブロックするアクティビティ作成プロセスの問題を修正しました。顧客が[!UICONTROL  クリックトラック ] モードで追跡されたコンポーネントに名前を付けることができなかったアクティビティ作成プロセスの問題を修正しました。 名前を入力した後、フィールドは編集可能に見えましたが、入力は保持されず、編集モードで「MY プライマリ GOAL 0」などの汎用ラベルがデフォルトで使用されます。 トラッキングされた要素フィールドが無効になり、目標の設定とトラッキング精度を向上させるために、名前付けの動作が明確になりました。 （TGT-51396）
...
* **AEMで書き出されたフラグメントの意図しないHTML編集を許可するアクティビティ作成プロセスの問題を修正**：お客様が[!DNL Adobe Experience Manager] （AEM）から[!DNL Target]内に書き出された[!DNL Experience Fragments] （XF）のHTMLを編集できるアクティビティ作成プロセスの問題を修正しました。 AEMから公開されたXFは引き続きロックされ、編集できなくなります。 この修正により、AEMで書き出されたフラグメントで「HTMLを編集」オプションが使用できなくなり、コンテンツの整合性と想定されるガバナンスが維持されます。 （TGT-53286）
* **更新されたUI内のアクティビティ作成プロセスにおけるXF コンテンツの断続的なプレビューの問題を修正**：更新されたUI内のプレビューモードでXF コンテンツを断続的にレンダリングできないアクティビティ作成プロセスの問題を修正しました。 コンテンツは正しく配信されましたが、プレビューが一貫して表示されないため、お客様がオファー設定を検証することが困難でした。 XF プレビューが確実に読み込まれるようになり、アクティビティ設定時の信頼性と効率性が向上しました。 （TGT-53318）
...
* **URL内の先頭スペースがQA リンクの破損を引き起こすアクティビティ作成プロセスの問題を修正**: アクティビティ URL内の先頭スペースが保存時に適切にトリミングされないアクティビティ作成プロセスの問題を修正しました。 これにより、バックエンドで無効なQA リンクと不正な書式設定が発生しました。 更新後、URLがクリーンに保存されるようになり、リンク切れを防ぎ、顧客のQA ワークフローの信頼性が向上しました。 （TGT-53427）
...
* **高度な検索で提案を提供できなかったカタログ検索UIの問題を修正しました**: [!UICONTROL 高度な検索]機能で提案を提供できなかった新しい[!UICONTROL  カタログ検索] UIの問題を修正しました。 ユーザーは、正確なスペルを入力して正確な値を入力する必要があり、検索が煩雑になり、ミスが発生しやすくなりました。 この修正により、[!UICONTROL 高度な検索]では、ユーザーが入力する際に関連する提案が表示されるようになりました。これにより、ユーザビリティが向上し、製品を探す際の摩擦が軽減されます。 （TGT-52008）
* **複数のUIとフィルタリングの問題を解決して、応答性とエンティティのインタラクションを向上しました**：小さな画面デバイスの条件の詳細に対する応答性の低下、環境フィルターで「すべてのホストグループ」を選択する際の結果の欠如、名前のないエンティティとのインタラクションの欠如など、いくつかの問題を解決しました。 これらの修正により、画面サイズ全体の使いやすさが向上し、正確なフィルタリングが可能になり、すべての製品エンティティとの完全なインタラクションが可能になり、ユーザーの全体的なエクスペリエンスが向上します。 （TGT-52992）
* **アクティビティの作成中にRecommendationsの詳細ビューで見つからない製品IDを修正**: [!DNL Recommendations] アクティビティ作成プロセスで、製品IDが製品詳細画面に表示されない問題を修正しました。これにより、ワークフロー中に顧客が製品IDをコピーしたり参照したりするのが困難になりました。 製品IDが詳細ビューに明確に表示されるようになり、可視性が向上し、顧客にとってより効率的な製品管理が可能になりました。 （TGT-51964）
* **商品メッセージをレコメンデーションビュー**&#x200B;に表示できなかったアクティビティ作成プロセスの問題を修正しました。[!DNL Recommendations] ビューの[!UICONTROL  メッセージ ]列に商品データが表示されていなかったにもかかわらず、メッセージが表示されないアクティビティ作成プロセスの問題を修正しました。 顧客は、コンテンツを一時的に表示するために、列を手動で削除したり再追加したりしなければならず、スクロールや検索時に再度表示されがちでした。 このアップデートにより、製品メッセージの一貫性のある可視性が復元され、カタログのナビゲーションとレビューのワークフローが改善されます。 （TGT-52777）
* **アクティビティ作成プロセスで「すべてのホストグループ」を選択すると、レコメンデーションビューで結果が返されない問題を修正しました**: アクティビティ作成プロセスで、[!DNL Recommendations] ビューで「すべてのホストグループ」環境を選択すると結果が返されない問題を修正しました。 顧客は、あらゆるホストグループの製品データを期待どおりに表示できるようになり、アクティビティの設定中の可視性とフィルタリングの精度が向上しました。 （TGT-53006）
* **アクティビティ作成プロセスで、保存後にフロントプロモーショントグルが保持されない問題を修正**：アクティビティ作成プロセスで、アクティビティ設定でフロントプロモーショントグルを無効にしても、保存後に保持されない問題を修正しました。 フロントプロモーションを削除しようとしているお客様は、アクティビティを再訪問したときにトグルが再度有効になっており、適切にカスタマイズできないことが判明しました。 この更新により、変更を確実に保存でき、顧客はプロモーション設定を完全に制御できるようになります。 （TGT-53215）
* **最終更新日]列[!UICONTROL による整理不一致の並べ替えを修正：** アクティビティ作成プロセスで、[!UICONTROL 最終更新日]列によるカタログの並べ替えに一貫性のない結果が生じる問題を修正しました。 顧客は、更新タイムスタンプに基づいて商品データを確実に整理できず、カタログのレビューと管理がより困難になりました。 並べ替えが期待どおりに機能するようになり、更新されたUIの精度と使いやすさが向上しました。 （TGT-53116）
...
* **アクティビティの読み込みを修正し、アクティビティ作成プロセスの[!UICONTROL  キャンセル ] ボタンの問題**&#x200B;を修正：特定のアクティビティの読み込みに失敗し、お客様が変更にアクセスできなくなるアクティビティ作成プロセスの問題を修正しました。 さらに、[!UICONTROL  キャンセル ] ボタンが応答しなかったため、お客様は読み込みプロセスを停止したり、編集ビューを終了したりすることができませんでした。 この修正により、アクティビティが確実に読み込まれ、[!UICONTROL  キャンセル ] ボタンが期待どおりに機能するようになり、Visual Experience Composerの全体的な安定性とユーザーエクスペリエンスが向上しました。 （VEC）（TGT-53218）
* **更新されたVEC UIで、編集がブロックされ、無効になったエクスペリエンス切り替えの問題を修正しました[!UICONTROL  キャンセル ] ボタン**：更新されたVEC UIで、エクスペリエンスのターゲット設定（XT）アクティビティでエクスペリエンスを切り替えるときに変更が読み込まれなかった問題を修正しました。 顧客は、最初に入力したエクスペリエンスを超えるエクスペリエンスを編集できず、[!UICONTROL  キャンセル ] ボタンが見つからないか、応答しませんでした。 この修正により、すべてのエクスペリエンスで変更が正しく読み込まれるようになり、ヘルパー拡張機能がなくても[!UICONTROL  キャンセル ] ボタンが確実に機能するようになり、編集ワークフローが改善され、フラストレーションが軽減されます。 （TGT-53256）
* **アクティビティ作成プロセスで複数のエクスペリエンスを切り替える際の白い画面の問題を修正**：複数のエクスペリエンスを切り替えると白い画面が発生する問題を修正しました。 また、アクティビティ内の複数のエクスペリエンスを変更しようとすると、顧客が白い画面に遭遇するアクティビティ作成プロセスの問題を修正しました。 この問題は、2つのエクスペリエンスに変更を適用してから3つ目のエクスペリエンスを選択した後に発生し、それ以上の編集を行うことができません。 このアップデートにより、エクスペリエンス間の移行がスムーズになり、顧客は中断することなく変更を加えることができます。 （TGT-53266）
* **編集セッション間でカスタムコードの変更が確実に保存されないVECの問題を修正**:Visual Experience Composer （VEC）で行われたカスタムコードの変更が1回の試行で確実に保存されない問題を修正しました。 お客様は、[!UICONTROL  コンテンツを編集] ボタンと最後の[!UICONTROL 保存] ボタンの両方を使用した後でも、スタイルの更新やHTML編集などの変更が、web サイトおよびQA URLから断続的に見つからないと報告しました。 このリグレッションが解決され、すべてのカスタムコードの変更が編集セッション間で期待どおりに保持されるようになりました**（TGT-53418）。
* **アクティビティの作成中に更新されたUIに`triggerViews`が見つからない問題を修正**：更新されたUIに`triggerViews`が正しく実装されていても、更新されたUIにが表示されないアクティビティ作成プロセスの問題を修正しました。 これにより、複数のお客様に影響が及び、アクティビティの設定中にビューベースのトリガーを検証することが困難になりました。 `TriggerViews`が期待どおりに表示され、お客様が確実に設定を完了してテストできるようになりました。 （TGT-53239）
* **更新されたUI**&#x200B;の特定のweb ページのアクティビティ作成プロセスにおけるビューの読み込みの問題を修正：アクティビティ作成プロセスで、配信またはインタラクション呼び出しで正しく実装され、表示されているにもかかわらず、特定のweb ページの更新されたUIにビューが読み込まれない問題を修正しました。 これは複数の顧客に影響を与え、ビューベースのターゲティングの検証を難しくしました。 サポートされているページ全体でビューが一貫して表示されるようになり、アクティビティの設定中の信頼性が向上しました。 （TGT-53246）
* **更新されたUI**&#x200B;のアクティビティ作成プロセスにおける断続的なビューの読み込みの問題を修正：アクティビティの編集中に、更新されたUIにビューが断続的に表示されないアクティビティ作成プロセスの問題を修正しました。 正しいビュー名がネットワークペイロードに存在していても、インターフェイスで一貫して認識されず、ビューベースのパーソナライゼーションを設定する顧客の機能に影響を与えました。 ビューが確実に表示され、よりスムーズな設定と検証ワークフローをサポートするようになりました。 （TGT-53223）
* **追跡されたアクション名が更新されたUIに保存されなかったアクティビティ作成プロセスの問題を修正しました**：追跡されたアクション指標を更新されたUIに保存できなかったアクティビティ作成プロセスの問題を修正しました。 トラッキングされた要素に名前を付けてアクティビティを保存すると、再度開いたときに名前がデフォルトのラベルにリセットされ、混乱が生じ、目標設定が中断されます。 トラッキングされたアクションに割り当てられた名前が保持されるようになり、顧客はコンバージョン指標を正確に設定および管理できるようになりました。 （TGT-53453）
...
* 更新されたUIに関するいくつかの問題を修正しました。これには、入力値の間隔によるページタイプの削除時のエラー、ワークスペース間の信頼性の低いアクティビティのコピー、QA環境でのページ配信ルールの誤動作などが含まれます。 （TGT-52703）
* 更新された[!DNL Target] UIで、[!UICONTROL 編集者]の役割を持つユーザーがライブアクティビティにアクセスし、編集を試みることができた問題を修正しました。この問題は制限する必要があります。 アクティビティリストと概要画面に、ライブアクティビティの編集オプションが誤って表示され、意図しない変更が発生する可能性があります。 （TGT-53055）
* [!UICONTROL 高度な検索] UIで、「値が存在する」または「値が存在しない」演算子を選択すると、ユーザーにオペランドの入力を求めるメッセージが表示される問題を修正しました。このオペランドは、これらの条件に必須ではありません。 （TGT-51961）
* 更新されたUIで、サンドボックス環境でも、ステージング環境から実稼動ワークスペースにアクティビティをコピーしようとすると、一貫して失敗する問題を修正しました。 有効な設定を使用し、適切なワークスペース権限を持っているにもかかわらず、「アクティビティで既に使用されている匿名オーディエンス」や「無効なオーディエンス ID」など、様々なエラーメッセージが発生しました。 （TGT-52394）
...
* コンテンツ配信で正しく動作しているにもかかわらず、[!UICONTROL Visual Experience Composer] （VEC）プレビューでエクスペリエンスフラグメント（XF）コンテンツがレンダリングできない問題を修正しました。 （TGT-53318）
...
* QA テナントの複数の言語環境で、「プロモーション」セクションの「プロモーションを追加」ボタンがローカライズされない問題を修正しました。 （TGT-53263）
...
* Visual Experience Composer （VEC）を介して既存のHTML オファーを編集すると、すべての変更内容がコンテンツ配信から削除される問題を修正しました。 変更は「修正」タブでグレー表示され、従来のUIで正しく適用されているにもかかわらず、QA プレビューには反映されません。 （TGT-52863）
* 更新された[!DNL Target] UIで、[!UICONTROL  ターゲティング ] タブから[!UICONTROL  エクスペリエンス ]に戻った後、[!UICONTROL Visual Experience Composer] （VEC）で行われたHTML オファーの変更が保持されない問題を修正しました。 （TGT-52874）
* 更新された[!DNL Target] UIで、同じセレクターの前後に1つのHTML オファーを挿入すると、場所が正しく生成されない問題を修正しました。 顧客が[!UICONTROL  ターゲティング ] タブから[!UICONTROL  エクスペリエンス ] タブに戻ったとき、1つのセレクターのみが保持され、変更の喪失とコンテンツ配信の破損につながりました。 この動作は、従来のUIとは異なり、場所IDが異なる両方の変更を正しく保持しました。 （TGT-52891）
* 更新された[!DNL Target] UIで、[!UICONTROL 変更] パネル内で追加されたオファーをクリックすると、右側の[!UICONTROL 設定] パネルが断続的に表示および非表示になり、オファー設定を操作するのが困難になる問題を修正しました。 （TGT-53288）
* アクティビティ内のオーディエンス固有のバリエーションに追加されたHTML オファーが、変更セクションに一貫して保存または表示されない問題を修正しました。 編集中にオーディエンスを切り替えた後、以前に適用されたオファーが消えたり、レンダリングできなかったりすることがあり、検証が中断され、アクティビティの準備が遅れることがあります。 （TGT-53440）
* オファーを含むアクティビティをコピーすると、新しいアクティビティで重複するオファーが作成される問題を修正しました。 この動作により、特にワークスペース間でアクティビティを移動する際に、不要な混乱や混乱が生じました。 この修正により、[!DNL Target]件のオファーが複製することなく宛先ワークスペースに正しくコピーされるようになり、アクティビティ管理が合理化され、ワークフロー効率が向上しました。 （TGT-53454）
...
* 更新されたVECで、顧客が[!UICONTROL  シングルページアプリケーション ] （SPA）ビューに変更を適用できない問題を修正しました。 古いUIで作成されたアクティビティではビュー固有のターゲティングがサポートされていましたが、新しいUIでは、ビューの切り替え制御が無効になっているので、これらのビューの検出または編集を許可できませんでした。 （TGT-52556）
...
* [!UICONTROL Visual Experience Composer]内のエクスペリエンスに対して行われた変更が表示されないか、UIで一貫性がないように見える問題を修正しました。 （TGT-TGT-53381）
* 更新されたVECで、「[!UICONTROL  コンテンツを管理]」セクションがエクスペリエンスのサムネール用の代替テキストを表示できなかった問題を修正しました。 この問題により、特にファイル名が長い場合や切り捨てられている場合は、ユーザーがドキュメントを識別することが困難でした。 （TGT-52291）
* VEC アクティビティで[!UICONTROL  ページ配信] ルールを設定する際に、顧客が誤った検証エラーを検出するエラーを修正しました。 具体的には、テキストをサポートする必要がある場合でも、テキスト値を入力する際に「どの演算子にも等しい」や「どの演算子にも等しくない」などの演算子が「演算子タイプに対して無効な入力」をトリガーします。 （TGT-52629）
* 「オファーを選択」ボタンを使用してオファーを選択する際に、更新されたUIの[!UICONTROL 変更] パネルで一貫性のない動作が発生する問題を修正しました。 既存のオファーを置き換える代わりに、UIは重複を追加し、いずれかのオファーと対話しようとすると、`selectorNotFound`などのコンソールエラーが発生しました。 さらに、一部のオファーでは「オファーを選択」ボタンが表示されず、編集可能なプロパティのみが表示されます。 （TGT-53321）
* 更新された[!DNL Target] UIで、アクティビティセットアップ中に行われたHTML コードの変更が、コントロールグループ用に正しく保存されていても、バリエーション ページに保持されない問題を修正しました。 何度も試し、ページをグループ化せずにテストを再作成したにもかかわらず、バリエーションのページは一貫して変更を保持できず、コンテンツ配信とQA検証に影響を与えました。 （TGT-53436）
* 更新されたVECで、ページ配信ルールの「いずれかに等しい」演算子が入力値を受け入れられない問題を修正しました。 すべてのmboxで顧客パラメーターを設定する際に、この演算子を選択すると「無効な入力」エラーが発生し、ルール作成が妨げられました。 （TGT-52623）
...
* ワークスペース間でアクティビティをコピーする際のワークフローを改善しました。 以前はワークスペース間でアクティビティをコピーすると、オーディエンスが欠落したり、プロパティが割り当てられていなかったりして、同期エラーが発生していました。 このアップデートにより、よりスマートで直感的なワークフローが導入され、コピーされたアクティビティが適切に設定され、公開準備が整います。 （TGT-47094）
* `copyActivityBatchOperations` ミューテーションでエラーが発生したため、顧客がワークスペース間でアクティビティをコピーできない問題を修正しました。 アクティビティを複製しようとすると、サーバーエラー（500）とnull応答ペイロードが発生しました。 （TGT-52405）
* 選択したワークスペース内で、設定で参照されているオファーにアクセスできない場合に、アクティビティの同期に失敗する問題を修正しました。 これにより、公開エラーが発生し、アクティビティが失敗した状態のままになりました。 （TGT-52535）
* 更新された[!DNL Target] UIのワークスペース間でアクティビティをコピーする際の複数の問題を修正しました。 ユーザーがソース ワークスペースと宛先ワークスペースの両方で「[!UICONTROL 承認者]」の役割を持っている場合でも、特にライブアクティビティでのオーディエンスアクセスに関連するエラーと、不正な権限検証が発生しました。 （TGT-53002）
* 更新されたUIで、同じワークスペース内のアクティビティをコピーすると、関連するオファーとオーディエンスが不必要に重複する問題を修正しました。 （TGT-53457）
* アドホック（匿名）オーディエンスが、デフォルト以外のワークスペース間またはデフォルト以外のワークスペースからデフォルトのワークスペースに正しくコピーされなかった場合に対処する問題を修正しました。 以前のアップデートでは、デフォルトからデフォルト以外および同じワークスペースのシナリオに対して適切な複製が保証されていましたが、この機能強化では、複数のワークスペースにまたがる組み合わせオーディエンス設定の保持に重点を置きました。 この修正により、ワークスペースのトランジション全体で一貫したオーディエンスの動作と正確なターゲティングが保証されます。 （TGT-53268）

+++

### [!DNL Target Standard/Premium] 25.7.4 （2025年8月1日）

このリリースでは、主に複雑な顧客カスタマイズによって発生した最近の問題を解決し、次の修正と改善が含まれています。

**アクティビティ**

+++詳細を見る

* 変更を加えずにライブアクティビティを保存しようとすると、お客様が「無効なユーザー入力」エラーに遭遇する問題を修正しました。 GraphQLの応答は、重複したLocalIdの問題を示します。 （TGT-53329 &amp; TGT-53373 &amp; TGT-53195）
* 更新されたVECでリダイレクトエクスペリエンスを作成できなかった問題を修正しました。 リダイレクト URLは無視され、元のページが代わりに表示されました。 （TGT-53306）
...
* [!UICONTROL 条件を作成] モーダルのローカライズの問題を修正しました。[!UICONTROL 価格規則を選択] ドロップダウンリストで「次の値の間」オプションを選択すると、[!UICONTROL 包含ルール ] セクションで「to」という文字列がローカライズ解除されました。 （TGT-49754）
* フィード作成ウィザードの[!UICONTROL 環境] ドロップダウンリストにある文字列「[!UICONTROL すべてのホストグループ ]」のローカライズに関する問題を修正しました。 （TGT-46737）
...
* QA環境が複数のタブ間でデータの読み込みに失敗し、インターフェイスが使用できなくなる問題を修正しました。 （TGT-53377）
* QA環境でアクティビティを作成できない問題を修正しました。 プロセスは、正常に完了する代わりに[!UICONTROL  アクティビティ ] ページにリダイレクトされました。 （TGT-53328）
...
* [!DNL Recommendations]でコレクションを作成する際に「ディープラーニング」オペランドにカーソルを合わせると、ページがクラッシュする問題を修正しました。 （TGT-53305）
* 更新されたUIの[!UICONTROL  カタログ検索]のフィルター候補が不正確だった問題を修正しました。 （TGT-52007）
* 「値が存在する」または「値が存在しない」演算子を使用する場合にOperands フィルターが表示される[!DNL Recommendations] UIの問題を修正しました。ただし、非表示にする必要があります。 （TGT-53012）
...
* ユーザーがAutomated Personalization（AP）アクティビティの編集中に[!UICONTROL  コンテンツの管理]をクリックし、[!UICONTROL 完了]をクリックすると、ページが空白になり、応答しなくなる問題を修正しました。 （TGT-53047 および TGT-52993）
* [!UICONTROL 目標と設定]の下の[!UICONTROL 表示mbox]のコンバージョン指標を選択すると、ページがクラッシュする問題を修正しました。 （TGT-53346、TGT-53343、TGT-53348）
* [!UICONTROL URLへのリダイレクト ]機能が期待どおりに機能しない問題を修正しました。有効なURLでもリダイレクトが発生しません。 （TGT-53307）
...
* ワークスペース間でアクティビティをコピーすると、重複した「オーディエンスコピー」エントリとIDの競合が発生する問題を修正しました。 オーディエンスは、結合されたオーディエンス（最大5 レベル）に対して、一意のID、ワークスペースコンテキスト、再帰処理でコピーされるようになりました。 （TGT-53081）
* ワークスペースが「[!UICONTROL すべてのワークスペース ]」に設定されている場合に、デフォルトのワークスペースに既に存在するアクティビティをコピーすると、誤ったエラーが発生する問題を修正しました。

  「デフォルト以外のワークスペースには、少なくとも1つのプロパティを含める必要があります。」

  コピーはデフォルトのワークスペース内にあるので、プロパティは必要ありません。 プロパティを追加して保存しようとすると、次のエラーが発生します。
...
主に複雑な顧客カスタマイズに関連して、最近特定された問題のため、このリリースには次の修正と更新が含まれています。

**アクティビティ**

+++詳細を見る

* ビルダークラスの`buildViews` メソッドが、割り当てられた最も高い`viewLocalId`ではなく、ビューの合計数`viewMaxLocalId`を誤って設定する問題を修正しました。 （TGT-53207）
* 更新された[!DNL Target] UIで、[!UICONTROL Automated Personalization] （AP）アクティビティで削除されたオファーが、元の名前ではなく`Deleted option with ID: X`と表示される問題を修正しました（例：従来のUIに表示される`Offer Name [Deleted]`）。 この修正により、削除されたオファーに対する意味のあるラベル付けが復元され、明確性が向上し、レポートをより正確でユーザーフレンドリーにすることができます。 （TGT-52921）
* [!DNL Target] フロントエンドから[!DNL Target] Centralに移行した一部のアクティビティで、以前に修正された同期バグが原因で、指標の設定に一貫性がなかった問題を修正しました。 具体的には、最初にコンバージョン指標を使用し、後で分析ベースの指標に更新されたアクティビティは、`primaryMetricType`および`successCriteria` フィールドに古い値を保持しました。 （TGT-52643）
* HTMLの変更に`contentEditable`属性が意図せず含まれたため、QA プレビューページのコンテンツ全体が編集可能になった問題を修正しました。 これにより、ユーザーはページ上のテキストをクリックして編集できるようになり、QA中にレイアウトの問題や混乱が発生する可能性がありました。 （TGT-53247）
* [!DNL Page Load]から[!UICONTROL  ビュー]に変更を移動すると、変更が重複し、[!UICONTROL  ページ読み込み]に残り、[!UICONTROL  ビュー]にも表示される問題を修正しました。 さらに、[!UICONTROL  ビュー]から変更を削除すると、[!UICONTROL  ページ読み込み]からも誤って削除されます。 （TGT-53270）
...
* バックエンド永続性レイヤーで、削除されたオプションが正しく保存されていても、既存のAPI エンドポイントからアクセスできない問題を修正しました。 その結果、フロントエンドアプリケーションは削除されたオプションに対して意味のある名前を取得できず、過去のレポートビューに影響を与えました。 この修正により、保存された削除されたオプションデータをUIで適切に表示できるようになりました。 （TGT-52973）
* JCR ベースのアクティビティから[!DNL Target] Centralへの削除されたアクティビティオプションの転送をサポートする新しい移行エンドポイントを実装しました。 この機能により、システム全体で一貫した追跡とレポートが可能になります。 この機能により、削除されたオプションが[!DNL Target] フロントエンドとバックエンド全体で保持および同期され、過去のレポートとデータの整合性が向上します。 （TGT-53217）
* ユーザーが以前に削除したアクティビティオプションをセカンダリデータベースから復元できる新しいAPI エンドポイントを導入しました。 この機能は、`RemovedCampaignElements`および`RemovedOptionInfo` クラスによって提供される既存のインフラストラクチャを活用し、削除されたオプションをアクティブなアクティビティにシームレスに再統合します。 （TGT-52903）
* 25文字を超える指標の名前を含む[!DNL Recommendations] アクティビティが、APIの制限により開いたり編集したりできない問題を修正しました。 この修正により、メトリック名が文字制限を超えて互換性が確保され、影響を受けるアクティビティへの完全なアクセスが復元されます。 （TGT-52839）
...
* [!UICONTROL Automated Personalization] （AP）アクティビティを作成または編集する際に&#x200B;**[!UICONTROL コンテンツを管理]** アイコン（![ コンテンツを管理](/help/main/assets/icons/Experience.svg)）をクリックすると、エディターがクラッシュする[!UICONTROL  フォームベースのExperience Composer]の問題を修正しました。 （TGT-53047）
...
* リストの下部にスクロールすると、[!UICONTROL  カタログ検索]が追加の結果を読み込めず、従来のUIと一致する動作を復元する問題を修正しました。 （TGT-53088）
* [!UICONTROL 条件の詳細] ダイアログボックスからの項目の削除をブロックする問題を修正しました。 （TGT-53245）
* 名前のない製品を開いたり、操作したりできない問題を修正しました。 この問題は、名前のない結果を返す環境を選択する際に発生し、製品の詳細にアクセスできなくなります。 （TGT-53007）
* 特定の製品を選択すると、[!UICONTROL  カタログ検索] ページがクラッシュし、空白の画面が表示される問題を修正しました。 （TGT-53087）
* ユーザーが[!DNL Target] UIでsite_cart_z1 [!DNL Recommendation] アクティビティを編集できない問題を修正しました。 アクティビティを開こうとすると、[!UICONTROL 概要] ページでエラーが発生し、エディターへのアクセスがブロックされました。 （TGT-53221）
...
* レポートソースを[!DNL Customer Journey Analytics]または[!DNL Analytics]から[!DNL Target]に切り替える際に、アクティビティデータベースのサンドボックスフィールドがクリアされない問題を修正しました。 以前は、UIがサンドボックス：nullを正しく送信していましたが、バックエンドはこの値を無視し、古いサンドボックスデータを残していました。 nullを受信すると、バックエンドがサンドボックスフィールドを適切にクリアするようになりました。 （TGT-52798）
* [!UICONTROL Automated Personalization] （AP）アクティビティの正確な履歴レポートをサポートするために、Target バックエンドの削除されたオプション永続性レイヤーを再実装しました。 以前は、オプションが削除されると、その名前が失われ、過去のパフォーマンスデータを解釈することが困難でした。

  **主な改善点**:

  * 削除されたオプションは、既存の`RemovedCampaignElements`および`RemovedOptionInfo` インフラストラクチャを使用して追跡されるようになりました。
  * AP アクティビティからオプションが削除されると、そのメタデータ（IDや名前など）が保持されます。
  * レポート UIで、元のオプション名（例：`Option Name [Deleted]`）を過去の指標と共に表示できるようになり、明瞭性と使いやすさが向上しました。

  このアップデートにより、オプションがアクティビティから削除された後でも、一貫した有意義なレポートが保証されます。 （TGT-52986）
...
* ビューに変更を適用すると重複が発生し、「無効なユーザー入力」エラーがトリガーされるVECの問題を修正しました。 （TGT-52886）
* VECで画像オファーを設定する際の[!UICONTROL 挿入前]および[!UICONTROL 挿入後] オプションの[!UICONTROL 取り消し]機能に関する問題を修正しました。

  以前は、画像オファーに対する[!UICONTROL 挿入前]または[!UICONTROL 挿入後] アクションを取り消すと、一貫性のない動作が発生したり、変更を適切に元に戻すことができませんでした。特に、従来の[!DNL Target] UIで作成されたアクティビティでは問題がありました。 この問題は、元に戻すアクションがこれらの変更に対して確実に機能するように解決されました。 （TGT-52809）

* `contentEditable`属性が誤ってtrueに設定され、保存されたHTML コンテンツに保持される問題を修正しました。 このアップデートにより、意図しない編集操作を行うことなく、クリーンで期待されるHTML出力が保証されます。 （TGT-52319）
* 削除されたオプションの永続的な損失を防ぎ、サービス間で一貫した動作を確保するために、UIおよび関連するマイクロサービスのオプションにソフト削除機能が実装されました。

  **主な変更**:

  * オプションは完全に削除されなくなりました。 代わりに、パラメーターXML オブジェクトに新しい削除済み：true フラグが付けられます。
  * このフラグは、削除されたオプションをレンダリングから除外し、エッジサービスに送信しないようにするために、更新された[!DNL Target] UIでのみ使用されます。
  * 削除されたオプションは、編集中もアクティビティペイロードの一部として残り、存在しないオプションの顧客への配信を避けながら、トレーサビリティを確保します。

  このアップデートは、データの整合性を向上させ、分散システムでの削除を管理するためのベストプラクティスと一致しています。 （TGT-52726）
...
* デフォルト以外のワークスペースからデフォルト以外のワークスペースに、またはデフォルト以外のワークスペース間でアクティビティをコピーする際の問題を修正しました。 オファーは、競合を防ぐために強化されたトラッキングと命名規則で複製されるようになりました。

  **主な改善点**:
  * オファーは、更新されたIDとメタデータを使用して、宛先ワークスペースで再作成されます。
  * コピーされたオファーの名前は、「オファー名コピー」の形式と、一意性を確保するためのランダムな番号またはタイムスタンプを使用して変更されます。
  * 新しいIDを反映するように、オファーとアクティビティの状態が更新されます。
  * この機能は、繰り返しコピーアクション中に複数の同じ「オファーコピー」名が原因で発生するエラーを防止します。
  * オファーは、宛先ワークスペースのオファーリストにすぐに表示されないかもしれませんが、適切に処理され、表示されます。

  このアップデートにより、複数のワークスペースをまたいでオファーを管理する際の信頼性とトレーサビリティが向上します。 （TGT-53080）
...
* 未保存の変更を含むアクティビティ編集をキャンセルする際に、追加の確認警告を追加しました。「このアクティビティを保存しますか？ 保存しないと、すべての変更が失われます。」 このメッセージは、偶発的なデータ損失を防ぐのに役立ちます。 （TGT-52865）
* レガシー機能を[!UICONTROL 目標と設定]の[!UICONTROL 優先度] スライダーに復元し、レガシーUIでサポートされているように、お客様が数値を直接入力できるようにしました。 （TGT-53185 および TGT-53219）
...
* カスタムオーディエンスを含むアクティビティを保存または編集できない問題を修正しました。 お客様は、「リクエストを完了できません。 問題が解決しない場合は、[!DNL Adobe Client Care]に連絡してください。」 特定のアクティビティに対して、変更を保存する場合や、変更なしで保存する場合。 （TGT-53189）
...
* 顧客が[!UICONTROL 目標と設定] ページで特定のアクティビティに関するレポートを表示した際に、Analyticsの[!UICONTROL  ビュー] リンクが実稼働環境ではなくQA環境を誤ってポイントする問題を修正しました。 （TGT-53163）

+++

**[!UICONTROL エクスペリエンス ]および[!UICONTROL  オファー]**

+++詳細を見る

* カスタムコードを使用して`triggerView`を呼び出すと、無限ループが発生する問題を修正しました。 （TGT-52885）
* アクティビティに対して定義された`LocalIds`と、エクスペリエンス定義で使用される`LocalIds`との間で不一致が発生する問題を修正しました。 （TGT-52669）
* アクティビティ [!UICONTROL 概要] ページで指標名が見つからない問題を修正し、正しい指標名の代わりに「オファー」のみを表示しました。 （TGT-53054）
...
* [!UICONTROL  フォームベースのExperience Composer]で、アクティビティの保存が妨げられ、「未定義のプロパティを読み取れません（「マップ」を読み取っています）」というエラーメッセージが表示される問題を修正しました。 （TGT-53145）
...
* [!UICONTROL  カタログ検索]から商品をクリックすると、「商品の詳細を取得できませんでした」というエラーが表示され、閉じるオプションなしでモーダルが開く問題を修正しました。 （TGT-53082）
* コレクションまたはプロモーションを変更する際に、[!UICONTROL A/B テスト ] アクティビティの[ オファーとしての推奨事項](/help/main/c-recommendations/recommendations-as-an-offer.md)が正しく更新されない問題を修正しました。 （TGT-52884）
* 更新されたUIでエンティティをクリックすると、「製品の詳細を取得できませんでした」というエラーが表示される実稼動環境の問題を修正しました。 この問題が解決しない場合は、[!DNL Adobe Client Care]にお問い合わせください。」 （TGT-53071）
...
* 注文の詳細をCSV ファイルに保存すると、空のファイルが生成される問題を修正しました。 （TGT-52225）
...
* 複数のエクスペリエンスで使用されるセレクターが選択されたように常に強調表示されない[!UICONTROL 目標と設定] ページの問題を解決しました。 （TGT-53062）
* アクティビティの編集を妨げ、「未定義のプロパティを読み取れません（「マップ」を読み取ります）」というエラーメッセージが表示される問題を修正しました。 （TGT-53161）
...
* ワークスペースを切り替える際のアドホックオファーの処理を改善しました。
  * デフォルトのワークスペースからデフォルト以外のワークスペース（またはデフォルト以外のワークスペース）に切り替えると、アドホックオファーが正しくコピーされるようになりました。 初期化時に、ワークスペースのコンテキストが更新され、一意性を確保するために新しいIDがオファーに割り当てられます。
  * 同じワークスペース内に留まっている場合は変更は発生しません。 （TGT-53079）
* 顧客が異なるワークスペース間でアクティビティを[ コピーできない問題を修正しました](/help/main/c-activities/edit-activity.md#section_45A92E1DD3934523B07E71EF90C4F8B6)。 （TGT-52753 および TGT-47094）
* ワークスペース間でプロパティを変更する際の問題を修正しました。
  * デフォルトのワークスペースをデフォルト以外のワークスペースに切り替える場合、現在のプロパティが宛先ワークスペースに存在する場合、プロパティは保持されます。
  * [!UICONTROL  プロパティ ] リストに警告が表示され（一部のプロパティが互換性がないことを示している可能性があります）、顧客が[!UICONTROL 追加]または[!UICONTROL 削除]をクリックしてから[!UICONTROL 保存]をクリックすると、宛先ワークスペースにないプロパティはすべて削除されます。 お客様が[!UICONTROL  キャンセル ]をクリックすると、宛先ワークスペースにプロパティが存在しない場合でも、すべてのプロパティが残ります。 （TGT-47094）
  * 同じワークスペースに留まる場合、またはデフォルト以外のワークスペースからデフォルトまたは別のワークスペースに切り替える場合、すべてがそのまま残ります。 （TGT-53078）
* アクティビティの元のワークスペースコンテキストを尊重するように、エンティティ検証ロジックを更新しました。 [!UICONTROL  エクスペリエンスフラグメント ] （XF）などのエンティティは、アクティビティが最初に作成されたワークスペースに基づいて検証されるようになりました。 例えば、XFがデフォルトのワークスペースに存在し、アクティビティがワークスペース Xからワークスペース Yにコピーされる場合、XFが元の（デフォルトの）ワークスペースで有効である限り、検証は引き続き渡されます。 （TGT-53196）
* アクティビティの複製中にアドホックオーディエンスをコピーする機能が強化されました。
  * 指標、レポート、ページ、アクティビティのみのタイプなどのアドホックオーディエンスが、次のシナリオで自動的にコピーされるようになりました。
    * デフォルトのワークスペースからデフォルト以外のワークスペースにアクティビティをコピーする場合。
    * 同じワークスペース内のアクティビティをコピーする場合。 （TGT-53197）
...
* [!UICONTROL  アクティビティ QA] URLに不要なクエリパラメーターが含まれている問題を修正しました：`at_preview_evaluate_as_true_audience_ids`。 （TGT-52907）
* プレビューURLに、ユーザーが明示的に入力したオーディエンス以外の追加オーディエンスが誤って含まれる問題を修正しました。 この動作は、QAまたはプレビューリンクを生成する際に、指定したオーディエンスのみが適用されるように修正されました。 （TGT-52912）
* トラフィック配分の設定中に[!UICONTROL 自動配分] （AA）が最初に選択された場合に、ユーザーが[!UICONTROL 自動ターゲット ] （AT）アクティビティを作成できない問題を修正しました。 この問題により、バックエンドの検証エラーが発生し、アクティビティが保存されなくなります。 （TGT-53096）
...
* [!UICONTROL 承認者]の役割を持つユーザーが、アクティビティのみのオーディエンスの絞り込みを追加または保存できない問題を修正しました。 これを試みると、ユーザーがアクティビティの承認と管理に十分な権限を持っていても、「[editor]」権限が必要であると記載された403 Forbidden エラーが発生しました。 （TGT-52984）
* アクティビティ固有のオーディエンスが「[!UICONTROL  オーディエンスの絞り込みを削除]」オプションを使用して削除されると、同じアクティビティ内で再選択するためにオーディエンスがオーディエンスリストに表示されなくなる問題を修正しました。 この動作により、最初から再作成しない限り、ユーザーは同じオーディエンスを再追加できません。 （TGT-52979）
* アクティビティが保存される前であっても、アクティビティのみのオーディエンスの絞り込みが、場所から削除された直後にUIから消える問題を修正しました。 この動作は、期待される機能とツールチップのガイダンスに矛盾しています。「アクティビティが保存されると、このライブラリからのすべての未使用のオーディエンスが削除されます。」 （TGT-52982）
* アクティビティに[!UICONTROL すべての訪問者]以外のオーディエンスを割り当てようとした場合の問題を修正しました。 保存すると、次のエラーメッセージが表示されます。「リクエストを完了できません。 問題が解決しない場合は、[!UICONTROL Adobe クライアントケア ]にお問い合わせください。」 （TGT-53008）
* アクティビティエディター内で新しいオーディエンスを作成して割り当てた後、アクティビティの保存がブロックされる問題を修正しました。 表示されたエラーメッセージは「リクエストを完了できません。 問題が解決しない場合は、[!UICONTROL Adobe クライアントケア ]にお問い合わせください。」 （TGT-52977）
...
* 既存のアクティビティをコピーし、レポートソースを[!DNL Adobe Analytics] （A4T）に変更すると、「無効なユーザー入力」エラーが発生する問題を修正しました。 `restart_same_experience`、`restart_random_experience`、`restart_new_experience`など、[!DNL Analytics]のレポートと互換性のない特定の指標アクションが元のアクティビティから保持されたときに、エラーがトリガーされました。 （TGT-52900）
* [!UICONTROL 目標と設定]手順でレポートソースとして[!DNL Adobe Analytics] （A4T）を選択すると、顧客がアクティビティの作成または保存をブロックする問題を修正しました。 この問題は、[!UICONTROL  カスタムイベント ]指標（例：「カスタムイベント 16」）を選択する際に特に発生し、「無効なユーザー入力」というエラーが発生しました。 （TGT-52910）
* Analytics]で「[!UICONTROL  ビュー」リンクをクリックすると、ユーザーが意図した[!DNL Analytics] ダッシュボードではなくホームページにリダイレクトされる問題を修正しました。 （TGT-53092 および TGT-53093）
...
* 更新された[!UICONTROL 概要] UIで[!DNL Recommendations] アクティビティを表示する際に、[!DNL Adobe Analytics] （A4T）がレポートソースとして選択されていると、[!UICONTROL 目標と設定] セクションが読み込まれない問題を修正しました。 次のエラーメッセージが表示されました：「問題が発生しました。 ご要望の処理は完了できません。 問題が解決しない場合は、アドビのカスタマーケアにお問い合わせください。」 （TGT-52999）
...
* すべてのコンテンツオプションを削除した後、[!UICONTROL  コンテンツを管理]機能が場所の状態を正しく検証しなかった問題を修正しました。 この問題は、アクティビティ設定を保存または続行しようとすると、動作の一貫性が失われたり、エラーが発生したりする可能性があります。 （TGT-52801）
* 新しいページを追加し、異なるエクスペリエンス内の特定の要素を削除する際に、ユーザーが「無効な入力」エラーに遭遇する問題を修正しました。 このエラーは、要素の操作中に、特にエクスペリエンスの切り替えや共有ページ構造の変更時に生成される重複`LocalIds`によってトリガーされます。 （TGT-52720）
* [!UICONTROL  アドホックオファーを生成]機能を使用すると、[!UICONTROL  コンテンツを管理] パネルに未定義の場所が表示される問題を修正しました。 （TGT-53076 および TGT-53070）
* HTML オファーを使用して行った変更が、[!UICONTROL  ターゲティング ]段階から[!UICONTROL  エクスペリエンス ]に戻る際に見つからない可能性があるお客様に対する動作を明確にしました。 この顧客の場合、影響を受けるweb サイトでは、ページの読み込みごとに変更される複数のDOM セレクターが動的に生成されました。 その結果、最初に修正に使用されたセレクターは、エディターを再度開いたときに見つからず、修正が見つからないか無効に見えます。 このシナリオは計画通りに機能しています。 エディターで変更が引き続き視覚的に表示されるようにするには、クライアントが安定した一貫性のあるセレクターを使用し、ページのリロード時に変更しないことをお勧めします。 （TGT-52874）
* 除外されたエクスペリエンスの一部であるオファーを削除または非アクティブ化しようとすると、「無効なユーザー入力」エラーが発生する問題を修正しました。 この問題は、オファーが含まれるエクスペリエンスで積極的に使用されていなかったにもかかわらず発生しました。 （TGT-52917）
...
* フォームベースのアクティビティで、エクスペリエンスを複製し、複製されたエクスペリエンスの1つでカスタムコードを編集すると、それらの変更がすべての複製されたエクスペリエンスに誤って適用される問題を修正しました。 各エクスペリエンスは、複製した後も独自のカスタムコードを個別に保持するようになりました。 （TGT-51600）
...
* フランス語（fr_FR）、ドイツ語（de_DE）、イタリア語（it_IT）、韓国語（ko_KO）、簡体字中国語（zh_CN）の新しいUIのローカライゼーション文字列を更新しました。
...
* 新しい[!DNL Recommendations] フィード [status](/help/main/c-recommendations/c-products/feeds.md#status)を追加しました：[!UICONTROL 部分的な読み込みに失敗]。 （KB-2215）
* [!UICONTROL  プロモーション ]で[!DNL Recommendations]を追加する際のアクティビティ作成ワークフローに影響する問題を修正しました。 ユーザーが「[!UICONTROL 属性で昇格]」を選択し、フィルタリングルール（[!UICONTROL  パラメータマッチング ]など）を追加した場合、アクティビティの保存と再編集後、選択したルールタイプとオペランド値は保持されませんでした。 再び開くと、フィルタリングルールタイプが予期せず変更され、オペランド値が欠落します。 （TGT-53059）
* ルールのロジックに関係なく、単一のルールで作成されたプロモーションが誤って解釈され、「アイテムのリスト」プロモーションタイプとして表示される[!DNL Recommendations] UIの問題を修正しました。 （TGT-53063）
* 更新された[!UICONTROL 概要]UIを使用する際に、「[!UICONTROL 推奨事項データをダウンロード ]」ボタンが[!DNL Recommendations]を含む[!UICONTROL  エクスペリエンスターゲティング ] （XT）アクティビティに表示されない問題を修正しました。 （TGT-52730 および TGT-52756）
* 以前は、Recommendations UIには、フィードから正常に読み込まれたエンティティの数のみが表示されていました。 ただし、バックエンドメッセージ形式には、読み込まれたエンティティの数と形式のエンティティの合計数の両方が含まれます：`# of entities imported / # of total entities`。 この不一致により、UIの最初の値（インポートされたカウント）のみが表示され、混乱が生じました。 UIに両方の数値が表示されるようになりました。 （TGT-53073）
* レコメンデーションを使用したフォームベースのA/B アクティビティで「[!UICONTROL 属性で昇格]」プロモーションを設定する際に、顧客がフィルタリングルールを保存できない問題を修正しました。 アクティビティを保存して再度開くと、フィルタリングルールが見つからず、アクティビティを正常に保存できませんでした。 （TGT-53057）
...
* [!UICONTROL  レポート ] ページから「[!UICONTROL 注文の詳細をCSV]に書き出し」を選択すると、空のファイルがダウンロードされる問題を修正しました。 この問題は、有効な注文データがアクティビティに存在する場合でも発生しました。 （TGT-52225）
* 新しいレポートオーディエンスを作成して割り当てた後にアクティビティを保存しようとした場合の問題を修正しました。 返されたエラーメッセージは「アクセスが拒否されました。 この操作を実行するには、次のすべての権限が必要です：[editor]。」 この問題は、ユーザーが承認者レベルのアクセス権を持っているにもかかわらず発生しました。 （TGT-53103）
...
* ビューに変更を適用すると、ビューが複製され、アクティビティが「無効なユーザー入力」エラーを返す問題を解決しました。 この修正により、重複や検証エラーをトリガーせずに、ビューの変更が正しく適用されるようになります。 （TGT-52886）
* 間違ったエクスペリエンスに対してカスタムコードの変更が誤って表示される問題を修正しました。 具体的には、1つのエクスペリエンスを対象とした変更が異なるエクスペリエンスで表示され、ライブアクティビティの混乱や潜在的な設定ミスにつながりました。 （TGT-52776）
* 新しいVEC UIでカスタムコードの変更を編集または保存できない問題を修正しました。 具体的：

  * カスタムコードブロックを編集して保存した後、変更はUIまたはQA プレビューに反映されませんでした。
  * 場合によっては、アクティビティを閉じて再度開かないと、変更を削除できません。
  * 回避策として、ユーザーはコードをコピーし、変更を削除し、更新されたコンテンツを使用して手動で再作成する必要がありました。 （TGT-53072）

* カスタムコードを編集して保存すると、[!UICONTROL 変更] パネルが応答しなくなる問題を修正しました。 （TGT-53075）
* バリアントエクスペリエンスのカスタムコードに対して行われた変更が、[!UICONTROL Control] エクスペリエンスに意図せず反映される問題を修正しました。 これにより、配信動作が意図しない変更になりました。 [!UICONTROL Control] エクスペリエンスは、他のエクスペリエンスに対して行われたカスタムコード編集から分離されたままになります。 （TGT-52413）
* エディターが完全に読み込まれる前に、ユーザーが2番目のエクスペリエンスをクリックした場合、1つのエクスペリエンス（エクスペリエンス Bなど）に対して行われた変更が、別のエクスペリエンス（エクスペリエンス A）に意図せず複製される問題を修正しました。 この動作により、最初に選択したエクスペリエンスに変更がない場合、変更が失われる可能性もあります。 （TGT-52597）
* アクティビティ作成の[!UICONTROL 変更] ステップで行われた変更が一貫して保存されない問題を修正しました。 場合によっては、すべての手順を完了し、[!UICONTROL 保存]をクリックした後、[!UICONTROL 変更] セクションに追加されたカスタムスクリプトは、保存処理中に表示されるエラーがないにもかかわらず、ライブサイトに反映されません。 （TGT-52661）
* カスタムコードの変更が正しく保存されず、同じアクティビティ内の複数のエクスペリエンス間で意図せずミラーリングされる問題を修正しました。 さらに、特定のアクティビティを開いたり更新したりすると、アクセスの問題が発生し、空白の画面が表示されます。 これらの問題は、安定したアクティビティ編集と正確なエクスペリエンスの分離を確保するために解決されました。 （TGT-52594）
* [!UICONTROL 参照モード ]でユーザーが別のURLを参照できない問題を修正しました。 これにより、テスターとエディターは、同じアクティビティセッション内で代替ページを検証またはプレビューできなくなりました。 （TGT-53052）
* アクティビティの作成中に複数の[!UICONTROL Visual Experience Composer] （VEC） インスタンスが同時に開く問題を修正しました。 この問題は、ユーザーが[!UICONTROL Enhanced Experience Composer] （EEC）を無効にし、[!UICONTROL  ページ配信]手順でweb サイト URLから末尾のスラッシュを削除した場合に発生しました。 （TGT-52782）
* ユーザーが別の指標を選択した後でも、[!UICONTROL 目標と設定] ステップの[!UICONTROL 収益]指標ドロップダウンが、訪問ごとの[!UICONTROL 収益] （RPVISIT）に間違ってデフォルトで設定される問題を修正しました。  この問題は、指標の設定パネルを折りたたんで再展開すると発生し、以前に選択した値がリセットされます。 （TGT-52811 および TGT-52878）
* [!UICONTROL Multivariate Testing] （AP）および[!UICONTROL Automated Personalization] （MVT）アクティビティでのオファーの命名とコンテンツの翻訳に関連するアクティビティ作成ワークフローのいくつかの問題を修正しました。

  対処された主な課題：

  * 同じ名前で複数のHTML オファーを作成する（例えば、「Experience」）際に「オファー名の重複は許可されていません」というエラーが発生しましたが、UIは、競合の原因となっているオファーを明確に示しませんでした。
  * 右側のパネルを使用してオファーの名前を変更すると、UIの名前が更新されましたが、変更は[!UICONTROL  コンテンツの管理] タブまたは[!UICONTROL  オファー] タブに反映されず、永続的な検証エラーが発生しました。
  * MVT アクティビティでは、名前を変更した後も重複した名前エラーは保持されませんでしたが、UIは、更新されたオファー名をタブ間で一貫して反映できませんでした。 （TGT-52933）
...
* 更新された[!UICONTROL Visual Experience Composer] （VEC） UIに[!UICONTROL Rearrange] オプションを追加し、従来のVECで利用できる機能に合わせました。 （TGT-46957 および TGT-52876）
* [!UICONTROL A/B テスト ] アクティビティでバリアント エクスペリエンス（エクスペリエンス Bなど）に対して行われた変更が保持されない問題を修正しました。 エクスペリエンスを切り替えると、バリエーションの変更は消えます。 この問題は、コントロールのエクスペリエンスには影響しません。 （TGT-52664）
* 特定の顧客がアクティビティを作成または保存できず、他の顧客が同じアクションを問題なく実行できる問題を修正しました。 問題がアカウント間で一貫性を失いました。（TGT-52842）
* 更新されたVECで、ユーザーが変更を[!UICONTROL  ページ読み込みイベント ]に移動できなかった問題を修正しました。これは、従来のUIに存在していた機能です。 （TGT-52617）
* 更新されたUIで、変更を作成する際に[!UICONTROL  ページ読み込み] イベントが[!DNL Target]に表示されない問題を修正しました。更新はビューにのみ適用されます。 （TGT-52604）
* 一部のアクティビティの変更が、更新されたVECに正しく表示されない問題を修正しました。 （TGT-52818）
* [!UICONTROL Automated Personalization] （AP）アクティビティのレポートデータの取得時に発生したnull ポインターの例外を修正しました。 （TGT-52362）
* [!UICONTROL Automated Personalization] （AP）アクティビティの.CSV ファイルにオファーレベルの詳細が表示されない問題を修正しました。 （TGT-52675）
* 更新されたVECで変更を適用する際に、最初に変更が正しく表示され、予想される[!UICONTROL  エクスペリエンスフラグメント ]が含まれる問題を修正しました。 ただし、エクスペリエンスを切り替えたり、追加の編集を行ったりすると、セレクターの問題により、一部の変更が適用されない場合があります。 （TGT-52679）
* 既存のアクティビティを複製して新しいアクティビティを作成すると、複製されたアクティビティ内のQA リンクが元のアクティビティのページ URLを誤って保持する問題を修正しました。 （TGT-52775）
* 更新されたVECで[!UICONTROL  オンデバイス決定]が利用できなくなっていた問題を修正しました。 （TGT-52371）
* 製品[!DNL Recommendations] アクティビティを編集できない問題を修正しました。 Target UIを介してVECにアクセスしようとすると、[!UICONTROL 概要] ページにエラーが表示され、編集ができなくなります。 （TGT-52823）
* エクスペリエンス名が50文字を超えた場合に[!DNL Recommendations] アクティビティを保存できない問題を修正しました。 （TGT-52619）
* 新しいUIで条件を変更した後、お客様がRecommendations アクティビティを保存できない問題を修正しました。 この問題は権限に関連しているようで、同じ役割を持つすべてのユーザーには影響しません。 （TGT-52816）
* [!UICONTROL 編集者]の役割を持つユーザーが[!DNL Recommendations] アクティビティを編集できない問題を修正しました。 デザインを変更してアクティビティを保存しようとすると、ユーザーが関連するワークスペースに既にその役割を持っていても、「[editor]」権限が必要であると記載された403 Forbidden エラーが発生しました。 （TGT-52836）

### [!DNL Target Standard/Premium] 25.6.3（2025年6月20日（PT））

このリリースには、次の修正および更新が含まれています。

* あるワークスペースから別のワークスペースにアクティビティをコピーすると、「nullにしないでください」や「問題が発生しました」などのエラーがトリガーされる問題を修正しました。 （TGT-52474）
* 特定のアクティビティに対して[!UICONTROL 自動セグメント ]および[!UICONTROL 重要属性] レポートが生成されない問題を修正しました。 （TGT-52904）
* 更新されたVECで、[!UICONTROL Automated Personalization] （AP）アクティビティのデフォルトコンテンツ処理が従来のUIと一致しない問題を修正しました。 グループが明示的に追加されていない場合、システムはデフォルトの`optionGroup`を`optionGroupLocalId = 0`に自動的に追加するようになりました。 このグループには、既定のオプション （例：`optionLocalId: 0`）が含まれています。 デフォルトのコンテンツが削除されると、対応するオプショングループも削除されます。 （TGT-52651）
* 以前に削除したエクスペリエンスから`experienceLocalId`を再利用できない[!UICONTROL 多変量テスト ] （MVT）アクティビティの問題を修正しました。 （TGT-52672）
* エクスペリエンスフラグメントを含むアクティビティをコピーまたは編集できない問題を修正しました。 次のエラーが発生しました：`Enum "AemOfferType" cannot represent value: "html"`。 （TGT-52635）
* アクティビティの場所のURLで、スラッシュ（/）などの無効な文字が原因でクエリパラメーターを表示できない問題を修正しました。 （TNT52845）
* バックエンド APIを介した[!DNL A/B Test]件のアクティビティ更新の検証エラーメッセージを改善しました。 重複する場所の名前が存在する場合、メッセージに次の内容が明確に表示されるようになりました：`locations.selectors`の「名前の重複は許可されていません」。 （TGT-52589）
* リクエストペイロードで認識されないプロパティが原因で、ライブ [!UICONTROL Recommendations] アクティビティを更新する際に発生したエラーを修正しました。 システムが「無効なJSON」を適切に処理するようになりました。 認識できないプロパティ名」エラー。 （TGT-52723）
* [!DNL Recommendations] デザインを作成できない問題を修正しました。 [!UICONTROL Create]をクリックすると、「スクリプト内に少なくとも1つのエンティティ変数が使用されている必要があります」というメッセージがトリガーされました。 （TGT-52395 および TGT-52899）
* 変更なしで[!DNL Recommendations]のデザインを再保存できない問題を修正しました。 （TGT-52879）
* [!UICONTROL Recommendations] アクティビティの保存時に「400 Bad Request」エラーが発生するバックエンド検証エラーを修正しました。 （TGT-52716）
* [!UICONTROL  フォームベースのExperience Composer]で、[!UICONTROL 場所] ドロップダウンの特殊文字を含むmboxにカーソルを合わせると、エディターが空白になり、「要素」で「querySelector」を実行できませんでした」という問題が発生する問題を修正しました。 エラーが表示されます。 （TGT-52717）
* 新しい「PARTIALLY_IMPORTED」インジケーターにより、フィードステータスの精度が向上しました。 以前は、ファイル内のすべての行が読み込まれなかった場合でも、フィードは「成功」とマークされていましたが、これは誤解を招く結果でした。 （TGT-52892）
* AP V2に移行した後、`/admin/rest/ui/v1/campaigns`への特定のAPI呼び出しでクライアントサイドエラー（HTTP 4xx）が返されるエラーを修正しました。 （TGT-52721）

### [!DNL Target Standard/Premium] 25.6.2（2025年6月12日（PT））

このリリースには、次の修正および更新が含まれています。

* 更新された[!DNL Target] UIと[!UICONTROL Visual Experience Composer] （VEC）に関する一般的な質問に対応するために、[新しいFAQ記事](/help/main/c-intro/updated-ui-faq.md)を追加しました。
* [!UICONTROL  ページ配信]の「[!UICONTROL URL – を含まない]」ルールが機能しない問題を修正し、コンテンツをブロックする必要がある場合でも表示できるようにしました。 （TGT-52754）
* 「[!UICONTROL  ページ配信]」がエラーメッセージを正しく表示しない問題を修正しました。「ページ URLの重複は許可されていません。 （TGT-52765）
* エクスペリエンスフラグメントを含む[!UICONTROL  ページ配信] URLのオーディエンスが#個の誤って追加されて作成される問題を修正しました。 （TGT-52786）
* [!UICONTROL 目標と設定] ページでアクティビティをコピーして設定を編集すると、[!DNL Target] UIが応答しなくなる問題を修正しました。 （TGT-52797）
* 更新された[!UICONTROL Visual Experience Composer] （VEC）で、[!UICONTROL A/B テスト ] アクティビティの追加ページを同じURLにリダイレクトすることが誤って許可されていた問題を修正しました。 （TGT-51838）
* アクティビティの編集時に、[!UICONTROL 目標と設定] ページの指標に対する変更が保存されない問題を修正しました。 （TGT-52799）
* Web エディターの読み込み中に新しいエクスペリエンスを追加すると、新しいエクスペリエンスが以前のエクスペリエンスのコンテンツを複製する問題を修正しました。 （TGT-51397）
* 以前は従来のTarget UIで使用されていた機能である`<head>` タグの外部でカスタムコードを使用する機能を復元しました。 （TGT-52304 および TGT-52300）
* アクティビティの作成中にデフォルトのワークスペースを選択する際の不要な検証を削除しました。 必須のプロパティ検証は、デフォルトのワークスペースには適用されなくなりましたが、デフォルト以外のワークスペースには引き続き適用されます。 （TGT-52449）
* 更新された[!UICONTROL Visual Experience Composer] （VEC）で、`triggerView()`呼び出しが検出されない問題を修正しました。 （TGT-52575）
* 更新された[!UICONTROL Visual Experience Composer] （VEC）で、ユーザーが[!UICONTROL  シングルページアプリケーション ] （SPA）ビューに変更を追加できない問題を修正しました。 （TGT-52556）
* 更新された[!DNL Target] UIで、顧客がオファーの詳細を表示できない問題を修正しました。 （TGT-52607）
* [!UICONTROL  オファーライブラリ ]でオファーに対して行われた更新が、更新された[!UICONTROL Visual Experience Composer] （VEC）に反映されない問題を修正しました。 （TGT-52637）
* アクティビティの作成中にオファーセクションが正しく表示されない問題を修正しました。 （TGT-52773）
* `optionGroups`で参照されているすべての`optionLocalIds`がoptions配列に存在することを確認するための検証を追加しました。 無効な参照は、アクティビティの作成中に自動的に削除されます。 （TGT-52687）
* 新しいオファーを追加した後、レポートグループと除外が保持されない問題を修正しました。 （TGT-52728）
* [!UICONTROL  アクティビティ QA] ボタンを持たないアクティビティに、空のオプションセレクターが表示される問題を修正しました。 （TGT-52733）
* QA リンクでコンテンツを適切にレンダリングできない問題を修正しました。 （TGT-52718）
* 要素をエクスペリエンスフラグメントに置き換えても、QA環境で変更が正しく反映されない問題を修正しました。 （TGT-52762）
* 更新された[!UICONTROL Visual Experience Composer] （VEC）で、ユーザーがエクスペリエンスフラグメントを追加しようとしたときに「無効な入力」エラーが発生する問題を修正しました。 （TGT-52701）
* 更新された[!UICONTROL Visual Experience Composer] （VEC）でオーディエンスターゲティングを編集する際に、「オーディエンスを編集」モーダルが空に表示される問題を修正しました。 （TGT-52749）
* 選択したワークスペースでエンティティにアクセスできないときにユーザーに通知するメッセージを追加しました。 （TGT-52767）
* UIで、環境IDを条件に手動で割り当てることができなかった問題を修正しました。 代わりに、[!UICONTROL 製品カタログ検索] ホストグループのIDにデフォルトで設定されます。 この修正により、デフォルトだけでなく、すべての環境で条件変更が適用されるようになりました。 （TGT-52817）
* [!UICONTROL  エクスペリエンスのターゲット設定] （XT）アクティビティに対して、「[!UICONTROL  レコメンデーションデータをダウンロード ]」オプションが見つからない問題を修正しました。 （TGT-52730 および TGT-52756）

### [!DNL Target Standard/Premium] 25.6.1（2025年6月6日（PT））

このリリースには、次の修正および更新が含まれています。

* QA リンクで、関連するアクティビティの正しいエクスペリエンスが配信されない問題を修正しました。 （TGT-52163 および TGT-52790）
* 関連するオーディエンス IDがQA リンクで欠落していた問題を修正しました。 （TGT-52722）
* 設定されたページ配信URL条件が正確に満たされた場合にのみエクスペリエンスが配信される問題を修正しました。 （TGT-52696）
* 顧客が[!DNL Recommendations] デザインテンプレートを作成できない問題を修正しました。 テンプレートを作成しようとすると、「スクリプト内に少なくとも1つのエンティティ変数が使用されている必要があります」というエラーが発生しました。 （TGT-52395）
* Velocity配列を使用して[!DNL Recommendations] デザインを保存できない問題を修正しました。 「スクリプト内で少なくとも1つのエンティティ変数を使用する必要があります」というエラーメッセージが正しくトリガーされませんでした。 （TGT-52734）
* 内部web ページのページの読み込みに失敗した場合に、[!UICONTROL Visual Experience Composer] （VEC）で変更にアクセスできない問題を修正しました。 （TGT-52488 &amp;TGT-52470）
* VECの小さい画面サイズで[!UICONTROL 変更] パネルが表示されない問題を修正しました。 （TGT-52470）
* 更新されたVECで、[!UICONTROL 参照] モードから[!UICONTROL  デザイン ] モードに切り替えるとコンソールエラーが発生し、それ以上の操作が妨げられる問題を修正しました。 （TGT-52532）
* 特定の要素をクリックすると、意図せずにそのサイズが拡大するVECの問題を修正しました。 （TGT-52497）
* 特定のページ要素の読み込みに失敗したり、VECで認識されたりする問題を修正し、ボタンやバナーの選択や、アクティビティでの正確なイベントトラッキングの中断などのインタラクションを防止しました。 （TGT-52663）
* [!UICONTROL Automated Personalization] （AP）アクティビティで、お客様がオファーを削除または削除できない問題を修正しました。 （TGT-52690）
* 複数ページのアクティビティでアクティビティの選定の動作に一貫性がない問題を修正しました。 （TGT-52694）
* アクティビティの[!UICONTROL 概要] ページで[!UICONTROL  アクティビティの場所]の無効なURLが表示される問題を修正しました。 （TGT-52695）
* 更新された[!DNL Target] UIで、アクティビティの場所に重複したエントリが表示される問題を修正しました。 （TGT-52693）
* `getAudiencesV3` エラーが発生し、顧客がアクティビティを編集またはコピーできない問題を修正しました。 （TGT-52709）
* アクティビティに[!UICONTROL  エクスペリエンスフラグメント ]またはHTML オファーを追加する際に無効なペイロードエラーが発生する問題を修正しました。 （TGT-52779 および TGT-52773）
* 更新された[!DNL Target] UIで、E[!UICONTROL  エクスペリエンスフラグメント ]が無効な入力エラーが原因で正しく表示できなかった問題を修正しました。 （TGT-52701）
* 無効なユーザーエラーが原因で、顧客が[!UICONTROL  フォームベースのExperience Composer]でアクティビティを編集できない問題を修正しました。 （TGT-52470）
* 以前の翻訳で基本多言語面の外の文字が使用されていた韓国語のローカライズの問題を修正しました。 更新された翻訳は、意図された意味を正確に伝える適切な文字を使用します。 （TGT-52508 および TGT-52509）
* アクティビティの開始日と終了日を選択する際に「日付」の翻訳が一貫しない韓国語のローカライズの問題を修正しました。 （TGT-52510）
...
* レガシー[!DNL Target] UIの機能に合わせて、[!UICONTROL  レポート ] > [!UICONTROL  レポート設定] （![ レポート設定アイコン ](/help/main/assets/icons/Setting.svg)）の下にQA モードトラフィック設定を追加しました。 （TGT-52228 および TGT-52329）
...
* 更新された[!UICONTROL Visual Experience Composer] （VEC）が`at_property`値を自動検出できなかった問題を修正しました。 （TGT-52347）
* フォーム要素の操作中にVECで[!UICONTROL 参照]と[!UICONTROL  デザイン ]のモードを切り替えた後に、1つだけの変更が予想される場合に2つの変更が記録される問題を修正しました。 （TGT-52455）
* セレクターが無効であるか、既に使用されているか、表示されていないことを示すエラーにより、更新されたVECの[!UICONTROL  クリック済み要素]設定を選択できなかった問題を修正しました。 （TGT-52467）
* 更新されたVECで[!UICONTROL Recommendation Offer] ボックスを追加すると、重複した（ゴースト）ボックスが表示される問題を修正しました。 エクスペリエンス A と B を繰り返し切り替えると、ゴーストボックスがさらに追加されました。 （TGT-52505 および TGT-52519）
* 更新された[!DNL Target] UIで、[!UICONTROL  オファー] メニューを介して行われたHTML オファーの変更が、関連するアクティビティに反映されない問題を修正しました。その逆も同様です。 この動作は、更新が[!UICONTROL  オファー] メニューとアクティビティの間で正しく同期される従来のUIと一致するようになりました。 （TGT-52540 および TGT-52541）
* アクティビティ内でエクスペリエンスフラグメントを使用しようとすると、[!UICONTROL  オファーライブラリ ]の[!UICONTROL  エクスペリエンスフラグメント ]に対する最近の更新が反映されない問題を修正しました。 （TGT-52659）
...
* [!UICONTROL  アクティビティ ] リストの名前検索機能が、複数単語のクエリで正しく機能しない問題を修正しました。 （TGT-52529）
* [!UICONTROL Automated Personalization] （AP）アクティビティからエクスペリエンスを除外できない問題を修正しました。 （TGT-52383）
* AP アクティビティでコンテンツを管理する際に、「[!UICONTROL Contains]」オプションが[!UICONTROL  フィルタールール ]に見つからない問題を修正しました。 （TGT-52384）
* [!UICONTROL Automated Personalization] （AP）アクティビティのレポートの不整合を修正しました。特に、[!DNL Target]の内部システムから`optionLocalId`個の値を使用してデフォルトのオファーを追跡およびレポートする方法に関連しています。
* QA リンクで、意図したアクティビティエクスペリエンスを配信できない問題を修正しました。 （TGT-52163）
* [!UICONTROL 承認者]権限を持つユーザーがライブアクティビティの編集を誤ってブロックされ、「アクセスが拒否されました」というエラーメッセージが表示される問題を修正しました。 （TGT-52416）
...
* [!UICONTROL 製品管理者]および[!UICONTROL  システム管理者]権限を持つ[!DNL Target] ユーザーが、[!DNL Target]での役割に関係なく、[!UICONTROL 管理] ページのすべての設定を編集できるようになりました。 これらの権限を持たないユーザーには、これらの設定に対して読み取り専用アクセス権があります。 この更新により、[管理設定](/help/main/administrating-target/administrating-target.md)に対するより厳格なアクセス制御が確保されます。 （TGT-48179）
...
* VECでサイトを読み込んだ後、[!UICONTROL  サイト環境設定] セクションでIDとクラスによる選択を顧客が有効にできない問題を修正しました。 [!UICONTROL  サイト環境設定]は、有効にした後でも自動的に無効に戻りました。 （TGT-52207）
* [ ページ配信](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#settings) URLがスラッシュ（/）で終了した場合、[!UICONTROL Visual Experience Composer] （VEC）で正しいページを表示できない問題を修正しました。 （TGT-52237）
...
* アクティビティ [!UICONTROL 概要] ページで[!DNL Customer Journey Analytics]が正しく表示されない問題を修正しました。 （TGT-52359）
* [!UICONTROL Automated Personalization] （AP）アクティビティでレポートグループが保持されない問題を修正しました。 （TGT-52368）
* オファー決定支援を含むアクティビティを保存できない問題を修正しました。 （TGT-52390）
* デフォルトのオファーが選択されたが、[!UICONTROL Automated Personalization] （AP）および[!UICONTROL 多変量テスト ] （MVT）アクティビティに表示されるその他のオファーコンテンツが表示される問題を修正しました。 （TGT-52372）
* 完全な組織アクセスと特定の組織 + ユーザーアクセス間の OR をチェックするように GET 権限ロジックを修正しました。 （TGT-52374）
* [!UICONTROL 選択済みのみを表示]が有効になっているにもかかわらず、[!UICONTROL 管理コンテンツ ]および[!UICONTROL  レポート対象オーディエンス ]のオーディエンスを選択した後にオーディエンス名が表示されない問題を修正しました。 （TGT-52393）
...
* [!UICONTROL  アクティビティ ]の設定ページと[!UICONTROL  レポート ]の概要ページの間でオーディエンスリストに矛盾が生じる問題を修正しました。 （TGT-52203）
...
* アクティビティの[!UICONTROL 概要] ページの場所が正しくない問題を修正しました。 （TGT-52182）
...
* 追加のページを追加し、[!UICONTROL  エクスペリエンス ] タブに戻ると変更が消える問題を修正しました。 （TGT-52264）
* [!UICONTROL  エクスペリエンスのターゲット設定] （XT）アクティビティで、顧客がオーディエンスを変更できない問題を修正しました。 （TGT-52191）
* サポートされていない UI ルールにより、XT アクティビティを編集できないエラーを修正しました。 （TGT-52273）
* 更新された[!UICONTROL Visual Experience Composer] （VEC）で、エディターの下部にパンくずリストが常に表示されないため、要素を正確に選択するのが困難になる問題を修正しました。 （TGT-52169）
* ページネーションが原因で[!UICONTROL Audience] ドロップダウンリストがすべてのオーディエンスを表示できなかった問題を修正しました。 （TGT-52204）
* [!UICONTROL Automated Personalization] （AP）アクティビティで新しいオファーを追加する際に、無効なユーザー入力メッセージが発生する問題を修正しました。 （TGT-52210）
* お客様がA4Tにアクセスできない場合でも、[!UICONTROL Analytics for Target] （A4T）がレポートソースとして誤って選択される問題を修正しました。 （TGT-52226）
* [!UICONTROL  ページを表示] URL指標を使用してアクティビティを保存できない問題を修正しました。 （TGT-52260）
...
2025年4月22日より、[!DNL Target] ワークスペースでの役割に関係なく、[!UICONTROL 管理] セクションの設定を更新できるのは[!UICONTROL 製品]および[!UICONTROL  ソリューション ]管理者のみです。 この権限を持たないユーザーは、[!UICONTROL 管理] セクションへの読み取り専用アクセス権を持ちます。
...
* [!UICONTROL 編集者]権限を持つ顧客がアクティビティを保存できない問題を修正しました。 （TGT-52227）
...
* 特定の[!UICONTROL  エクスペリエンスのターゲット設定] （XT）アクティビティのオーディエンス情報ポップアップを顧客が開けないエラーを修正しました。 （TGT-52049）
* [!UICONTROL Visual Experience Composer] （VEC）で行われた変更に続いて、オーディエンス設定が「[!UICONTROL すべての訪問者]」に戻る問題を修正しました。 （TGT-52132）
* 特定のアクティビティのオーディエンスの絞り込みが表示されない問題を修正しました（TGT-52057）
* 顧客がデフォルトのワークスペースに[!UICONTROL  エクスペリエンスフラグメント ]を挿入できない問題を修正しました。 （TGT-52073）
* [!UICONTROL Automated Personalization] （AP）アクティビティの[!UICONTROL  オファー] ページにオファーが「見つかりません」と表示され、表示されない問題を修正しました。 （TGT-52150）
* アクティビティ内で重複するオーディエンスを許可する機能を追加しました。 （TGT-51200）
* 編集後にXT アクティビティの[!UICONTROL 目標と設定] ページに間違ったmbox名が表示される問題を修正しました。 （TGT-52026）
...
* [!UICONTROL 目標と設定] ページの[!UICONTROL  レポート設定]の[!UICONTROL 最適化目標]を編集後に再設定する必要がある問題を修正しました。 （TGT-52071）
* ページ配信ルールのないアクティビティが[!UICONTROL 概要] ページに複数のルールを表示する問題を修正しました。 （TGT-52084）
...
* 編集中に、更新された[!UICONTROL Visual Experience Composer] （VEC）にClickTrack指標が表示されない問題を修正しました。 （TGT-52152）
* アクティビティの場所としてクエリパラメーターを含むURLが、アクティビティの[!UICONTROL 概要] ページにクエリパラメーターを表示しない問題を修正しました。 （TGT-51635）
* [!UICONTROL Visual Experience Composer] （VEC）内の[!UICONTROL 参照モード ]でエクスペリエンス URL全体が表示されない問題を修正しました。 （TGT-52101）
* アクティビティを編集するとページ配信で URL の末尾に「/」が追加され、無効になる問題を修正しました。 （TGT-52114）
* [!UICONTROL  フォームベースのExperience Composer]の[!UICONTROL Activity QA] リンクが[!DNL Adobe Experience Cloud] ホームページに誤ってリダイレクトされる問題を修正しました。 （TGT-52055）
* 保存して再度開いた後、[!UICONTROL A/B テスト ] アクティビティに追加されたページが保持されない問題を修正しました。 （TGT-51994）
* お客様がインラインスタイルセクションでスタイルを削除できない問題を修正しました。 （TGT-52070）
* 従来のUIと同様に、[!UICONTROL Activity QA] ダイアログボックスの[ オーディエンス定義カード ](/help/main/c-target/c-audiences/audiences.md#section_11B9C4A777E14D36BA1E925021945780)へのアクセスを復元しました。 （TGT-52056）
...
* Ove[!UICONTROL r]表示ページおよびアクティビティ編集中に、ターゲティンググラフ内のオーディエンスを表示できない問題を修正しました。 （TGT-52050）
* [!UICONTROL  エクスペリエンスターゲティング ] （XT）アクティビティで、顧客が優先順にエクスペリエンスを並べ替えられなかった問題を修正しました。 （TGT-52054）
* テキストスタイルの変更を元に戻す際に、誤ったレンダリングが発生する問題を修正しました。 （TGT-51876）
* リダイレクトオファーを変更する際に、[!DNL Target]はそのオファーに関連付けられている[!UICONTROL ClickTrack] セレクターも削除する問題を修正しました。 （TGT-51936）
* [!UICONTROL ClickTrack]のキャンセル時に[!DNL Target]がセレクターを誤って保存する問題を修正しました。 （TGT-51937）
* [!UICONTROL 目標と設定] ページでmbox ピッカーを開いたり閉じたりした後、変更を加えずに無効な名前エラーが発生する問題を修正しました。 （TGT-51983）
...
* ナビゲーションバーの前にコンテンツを挿入するために[!UICONTROL 前に挿入] オプションを使用できない問題を修正しました。 （TGT-52031）
...
* [!UICONTROL  アクティビティ ] ページの読み込みが遅くなる問題を解決しました。 （TGT-51151）
...
* アクティビティの作成中または編集中にオーディエンスを「[!UICONTROL すべての訪問者]」に置き換えると、「オーディエンスの重複は許可されていません」エラーが発生する問題を解決しました。 （TGT-51978）
* [!UICONTROL A/B テスト ] アクティビティの保存時に「無効なユーザー入力」エラーが発生する問題を解決しました。 （TGT-51976）
* [!UICONTROL 目標と設定] ページで計算指標が正しく表示されない問題を解決しました。 （TGT-51975）
...
* ページオーディエンスを削除すると、[!UICONTROL ClickTrack] セレクターも削除される問題を解決しました。 （TGT-51935）
* [!UICONTROL 概要] ページを開いた後にアクティビティを編集できない問題を解決しました。 （TGT-51931）
* アクティビティ作成中に、`[Unused optionLocalIds: 0]]` エラーが発生する問題を修正しました。 （TGT-51920）
* テキストスタイルの変更を削除した後に、一部の変更が正しく翻訳されない問題を修正しました。 （TGT-51876）
* [!UICONTROL  フォームベースのExperience Composer]で、ターゲットオーディエンスが正しく更新されない問題を解決しました。 （TGT-51845）
* アクティビティのナビゲーション中に[!UICONTROL Visual Experience Composer]のURLが正しく更新されない問題を解決しました。 （TGT-51832）
* アクティビティの作成時やオファーの追加時に正しく表示されても、[!UICONTROL  オファー] UIにオファーが表示されない問題を解決しました。 （TGT-51805）
* パーソナライズされたコンテンツやターゲットコンテンツを配信できない場合に、一部のアクティビティでデフォルトコンテンツを表示するためのフォールバック画面が欠落する問題を修正しました。 （TGT-51638）
* ライブオファーと特定のフォルダーが[!UICONTROL  オファー] UIに正しく表示されない問題を解決しました。 （TGT-51628）
* 一部の URL 文字列と goURL が正しくローカライズされない問題を修正しました。 （TGT-35741）
* 役割（[!UICONTROL 承認者]、[!UICONTROL 編集者]、および[!UICONTROL  オブザーバー]）が[!DNL Target] UIで正しくローカライズされない問題を修正しました。 （TGT-29925）

### [!DNL Target Standard/Premium] 25.3.6（2025年3月14日（PT））

このリリースには、次の修正および更新が含まれています。

* 同じ[!UICONTROL ClickTrack] セレクターを複数回使用する場合に[!UICONTROL  クリックトラッキング ]が有効になっている[!UICONTROL Visual Experience Composer] （VEC）アクティビティの「無効なユーザー入力」エラーを解決しました。 （TGT-51921）
...
* ユーザーが[!UICONTROL 変更] パネルでオファーを変更できない問題を解決しました。 （TGT-51800）
* [!UICONTROL ClickTrack] モードを含め、エクスペリエンスとオーディエンスの左側のパネルにアクションが誤って表示される問題を解決しました。 （TGT-51895）
...
* アクティビティのみのオーディエンスが[!UICONTROL  オーディエンス ] パネルに表示されず、編集や再利用が妨げられる問題を解決しました。 （TGT-51860）
* [!UICONTROL Analytics for Target] （A4T）レポートを使用して、[!DNL Target Standard]人の顧客がアクティビティを作成するのをブロックする問題を修正しました。 （TGT-51854）
...
* アクティビティのみのオーディエンスで[!UICONTROL  エクスペリエンスのターゲット設定] （XT）アクティビティを編集できない問題を修正しました。 （TGT-51846）
* [!UICONTROL Visual Experience Composer] （VEC）が最初の編集時にエクスペリエンスに変更を正しく適用できなかった問題を修正しました。 （TGT-51843）
* VEC 内の特定の要素をクリックした際に、「ID」エラーがトリガーされる問題を修正しました。 （TGT-51814）
* アクティビティ作成中の VEC のエラー処理を更新しました。 （TGT-51759）
* アクティビティの保存時に[!UICONTROL 変更] パネルでビューが見つからない場合に、「無効なユーザー入力」エラーが発生する問題を修正しました。 （TGT-51827）
* レコメンデーション条件を作成できない問題を修正しました。 （TGT-51834）
...
* 結合されたオーディエンスにはサブグループを含めることができ、サブグループにはそれぞれ複数のオーディエンスが含まれます。 このリリースでは、[!UICONTROL  ルール ] ダイアログボックスにサブグループオーディエンスが表示されない問題を修正しました。 （TGT-51813）
* 古いアクティビティを開くと、一部のエクスペリエンスオーディエンスが[!UICONTROL すべての訪問者]に置き換えられる問題を解決しました。 （TGT-51812）
...
* アクティビティを作成する[!UICONTROL  ターゲティング ]手順で、オーディエンスの除外ルールがオーディエンスの情報パネルに表示されない問題を解決しました。 （TGT-51579）
...
* アクティビティ作成プロセスをキャンセルすると、すべてのステート内オーディエンスの変更が削除される問題を解決しました（[!UICONTROL  オーディエンスを追加]ではなく[!UICONTROL  キャンセル ]を選択）。 （TGT-51769 および TGT-51770）
* 一部のアクティビティ（特にカスタムコードを使用した場合）で、[!UICONTROL Visual Experience Composer] （VEC）の読み込みに失敗する問題を解決しました。  この問題により、VEC に空白の画面が表示されたり、[!DNL Target] UI が古いバージョンに戻ったりしました。 （TGT-51758）
* オーディエンス向けのページ配信を編集した後に、変更が破棄される問題を修正しました。 （TGT-51756）
* [!UICONTROL 目標と設定] ページで指標の種類を変更すると、指標ではないオーディエンス（ページおよびエクスペリエンスオーディエンス）がすべてアクティビティから削除される問題を解決しました。 （TGT-51753）
* アクティビティの編集中に[!UICONTROL  キャンセル ]をクリックすると、[!UICONTROL  アクティビティの詳細] ページではなく、[!UICONTROL  アクティビティ リスト ]にTarget UIが移動する問題を解決しました。 （TGT-51731）
* [!UICONTROL  レポートをCSVに書き出し] オプションを使用してレポートをダウンロードできない問題を解決しました。 （TGT-51708）
* [!DNL Target Standard]人の顧客が[!DNL Target Premium]機能である[!UICONTROL  プロパティ ]を使用していると誤って表示されるフォームベースのExperience Composerの問題を解決しました。 （TGT-51678）
* 新しいオファーを作成する際に、[!DNL Adobe Experience Platform] 属性が表示されない問題を修正しました。 （TGT-51665）
* [!DNL Recommendations] インベントリのすべてのアクティブなフィルターをクイック検索に移動し、UIを[!UICONTROL  フィルター] レールではなく[!UICONTROL  カタログ検索]に合わせました。 （TGT-50723）
...
* [!UICONTROL  アクティビティ ] ユーザーインターフェイスの更新
* [!DNL Recommendations] ユーザーインターフェイスの更新

#### [!UICONTROL  アクティビティ ] ユーザーインターフェイスの更新

[!DNL Adobe Target] UIの近代化の取り組みが続いているため、更新された[!UICONTROL  アクティビティ ] ユーザーインターフェイスの一般公開をお知らせします。

>[!NOTE]
>
>2月17日以降、お客様は新しい[!UICONTROL  アクティビティ ] UIに徐々にアクセスできるようになります。 すべてのお客様にシームレスなロールアウトを確実に行えるように、このリリースは制御したステージでデプロイされます。 最初のステージでは、[!DNL Target]のお客様の最初のグループを新しい[!UICONTROL  アクティビティ ] UIにアップグレードします。 後続のステージでは、残りのお客様をアップグレードします。
...
* [[!UICONTROL 変更ログ ]](/help/main/c-activities/change-log.md) ページを更新しました。リアルタイム インサイト用に[[!DNL Audit Query API]](https://experienceleague.adobe.com/ja/docs/experience-platform/landing/governance-privacy-security/audit-logs/audit-api/overview){target=_blank}から情報を取得できるようになりました。
...
* ブラウザープロバイダーからの最新のセキュリティアップデートと最新のユーザーインターフェイスをサポートして、[完全に[!UICONTROL Visual Editing Composer]](/help/main/c-experiences/c-visual-experience-composer/visual-experience-composer.md)を再構築しました。
...
>[!NOTE]
>
>2月17日以降、お客様は新しい[!UICONTROL Recommendations] UIに徐々にアクセスできるようになります。 すべてのお客様にシームレスなロールアウトを確実に行えるように、このリリースは制御したステージでデプロイされます。 最初のステージでは、[!DNL Target]のお客様の最初のグループを新しい[!UICONTROL  アクティビティ ] UIにアップグレードします。 後続のステージでは、残りのお客様をアップグレードします。

この更新では、最新の [!DNL Adobe Spectrum] デザインシステムに基づいて、以前は一貫性がなかったデザインパターンを標準化すると共に、次のような新しい機能強化が追加されています。

* [製品カタログ検索](/help/main/c-recommendations/c-products/catalog-search.md)には、製品のリアルタイム同期を可能にする更新されたデータベースが用意されています。
* API経由で作成された[!UICONTROL Recommendations] オブジェクト （[!UICONTROL 条件]、[!UICONTROL  デザイン ]、[!UICONTROL  コレクション ]、および[!UICONTROL 除外]）が、UI](/help/main/c-recommendations/c-recommendations-faq/recommendations-faq.md)で利用できるようになりました。[
* [推奨事項の設定](/help/main/administrating-target/recommendations-settings.md)は、[!UICONTROL 管理] セクションに統合されました。
...
#### [!UICONTROL  オファーライブラリ ] ユーザーインターフェイスの更新

[!DNL Adobe Target] ユーザーのユーザーエクスペリエンスを向上させるために、このリリースでは、[!UICONTROL  オファーライブラリ ] ユーザーインターフェイスが更新されます。
...
* **[!UICONTROL コードエディター]のアップグレード**：構文の強調表示と行番号を使用して、HTMLおよびJSON エディターを更新しました。
...
* JSON オファーの使用
* [[!UICONTROL  フォームベースのExperience Composer]](/help/main/c-experiences/form-experience-composer.md)で作成されたアクティビティでのカスタム決定範囲の使用
* グローバルな`__view__` スコープを使用する[!UICONTROL  フォームベースのExperience Composer]を使用して作成されたアクティビティの取得時に`renderDecisions: true`を使用しない
...
* **パーソナライゼーションと測定のための単一の呼び出し。**
  ...
  * バウンス数の減少。
  * A4Tと[!UICONTROL  ページビュー]のヒットが結合されていないため、[!DNL Analytics]個のeVarとイベントを使用してA4T レポートの特定の分類と相関関係を実行することが困難です。
...
### [!UICONTROL Visual Experience Composer] ヘルパー拡張機能（2024年4月23日）
...
### [!UICONTROL  ブラウザー]のオーディエンス属性の`Browser:iPad`と`Browser:iPhone`の更新（2024年4月30日）

| アップデート | 詳細 |
|--- |--- |
| [!UICONTROL  ブラウザー:iPad]と[!UICONTROL  ブラウザー:iPhone]が、オーディエンスの作成時に使用される[ ブラウザー属性](/help/main/c-target/c-audiences/c-target-rules/browser.md)で更新されました。 | [!DNL Adobe Target] を使用すると、ページを訪問した際に特定の[ブラウザーやブラウザーオプション](/help/main/c-target/c-audiences/c-target-rules/browser.md)を使用する訪問者など、[いくつかのカテゴリ属性のいずれかをターゲットに設定](/help/main/c-target/c-audiences/c-target-rules/target-rules.md)できます。<P>[!DNL Target] Standard／Premium 24.3.1（2024年3月4～6日（PT））以降、`Browser:iPad` や `Browser:iPhone` などのターゲット UI を使用して作成されたビルトインのオーディエンスは、`profile.mobile.deviceVendor`、`profile.mobile.isMobilePhone`、`profile.mobile.isTablet` を使用して [!DNL iPad] と [!DNL iPhone] に対して適切なターゲティングを実行するように更新されます。<P>このアップデートでは、お客様側でのアクションは必要ありません。<p><B>重要</b>：お客様がプロファイルスクリプト（および JavaScript セグメント）で [!DNL iPad] と [!DNL iPhone] を適切にターゲティングを実行するには、**2024年4月30日（PT）**&#x200B;までにお客様が手動で変更を行う必要があります。 手動で変更する必要がある代替設定の例については、[!UICONTROL  ブラウザー]のオーディエンス属性](/help/main/c-target/c-audiences/c-target-rules/browser.md#updates)の[更新 [!DNL iPad] および [!DNL iPhone] を参照してください。 |

### [!UICONTROL Visual Editing Helper]拡張機能（2024年3月14日）
...
* [!UICONTROL Visual Experience Composer] （VEC）でオーサリングを実行中に拡張機能がCookieを複製する問題を修正しました。
...
* [!UICONTROL Analytics for Target] （A4T）レポートで設定された[!UICONTROL 多変量] （MVT）アクティビティが正しく表示されない問題を修正しました。 （TGT-47490）
...
* 収益目標の指標を持つTarget] （A4T）アクティビティの[!UICONTROL Analyticsでは、列名として「収益」が表示されず、収益指標がレポートに（$）形式で表示されませんでした。 これは、表面上の問題で、既に修正されています。 （TGT-46995）
* レポートの日付間隔が正しく機能しない問題を修正しました。 （TGT-47396）
* 顧客が[!UICONTROL 詳細アクション ] アイコンを使用してアクティビティをアクティブ化または非アクティブ化した後、誤ったステータスが[!UICONTROL すべてのアクティビティ ] ページに表示される問題を修正しました。 （TGT-47367）
* 1人の顧客に対して[!UICONTROL 重要属性] レポートが表示されない問題を修正しました。 （TGT-47272）
...
* [!UICONTROL Automated Personalization] アクティビティでのエクスペリエンスに対して[重複オファー](/help/main/c-activities/t-automated-personalization/managing-exclusions.md)を許可しない機能をサポートするように、[ アクティビティ QA](/help/main/c-activities/c-activity-qa/activity-qa.md)機能を強化しました。 （TGT-46627）
* コントロールエクスペリエンスにトラフィックが割り当てられていない場合、アクティビティレポートでデータが使用できない理由をお客様が理解できるように、[!DNL Target] UI にツールチップを追加しました。 詳細な情報へのリンクがツールヒントに含まれています：[自分のアクティビティのレポートに利用できるデータがないのはなぜですか？](/help/main/c-reports/reporting-frequently-asked-questions.md#section_E4722F6445884130951DF79981C8289B)。 （TGT-46610）
* 少数の顧客の[!UICONTROL  アクティビティ ] ページでアクティビティが正しく表示されない問題を修正しました。 （TGT-46830）
* レポート ソースとして[[!UICONTROL Analytics for Target]](/help/main/c-integrating-target-with-mac/a4t/a4t.md) （A4T）を使用するアクティビティに影響を与えた次の問題を修正しました。
  * 一部の顧客がレポートデータを表示できない問題を修正しました。 （TGT-46557）
  * アクティビティレポートページの[!UICONTROL Analytics] リンクの表示が正しく機能しないことがある問題を修正しました。 （TGT-46731）
  * [!UICONTROL Lift]および[!UICONTROL Confidence]のデータが[!DNL Target] UIで正しく表示されない問題を修正しました。 （TGT-46592、TGT-46554、TGT-46586）

### [!UICONTROL  アクティビティ ] ページのユーザーインターフェイスの更新（2023年10月25日（PT））

[!DNL Target]人のユーザーのユーザーエクスペリエンスを向上させる[!DNL Adobe Target] チームの継続的な取り組みの一環として、このリリースでは、[!DNL Target] UIの[!UICONTROL  アクティビティ ] ページが更新されます。 この更新では、以前は一貫性がなかったデザインパターンが統合および標準化されたほか、新しい機能強化が追加されています。
...
* 新しい[!UICONTROL  アクティビティ ] UIを強化して、新しいアクティビティを作成する際に`selectorCriteria`のデフォルト設定で[!UICONTROL Visual Experience Composer] （VEC）が開くように変更しました。 （TGT-46586）
* VECの使用時に[!UICONTROL Composer] モードで一部のお客様が要素を編集できない問題を修正しました。 （TGT-46470）
* カスタム属性を使用する際に、汎用的な優先セレクターを指定する機能を追加しました。 （TGT-46545）
* [!DNL Adobe Analysis Workspace]でレポートが正しく表示されていても、[!UICONTROL Analytics for Target] （A4T）を使用する[!UICONTROL 自動ターゲット ] レポートが[!DNL Target] UIに表示されない場合がある問題を修正しました。 （TGT-46494）
...
| 機能 | 詳細 |
| --- | --- |
| [!DNL Recommendations] 実装パターン | *at.js を使用したレコメンデーション実装パターンの*&#x200B;記事は、at.js JavaScript ライブラリを使用する場合の [!DNL Adobe Target Recommendations] 実装の理解と作成に役立ちます。<P>詳しくは、*Adobe Target 開発者ガイド*&#x200B;の [at.js の概要を使用したレコメンデーション実装パターン](https://experienceleague.adobe.com/docs/target-dev/developer/implementation-patterns/atjs/recs-implementation-pattern-atjs.html?lang=ja){target=_blank}を参照してください。 |

* 動的フレームワーク用に[!UICONTROL Visual Experience Composer] （VEC）の機能強化を追加しました。 （TGT-44064）
...
* Lightning Web コンポーネント （Light DOM）をサポートするように[!UICONTROL Visual Experience Composer] （VEC）を強化しました。 （TGT-45422）
* VEC アクションが正しくない順序で適用される問題を修正しました。 VECが一部の変更を非同期で適用し、要素に追加の変更を追加すると、その要素が[!UICONTROL 挿入] アクションの後に表示された場合にエラーが発生する場合があります。 また、アンカーリンクをクリックすると更新されるようになった VEC の URL の修正も行っています。 （TGT-45983）
* シャドウ DOMの要素をサポートするようになったVEC [!UICONTROL  オーバーレイ ]機能の問題を修正しました。 （TGT-45202 および TGT-45262）
* VECでシングルページアプリケーション（SPA）ページを開き、[!UICONTROL 参照] モードに移行すると、戻る矢印と進む矢印が正しく機能しない問題を修正しました。 （TGT-45956）
...
* 一部のお客様の[!UICONTROL Automated Personalization] （AP）アクティビティに影響を与えた問題を修正しました。これには、[!DNL Target] バックエンドのアクティビティをタイムリーに同期し、プレビューリンクで期待されるエクスペリエンスを配信することが含まれます。 （TGT-46202）

### [!DNL Target] Standard/Premium 23.9.1（2023年9月6日～11日（PT））

このリリースには、以下の機能強化および修正が含まれています。

* [!UICONTROL Analytics for Target] （A4T）をレポートソースとして使用する[!UICONTROL 自動割り当て] アクティビティの[!DNL Target] UIと[!DNL Adobe Analytics] UIで一貫性のないレポートデータが発生する問題を修正しました。 （TGT-46112）
...
* [!UICONTROL  アクティビティ ] リストページの「[!UICONTROL  ステータス ]」列に示すように、アクティビティが正常に同期されない場合がある問題を修正しました。 （TGT-46010 および TGT-44831）
* 「[!UICONTROL Analytics for Target] （A4T）をレポートソースとして使用するアクティビティの[!UICONTROL  レポート ] ページに「]Analyticsで表示」リンクが表示されない場合がある問題を修正しました。 [!UICONTROL （TGT-45808）
* テーブル内の値の表示を、小数点以下の数値ではなくパーセンテージで表示するように調整しました。 例えば、.08 ではなく 8％で表示します。 （TGT-45548）
* [!UICONTROL  エクスペリエンスのターゲット設定] （XT）アクティビティの[!UICONTROL 目標と設定] ページで、キーボードフォーカスを使用して次の要素に移動できない問題を修正しました。 （TGT-44526）
...
* [Visual Experience Composer] （VEC）のDOM パス ](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#dom-path)を使用して要素を移動し、シャドウ DOM要素を含める場合の検索を改善しました。 [!UICONTROL （TGT-45262）
...
| 機能 | 詳細 |
|--- |--- |
| [!UICONTROL Automated Personalization] アクティビティの[!UICONTROL QA モード ] | [!DNL Adobe Target] [!UICONTROL QA モード ]が[!UICONTROL Automated Personalization] アクティビティで利用できるようになりました。これは、[!UICONTROL  プレビューリンク ]機能に代わるものです。<P>詳しくは、[アクティビティ QA](/help/main/c-activities/c-activity-qa/activity-qa.md) を参照してください。 |

* [!UICONTROL Automated Personalization]のアクティビティで[除外事項](/help/main/c-activities/t-automated-personalization/managing-exclusions.md#concept_4EF78013F80E48EFA024AE0274C9F037)を管理する際に、機能の重複（読み込み時間の短縮を含む）を禁止するパフォーマンスの機能強化。
...
* 画像名にGB 18030文字が含まれている場合に、[!UICONTROL  デザインを作成] パネルに画像が表示されない問題を修正しました。 （TGT-44614）
* エクスペリエンスのテキスト／HTML で、一部の GB 18030 シンボル文字が正しくエスケープされなかった問題を修正しました。 （TGT-44600）
* 分析中に[!UICONTROL 自動Personalization] アクティビティのレポートがフリーズする問題を修正しました。 （TGT-44820）
* アクティビティ名に角括弧（[または]）が含まれている場合、[!UICONTROL  アクティビティ ] ページでアクティビティを検索できない問題を修正しました。 （TGT-44777）
...
* [!UICONTROL Visual Experience Composer] （VEC）でページを読み込めないURLにクエリパラメーターが追加される問題を修正しました。 （TGT-44873）
* [!DNL Target] UI 全体で様々なローカライゼーションの修正を行いました。

### [!DNL Target] [!UICONTROL Real-Time CDP プロファイル属性]と共有されたReal-Time CDP プロファイル属性（2023年6月13日）

このリリースには、次の機能拡張が含まれています。

| 機能 | 詳細 |
|--- |--- |
| [!DNL Target] と共有される Real-Time CDP プロファイル属性 | [!UICONTROL Real-Time CDP プロファイル属性]は、[!DNL Target]と共有して、HTMLおよびJSON オファーで使用できます。<P>詳しくは、[Real-Time CDP プロファイル属性の  [!DNL Target]](/help/main/c-integrating-target-with-mac/integrating-with-rtcdp.md#rtcdp-profile-attributes)との共有を参照してください。 |
...
| 機能 | 詳細 |
|--- |--- |
| AEM [!UICONTROL  コンテンツフラグメント ]：ヘッドレスでのパーソナライゼーションと実験 | [!DNL Target]件のアクティビティで[!DNL Adobe Experience Manager] （AEM） [!UICONTROL  コンテンツフラグメント ]を使用します。 AEM の使いやすさと機能を、[!DNL Target] の強力な人工知能（AI）および機械学習（ML） 機能と組み合わせると、ヘッドレスパーソナライゼーションと実験を支援できます。<P>詳しくは、[AEM [!UICONTROL  コンテンツフラグメント ]](/help/main/c-integrating-target-with-mac/aem/content-fragments-aem.md)を参照してください。 |
| [*Adobe Target 開発者ガイド*](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=ja){target=_blank} | *Adobe Target開発者ガイド*&#x200B;が&#x200B;*[!UICONTROL Adobe Experience League]*&#x200B;に再配置されました。 *[!UICONTROL Experience League]*&#x200B;への移行により、テキストのローカライズが強化され、*Experience League*&#x200B;内での検索が統合され、*[!UICONTROL Adobe Targetビジネス実践ガイド]*&#x200B;と&#x200B;*[!UICONTROL Adobe Target開発者ガイド]*&#x200B;の両方から検索結果を表示して提供できるようになり、さらに多くのメリットが得られます。<P>以前の場所から&#x200B;*[!UICONTROL Experience League]*&#x200B;に自動的にリダイレクトされます。 必要に応じてブックマークを更新してください。 |
...
| 機能 | 詳細 |
|--- |--- |
| [!UICONTROL 自動配分]および[!UICONTROL 自動ターゲット ]のA4T指標を最適化しました<p>（リリース日：2023年3月30日（PT）） | [!DNL Target]では、[!UICONTROL 自動配分]および[!UICONTROL 自動ターゲット ] アクティビティに[!UICONTROL A4T]を使用する場合、連続イベントに基づいて二項イベントまたは指標に基づいて指標を選択できます。<P>サポートされる指標には、次の変更点があることに注意してください。<ul><li>[!DNL Target]2023年9月9日（PT）まで、既存のアクティビティの以前の動作が維持されます。 この日以降、既存のアクティビティを新しい動作に強制的に移行するため、サポートされていない指標を使用しているアクティビティは廃止されます。</li></ul>詳しくは、[!UICONTROL 自動配分]および[!UICONTROL 自動ターゲット ] アクティビティ ](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md#supported)の[A4T サポートの「サポートされている目標指標」を参照してください。<br>この機能により、次のチュートリアルが更新されました。<ul><li>[自動配分] アクティビティ ](https://experienceleague.adobe.com/docs/target-learn/tutorials/integrations/set-up-a4t-reports-in-analysis-workspace-for-auto-allocate-activities.html?lang=ja){target=_blank}の [!DNL Analysis Workspace] でA4T レポートを設定しています[!UICONTROL </li><li>[[!UICONTROL 自動ターゲット ] アクティビティ ](https://experienceleague.adobe.com/docs/target-learn/tutorials/integrations/set-up-a4t-reports-in-analysis-workspace-for-auto-target-activities.html?lang=ja){target=_blank}に対して [!DNL Analysis Workspace] でA4T レポートを設定しています</li></ul> |
...
* ユーザーが[!UICONTROL 管理]/[!UICONTROL Visual Experience Composer]/[!UICONTROL  デフォルト URL]の下の[!UICONTROL  デフォルト URL]を削除できるようにする拡張UI。 この変更により、お客様はデフォルトの URL を空の文字列に戻すことができます。以前は初回設定後にはできませんでした。 （TGT-44577）
* お客様が標準のオーディエンス（予約済みの名前を持つオーディエンス）を編集または削除できないようにする制限を削除しました。 （TGT-44655）
* [組み合わせオーディエンス ](/help/main/c-target/combining-multiple-audiences.md)を作成する際に、スピナーの読み込み中に「[!UICONTROL 完了]」オプションを無効にしました。[!DNL Target] UIにスピナーが表示されました。 （TGT-44079）
* [!UICONTROL  オーディエンス ] ページの下部にある[!UICONTROL 言語] リンクを修正して、「[!UICONTROL  アカウント通信設定]」ページに正しくリンクするようにしました。 （TGT-43562）
* [!UICONTROL 管理] > [!UICONTROL  レポート ] > [!UICONTROL Reporting Experience Cloud ソリューション ]の下の[!UICONTROL Adobe Analytics] オプションを選択した後、お客様が[!UICONTROL A/B テスト ] アクティビティを作成できない場合がある問題を解決しました。 （TGT-44844）
* [!UICONTROL Visual Experience Composer] （VEC）内から多くのエクスペリエンスを含む[!UICONTROL 多変量テスト ] アクティビティで、顧客が最後のエクスペリエンスを表示できない問題を修正しました。 VEC の下部にある [DOM パス](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#dom-path)により、お客様が最後のエクスペリエンスを表示できないことがありました。 （TGT-44578）
* ページで認証を必要とするか、リダイレクトを呼び出す場合に、VEC の参照 URL が、通常のブラウザーセッションで表示される現在のページを反映しない問題を修正しました。 （TGT-44350）
* [!UICONTROL Recommendations] > [!UICONTROL Settings]の[!UICONTROL Filter Incompatible Criteria]設定を変更できなかった問題を修正しました。 （TGT-44398）
* 名前にドットが付いたレポートスイートで[!UICONTROL Analytics Classifications]を使用すると、[!DNL Recommendations] フィードを作成するPOST リクエストが失敗する問題を修正しました。 （TGT-44598）
...
* [!UICONTROL Visual Experience Composer] （VEC）を使用したカスタム web コンポーネントのオーサリングの更新：
...
* サーバーサイドのレコメンデーションフィードに影響を与える可能性があるセキュリティの問題を修正しました。 （TGT-43769）
...
| 機能 | 詳細 |
| --- | --- |
| Automated Personalization（AP）での [JSON オファー](/help/main/c-experiences/c-manage-content/create-json-offer.md)のサポート | フォームベースのExperience Composerを使用した[!UICONTROL Automated Personalization] （AP）アクティビティでのJSON オファーのサポートを追加しました。 （TGT-41460） |
...
* [!UICONTROL Automated Personalization] アクティビティの[!UICONTROL  オファーレベル ] レポートで、表示されているオファー名から場所を削除しました。 この変更により、レポートが読みやすくなります。 （TGT-44294）
* 45日間および90日間のカレンダーオプションをAPから削除し、[!DNL Target] UIの[!UICONTROL Personalization インサイト ]および[!UICONTROL 重要な属性] レポートの[!UICONTROL 自動ターゲット ]を削除しました。 使用パターンおよびパフォーマンス向上のため、これらの日付範囲は非推奨（廃止予定）になりました。 現在許可されている範囲（15 日、30 日および 60 日）を反映するように UI を更新しました。 （TGT-39357）
* アクティビティの公開後、[!UICONTROL 目標と設定] ページの[!UICONTROL 最適化目標と同じ]設定を変更する機能を許可しませんでした。 （TGT-43923）
...
* [!UICONTROL 実装] ページ （[!UICONTROL 管理] > [!UICONTROL 実装]）の「オンデバイス決定を使用した実装方法」のリンクを、サポートされているすべてのSDKでオンデバイス決定を使用する方法を説明するページ（Node.js、Java、.NET、Python）に変更しました。 詳しくは、[Adobe Target 開発者ガイド](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=ja){target=_blank}の [Target SDK の開始方法](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/getting-started.html?lang=ja){target=_blank}を参照してください。
...
新しい[!DNL Adobe Target] モデル APIは、[!UICONTROL Automated Personalization] （AP）および[!UICONTROL 自動ターゲット ] （AT）アクティビティのマシンラーニングモデルで使用される機能のリストを表示および管理できます。
...
* A4Tを使用する[!UICONTROL 自動ターゲット ] アクティビティを作成または編集する際に、お客様が最適化条件を選択できるように、[!DNL Target] UIにツールヒントを追加しました。
...
| 機能 | 詳細 |
| --- | --- |
| [!DNL Adobe Experience Manager]（AEM）エクスペリエンスフラグメント | AEM エクスペリエンスフラグメント機能の更新内容は次のとおりです。<ul><li>[!UICONTROL  オファー] リストで、AEM エクスペリエンスフラグメントをタイプ（HTMLまたはJSON）でフィルタリングする機能を追加しました。 （TGT-43121）</li><li>VECを使用する際に、顧客がJSON [!UICONTROL  エクスペリエンスフラグメント ]のオファーを挿入できる問題を修正しました。これはサポートされていません。 JSON オファーは、[!UICONTROL  フォームベースのエクスペリエンス ] コンポーザーを使用する場合にのみ挿入できます。 （TGT-43846）</li></ul>詳しくは、AEM [エクスペリエンスフラグメント](/help/main/c-experiences/c-manage-content/aem-experience-fragments.md)を参照してください。 |
| Google Chrome用の新しい[!UICONTROL Visual Experience Composer]拡張機能 | Chrome用の新しい[!DNL Adobe Target] [!UICONTROL Visual Experience Composer] （VEC）拡張機能がChrome Web ストアで利用できます。<br>2023年1月から、現在の[!DNL Target]VEC Helper拡張機能はGoogle Chromeでの動作を停止します。これは、GoogleでManifest V2を使用する拡張機能が許可されないためです。 新しい拡張機能をダウンロードすれば、新年から引き続き [!DNL Target] で web サイトを視覚的に作成できます。<br>以下のリンクは、Chrome ウェブストアの 2 つの拡張機能を示しています。<ul><li>[新しい拡張機能](https://chrome.google.com/webstore/detail/adobe-experience-cloud-vi/kgmjjkfjacffaebgpkpcllakjifppnca){target=_blank}</li><li>[古い拡張機能](https://chrome.google.com/webstore/detail/adobe-target-vec-helper/ggjpideecfnbipkacplkhhaflkdjagak){target=_blank}</li></ul>詳しくは、[Visual Editing Helper 拡張機能](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/visual-editing-helper-extension.md)を参照してください。 |

* オーディエンスのルール情報が[!UICONTROL Audiences Refinements]情報ウィンドウに正しく表示されない問題を修正しました。 （TGT-43917）
...
* [!UICONTROL 構成]から[!UICONTROL 参照] モードに切り替えた後、VECでアクティビティを作成または編集する際に、[!UICONTROL  エクスペリエンス ] ページの[!UICONTROL 変更] パネルに一部のコンポーネントが正しく表示されない問題を修正しました。 （TGT-43300）
...
* at.js 2.10.0 （およびそれ以降）をダウンロードする際に、[!UICONTROL  クロスドメイン ] オプションを追加し、サードパーティ Cookieの設定を許可または無効にしました。 （TGT-43674）
* [!DNL Recommendations] フィードの読み込みが失敗した場合に顧客に通知する [!DNL Target] UI の通知を更新しました。 （TGT-35811）
* [!UICONTROL Visual Experience Composer] （VEC）内で[!UICONTROL 決定オファー]が正しく機能しない問題を修正しました。 （TGT-43866）
* [!UICONTROL Multivariate Testing] （MVT）アクティビティの作成中に[!UICONTROL  クリックしたElement] コンバージョン目標を選択すると、エラーメッセージが表示される問題を修正しました。 （TGT-43842）
* [!UICONTROL Automated Personalization] （AP）アクティビティのダウンロード済みCSV レポートファイルに[!UICONTROL  インプレッション ]列が表示されない問題を修正しました。 （TGT-43780）
* [!UICONTROL  フォームベースのExperience Composer]を使用する際に、エクスペリエンスを複製した後、HTML/JSON オファーを編集できない問題を修正しました。 （TGT-43633）
* 顧客が[!UICONTROL A/B テスト ] アクティビティをデフォルト以外のワークスペースから別のデフォルト以外のワークスペースにコピーできない問題を修正しました。 （TGT-41910）
* [!UICONTROL A/B テスト ]および[!UICONTROL  エクスペリエンスのターゲット設定] （XT）アクティビティの[!DNL Recommendations] オブジェクト（デザイン、条件、コレクションなど）の使用状況を、お客様が適切に表示できるようにし、[!DNL Target]のUIおよび[!DNL Recommendations] バックエンドから使用されなくなった条件オブジェクトを削除できるようにする問題を修正しました。 （TGT-42331）
...
| 機能 | 説明 |
| --- | ---  |
| 製品内通知 | 次の関連する製品内通知を取得します。<ul><li>**アクティビティ**：手動で、または開始日または終了日に達したことにより、アクティビティが承認または無効化されたときの、すべてのアクティビティタイプに関する通知。 通知には、アクティビティの名前と、アクティビティの概要ページへのリンクが含まれます。</li><li>**プロファイルスクリプト** プロファイルスクリプトが手動または Target によって有効化または無効化されたときの通知。</li><li>**レコメンデーションフィード**：レコメンデーションフィードが手動または Target によってアクティブ化または非アクティブ化されたときの通知。 また、レコメンデーションフィードに失敗した場合には通知も送信されます。</li></ul> デフォルトでは、製品管理者、発行者および承認者が通知を受信します。 通知は、Experience Cloud の環境設定内で設定できます。<br>詳しくは、[通知とお知らせ](/help/main/c-intro/understand-the-target-ui.md#notifications-announcements)を参照してください。 |
| *Adobe Target 開発者ガイド* | *Adobe Target 開発者ガイド*&#x200B;は、すべての [!DNL Target] 開発者コンテンツを 1 つの便利なガイドにまとめたものです。 このガイドには、[!DNL Target]および[!DNL Recommendations]、[!DNL Target] SDK、および[!DNL Target] APIの実装に関する情報が含まれています。<br>詳細については、[Adobe Target開発者ガイド ](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=ja){target=_blank}を参照してください。 |

* [!UICONTROL 編集者]の役割を持つユーザーは、ライブアクティビティでオーディエンスを編集できなくなりました。 （TGT-43582）
* お客様が感嘆符（!）でオーディエンスを保存しようとすると、警告メッセージが表示されます。 ）をオーディエンス名の最初の文字として使用します（!Londonなど）。 （TGT-43643）
...
* 新しい[!UICONTROL Audiences] ページに対して、過去にオーディエンスが保存された古いデータベースと、バックエンドから直接情報を取得する新しいアーキテクチャとの間で一貫性のない状態が発生するのを防ぐために、機能強化が提供されました。 （TGT-43552）
...
* JavaScript エラーが発生し、一部のお客様が特定の[!UICONTROL Automated Personalization] （AP）アクティビティのアクティビティの詳細にアクセスできない問題を修正しました。 （TGT-43526）
...
* 新しい[!UICONTROL Audiences] UIがデプロイされる前に絞り込みとオーディエンスが作成された場合、1人の顧客がアクティビティのエクスペリエンスレベルの絞り込みとオーディエンスを編集できない問題を修正しました。 （TGT-43433）
...
* [!UICONTROL  フォームベースのExperience Composer]で作成されたアクティビティで、アドホックオファーを持つ場所を顧客が削除できない問題を修正しました。 （TGT-43315）
...
* このリリースでは、[!UICONTROL 自動Personalization] （AP）および[!UICONTROL 自動ターゲット ] （AT）アクティビティのコンテンツを先行取得できます（以前は[!DNL Target]によって返されませんでした）。 AP/AT アクティビティが配信パス上にあり、コンテンツ配信に同じ場所を使用する他の AB/XT アクティビティよりも優先度が高い場合、プリフェッチ呼び出し（「実行」フローに対する変更なし）ではエンドユーザーに表示されるエクスペリエンスが変更される可能性があります。
...
* 新しい[!UICONTROL Audiences] UIは、[!DNL Target Standard]人の顧客すべてに対して有効になります。

### Target Standard/Premium のカスタマーエンジニアリング修正（2022年3月22日（PT））

このメンテナンスリリースには、次の機能拡張が含まれています。

* [!UICONTROL Analyticsをレポートソース ] （A4T）として使用するアクティビティで[!UICONTROL 配信API]を使用する場合、`prefetch` ビューと`pageLoad` クリック指標の[!DNL Analytics] ペイロードデータを返す機能を追加しました。 （TNT-43198）
* ボットフィルタリングユーザーエージェントリストを更新し、日本で一般的に使用されるブラウザータイプを許可しました。 （TNT-43867）

### Target Standard/Premium 22.2.1（2022年2月1日（PT））

このメンテナンスリリースには、Target Standard/Premium 22.1.2 リリースで発表された新しい[!UICONTROL Audiences] UIに関する次の修正と機能強化が含まれています。このリリースは、今後6週間以内にすべての地域のお客様にロールアウトされます。 これらの修正により、 [!DNL Adobe Target Standard/Premium] で作成されたオーディエンスの機能が調整されます。
...
* [!DNL Adobe Experience Platform]、[!DNL Adobe Experience Cloud]、[!DNL Adobe Target Classic]からインポートしたオーディエンスの[!UICONTROL  オーディエンス ] リストに[!UICONTROL 削除] オプションを追加しました。 また、一括削除機能も追加しました。 （TGT-42914）

### at.js バージョン 2.8.1（2022年1月28日（PT））

* デバイス決定] （ODD） ハイブリッド実行モードで[!UICONTROL On Device Decisioningで、`pageLoad`がtarget-global-mboxにマッピングされない問題を修正しました。
...
| [!UICONTROL  オーディエンス ] UIの更新 | [!DNL Target]人のユーザーのユーザーエクスペリエンスを向上させる[!DNL Adobe Target] チームの継続的な取り組みの一環として、このリリースでは、[!DNL Target] UIの[!UICONTROL  オーディエンス ]および[!UICONTROL  プロファイルスクリプト ] ページが更新されます。 この更新では、以前は一貫性がなかったデザインパターンが統合および標準化されたほか、次のような新しい機能強化が追加されています。<ul><li>複数のオーディエンスを同時に選択および削除する機能</li><li>[オーディエンスビルダーのデザイン](/help/main/c-target/c-audiences/create-audience.md)の更新</li><li>[!UICONTROL Audience] ライブラリルールビルダーでの除外ルールのサポート</li><li>新しい「オーディエンスソース」フィルターによるオーディエンス検出の迅速化</li><li>セッションの永続的な検索およびフィルターオプション</li><li>[!DNL Target Premium] 顧客のワークスペース間でオーディエンスを移動する機能。</li></ul>詳細については、[ オーディエンス ](/help/main/c-target/target.md)を参照してください。<br>**メモ**：この機能は、今後8週間以内に別の地域の顧客にロールアウトされます。 |
| [!UICONTROL  プロファイルスクリプト ] UIの更新 | [!UICONTROL  プロファイルスクリプト ] ライブラリも更新され、更新されたインターフェイスと、いくつかの生産性の更新が含まれています。<ul><li>複数のプロファイルスクリプトを同時に選択および削除する機能</li><li>プロファイルスクリプトの新しいコードエディター</li><li>コードエディター内での構文のハイライト表示とエラーチェック</li><li>キーボードショートカットを使用したトークン（mbox またはプロファイル）パラメーターのオートコンプリート</li></ul>詳しくは、[訪問者プロファイル ](/help/main/c-target/c-visitor-profile/visitor-profile.md)を参照してください。<br>**メモ**：この機能は、今後8週間以内に様々な地域の顧客にロールアウトされます。 |
...
* アクティビティの編集時に、[!UICONTROL 含める]および[!UICONTROL 除外] オプションが組み合わせオーディエンスに対して無効になる問題を修正しました。 （TGT-43422）
* アクティビティの編集中に一部のお客様が使用可能なオーディエンスのリストを表示できない問題を修正しました。 （TGT-43404）
* [!UICONTROL 管理] > [!UICONTROL  レポート ]で、「[!UICONTROL IPs」からIP アドレスを削除して[!DNL Target] レポートデータ ]」リストから除外できなかった問題を修正しました。 （TGT-43384）
* 任意の変数が「次よりも大きい」、「次よりも大きいか等しい」、「次よりも小さい」、「次よりも小さいか等しい」ことを確認するオーディエンス条件で負の数値を使用できない問題を修正しました。 （TGT-43367）
* 組み合わせオーディエンスを作成する際に、顧客が[!UICONTROL  オーディエンスの詳細] カードを表示できない問題を修正しました。 （TGT-43303）
...
* [!DNL Target]のアクティビティレポートで「[!UICONTROL Analyticsで表示]」ボタンをクリックすると、顧客が[!DNL Analysis Workspace]で[!UICONTROL A4T] パネルを開けなくなる問題を修正しました。 （TGT-42099、TGT-42100）
* [!UICONTROL  フォームベースのExperience Composer]を使用して[!UICONTROL A/B テスト ]および[!UICONTROL Experience Targeting] （XT）アクティビティを編集する際に、[!UICONTROL  デザインを編集] ボタンが表示されない問題を修正しました。 （TGT-41980）
* 新しい[!UICONTROL Recommendations] アクティビティの作成中に[!UICONTROL Compatible] チェックボックスが条件選択に表示されない問題を修正しました。 （TGT-42053）
...
[!DNL Target] [!UICONTROL  オーディエンス ]を[!DNL Adobe Experience Platform Web SDK]と共に使用する際に、次の機能強化が追加されました。
...
* オーディエンスがソースで削除され [!DNL Target] アクティビティで使用できなくなったことを示す警告アイコン、ポップオーバーおよびメッセージが、[!DNL Target] UI の様々な場所に追加されました。
  ...
  * アクティビティ作成ワークフローの[!UICONTROL  エクスペリエンス ] ステップ：
    ...
    ![ エクスペリエンス ] ページ ](assets/deleted-at-source-experiences.png)のソースメッセージでオーディエンスが削除されました[!UICONTROL 

  * アクティビティ作成ワークフローの[!UICONTROL  ターゲティング ] ステップ：

    ![ ターゲティング ] ページ ](assets/deleted-at-source-targeting.png)のソースメッセージでオーディエンスが削除されました[!UICONTROL 

  * アクティビティ作成ワークフローの[!UICONTROL 目標と設定] ステップ：

    ![目標と設定] ページ ](assets/deleted-at-source-goals-settings.png)のソースメッセージでオーディエンスが削除されました[!UICONTROL 

  * オーディエンスの絞り込み（[!UICONTROL  アクティビティ作成ワークフローの[!UICONTROL  ターゲティング ] ステップでオーディエンス ]を置き換える）:

* オーディエンスの結合機能を使用しようとして、ソースでいずれかのオーディエンスが削除された場合、[!UICONTROL 保存]は無効になります。
...
* VECで「[!UICONTROL Insert After]」を使用する場合、[ エクスペリエンスフラグメント ](/help/main/c-experiences/c-manage-content/aem-experience-fragments.md)を含むセレクターを変更できない問題を修正しました。 （TGT-41802）
* オファー内の空の JSON コンテンツがバックエンドに送信されない問題を修正しました。 [!DNL Target] は、空の場合でも JSON オブジェクトを送信するようになりました。 （TGT-41555）
* 顧客がレポートの表示中に「[!UICONTROL Analyticsで表示]」をクリックすると、従来の[!DNL Analytics] レポートが[!DNL Analysis Workspace]ではなく開く問題を修正しました。 （TGT-41867）
* お客様が[!UICONTROL Automated Personalization] アクティビティのレポートソース（A4T）として[!DNL Analytics]を選択しようとすると、表示されるUI メッセージにさらに明確な説明が追加されました。 メッセージには、「[!DNL Target]」が「[!UICONTROL Automated Personalization] アクティビティでサポートされている唯一のソース」と記載されています。 （TGT-41954）
* ユーザーがホストをコンマではなく「改行」で区切ろうとした場合のエラーメッセージに明確な説明を追加しました。 （TGT-40671）
* 一部のアクティビティ「[!UICONTROL 最終更新日]」の日付が、スペイン語と日本語のお客様の英語UIと異なる問題を修正しました（スペイン語と日本語でUIを表示する場合）。 （TGT-38980）
...
* [!UICONTROL  フォームベースのExperience Composer]で作成された[!UICONTROL 自動Personalization] アクティビティに関するレポートが、レポートで削除されたオファーを参照する問題を修正しました。 この問題により、次のエラーメッセージが表示されました。「このレポートのデータの取得に問題があります。 問題が解決しない場合は、アドビのカスタマーケアにお問い合わせください。」 （TGT-41028）
...
| 機能 | 詳細 |
| --- | --- |
| [!UICONTROL Analytics for Target]（A4T） | [!DNL Analytics]をレポートソース （A4T）として使用するアクティビティから[!UICONTROL Reports] ページの「[!UICONTROL Analytics]」リンクをクリックすると、[!DNL Analysis Workspace]が開きます。 以前は、このリンクをクリックすると、[!DNL Analytics] レポートが開きました。 （TGT-36959） |
...
* [!UICONTROL 管理] > [!UICONTROL 実装]で、「グローバル mboxが同期していない可能性があります」というエラーメッセージが表示されることがある問題を修正しました。 再保存してみてください。」というエラーメッセージが表示されることがある問題を修正しました。
...
| 機能 | 詳細 |
| --- | --- |
| Target] （A4T）の[!UICONTROL Analyticsでのリダイレクトのサポート | [A4T](/help/main/c-integrating-target-with-mac/a4t/a4t.md).<br>詳細については、[Analytics for [!DNL Target] implementation](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)を参照してください。[!DNL Target] リダイレクトがサポートされるようになりました。 |
...
* オーディエンスを[!UICONTROL すべての訪問者]に変更した後、アクティビティが同期されない問題を修正しました。 （TGT-40259）
* [!UICONTROL 重複を許可しない] オプションが有効になっていても、[!UICONTROL Automated Personalization] アクティビティの異なる場所で使用すると、オファーが重複しない問題を修正しました。 （TGT-39567）
...
### レコメンデーションフィード処理サーバーの IP アドレスの変更（2021年3月16日（PT））

[!DNL Target Recommendations] フィード処理サーバーの IP アドレスが 2021 年 3 月 16 日に更新されました。 詳しくは、[レコメンデーションフィード処理サーバーで使用される IP アドレス](/help/main/c-recommendations/c-recommendations-faq/ip-addresses-marketing-cloud.md)を参照してください。
...
* [!UICONTROL 自動ターゲット ] （AT）および[!UICONTROL Personalization] （AP）アクティビティに関する[!UICONTROL Automated Personalization インサイト ] レポートが毎日作成されるようになりました。 過去15、30、60日間の[!UICONTROL 自動セグメント ]または[!UICONTROL 重要な属性]を提供するレポートを選択できます。 45 日と 90 日のオプションが削除され、その他のルックバックウィンドウ設定を毎日実行できるようになりました。 （TGT-39472）
* ユーザーがアクティビティの[!UICONTROL 目標と設定] ページで[!UICONTROL 依存関係を編集]をクリックすると、現在の依存関係が表示されない問題を修正しました。 （TGT-39340）
* ワークスペースの[!UICONTROL  オーディエンスライブラリ ]を更新する際の問題を修正しました。 更新の前は、現在選択されているワークスペースのオーディエンスが表示されていました。 更新後、[!UICONTROL  デフォルトのWorkspace]とそのオーディエンスが表示されます。 現在のワークスペースとそのオーディエンスは、更新後も保持されるようになりました。 （TGT-38871）
* [!UICONTROL Recommendations] アクティビティをコピーし、後でその条件シーケンスを変更して元のアクティビティを編集する際の問題を修正しました。 元のアクティビティで条件シーケンスを変更すると、コピーされたアクティビティに誤って適用されていました。 （TGT-39155）
* [!UICONTROL Recommendations]の除外用に表示される製品数が正しくない問題を修正しました。 （TGT-39599）
...
* [!UICONTROL 自動ターゲット ] アクティビティで[!UICONTROL Analyticsをレポートソース ] （A4T）として使用する際に、[!DNL Adobe Analytics]指標を選択する際に警告を追加しました。 [!UICONTROL 自動ターゲット ] モデルは、バイナリ（コンバージョンベース）指標を操作するように最適化されています。 売上などの継続的な指標を選択すると、結果が最適でない可能性があり、[!UICONTROL Personalization インサイト ] レポートが正確でない可能性があります。 （TGT-38926）
* A4Tを使用する[!UICONTROL 自動ターゲット ] アクティビティの[!UICONTROL 自動ターゲット概要] レポートにステータスアイコンを追加しました。 レポートの各エクスペリエンスの横にある緑のチェックアイコンは、対象のエクスペリエンス用にパーソナライズされた機械学習モデルが作成されたことを意味します。 時計アイコンは、トラフィックの量が不十分でモデルがまだ作成されていないことを意味します。 （TGT-38925）
* A4Tおよび[!DNL Analytics] コンバージョン指標を使用する[!UICONTROL 自動ターゲット ] アクティビティの[!UICONTROL 自動セグメント ]および[!UICONTROL 重要属性] レポートが生成され、レポートソースとして[!DNL Target]を使用する場合と同じように見えます。 （TGT-38931）
* 環境フィルターオプションを[!UICONTROL Recommendations] [!UICONTROL  コレクション ] リストに追加しました。 （TGT-38353）
* [!UICONTROL Recommendations] コレクションに誤った製品数が表示される問題を修正しました。 （TGT-39162）
* [!UICONTROL 最終更新日] フィルターを[!UICONTROL 推奨事項] [!UICONTROL  カタログ検索]に追加しました。 （TGT-38340）
* 業界の垂直方向を変更した後に[!UICONTROL  シーケンスを作成] ページがハングする原因となった[!UICONTROL Recommendations]の問題を修正しました。 （TGT-38160）
* [!UICONTROL Automated Personalization] （AP）アクティビティのオファーからオーディエンスを削除できない問題を修正しました。 （TGT-39058）
* 一部のお客様の[!UICONTROL  オーディエンス情報] カードに正しくない時間枠（開始日と終了日）が表示される問題を修正しました。 （TGT-39150）
* 一部のお客様が[!UICONTROL 既定のWorkspace]でアクティビティのリストを表示できない問題を修正しました。 （TGT-38526）
...
| 機能 | 詳細 |
| --- | --- |
| ![Premium バッジ](/help/main/assets/premium.png)自動ターゲットアクティビティに対する Analytics for Target（A4T）のサポート | [!UICONTROL 自動ターゲット ] アクティビティで、Target](/help/main/c-integrating-target-with-mac/a4t/a4t.md)の[Analyticsがサポートされるようになりました。<br>この統合により、[!UICONTROL 自動ターゲット ]のアンサンブルマシンラーニングアルゴリズムを使用して、訪問者のプロファイル、行動、コンテキストに基づいて、各訪問者に最適なエクスペリエンスを選択できます。<br>A/B テストおよびエクスペリエンスのターゲット設定アクティビティで使用するA4T](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)を既に[実装している場合、設定は完了です！<br>詳細については、「[自動配分と自動ターゲット アクティビティのA4T サポート ](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md)」を参照してください。 |

### Target Standard/Premium 20.8.2（2020 年 9 月 10 日）

| 機能 | 詳細 |
| --- | --- |
| ![Premium バッジ](/help/main/assets/premium.png)条件のシーケンス内のコントロールレコメンデーションスロット | 条件シーケンスを使用して、各レコメンデーション条件で使用されるスロット数を制御できるようになりました。異なるタイプの項目や異なるアルゴリズムロジックを組み合わせて一致させることができます。<br>詳しくは、[条件シーケンスを作成する](/help/main/c-recommendations/c-algorithms/create-criteria-sequence.md#sequence)を参照してください。 |

### Target Standard/Premium 20.8.1（2020 年 9 月 2 日）

このリリースには、次の機能強化、修正および変更が含まれています。

* 組織を切り替えた後、新しい[!UICONTROL 管理] ページを読み込むとエラーが表示される問題を修正しました。 （TGT-37730）
* [!UICONTROL 管理/実装] ページに正しくないクライアントコードが表示される問題を修正しました。 （TGT-37849）
* VECを正常に読み込んだ後、[!UICONTROL Visual Experience Composer] （VEC）の編集機能をユーザーが使用できないことがある問題を修正しました。 （TGT-37162）
...
* 組織を切り替えた後、[!UICONTROL 管理/実装] ページからat.jsをダウンロードできない場合がある問題を修正しました。 （TGT-37668）
* ユーザーがダウンロードボタンを複数回クリックした場合でも [!DNL Target] が複数のリクエストを送信しないように、読み込み中は at.js のダウンロードボタンが無効化されるようになりました。 （TGT-37633）
* [!UICONTROL  エクスペリエンスのターゲット設定] （XT）アクティビティで、エクスペリエンスが長期間にわたって「結果を取得」を表示する問題を修正しました。 （TGT-37684）
...
* ワークスペース名（[!UICONTROL Adobe Admin Console for Enterprise]で指定）にアポストロフィが含まれている場合に、ユーザーが[!UICONTROL  アクティビティ ] ページを表示できない問題を修正しました。 （TGT-37709）
* 最適化およびコンバージョン指標を選択する際に[!UICONTROL 自動割り当て] アクティビティで、レポートスイートが既に指定されていても、エラーメッセージがレポートスイートの選択をユーザーに誤って通知する問題を修正しました。 （TGT-37689）
* [!UICONTROL  ターゲティング ] ページに移動してから戻ると、[!UICONTROL 目標と設定] ページの指標が空白になる場合がある問題を修正しました。 （TGT-37691）
...
#### [!UICONTROL 管理] セクション UIの更新

パフォーマンスを向上させ、新機能をリリースする際に必要なメンテナンス時間を短縮し、製品全体のユーザーエクスペリエンスを改善するために、新しい技術スタックを使用して [!DNL Target] の UI 全体を徐々に書き直しています。 最初に更新されたセクションは[!UICONTROL 設定] セクションで、[!UICONTROL 管理]という名前に変更されました。

この更新の一環として、[!UICONTROL 管理] セクションのページを使用して、次のような多くのアクションを簡単に実行できるようになります。

* 最新のat.js ファイルを「[!UICONTROL 実装]」タブ（**[!UICONTROL 管理]** > **[!UICONTROL 実装]**）からダウンロードします。
* at.js設定をカスタマイズし、変更を簡単に確認できます（**[!UICONTROL 管理]** > **[!UICONTROL 実装]**）。
* デフォルトの通貨とタイムゾーン、レポートから除外するIPなどの拡張レポート設定を変更します（**[!UICONTROL 管理]** > **[!UICONTROL レポート]**）。
* プライバシー上の理由により、訪問者のIP アドレスを難読化します（**[!UICONTROL 管理]** > **[!UICONTROL 実装]**）
* Adobe Admin Console （**[!UICONTROL 管理]** > **[!UICONTROL ユーザー]**）で管理する前に、ワークスペースごとの既存のユーザーのリストとその役割を表示します。
* [!UICONTROL 管理] セクションのすべてのテーブルを検索してフィルター処理します。
...
* [!UICONTROL Insert After] > [!UICONTROL Image]がScalable Vector Graphics （SVG）画像で正しく機能しない問題を修正しました。 （TGT-37242）
* ドラフトアクティビティの削除を妨げる[!UICONTROL 発行者]の役割を持つユーザーの問題を修正しました。 （TGT-37358）
* [!UICONTROL すべてのマイワークスペース ]を選択すると、ユーザーがアクティビティを編集できない問題を修正しました。 （TGT-37276）

### Target Standard／Premium 20.5.1（2020 年 6 月 17 日）

| 機能／拡張機能 | 説明 |
| --- | --- |
| [!UICONTROL 自動配分] アクティビティのAnalytics for Target （A4T）のサポート | [!UICONTROL 自動割り当て] アクティビティで、Target](/help/main/c-integrating-target-with-mac/a4t/a4t.md)の[Analyticsがサポートされるようになりました。<br>この統合により、[!UICONTROL 自動割り当て] マルチアームドバンディット機能を使用して、トラフィックを勝者エクスペリエンスに誘導しながら、[!UICONTROL Adobe Analytics]目標メトリックおよび/または[!UICONTROL Adobe Analytics] レポートおよび分析機能を使用できます。<br>A/B テストおよびエクスペリエンスのターゲット設定アクティビティで使用するA4T](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)を既に[実装している場合、設定は完了です！<br>詳細については、「[自動配分と自動ターゲット アクティビティのA4T サポート ](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md)」を参照してください。 |
| 自動ターゲットおよび Automated Personalization アクティビティのトラフィック配分方法の応答トークン | [!UICONTROL 自動ターゲット ]および[!UICONTROL Automated Personalization] アクティビティに2つの[応答トークン ](/help/main/administrating-target/response-tokens.md)が追加され、訪問者が「コントロール」または「ターゲティング」トラフィックに割り当てられた結果として特定のエクスペリエンスを受け取ったかどうかを判断できるようになりました。<ul><li>`experience.trafficAllocationId` は、訪問者が「コントロール」トラフィックからエクスペリエンスを受け取った場合は 0 を、訪問者が「ターゲット」トラフィック分布からエクスペリエンスを受け取った場合は 1 を返します。</li><li>`experience.trafficAllocationType` は、「control」または「targeted」を返します。</li></ul>コントロールトラフィックとターゲットトラフィックの比較について詳しくは、[Automated Personalization または自動ターゲットアクティビティのコントロールを選択](/help/main/c-activities/t-automated-personalization/experience-as-control.md)を参照してください。 |
| [!UICONTROL 発行者]の役割 | この新しい役割は、現在の[!UICONTROL Observer]の役割と似ています（アクティビティを表示できますが、作成または編集することはできません）。 ただし、[!UICONTROL 発行者]の役割には、アクティビティをアクティブ化するための追加の権限があります。<br>詳しくは、次を参照してください。 <ul><li>**Target Standard ユーザー**：*ユーザー*&#x200B;の[役割と権限を指定](/help/main/administrating-target/c-user-management/c-user-management/user-management.md#roles-permissions)します。</li><li>**Target Premium ユーザー**：[手順 6：役割と権限の指定](/help/main/administrating-target/c-user-management/property-channel/properties-overview.md#section_8C425E43E5DD4111BBFC734A2B7ABC80)は、*エンタープライズ権限の設定*&#x200B;でおこないます。</li></ul> |
| [!DNL Analysis Workspace]<br> における A4T のサポート（2020 年 6 月 25 日） | [!UICONTROL Target] （A4T）の分析は、[!DNL Analysis Workspace]でサポートされるようになりました。 [!UICONTROL Analytics for Target （A4T） パネル ]を使用すると、[!DNL Analysis Workspace]で[!DNL Adobe Target]のアクティビティとエクスペリエンスを分析できます。<br>詳細については、*A4T レポート*&#x200B;の[Analytics](/help/main/c-integrating-target-with-mac/a4t/reporting.md)および[Analytics for Target （A4T） パネル ](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/panels/a4t-panel.html?lang=ja)の&#x200B;*分析ツールガイド*&#x200B;を参照してください。 |

**機能強化、修正および変更**

* 「ユニーク訪問者（UniqueVisitors）」ではなく、「訪問者（visitors）」指標がアクティビティの定義に保存される問題を修正しました。 （TGT-37098）
* [!UICONTROL  オーディエンス ] ページで垂直スクロールバーが正しく機能しない[!DNL Target] UIの問題を修正しました。 （TGT-36968）
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| at.js 1.8.2 | at.js のこのリリースはメンテナンスリリースで、次の修正が含まれています。<ul><li>CNAMEとエッジの上書きを使用する際の問題を修正しました。at.js 1.*x*&#x200B;がサーバードメインを誤って作成し、[!DNL Target] リクエストが失敗する可能性があります。 （TNT-35064）</li></ul>詳しくは、[at.js のバージョンの詳細](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=ja){target=_blank}を参照してください。 |
| at.js 2.3.1 | at.js のこのリリースはメンテナンスリリースで、次の機能強化および修正が含まれています。<ul><li>[targetGlobalSettings](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/targetglobalsettings.html?lang=ja){target=_blank} 経由での `deviceIdLifetime` 設定のオーバーライドを可能にしました。 （TNT-36349）</li><li>CNAMEとエッジの上書きを使用する際の問題を修正しました。at.js 2.*x*&#x200B;がサーバードメインを誤って作成し、[!DNL Target] リクエストが失敗する可能性があります。 （TNT-35065）</li><li>[!DNL Target] [!DNL Launch] 拡張機能 v2 と [!DNL Adobe Analytics] [!DNL Launch] 拡張機能を使用すると、[!DNL Target] による [!DNL Analytics] `sendBeacon` 呼び出しが遅延する問題を修正しました。 （TNT-36407、TNT-35990、TNT-36000）</li></ul>詳しくは、[at.js のバージョンの詳細](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=ja){target=_blank}を参照してください。 |
...
* Target UI で、1 つのエクスペリエンスでユーザーがコンテンツを置き換えた後に、編集済みのコンテンツが他のレコメンデーションオファープレビューに表示される問題を修正しました。 （TGT-36053 および TGT-36894）
* 一部のユーザーがレコメンデーションカタログから項目を削除できない問題を修正しました。 （TGT-36455）
* 複数ページのアクティビティで、レコメンデーション条件を保存できない問題を修正しました。 （TGT-36249）
* 2 回連続して条件を編集すると、行動データソースのラジオボタンが消える問題を修正しました。 （TGT-36796）
* レコメンデーションアルゴリズムで、長期間「結果を取得」と表示される表示の問題を修正しました。 （TGT-36550 および TGT-36551）
...
* [!DNL Adobe Experience Cloud] にログインすると、新しいヘッダーナビゲーションに移動します。 上部に黒いバーが表示された前のナビゲーションと非常に似ていますが、次の点が改善されています。
  ...
  * ユーザーヘルプの向上：検索結果には、[!DNL Target] 製品ドキュメントの結果、コミュニティフォーラムやビデオコンテンツなどが含まれ、より多くのコンテンツに簡単にアクセスして [!DNL Target] を最大限に活用できます。 また、[!UICONTROL  ヘルプ ] メニューにフィードバック メカニズムが追加され、問題を報告したり、アイデアを共有したりしやすくなりました。

  * ネットプロモータースコア（NPS）のフィードバック機能が改善され、調査モーダルで作業の流れが妨げられなくなりました。

  * [!DNL Target]の通知は、現在、ヘッダーの[!UICONTROL 通知] ドロップダウンでは利用できません。
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| Java SDK | [!DNL Target] Java SDK を使用すれば、サーバーサイドで [!DNL Target] をデプロイできます。 このJava SDKは、[!DNL Target]を[!DNL Adobe Experience Cloud Identity Service]、[!DNL Adobe Analytics]、[!DNL Adobe Audience Manager]などの他の[!DNL Adobe Experience Cloud] ソリューションと簡単に統合するのに役立ちます。<br>Java SDKは、ベストプラクティスを導入し、配信APIを介して[!DNL Target]と統合する際の複雑さを排除することで、エンジニアリングチームがビジネスロジックに集中できるようにします。 最新バージョンで導入される主な機能は次のとおりです。<ul><li>プリフェッチおよび通知のサポートを追加しました。これにより、キャッシュを使用してパフォーマンスを最適化できます。</li><li>Web ページとサーバーサイドの両方で [!DNL Target] のハイブリッド統合を使用する場合の、パフォーマンスの最適化に対するサポート。 at.js 2.2 がエクスペリエンスを取得するために追加のサーバー呼び出しをおこなわないように、サーバーサイドで取得したエクスペリエンスによって生成される `serverState` という設定を導入します。 このアプローチは、ページ読み込みのパフォーマンスを最適化します。</li><li>VEC で作成されたアクティビティの Java SDK 経由での取得のサポート。これは新しい配信 API で可能になりました。</li><li>開発者が [Target Java SDK](https://github.com/adobe/target-java-sdk) に投稿できるよう、オープンソース化しました。</li></ul>Target Java SDK の詳細情報については、Adobe テクニカルブログ：[新しい Target Java SDK を使用したサーバサイドの最適化](https://medium.com/adobetech/server-side-optimization-with-the-new-target-java-sdk-421dc418a3f2)を参照してください。 |
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| ![Premium バッジ](/help/main/assets/premium.png)ユーザーベースのレコメンデーション<br>（2019年10月24日（PT）） | 各訪問者のブラウジング、表示および購入履歴に基づいて項目をレコメンドします。 これらのアイテムは、一般的に「おすすめ」と呼ばれます。<br>この条件を使用すると、新規訪問者と再訪問者の両方にパーソナライズされたコンテンツとエクスペリエンスを配信できます。 レコメンデーションのリストは、訪問者の最新のアクティビティに対して重み付けされます。また、セッション内で更新され、訪問者がサイトを閲覧するにつれてより詳細にパーソナライズされます。<br>詳細情報は、[条件／アルゴリズム](/help/main/c-recommendations/c-algorithms/algorithms.md#criteria-algorithms)の「ユーザーベースのレコメンデーション」を参照してください。 |
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| Node.js SDK バージョン 1.0 | Target Node.js SDKを使用すると、Target サーバーサイドでTargetをデプロイできます。<br>このNode.js SDKを使用すると、Adobe Experience Cloud Identity Service、Adobe Analytics、Adobe Audience Managerなどの他のExperience Cloud ソリューションとTargetを簡単に統合できます。<br>Node.js SDKはベストプラクティスを導入し、Adobe Targetと連携する際の複雑さを解消します。これにより、エンジニアリングチームはビジネスロジックに集中できます。 最新バージョンで導入される主な機能は次のとおりです。<ul><li>プリフェッチおよび通知のサポートを追加しました。これにより、キャッシュを使用してパフォーマンスを最適化できます。</li><li>Web ページとサーバーサイドの両方で Target のハイブリッド統合を使用する場合のパフォーマンスの最適化に対するサポート。 at.js 2.2 がエクスペリエンスを取得するために追加のサーバー呼び出しをおこなわないように、サーバーサイドで取得したエクスペリエンスによって生成される `serverState` という設定を導入します。 このアプローチは、ページ読み込みのパフォーマンスを最適化します。</li><li> VEC で作成されたアクティビティの Node.js SDK 経由での取得のサポート。これは新しい配信 API で可能になりました。</li><li>開発者が Node.js SDK に投稿できるよう、オープンソース化しました。</li></ul> |
| 配信 API | まったく新しい配信 API エンドポイント（/v1/ 配信）を本番環境で使用できます。 主な機能は次のとおりです。<ul><li>1 つのエンドポイントで、1 つ以上の mbox のエクスペリエンスを取得できます。</li><li>VECで作成されたアクティビティを API 経由で取得します。</li><li>シングルページアプリ（SPA）およびモバイルアプリケーションで使用される、「ビュー」というまったく新しいオブジェクトのサポート。</li></ul> |
...
* A/B テストおよびエクスペリエンスのターゲット設定アクティビティで、DIV に加えて、DIV 以外の要素（P、UL、H1 など）にレコメンデーションオファーを追加できるようになりました。 （TGT-34333）
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| ![Premium バッジ ](/help/main/assets/premium.png)<br>A/B テストおよびエクスペリエンスのターゲット設定（XT）アクティビティのレコメンデーション | レコメンデーションオファー（アルゴリズム）ステータスは、レコメンデーションオファーを含む A/B テストおよび XT アクティビティの概要ページに表示されます。 ステータスには、「結果の準備ができました」、「結果の準備ができていません」および「フィードエラー」があります。 （TGT-33649）<br>[オファーとしてのレコメンデーション](/help/main/c-recommendations/recommendations-as-an-offer.md#status)を参照してください。 |
| Experience Cloud ID（ECID）ライブラリによる at.js 2.0 以降のクロスドメイントラッキングサポート | 以前、クロスドメイントラッキングはat.js 2.*x*&#x200B;ではサポートされていませんでした。 このリリースでは、at.js 2.0 以降を使用するお客様は、ECID ライブラリ経由でクロスドメイントラッキングを使用できるようになりました。 クロスドメイントラッキングを機能させるには、ECID ライブラリが、at.js 2.0 以降と共にページにインストールされている必要があります。 [Experience Cloud ID ライブラリ 4.3.0 以降](https://experienceleague.adobe.com/docs/id-service/using/release-notes/release-notes.html?lang=ja)を使用する必要があります。<br>[at.js 2.x でのクロスドメイントラッキングサポート](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=ja){target=_blank}を参照してください。 |
| Experience Cloud ID（ECID）ライブラリ 4.3 による Target での Apple の ITP 2.1 および ITP 2.2 のサポート | 現在、Target のお客様は、アドビの CNAME 証明書プログラムを利用して、Apple の ITP 2.1 および ITP 2.2 を緩和できます。<br>このリリースでは、TargetはECID ライブラリ 4.3とのシームレスな統合を導入しました。これは、サーバーサイド Cookieを活用してITP 2.1とITP 2.2を軽減します。 Targetのお客様は、今後のITP リリースを軽減するために、[ECID ライブラリ 4.3+](https://experienceleague.adobe.com/docs/id-service/using/release-notes/release-notes.html?lang=ja)をTargetのJavaScript ライブラリと組み合わせてデプロイすることを強くお勧めします。 ECID ライブラリでは、ブラウザーによって導入される、常に変化する cookie ポリシーに対する堅牢なソリューションを提供する機能強化が引き続き公開されます。<br> [Apple Intelligent Tracking Prevention（ITP）2.x](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/apple-itp-2x.html?lang=ja){target=_blank} を参照してください。 |

**機能強化、修正、変更点**

* 重複した値を追加すると、レコメンデーションアクティビティで除外値がクリアされなかった問題を修正しました。 （TGT-34996）
* 現在は、ターゲティングページ（3 ステップのガイドによるワークフローのステップ 2）からレコメンデーションアクティビティのデザインを削除できるようになりました。 デザインを削除するには、1 つ以上のデザインを選択する必要があります。 （TGT-35118）
...
* 少数のお客様に対して、レコメンデーションオファーが web サイトに表示されない問題を修正しました。 お客様に配信 API 呼び出しのオファーコンテンツが表示されたとしても、そのオファーは Web サイトに適用されていませんでした。 （TNT-32680）
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| Visual Experience Composer（VEC） | **新しい VEC メニューオプション**：VEC でページ要素をクリックすると、その要素のタイプで利用可能なメニューのオプションが表示されます。<ul><li>[!UICONTROL  スタイル/背景] オプションを使用して、選択した要素の背景画像と色を変更できるようになりました。 （TGT-15001）</li></ul>[ ビジュアルエクスペリエンスオプション ](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#styles)の&#x200B;*スタイル*&#x200B;を参照してください。<br>**クリックトラッキングの改善**: VECおよびシングルページアプリケーション（SPA） VEC内でクリックトラッキングを設定するプロセスが改善されました。<ul><li>クリックの追跡で使用する要素を選択する際に、すべての使用可能な要素の名前が右側の変更パネルに表示され、すばやく簡単に目的の要素を選択できます。</li><li>3部構成のガイド付きアクティビティワークフローの[!UICONTROL 目標と設定] ページには、クリックの追跡に選択された要素の数を表す数値が表示されます。 この数にマウスポインターを置くと、すべての選択された要素の名前が表示されます。 （TGT-33878）</li></ul>[クリックの追跡](/help/main/c-activities/r-success-metrics/click-tracking.md)を参照してください。 |
| シングルページアプリケーション Visual Experience Composer （SPA VEC） | **ガイドによるワークフロー**：新しいガイドによるワークフローを使用すると、シングルページアプリ用にアクティビティを正常に実行するには、ページ配信ルール設定をどのように設定する必要があるかを理解できます。 （TGT-33718） <br> [ シングルページアプリ （SPA） Visual Experience Composer](/help/main/c-experiences/spa-visual-experience-composer.md#page-delivery-settings)を参照してください。<br>**変更の複製**: SPA VECを使用して変更を定義し、その変更を複製して、シングルページアプリの他のビューで使用できるようになりました。 （TGT-33882）<br>[シングルページアプリケーション（SPA）の Visual Experience Composer](/help/main/c-experiences/spa-visual-experience-composer.md) を参照してください。 |
| ![Premium バッジ](/help/main/assets/premium.png) Automated Personalization（AP）と自動ターゲット | **特定のエクスペリエンスをコントロールとして**：AP または自動ターゲットアクティビティを作成する際、コントロールとして使用するエクスペリエンスを選択できます。 この機能を使用すると、アクティビティで設定されたトラフィック配分率に基づいて、コントロールトラフィック全体を特定のエクスペリエンスにルーティングできます。 その後、そのエクスペリエンスへのコントロールトラフィックに対するパーソナライズされたトラフィックのパフォーマンスレポートを評価できます。 現在のコントロール オプション（ランダムに提供されるエクスペリエンス）は、引き続き使用できます。 （TGT-32801、TGT-26572、およびTGT-26571） <br>詳しくは、[Automated Personalizationまたは自動ターゲットアクティビティのコントロールを選択](/help/main/c-activities/t-automated-personalization/experience-as-control.md)を参照してください。<br>**Personalization インサイトレポート**：訪問者が特定の場所にある特定のコンテンツを見たときに、マーケターにとって使いやすい属性の命名規則が、より意味のある情報を提供します。 （TGT-33421、TGT-34957）<br>[Target パーソナライゼーションアルゴリズムのデータ収集](/help/main/c-activities/t-automated-personalization/ap-data.md)を参照してください。 |
| ![Premium バッジ](/help/main/assets/premium.png) レコメンデーション | 「最近表示したアイテム」ロジックを作成する際、「以前購入された品目をレコメンデーション」切替スイッチを使用できます。 （TGT-34030）<br>詳しくは、「条件の作成」の[最近表示された項目](/help/main/c-recommendations/c-algorithms/create-new-algorithm.md#previously-purchased)を参照してください。 |
...
| クライアントサイド分析ログ | クライアントサイドでもサーバーサイドでも、Adobe AnalyticsへのAnalytics データの送信方法を完全に制御できます。<br>詳細については、*の[ クライアントサイド Analytics ログ ](/help/main/c-integrating-target-with-mac/a4t/before-implement.md#client-side)を参照してください。*&#x200B;を実装する前に。 |
| 通知の送信 | `applyOffer()`または`applyOffers()`を使用する代わりに、コードでエクスペリエンスをレンダリングする際に、開発者が通知を送信できるようにします。<br>詳細については、[adobe.target.sendNotifications （options） ](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/adobe-target-sendnotifications-atjs-21.html?lang=ja){target=_blank}を参照してください。 |
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| [!UICONTROL Visual Experience Composer] | [!UICONTROL Visual Experience Composer] （VEC）には、作業をより迅速かつ効率的にするための次の機能強化が含まれています。<ul><li>クリック追跡の設定時に DOM パス機能が使用できるようになりました。<br>詳しくは、[クリックの追跡](/help/main/c-activities/r-success-metrics/click-tracking.md#considerations)を参照してください。</li><li>スタイルパネルを使用すると、選択した要素の既存のスタイルの値を表示または編集できます。 その他のスタイルを追加することもできます。<br> スタイルパネルにアクセスするには、VEC内からページ要素をクリックし、[!UICONTROL 編集] > [!UICONTROL  スタイル ]をクリックします。<br> スタイルパネルは、VECの右側に表示されます。 このパネルには、編集または選択した要素に追加できるスタイルのリストが含まれています。 リアルタイム CSS エディターを使用すると、カスケーディングスタイルシート（CSS）を使用している場合や開発者からコードを受け取った場合に、変更を表示したりスタイルを追加したりできます。<br>詳しくは、*Visual Experience Composer オプション*&#x200B;の[スタイル](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#styles)を参照してください。</li><li>リッチテキストエディターで、ネストされたHTML5要素がサポートされるようになりました。<br>HTML5の仕様により、ネストするタグの新しい組み合わせが可能になります。 以前のバージョンのリッチテキストエディターでは、HTML5 の仕様で許可されている新しいタグのネストをサポートしていませんでした。 その結果、VEC で選択されたネストされた要素が正しく処理されず、必要のない HTML の変更が行われていました。 （TGT-33618）<br>詳しくは、*Visual Experience Composer オプション*&#x200B;の[テキスト／HTML の編集](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#edit-text-html)を参照してください。</li> |

#### 機能強化、修正、変更点

* VEC を使用してアセットを削除する際のワークフローを改善しました。 削除されたアセットは、[!UICONTROL  オファーライブラリ ]および[!DNL Scene7]から（該当する場合）削除されるようになりました。 削除されたアセットは、検索結果に表示されなくなりました。 （TGT-31981）
* 画像が含まれている場合（空ではないフォルダー）、アセットフォルダーを削除できるようになりました。 （TGT-33265）

  以前は、Target画像オファーライブラリ（[!UICONTROL  オファー] > [!UICONTROL 画像オファー]）から空でないフォルダーを削除できませんでした。 UI からフォルダーを削除しようとすると、「フォルダーが空ではありません！」というメッセージが表示されます。 通知が表示されます。  この機能とあわせて、フォルダーの削除を実行して、任意の数のアセットとサブフォルダーが含まれるフォルダー全体を削除する機能を追加します。 この機能は、Adobe Experience Cloud Assets UI でも Target UI で使用できます。
...
* コレクション ピッカーから[!UICONTROL Recommendations] コレクションを選択したら、「[!UICONTROL 保存]」ボタンをクリックする必要があります。 このワークフローは、 内の他のワークフローと一致します[!DNL Target]。 （TGT-33205）
...
>[!NOTE]
>
>一般データ保護規則（GDPR）に対して[!DNL Adobe] オプトインサポートが必要な場合は、at.js 1.7.1を実装する必要があります。 オプトインのサポートは、現在at.js 2.*x*&#x200B;ではサポートされていません。
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| at.js バージョン 2.0.0<br>（2019 年 2 月 19 日） | at.js 2.x がリリースされました。<br>最新バージョンの at.js には、次世代クライアント側テクノロジーでのパーソナライゼーションを実行するためにビジネスを均等化する豊富な機能セットが用意されています。 この新しいバージョンでは、at.jsをアップグレードして、シングルページアプリケーション（SPA）との調和のとれたインタラクションを実現することに重点が置かれています。<br>以前のバージョンでは利用できないat.js 2.xを使用するメリットを以下に示します。<ul><li>ページ読み込み時にすべてのオファーをキャッシュして、単一のサーバーコールに対する複数のサーバー呼び出しを減らす機能。</li><li>従来のサーバー呼び出しで発生する遅延時間なしで、キャッシュ経由でオファーが即座に表示されるため、サイトでのエンドユーザーのエクスペリエンスが著しく向上します。</li><li>単純な 1 行のコードと一度限りの開発者セットアップで、マーケターは、単一ページアプリケーション上の Visual Experience Composer（VEC）を介して A/B およびエクスペリエンス（XT）アクティビティを作成して実行できます。</li></ul>at.js 2.x では、次の新しい関数が導入されています。<ul><li>getOffers（）</li><li>applyOffers（）</li><li>triggerView（）</li></ul>at.js 2 x の導入に伴い、次の関数が廃止されました。<ul><li>mboxCreate()</li><li>mboxDefine</li><li>registerExtension()</li></ul>詳細については、「[at.js 1.xからat.js 2.x](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=ja){target=_blank}へのアップグレードおよび[at.js関数](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=ja){target=_blank}.<br>**注意**:[General Data Protection Regulation](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/cmp-privacy-and-general-data-protection-regulation.html?lang=ja){target=_blank} （GDPR） {target=_blank}に対するAdobe オプトインサポートが必要な場合は、現在at.js 1.7.0を使用する必要があります。 オプトインのサポートは、at.js 2.xではサポートされていません。 |
| at.js バージョン 1.7.0<br>（2019 年 2 月 14 日） | at.js 1.7.0 を使用できます。<br>このリリースでは、Adobe Opt-in サポートが導入されています。 Adobe Opt-In は、アドビソリューションと同意管理プラットフォームの統合を簡略化する方法です。<br>Adobe オプトインについて詳しくは、[ プライバシーおよび一般データ保護規則](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/cmp-privacy-and-general-data-protection-regulation.html?lang=ja){target=_blank} （GDPR） {target=_blank}を参照してください。<br>このリリースでは、リダイレクト URLから取得されるパラメーターでTargetがリダイレクト URL パラメーターを上書きする問題も修正されています。<br>**注**: Adobe オプトインのサポートが必要な場合は、現在at.js 1.7.0を使用する必要があります。 at.js 2.xでは、オプトインのサポートはサポートされていません。<br>すべてのバージョンのリストについては、[at.js バージョンの詳細](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=ja){target=_blank}を参照してください。 |
...
| ![ プレミアムバッジ ](/help/main/assets/premium.png)<br>[!UICONTROL A/B テスト ]および[!UICONTROL  エクスペリエンスのターゲット設定] アクティビティの推奨事項 | [!UICONTROL A/B テスト ] （[!UICONTROL 自動割り当て]および[!UICONTROL 自動ターゲット ]を含む）および[!UICONTROL  エクスペリエンスのターゲット設定] （XT）アクティビティ内に推奨事項を含めることができるようになりました。 これにより、次のようなまったく新しい機能が可能になります。<ul><li>同じアクティビティ内のレコメンデーションと非レコメンデーションのコンテンツをテストおよびターゲット設定します。</li><li>複数のレコメンデーションの順序など、レコメンデーションのページ配置を簡単に試行します。</li><li>[!UICONTROL 自動配分]を使用して、最もパフォーマンスの高いレコメンデーションエクスペリエンスにトラフィックを自動的にプッシュします。</li><li>[!UICONTROL 自動ターゲット ]を使用して、個々のプロファイルに基づいて、カスタマイズされたレコメンデーションエクスペリエンスに訪問者を動的に割り当てます。</li></ul>開始するには、VECを使用して[!UICONTROL A/B テスト ]または[!UICONTROL  エクスペリエンスのターゲット設定] アクティビティを作成し、[!UICONTROL 前に挿入]、[!UICONTROL 後に挿入]、[!UICONTROL 後に置換] アクションを使用して、エクスペリエンスに推奨事項を追加します。 （RECS-6166）<br>詳しくは、[オファーとしてのレコメンデーション](/help/main/c-recommendations/recommendations-as-an-offer.md)を参照してください。 |
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| ターゲティング<br>2019 年 1 月 25 日 | プロファイルスクリプト、プロファイルパラメーターなど、プロファイルスクリプトまたはその他の入力ソースによって返される小数値と小数値の比較が、「等しい」比較の関数と一致するように変更されました。<br>詳しくは、[Target とオーディエンス](/help/main/c-target/c-troubleshooting-targets-and-audiences/troubleshooting-targets-and-audiences.md) の FAQ を参照してください。 |
| プロファイルスクリプト<br>2019 年 1 月 17 日 | パフォーマンス上の理由から、256 文字以下の戻り値を返すことをお勧めします。<br>文字列戻り値の場合、戻り値のサイズが 2048 文字を超えると、スクリプトはシステムによって無効になります。<br>配列の戻り値の場合、配列の連結された値のサイズが2048文字を超えると、スクリプトはシステムによって無効になります。<br>文字の制限とその他の制限（オファーのサイズ、オーディエンス、プロファイル、値、パラメーターなど）について詳しく知りたい場合 Targetのアクティビティやその他の要素に影響する要素については、[制限](/help/main/r-troubleshooting-target/target-limits.md)を参照してください。 |
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| ![Target Premium バッジ ](/help/main/assets/premium.png)<br/>[!UICONTROL  エンタープライズ権限]の[!DNL Target] APIでのサポート | [Adobe Target Admin API](https://developers.adobetarget.com/api/#admin-apis) では、Target UI と同じエンタープライズパーミッションをフル活用できるようになります。 **2019 年 2 月 21 日**&#x200B;より、システム管理者は、任意のワークスペース内で、プログラムデータにプログラム的にアクセスしたり、アクティビティ、オファー、オーディエンスの作成および管理したりできます。 これらのアクションは、以前はデフォルトのワークスペースのみに制限されていました。 Automated Personalization（AP）アクティビティのサポートは、将来のリリースで導入されます。 |
| ![Target Premium バッジ ](/help/main/assets/premium.png)<br/>[!UICONTROL Recommendations]：環境別のコレクションと除外のフィルター（ホストグループ） | 選択した環境（ホストグループ）の[!UICONTROL Recommendations] コレクションと除外の内容をプレビューできるようになりました。<br/>以前は、コレクションまたは除外を表示したときに、表示された項目はデフォルトのホストグループ（[!UICONTROL Recommendations/Settings/Default Host Group]で指定）の結果でした。<br/> コレクションまたは除外を作成または更新する際に、[!UICONTROL 環境] セレクターを使用して結果をプレビューできます。 新しい[!UICONTROL 環境] フィルターは、[!UICONTROL 設定] ページに移動して、コレクションと除外を作成または編集する前に適切なデフォルトホストグループを選択する必要がなくなるため、時間と労力を節約します。<br/>**注：**&#x200B;選択した環境を変更した後、[!UICONTROL 検索]をクリックして返された結果を更新する必要があります。<br/>新しい[!UICONTROL 環境] フィルターは、[!DNL Target] UIの次の場所から利用できます。<ul><li>[!UICONTROL  カタログ検索] （[!UICONTROL おすすめ> カタログ検索]）</li><li>[!UICONTROL  コレクションを作成] ダイアログボックス （[!UICONTROL おすすめ/ コレクション / 新規作成]）</li><li>[!UICONTROL  コレクションの更新] ダイアログボックス （[!UICONTROL おすすめ/ コレクション / 編集]）</li><li>[!UICONTROL 除外を作成] ダイアログボックス （[!UICONTROL 推奨事項>除外>新規作成]）</li><li>[!UICONTROL 除外を更新] ダイアログボックス （[!UICONTROL おすすめ/除外/編集]）</li></ul><br>詳しくは、次のトピックを参照してください。<uL><li>[コレクション](/help/main/c-recommendations/c-products/collections.md)</li><li>[除外](/help/main/c-recommendations/c-products/exclusions.md)</li><li>[カタログ検索](/help/main/c-recommendations/c-products/catalog-search.md)</li><li>[設定](https://experienceleague.adobe.com/docs/target-dev/developer/recommendations.html?lang=ja){target=_blank}</li><li>[レコメンデーション：環境（ホストグループ）別にコレクションと除外をフィルター](/help/main/administrating-target/hosts.md)</li></ul>（TGT-20622）</ul> |
...
11 月 12 日の [!DNL Target] Standard／Premium リリースには、バックエンドの機能強化、修正および変更が含まれています。 [!UICONTROL パーソナライゼーションインサイト]レポートは、11 月 15 日から利用できるようになります。
...
* [!UICONTROL 管理] ページでAdobe Analyticsがレポートソースとして選択されたときに、特定の状況のユーザーがアクティビティを作成できない問題を修正しました。 以前は、レポートスイートを選択できない場合でも、「レポートスイートを選択してください」というメッセージが表示されていました。 （TGT-31968）
...
<table id="table_7E3513EABA4948DC92EADCCE0234A9FF"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p>レコメンデーション </p> </td> 
   <td colname="col2"> <p> 
...
* Automated Personalization（AP）および多変量分析テスト（MVT）アクティビティの Target 手順（3 つの手順から成るガイド付きワークフローの手順 2）を再設計することで、A/B、エクスペリエンスのターゲット設定（XT）、レコメンデーションの各アクティビティに使用されるデザインとの一貫性が向上しました。 （TGT-30712）
...
<table id="table_5A60FFE5E86148F4BDC6A7031D03D6BA"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>Visual Experience Composer（VEC） </p> </td> 
   <td colname="col2"> <p>変更パネルでアクションをクリックすると、VEC が自動的に Web ページをスクロールして対応する要素をハイライトするようになりました。 手動で下方にスクロールして、変更の影響を受けた HTML 要素を見つける必要はなくなりました。 </p> <p> <img src="assets/modifications_panel.png" id="image_6E01280636E34ADDA9527AD18A34310B" /> </p> <p>（TGT-30441） </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>サポートされているブラウザー </p> </td> 
   <td colname="col2"> <p>Target UI およびコンテンツ配信用のブラウザーとして Microsoft Edge がサポートされるようになりました。 </p> <p>詳しくは、を参照してください。<a href="https://experienceleague.adobe.com/docs/target-dev/developer/implementation/supported-browsers.html?lang=ja" format="dita" scope="local"> サポートされているブラウザー</a> （TGT-14102） </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1" class="premium"> <p>レコメンデーション </p> </td> 
...
<table id="table_6D99C48B72D24728BF623608053931D3"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Target アクティビティでAEMで作成されたエクスペリエンスフラグメントを使用すると、AEMの使いやすさとパワーを、Targetの強力な自動インテリジェンス（AI）および機械学習（ML）機能と組み合わせて、エクスペリエンスを大規模にテストおよびパーソナライズできます。&amp;nbsp;&amp;nbsp; </p> <p>AEM では、パーソナライゼーション戦略に生かせるよう、すべてのコンテンツとアセットが一元化されます。 AEM では、コードを記述しなくても、デスクトップ、タブレット、モバイルデバイス向けのコンテンツを 1 か所で簡単に作成できます。 デバイスごとにページを作成する必要はありません。AEM ではコンテンツを基に、各デバイスのエクスペリエンスが自動調整されます。 </p> <p> Adobe Targetなら、行動やコンテクスト、オフラインの変数を組み込んだ、ルールベースのAI主導のマシンラーニングアプローチと組み合わせて、パーソナライズされたエクスペリエンスを大規模に提供できます。&amp;nbsp; Adobe Targetなら、A/B アクティビティや多変量アクティビティを容易に設定および実行し、最適なオファー、コンテンツ、エクスペリエンスを決定できます。 </p> <p>エクスペリエンスフラグメントは、コンテンツ／エクスペリエンスの作成者や管理者と、ビジネス成果の向上に Target を使用している最適化／パーソナライゼーションの専門家との連携を大幅に強化できる機能です。 </p> <p>詳しくは、 <a href="/help/main/c-experiences/c-manage-content/aem-experience-fragments.md#topic_1E1E4EA01F074349B2CF8785387B5FE8" format="dita" scope="local">AEMエクスペリエンスフラグメント </a>を参照してください。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>レポート </p> </td> 
   <td colname="col2"> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 
     <ul id="ul_C595EEF916494342AD99FF0FDF999927"> 
      ...
      <li id="li_21B31F91685C46CAA47688FDE5735312"> <p>Targetは、レポートで統計的に有意な場合に上昇境界を表示するようになりました。（TGT-27301、TGT-27794、およびTGT-26387） </p> </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>アクティビティの作成ワークフローに、次の機能強化が加えられました。 </p> <p> 
     <ul id="ul_2D251AC11FC54E86AE84DEFFB6FDF43C"> 
      ...
      <li id="li_36EF9AD13B2D40ADB99343C9F758D5FD"> <p>3 ステップのガイドによるワークフローのステップ 2 でターゲティングを選択する際に、<span class="wintitle">オーディエンスを選択</span>ダイアログボックスでオーディエンスにマウスポインターを置くことで、オーディエンスを編集またはコピーできるようになりました。 </p> <p> <img src="assets/audience_picker_hover.png" id="image_6DC33A0856A346948E517F0BA4C9039F" /> </p> </li> 
...
* 管理、オーディエンス、レコメンデーションページでのオプションの設定時に、ユーザーの Target セッションがタイムアウトした場合のワークフローを向上しました。 「保存」をクリックするとセッションの有効期限切れを示すメッセージが表示されますが、ログインし直すと、ログインが成功したことを示すダイアログが表示され、Target の UI には同じページが表示されます。データの損失も発生しません。 （TGT-25557）
...
<table id="table_701D8D53D1DF4F28ADAC6EC221B0208A"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p>エクスペリエンスのターゲット設定（XT） </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>これまでは、Target で単一の指標の依存関係を指定したり、その指標に到達したらカウントを増分するよう設定したりすることができました。 現在では、複数の指標の依存関係を指定したり、指標に到達したらカウントを増分するか、指標に到達しなかったらカウントを増分するかを柔軟に選択したりできるようになりました。 </p> <p>複数の指標の依存関係を設定できる機能は、以下には対応していません。 </p> <p> 
     ...
     <ul id="ul_4B0EFFDD257C42579E19569DCBE15BE3"> 
      ...
      <li id="li_57C36A7945A24A52BCBD62CA0F15B668"> <p>エクスペリエンスのターゲット設定（XT）：<a href="/help/main/c-activities/t-experience-target/t-xt-create/xt-goals-and-settings.md#reference_B25389FD6F3A4989801E740364B089CC" format="dita" scope="local">目標と設定 </a> </p> </li> 
...
* アクティビティまたはオファーを作成、編集しているときにユーザーの [!DNL Target] セッションがタイムアウトした場合のワークフローを改善しました。 ユーザーが[!UICONTROL 保存]をクリックすると、セッションの有効期限が切れたメッセージが表示されますが、再度ログインすると、ログインが成功したことを知らせるダイアログがユーザーに表示され、UIは[!DNL Target]内の同じページに残り、データが失われることはありません。
  ...
  <tr> 
   ...
   <td colname="col2"> <p> <span class="filepath">at.js</span> バージョン 0.9.6 がリリースされました。 詳しくは、<a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/deploy-at-js/implement-target-without-a-tag-manager.html?lang=ja" format="dita" scope="local">at.js のダウンロード</a>を参照してください。 </p> <p><span class="filepath">at.js</span> バージョン 0.9.6 には、次の機能強化および修正が含まれています。 </p> <p> 
     <ul id="ul_108DF85393614C69988E299485D338FD"> 
      ...
      <li id="li_3B1F618DEC744056B5BB172C4DBB359A"> <p>cookieの保存時に使用すべきトップレベルドメインを抽出するアルゴリズムが、<span class="codeph"> at.js </span> バージョン 0.9.6で変更されました。 この変更により、IPを使用するアドレスにCookieを保存できません。 IP アドレスはテスト目的で使用されるケースがほとんどですが、DNS エントリを使用したり、ローカルボックスのホストファイルを変更したりすることで対処できます。 </p> </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>オーディエンスのターゲット設定の機能が以下のように強化されました。 </p> <p> 
     <ul id="ul_C920198404654C97A33190A29ACA6990"> 
      <li id="li_DB52EF909C9640649981940460CDF2B5"> <p><b>週と日付の分割：</b>「<span class="wintitle">週と日付の分割</span>」オプションを設定して、オーディエンスのターゲティングの繰り返しパターンを作成できます。 </p> <p>詳しくは、<a href="/help/main/c-target/c-audiences/c-target-rules/time-frame.md#concept_0FE1E8DACD104F8B870B0BADE3197F0A" format="dita" scope="local">時間枠</a>を参照してください。 </p> </li> 
...
* [!DNL Target] ユーザーインターフェイスが更新され、[!UICONTROL Analytics for Target] （A4T）をレポートソースとして使用するアクティビティでのリダイレクトオファーがサポートされるようになりました。 この機能には、まもなく提供される [!DNL at.js] 0.9.6 が必要になります。
* いくつかの場所で、[!DNL Target] ユーザーインターフェイスが更新されました。

  * レポートとアクティビティでは、一部のオプション （[!UICONTROL 編集]、[!UICONTROL  フィードに共有]、[!UICONTROL  エクスペリエンス URLの表示]など） [!UICONTROL 詳細オプション ] アイコン （![icon_more_options画像](assets/icon_more_options.png)をクリックしてアクセスできるようになりました
...
>[!NOTE]
>
>FP-11577以降を含む[!DNL Adobe Experience Manager] 6.2では、[!UICONTROL Adobe Target Cloud Services]との統合で[!DNL at.js]実装がサポートされるようになりました。 詳しくは、*Adobe Experience Manager 6.2* ドキュメントの[機能パック](https://experienceleague.adobe.com/docs/?lang=ja)および [Adobe Target との統合](https://experienceleague.adobe.com/docs/?lang=ja)を参照してください。
...
* アクティビティのページにat.js （バージョン 0.9.1以降）が読み込まれている場合は、トラッキングサーバーが不要であることをユーザーに通知する[!UICONTROL Analytics for Target] （A4T）を使用しているすべてのアクティビティの[!UICONTROL 目標と設定] ページにツールヒントを追加しました。 （TGT-22607）
...
* 非表示にする項目の選択時の [!UICONTROL Visual Experience Composer]（VEC）のユーザーインターフェイスが改善されました。
（TGT-22224）
...
* ダウンロードした[!UICONTROL 注文の詳細]CSV レポートで極端な注文が正しくマークされない問題を修正しました。 （TGT-22500）
...
* 自動作成されたグローバル mbox（`target-global-mbox`）以外の mbox と共にフォームベースの Experience Composer を使用し、エンゲージメント指標を成功指標として選択すると、指標はアクティビティで使用される mbox を持つページでのみ増分します。 例えば、mboxが`homepage_mbox`の場合、[!UICONTROL 訪問当たりのページ数]指標は、その訪問中の`homepage_mbox`へのヒット数です。
  ...
  <tr> 
   ...
   <td colname="col2"> <p>オーディエンスの機能が次のように強化されました。 </p> <p> 
     <ul id="ul_F1D1F97266134D4ABE627CF2DCE2C6D4"> 
      <li id="li_99A611FCC1254D229D79B8FD075B952A"> <p> モバイルデバイスに対してターゲティングしたオーディエンスを作成する際、ドロップダウンリストのビルトインオプションで「<span class="wintitle">デバイスのマーケティング名</span>」を選択できるようになりました。 </p> <p>この変更により、適切なデバイスモデル番号を検索するよりも簡単に、デバイスモデル名を選択できるようになりました。 例えば、Galaxy S7 のマーケティングデバイス名は「Samsung Galaxy S7 Edge」ですが、デバイスモデルは「SM-G9350」です。 （TGT-18393） </p> <p>詳しくは、<a href="/help/main/c-target/c-audiences/c-target-rules/mobile.md#concept_2A794199DC1A4D349FFFBC7DCF1FEB89" format="dita" scope="local">モバイル</a>を参照してください。 </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1" class="premium"> <p>レコメンデーション </p> </td> 
...
<table id="table_F8C01B2A9F07443490DB3025AC3AAC2A"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>プロモーション項目を追加して、レコメンデーションデザインでの配置を制御します。 </p> <p>詳しくは、<a href="/help/main/c-recommendations/t-create-recs-activity/adding-promotions.md#task_CC5BD28C364742218C1ACAF0D45E0E14" format="dita" scope="local">プロモーションの追加</a>を参照してください。 </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_EBA34BD2F5C745DD9EC5231AD79F6C00"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> 複数の値があるレコメンデーション属性 </td> 
   <td colname="col2"> <p>すべてのカスタム<span class="keyword">レコメンデーション</span>属性は複数のエンティティ値を含むことができるようになりました。 </p> <p>詳しくは、<a href="/help/main/c-recommendations/c-products/custom-entity-attributes.md#concept_E5CF39BCAC8140309A73828706288322" format="dita" scope="local">カスタムエンティティ属性</a>を参照してください。 </p> </td> 
  </tr> 
  <tr> 
...
* リダイレクトを含むエクスペリエンスのエクスペリエンスプレビュー URL に問題があります。 対応策は、Experience Composer で「**[!UICONTROL 設定]**」をクリックして、「**[!UICONTROL 複数オーディエンス]**」を選択し、**[!UICONTROL すべての訪問者]**&#x200B;を唯一のオーディエンスとして追加します。 続けてアクティビティを保存します。 アクティビティの配信は変更されませんが、プレビューは動作するようになります。 これは、7 月の Adobe Target リリースで修正される予定です。
...
<table id="table_DDC5356FD6B8443EAA6EB81C03ADF73A"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>CSVのダウンロードでは、エンティティの推奨事項がない環境を含むすべての環境に対して行が設定されるようになりました（例： 
     <code>
       # environment: 1724 
     </code>). </p> </td> 
  </tr> 
...
<table id="table_162CC5A0DB324B38A8A4252A18976686"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> レコメンデーション：<span class="codeph">mboxTrace</span> を使用したテキストエリアでの新しいデバッグ機能 </td> 
...
<table id="table_F2A89DF1EAB443B4B4C7E0BC6118384B"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>この機能は、以前に別の成功指標に到達している場合にのみ、訪問者を成功指標に到達したものとしてカウントするために、成功指標ごとにオプションを提供します。 </p> <p> 例えば、テストによってホームページのメイン画像が変更される可能性があります。 マーケティング担当者は、メイン画面をクリックした訪問者のコンバージョンだけをカウントしたいことがあります。 その場合、マーケティング担当者は、成功指標を「ホームページのメイン画像をクリックした」に設定して、購入用に別の指標を設定できます。 次に、マーケティング担当者は、訪問者が最初に「ホームページのメイン画像をクリックした」成功指標を達成するようにするルールを「購入」指標に追加できます。 </p> <p> <p>注意：成功指標で、場所でターゲティングしたオーディエンスが設定された場合、この機能はその指標をサポートしません。 </p> </p> <p> 従属成功指標は、AB、XT および MVT アクティビティのみをサポートします。 Automated Personalizationおよびレコメンデーションのサポートは、後日利用できるようになります。 </p> <p>詳しくは、<a href="/help/main/c-activities/r-success-metrics/success-metrics.md#reference_D011575C85DA48E989A244593D9B9924" format="dita" scope="local">成功指標</a>を参照してください。 </p> </td> 
  </tr> 
  <tr> 
...
* Chrome バージョン 48 のアップデートで、Visual Experience Composer が Chrome で間違って機能する問題が発生しました。 Google は、解決に向けて作業しています。 詳しくは、[https://code.google.com/p/chromium/issues/detail?id=582603](https://code.google.com/p/chromium/issues/detail?id=582603) を参照してください。 この問題を回避するには、次のようにします。
  ...
  * 拡張Experience Composerを有効にします。この機能は、**[!UICONTROL 管理]** > **[!UICONTROL Visual Experience Composer]** タブ内から設定できます。
...
<table id="table_8525ECC9B6D0435ABEF8C27F747B7A0C"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p class="premium">レコメンデーション： </p> <p>グローバル mbox 配信コーディングの変更 </p> </td> 
...
* フォームベースのワークフローで使用すると、HTML 以外のレコメンデーションテンプレートを `<div>` 要素でラップする問題を修正しました。
...
<table id="table_EE13D9B959DA4FB0AD6BC03FBF78AEF6"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">レコメンデーション </p> <p class="Premium">過去の購入の除外 </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">レコメンデーション </p> <p> 属性の重み付け </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">レコメンデーション </p> <p>フィード処理のための時間の設定 </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">レコメンデーション </p> <p>フィードリストを使用してフィードを実行しないように設定する </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">レコメンデーション </p> <p>コンテンツの類似性に基づいて新しい条件タイプを設定する </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">レコメンデーション </p> <p>機能強化：業界関連の条件の設定 </p> </td> 
   <td colname="col2"> <p>設定中に関係のないオプションは削除されました。 以前は、メディアなど一部の業種向けの一部の設定オプションは、必ずしも関連があるとは限りませんでした。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">レコメンデーション </p> <p>新しいあらかじめ用意されている条件 </p> </td> 
   <td colname="col2"> <p> 
...
<table id="table_907A952F54084C2A9C61F10E2B7A7BFF"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">レコメンデーション </p> <p class="Premium">フォームベースのレコメンデーション </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">レコメンデーション </p> <p> デザインでの主要品目に関する情報の表示 </p> </td> 
...
<table id="table_DF5B434D639345B4ACE2467B8966A908"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>マーケターは、Automated Personalization のオファーにターゲティングルールを適用できます。 指定したグループのメンバーに対して特定のオファーが表示されるのを除外することができます。 </p> <p><a href="/help/main/c-activities/t-automated-personalization/ap-target-offers.md#task_F207ED7A41B84FD39BB6FCBFABF4B23E" format="dita" scope="local">AP オファーのターゲット設定</a>を参照してください。 </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_3985F758176F4884A94AFDFB78B24209"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> <a href="/help/main/c-activities/t-test-ab/t-test-create-ab/test-create-ab.md" format="dita" scope="local">4 つのステップではなく、3 つの手順で A/B </a>および<a href="/help/main/c-activities/t-experience-target/t-xt-create/xt-create.md#task_D6B3429AC31549E1A70EDF04B3DDC765" format="dita" scope="local">エクスペリエンスのターゲット設定</a>アクティビティを作成します。 この変更により、これらのアクティビティの作成プロセスが、Automated Personalizationや多変量分析テストなどの他の種類のアクティビティのワークフローと似たものになりました。 </p> </td> 
  </tr> 
  <tr> 
...
* Analytics が Target のレポートソースとして使用されている場合に、Target Standard／Premium での多変量テストに対する Target ベースのコンバージョンイベントが報告されません。 この問題は、まもなく修正される予定です。
...
<table id="table_1558E5A5BAB64CC281C193F5A49E2ECE"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p class="premium">新しいアクティビティタイプ：レコメンデーション </p> </td> 
   <td colname="col2"> <p>レコメンデーションのアクティビティは、以前のユーザーアクティビティを基にして、顧客が興味を持つ可能性のある製品またはコンテンツを自動的に表示します。 レコメンデーションにより、顧客が関心を持ちそうな商品を積極的に紹介することが可能になります。 </p> <p>Recommendations は Target Premium ソリューションの一部として使用できます。 Target Premium ライセンスのない Target Standard には含まれません。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
* ユーザーが[!UICONTROL 目標と設定] ページで&#x200B;**[!UICONTROL リンクを選択]**&#x200B;をクリックしたときに、選択したリンクが選択済みとしてマークされない問題を修正しました。
...
[Target Standard 1.7 リリースウェビナー](https://my.adobeconnect.com/p1oabaz3cxi/)
...
<table id="table_11CD9EE0C9534FF19C9154784C4BFCF0"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> エクスペリエンスのターゲット設定アクティビティタイプ </td> 
