...
* **VECは、ルート要素なしで変更を読み込むようになりました。明確なルート要素を欠く変更を読み込むと、エクスペリエンスが停止する問題をVECで解決しました。 これらの変更により、UIが「A[!UICONTROL 最初の変更を適用する]」に無期限にハングしていました。 （TGT-53799）
...
* **簡体字中国語の「詳細を表示」の用語の不整合を修正しました。以前は、「詳細」という用語は、簡体字中国語（zh_CN）ロケールで「详情」と誤って翻訳され、確立された用語ガイドラインに違反していました。 これは、用語ベースとの一貫性を確保するために、「详细信息」に修正されました。 （TGT-53741）
...
* **スクロールバーを操作すると、レポートがクラッシュする。** 「[!UICONTROL  レポート ]」タブのスクロールバーをクリックすると、ページがクラッシュし、JavaScript エラーが発生しました。  エラーやクラッシュを引き起こさずに、`SyntaxError: Failed to execute 'querySelector' on 'Element': '[data-key="a-currentcopy"hiretalent""]' is not a valid selector.`件のレポートの読み込みとスクロールが正しく行われるようになりました。 （TGT-53828）
...
* **VECでパンくずリストをクリックしても、編集メニューが表示されない。（VEC）のパンくずリストを使用してHTMLのエレメントを選択すると、編集メニューが断続的に表示されなかったり、すぐに表示されなくなったりして、エレメントの選択が信頼できないようになります。 パンくずリストを使用して移動する際に、編集メニューが一貫して表示されるようになり、更新されたアクティビティ作成プロセスでエレメント選択ワークフローが改善されました。 （TGT-52873）
...
* Analytics]で「[!UICONTROL  ビュー」リンクをクリックすると、ユーザーが意図した[!DNL Analytics] ダッシュボードではなくホームページにリダイレクトされる問題を修正しました。 （TGT-53092 および TGT-53093）  <!-- * Fixed an issue when cloning an existing activity and changing the reporting source from [!DNL Target] to [!DNL Adobe Analytics], users encounter a "400 - Invalid User Input" error, preventing the activity from being saved. (TGT-52875)-->
...
* 9月15日（PDT）から10月6日（PDT）の午前9:25まで（PDT）にレポートする[!DNL Adobe Target Premium] ユーザーの[!UICONTROL 自動ターゲット ]に影響を与える問題を修正しました。 :30影響を受けるコンバージョン指標のレポートを表示する際（「[!UICONTROL  ページを表示]」または「[!UICONTROL mbox] クリック」オプションを使用して設定）に、コンバージョン率が誤って報告されます。 現時点では、配信に関する既知の問題はありません。
