...
* **エンティティ属性が一致する[!UICONTROL 属性によるプロモーション &#x200B;]のレコメンデーションキーが読み込まれません。** アクティビティの保存後に編集した際に、ルールタイプ [!UICONTROL &#x200B; エンティティ属性の一致]を持つタイプ [!UICONTROL 属性によるプロモーション &#x200B;]のプロモーションで、レコメンデーションキーが読み込まれない問題を解決しました。 GraphQLを通じてレコメンデーションキーが正しく取得され、プロモーションが期待どおりに表示され、機能するようになりました。 （TGT-53917）
...
* **エクスペリエンスのターゲット設定 （XT）アクティビティの注文の詳細をCSV問題にエクスポートします。** [!UICONTROL 注文詳細をCSV]に書き出すオプションが誤ってXT アクティビティに表示され、空のファイルが返される問題を修正しました。 このオプションは、AP アクティビティに対してのみ表示されるようになりました。これにより、正確な書き出し機能を確保し、混乱を防ぐことができます。 （TGT-53798）
...
* **コンポーザー モードの ボタンのクリックにより、ページのリダイレクトがトリガーされなくなりました。** 更新されたVEC UIで、[!UICONTROL Composer] モードでボタンをクリックすると、ボタンのターゲット URLに予期しないリダイレクトが発生する問題を解決しました。 これにより、call-to-action（CTA）要素を編集できず、オーサリングワークフローが中断しました。 （TGT-53137）
...
* **Recommendations UI 内のコレクションに製品が表示されない問題を修正しました。** 単一テナントの顧客から、新しいUIの[!UICONTROL Recommendations] セクションで特定のコレクションを表示する際に、製品リストを読み込めなかったことが報告されました。 環境を切り替えることで一時的に問題が解決されましたが、この回避策ではユーザーエクスペリエンスが低下しました。 環境の切り替えを必要とせずに、製品エンティティが一貫して読み込まれるようになりました。 （TGT-53783）
...
* **[!UICONTROL 重要な属性] ビューで復元された完全なCSV レポート  ボタンをダウンロードします。** 更新されたアクティビティ作成UIで、レポートビューの「[!UICONTROL 重要な属性]」セクションに「[!UICONTROL CSV レポート全体をダウンロード &#x200B;]」ボタンが表示されない問題を解決しました。 この修正により、ダウンロード可能なインサイトへのアクセスが復元され、更新されたUIと従来のUIの両方で一貫した機能が保証されます。 （TGT-53238）
...
* **製品カタログ検索高度な検索**&#x200B;で大文字と小文字を区別するフィルタリングが発生する問題を修正しました。[!UICONTROL 製品カタログ検索] ページの高度な検索フィルタリングで、バックエンドとGraphQL サービスの両方の動作に合わせて、大文字と小文字が区別されなくなりました。 このアップデートにより、テキストの大文字と小文字を問わず、顧客に対して一貫性のある正確な提案結果が提供されます。 （TGT-53585）
...
* **カタログ検索**&#x200B;での画像の読み込みに一貫性がなかった問題を修正：[!UICONTROL &#x200B; カタログ検索]のサムネールと画像が、アクティビティ作成プロセスで一貫して読み込まれなかった問題を修正しました。 「サムネール URL」列が表示され、一部の製品画像がナビゲーションまたは検索アクションの後に部分的に読み込まれるか、まったく読み込まれない場合を除き、画像が表示されませんでした。 画像の読み込み動作が安定し、列の表示やナビゲーションアクションに関係なく、サムネールが確実に表示されるようになりました。 （TGT-52778）
...
* **レコメンデーションカタログ が完全な製品属性データを断続的に表示できなかった問題を修正しました**：更新された[!DNL Recommendations] UIで、顧客は、データがフィードに存在していたにもかかわらず、メッセージなどの特定の製品属性が[!UICONTROL &#x200B; カタログ検索]結果に一貫して表示されない問題を経験しました。 この問題では、欠落している値を取得するために、列の表示を手動で再設定する必要がありました。 [!UICONTROL &#x200B; カタログ検索]では、設定済みのすべての属性が確実に表示されるようになったため、列を手動でリセットする必要がなくなりました。 （TGT-52769）
...
* **自動セグメント  レポートにnull オーディエンス値が表示される問題を修正しました**：顧客は、アクティビティレポートの[!UICONTROL 自動セグメント &#x200B;]がオーディエンスデータにnullを表示しており、セグメントパフォーマンスの正確な分析が妨げられていると報告しました。 この問題は、有効なオーディエンスデータが想定されていたにもかかわらず、[!UICONTROL &#x200B; レポート &#x200B;] セクションにアクセスし、[!UICONTROL 自動セグメント &#x200B;]を選択したときに発生しました。 [!UICONTROL 自動セグメント &#x200B;]は、オーディエンス値を正しく表示し、信頼性の高いレポートとセグメント化インサイトを確保できるようになりました。 （TGT-52793）
...
* **最終更新日列による整理不一致の並べ替えを修正：** アクティビティ作成プロセスで、[!UICONTROL 最終更新日]列によるカタログの並べ替えに一貫性のない結果が生じる問題を修正しました。 顧客は、更新タイムスタンプに基づいて商品データを確実に整理できず、カタログのレビューと管理がより困難になりました。 並べ替えが期待どおりに機能するようになり、更新されたUIの精度と使いやすさが向上しました。 （TGT-53116）
...
* Analyticsで「 ビュー」リンクをクリックすると、ユーザーが意図した[!DNL Analytics] ダッシュボードではなくホームページにリダイレクトされる問題を修正しました。 （TGT-53092 および TGT-53093）
...
* 収益目標の指標を持つTarget （A4T）アクティビティのAnalyticsでは、列名として「収益」が表示されず、収益指標がレポートに（$）形式で表示されませんでした。 これは、表面上の問題で、既に修正されています。 （TGT-46995）
...
* 「[!UICONTROL Analytics for Target] （A4T）をレポートソースとして使用するアクティビティの[!UICONTROL &#x200B; レポート &#x200B;] ページに「Analyticsで表示」リンクが表示されない場合がある問題を修正しました。 （TGT-45808）
...
* [Visual Experience Composer] （VEC）のDOM パス (/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#dom-path)を使用して要素を移動し、シャドウ DOM要素を含める場合の検索を改善しました。 （TGT-45262）
...
* デバイス決定 （ODD） ハイブリッド実行モードでOn Device Decisioningで、`pageLoad`がtarget-global-mboxにマッピングされない問題を修正しました。
...
* オーディエンスがソースで削除され [!DNL Target] アクティビティで使用できなくなったことを示す警告アイコン、ポップオーバーおよびメッセージが、[!DNL Target] UI の様々な場所に追加されました。
  ...
  * アクティビティ作成ワークフローの[!UICONTROL &#x200B; エクスペリエンス &#x200B;] ステップ：
    ...
    ![ エクスペリエンス ] ページ (assets/deleted-at-source-experiences.png)のソースメッセージでオーディエンスが削除されました

  * アクティビティ作成ワークフローの[!UICONTROL &#x200B; ターゲティング &#x200B;] ステップ：

    ![ ターゲティング ] ページ (assets/deleted-at-source-targeting.png)のソースメッセージでオーディエンスが削除されました

  * アクティビティ作成ワークフローの[!UICONTROL 目標と設定] ステップ：

    ![目標と設定] ページ (assets/deleted-at-source-goals-settings.png)のソースメッセージでオーディエンスが削除されました
...
| 機能 | 詳細 |
| --- | --- |
| Target （A4T）のAnalyticsでのリダイレクトのサポート | [A4T](/help/main/c-integrating-target-with-mac/a4t/a4t.md).<br>詳細については、[Analytics for [!DNL Target] implementation](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)を参照してください。[!DNL Target] リダイレクトがサポートされるようになりました。 |
