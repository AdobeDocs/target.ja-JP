...
* **一部のアクティビティに場所がありません。** 一部のアクティビティで&#x200B;**[!UICONTROL Location]**&#x200B;が見つからない問題を修正しました。 （TGT-54951）
...
* **追加のビューに適用すると、カスタムコードが保持されます。** 同じ&#x200B;**[!UICONTROL Activity]**&#x200B;内の別の&#x200B;**[!UICONTROL View]**&#x200B;のカスタムコードを追加または保存する際に、1つの&#x200B;**[!UICONTROL View]**&#x200B;に適用されたカスタムコードが削除される問題を修正しました。 （TGT-53933）
+++
...
* **アクティビティ ビューに表示されるオーディエンス属性。** **[!UICONTROL Activity]**&#x200B;から表示されたオーディエンスルールの詳細で、同じオーディエンスを&#x200B;**[!UICONTROL Audiences]** セクションから開いたときに表示された特定の属性が表示されない問題を修正しました。 （TGT-54742）

* **アクティビティ リスト ページでCSVをエクスポートします。** 日常的な書き出しのAPIのみに依存することなく、フィルターが適用される場合を含め、ユーザーインターフェイスからアクティビティリストを書き出せるように、**[!UICONTROL Export CSV]** アクションを追加しました。 （TGT-51466）
...
* **[!UICONTROL Automated personalization]件のアクティビティ。** インターフェイスとアクティビティの読み込みの問題を修正し、ユーザーがAutomated Personalization アクティビティを確実に作成、編集、管理できなくなり、キャンペーンの設定がブロックされ、パーソナライゼーションのユースケースが遅れました。 （TGT-54421）
...
* アクティビティからオーディエンスを作成する際に&#x200B;**オーディエンス名と説明が表示されます。** **[!UICONTROL Audiences]**&#x200B;の直下にオーディエンスを作成する場合と比較して、アクティビティフローからオーディエンスを作成または編集する場合に、オーディエンス **[!UICONTROL Name]**&#x200B;と&#x200B;**[!UICONTROL Description]** フィールドが明確に表示されない問題を修正しました。 （TGT-54837）
...
* **[!UICONTROL Live Activities]個がインサイトを利用しています。** インサイトダッシュボードの&#x200B;**[!UICONTROL Live Activities]**&#x200B;指標が、**[!UICONTROL All Activities]**&#x200B;でライブとして表示されたアクティビティの数よりも多い合計を報告する可能性がある問題を修正しました。 （TGT-54788）
...
* [!UICONTROL Global Exclusions]の&#x200B;**長いID リスト。** **[!UICONTROL Global Exclusions]**&#x200B;のIDの長いリストを貼り付けたり入力したりすると、更新されたインターフェイスでレガシーと比較して切り捨てられる可能性があり、不完全な除外リストが発生する問題を修正しました。 （TGT-54422）
...
* [!UICONTROL Visual Experience Composer]の&#x200B;**Enhanced Experience Composer （EEC） ステータス インジケーター。** EEC インジケーターは、拡張Experience Composerが有効かどうかを示します。 表示が修正され、インタラクティブではないステータス表示としてのみ機能するため、インタラクティブなトグルに似なくなりました。 （TGT-54828）

* **[!UICONTROL Visual Experience Composer].**&#x200B;の折りたたみ可能な左パネル アクティビティを編集用に開いている間に、左側のパネルを折りたたむことができるようになりました。 これにより、小さなディスプレイを含め、複数のオーディエンスとページを含むアクティビティの&#x200B;**[!UICONTROL Components]**&#x200B;と&#x200B;**[!UICONTROL Properties]**&#x200B;へのアクセスが改善されます。 （TGT-54269）
...
* **Visual Experience Composer （Chrome）のプライベート ネットワーク URL。** Chromeでは、ローカルネットワークへのアクセスが許可されていない限り、プライベートネットワーク上のページをブロックできます。 **[!UICONTROL Visual Experience Composer]**&#x200B;は、Experience Cloudが使用するフレーミングで権限をリクエストするようになりました。これにより、Chromeはサイレントで失敗するのではなく、許可プロンプトを表示できます。 アクセスを許可すると、内部URLは、お使いの環境でサポートされているときにVECに読み込まれる可能性があります。 （TGT-54346）
...
* **実装画面のちらつき管理トグルを追加します。** フリッカー管理設定の有効化を制御するために、[!UICONTROL Implementation]画面に新しい切り替えスイッチが追加されました。 この切り替えにより、管理者は実装画面から直接ちらつき管理を設定できます。 （TGT-52247）
...
* **URL管理機能を許可リストに加えるしました。** 許可されたリモート オファーのURL パターンを管理するための新しい「許可リストに加えるURL」管理機能が[!UICONTROL Administration] セクションに追加されました。 この機能には、許可リストに加えるされたURLを表に表示する機能、URLを検索してフィルタリングする機能、モーダルダイアログを使用して新しいURLを追加する機能、確認を行ってURLを削除する機能が含まれています。 この機能により、顧客はリモートオファーに使用できるURLを制御できるようになり、セキュリティとコンプライアンスが向上します。 （TGT-54486）
...
* **Web エディターのオファーでワークスペース列を有効にします。** この更新プログラムは、Web エディター内の他のワークスペースに表示される[!UICONTROL Default Workspace]からのオファーによって引き起こされる顧客の混乱に対処します。 この動作は設計どおりに機能していますが、[!UICONTROL Default Workspace]件のオファーは、すべてのワークスペースで意図的に表示されていますが、お客様は、UIがワークスペースの出所を明確にしていないと報告しました。特に、「承認者」などのデフォルト以外のワークスペースでアクティビティを作成する場合に問題が発生します。 わかりやすくするために、Web エディターのオファーリストで[!UICONTROL Workspace]列が有効になり、ユーザーは各オファーがどのワークスペースに属しているかを簡単に識別でき、表示される追加オファーの誤解釈を防ぐことができます。 （TGT-54138）
* **target=&quot;_blank&quot;のリンクが新しいタブで開きます。** この修正は、[!UICONTROL Browse] モードでクリックすると、~target=&quot;_blank&quot;~のリンクを含む作成されたweb サイトが新しいブラウザータブで開き、エディター内のプレビューエクスペリエンスが中断される問題を解決します。 この動作は、アンカー要素が変換され、そのターゲットがエディター内でのナビゲーションを維持するために上書きされた従来のUIとは異なり、作成されたページのネイティブリンク属性が拡張機能の挿入されたJavaScriptによってインターセプトされなかったために発生しました。 このアップデートにより、~target=&quot;_blank&quot;~を使用するリンクがWeb エディター内で適切に処理されるようになり、オーサリング中に外部タブが開かなくなります。 （TGT-54134）
...
* **[!UICONTROL Workspaces]ドロップダウンは、[!UICONTROL Users] セクションで20に制限されています。** この修正は、ユーザーがもっと多くのアクセス権を持っていた場合でも、[!UICONTROL Administration] > [!UICONTROL Users] セクションの[!UICONTROL Workspaces] ドロップダウンが20個のワークスペースのみを表示する問題を解決します。 `licenseGroups`の基礎となるGraphQL呼び出しも20件に制限され、ユーザーが組織内のより多くのワークスペースにアクセスできたにもかかわらず、UIに不完全なリストが表示されました。 この更新により、このハード制限が削除され、使用可能なワークスペースの完全なセットが返され、正しく表示されるようになりました。 （TGT-53820）
* **オファーのモーダルでワークスペース列が表示されない問題を修正しました。** 更新されたUIでオファーのモーダルにワークスペース列が表示されない問題を修正しました。 これは、[!UICONTROL Default Workspace]からのオファーが、選択したワークスペースからのオファーと一緒に表示され、その出所を示さなかったため、顧客に混乱をもたらしました。 ワークスペース列が有効になり、顧客は各オファーがどのワークスペースに属しているかを明確に識別できます。 （TGT-52320）
...
* **既に削除されている場合、アクティビティ編集で自動検出されたプロパティを追加しないでください。** この修正は、アクティビティを編集すると、ユーザーが以前に削除した自動検出プロパティが自動的に再表示される問題を解決します。 編集のためにアクティビティを再開すると、削除されたプロパティが誤って復元され、[!UICONTROL Properties List]の動作が不安定になり、混乱が生じます。 このアップデートにより、自動検出されたプロパティが削除されると、その後のすべての編集中に削除され、ユーザーが明示的に追加しない限り再び表示されなくなります。 （TGT-54182）
* **既に削除されている場合は、自動検出されたプロパティを追加しないでください。** この修正により、ユーザーがアクティビティから自動検出されたプロパティを手動で削除すると、その後のアクティビティエディター内でのナビゲーション中にシステムが再導入されなくなります。 以前は、ユーザーが自動検出されたプロパティの選択を解除し、[!UICONTROL Targeting] ステップに移動してから[!UICONTROL Experiences]に戻った場合、エディターは、アクティビティエディターの状態スライスに保存されている自動検出リストに基づいて、削除されたプロパティを再入力していました。 更新されたロジックは、自動検出されたプロパティを~ActivityState~ スライスの現在のプロパティと比較し、ユーザーが既に削除した自動検出されたプロパティを再追加しないようにしました。 これにより、ステップをまたいで一貫性のある行動を実現し、ユーザーの意図を尊重できます。 （TGT-54181）
* **自動検出されたテキストをプロパティ リストに追加します。** この機能強化により、[!UICONTROL Properties List]が更新され、システムによって自動的に検出されたプロパティに明確なラベルが付けられます。 自動検出されたプロパティがユーザーに表示される[!UICONTROL Properties List]にも存在する場合、~ActivityEditorSlice~状態に保存されている値を使用して、その名前の横に「（自動検出）」テキストが表示されるようになりました。 これは、従来のUIの動作を反映し、ユーザーが手動で選択したプロパティと自動的に識別されたプロパティを簡単に区別するのに役立ちます。 （TGT-54120）
* **自動検出された[!UICONTROL Properties]を状態に追加します。** この更新により、~ActivityEditorSlice.ExperienceEditor~状態で、Web エディターからアクティビティ [!UICONTROL Experiences] タブに渡されたすべての自動検出プロパティ IDの最新リストが常に維持されます。 ユーザーが[!UICONTROL Experiences] タブに移動するたびに、状態は新しく検出されたプロパティで更新され、重複を防ぎながら、正確な追跡と信頼性の高い下流の動作が保証されます。 （TGT-54119）
...
* **[!UICONTROL Reports]矢印が拡張可能な列を明確に示さない問題を修正しました。** 更新されたUIで追加の列を拡張できることがレポートテーブルに明確に表示されない問題を修正しました。 列ヘッダーの近くにある[!UICONTROL Reports]矢印に消えるツールチップが追加され、より多くの列が使用可能であることを顧客が理解しやすくなりました。 （TGT-53479）
...
* 新しいVEC UI **で**[!UICONTROL Experience Fragment]&#x200B;名が切り捨てられました（TGT-54312）
* **指標[!UICONTROL Revenue]に[!UICONTROL Advanced Settings]を使用できません。** この修正により、[!UICONTROL Goals & Settings]の[!UICONTROL Revenue]指標に[!UICONTROL Advanced Settings]を設定する際に、ユーザーが403 「アクセス拒否」エラーを検出した問題が解決されます。 プライマリ目標に関連付けられた依存関係の条件を追加する際に問題が発生しました。バックエンドでは、既にアクティビティの作成と編集に十分な権限を持っているユーザーであっても、誤ってエディター権限が必要でした。 その結果、有効な設定にもかかわらず、アクティビティの保存に失敗しました。 このアップデートでは、権限チェックが修正され、適切なアクセス権を持つユーザーが禁止リソースエラーをトリガーせずに収益メトリックの依存関係を正常に追加できるようになりました。 （TGT-54092）
* **選択した画像に追加ボタンが適用されない問題を修正しました。** アクティビティ作成プロセスで画像を選択または更新する際に、顧客が特定の画像を追加できない問題を修正しました。 例えば、顧客が特定のアセットを検索した場合、「ipp」を検索したときに返された画像は、[!UICONTROL Add] ボタンをクリックしても選択した画像が適用されず、変更は作成されませんでした。 `Homepage-banner-1-moz.jpg`などの他の画像を選択しても、期待どおりに機能し続けました。 このアップデートにより、更新されたUIですべての有効な画像を一貫して適用できるようになります。 （TGT-53610）
* **URL条件を削除すると、目標指標の設定がリセットされる問題を修正しました。** [!UICONTROL Goal]指標で単一のURL条件を削除すると、更新されたUIで設定全体がリセットされる問題を修正しました。 お客様が[!UICONTROL Conversion] > [!UICONTROL Viewed a Page]の下に保存されたURL条件を削除しようとすると、目標タイプが予期せず[!UICONTROL Viewed an Mbox]に切り替わり、以前に設定されたすべての設定が削除されました。 このアップデートにより、選択したURL条件のみが削除され、残りのすべての目標設定が維持されます。 （TGT-53271）
...
* **[!UICONTROL Message]列が、更新されたUIに製品データを断続的に表示できませんでした。** 更新された[!UICONTROL Recommendations] UIで、[!UICONTROL Catalog Search]の[!UICONTROL Message]列がフィードに値が存在していたにもかかわらず、製品データを断続的に表示できなかった問題を解決しました。 列は、すべての製品で正しいメッセージ値を一貫して表示するようになり、列の再設定を手動で必要とせずに信頼性の高い可視性を確保します。 （TGT-52777）
* 更新されたUIにアクティビティを保存すると、**[!UICONTROL Download Recommendations Data]ボタンが表示されません。** 更新されたUIで、再保存後でも特定の保存済みアクティビティに[!UICONTROL Download Recommendations Data] ボタンが表示されない問題を解決しました。 このボタンは、あらゆるアクティビティで一貫して表示されるようになったため、ユーザーは回避策を必要とせずにレコメンデーションデータを確実に書き出すことができます。 （TGT-53802）
...
* 更新されたUIで[!DNL Adobe Analytics]をレポートソースとして使用すると、**[!UICONTROL Goals & Settings]エラーメッセージが表示されます。** 更新された[!UICONTROL Overview] UIで、「目標」セクションに「問題が発生しました」というエラーが表示される問題を解決しました。 ご要望の処理は完了できません。 [!DNL Adobe Analytics] （A4T）がレポートソースとして選択されたときに問題が解決しない場合は、[!DNL Adobe Client Care]にお問い合わせください。 目標が[!UICONTROL Adobe Analytics]指標で正しく表示されるようになり、レポートソース全体で一貫した可視性が確保されました。 （TGT-54021）
...
* **更新されたUIで決定オファーを編集または置き換えることができません。** 更新されたUIで、[!UICONTROL Modifications] パネルで決定オファーを編集または置き換えることができず、オファー名が空白で表示される問題を解決しました。 決定オファーは完全にアクセス可能で編集可能になり、従来のUIとの整合性が回復され、顧客がアクティビティ内で直接オファーを管理できるようになります。 （TGT-53884）
...
* **エンティティ属性の一致を持つ属性によるプロモーションは、アクティビティの保存後にレコメンデーションキーを読み込めませんでした。** アクティビティを保存した後に編集されたときに、ルールタイプ [!UICONTROL Entity Attribute Matching]のタイプ [!UICONTROL Promotion by Attribute]のプロモーションがレコメンデーションキーを読み込まない問題を修正しました。 この問題は、`customKeyId`がGraphQLを通じてリクエストされていないことが原因で発生しました。 プロモーション編集中にレコメンデーションキーが正しく読み込まれるようになりました。 （TGT-53117）
...
* **一致する[!UICONTROL Entity Attribute]件の[!UICONTROL Promotion by Attribute]のRecommendation キーが読み込まれていません。** アクティビティの保存後に編集した場合、ルールタイプ [!UICONTROL Entity Attribute Matching]のタイプ [!UICONTROL Promotion by Attribute]のプロモーションでレコメンデーションキーが読み込まれない問題を解決しました。 GraphQLを通じてレコメンデーションキーが正しく取得され、プロモーションが期待どおりに表示され、機能するようになりました。 （TGT-53917）
* **更新されたUIで非表示のHTML要素に関する推奨事項を編集できません。** 非表示のHTML要素に適用されたレコメンデーションアクティビティを編集できない[!UICONTROL New Create]およびVEC UIの問題を解決しました。 この機能が期待どおりに動作するようになり、従来のUIとのパリティを復元し、要素の表示に関係なくレコメンデーションを変更できるようになりました。 （TGT-53953）
...
* 更新されたUIの[!DNL Recommendations] アクティビティに&#x200B;**[!UICONTROL Download Recommendations]ボタンがありません。** 更新された[!DNL Recommendations] UIで、レコメンデーションを使用してA/B アクティビティに[!UICONTROL Download Recommendations] ボタンが表示されない問題を解決しました。 ボタンが正しく表示され、レコメンデーションデータを従来のUIの機能と一致した形式で書き出せるようになりました。 （TGT-53768）
* 更新された概要UIに&#x200B;**[!UICONTROL Download Recommendation Data]ボタンがありません。** 更新された[!UICONTROL Overview] UIで、レコメンデーションを含むアクティビティに[!UICONTROL Download Recommendation Data] ボタンが表示されない問題を解決しました。 レコメンデーションデータを直接書き出すことができるため、従来のUIに切り替える必要はありません。 （TGT-53772）
* **アクティビティ条件を編集すると、更新されたUIに空白の画面が表示されることがあります。** 更新されたUIで[!UICONTROL Edit Criteria in Experiences]をクリックすると、特定のアクティビティに対して空白の画面が表示される問題を解決しました。 条件エディターがすべてのアクティビティで確実に読み込まれるようになり、ユーザーが中断することなく編集できるようになりました。 （TGT-53961）
* **更新されたUIでシーケンス条件を編集できません。** 更新されたUIで[!UICONTROL Sequence Criteria]を編集しようとすると、読み込み中に条件ポップアップが表示されたままになり、空白の画面が表示される問題を解決しました。 条件エディターが正しく読み込まれるようになり、ユーザーは中断することなくシーケンス条件を編集および更新できるようになりました。 （TGT-53985）
...
* **[!UICONTROL Multivariate Test]（MVT）の場所とグラフ レポートの問題により、レポートの生成が妨げられました。** MVT アクティビティがTarget UIで[!UICONTROL Location Contribution]およびグラフレポートの生成に失敗し、「問題が発生しました」というエラーが表示される問題を解決しました。 お客様のリクエストを完了できません。」 レポートがUI内で正しく読み込まれるようになり、完全に表示されるようになりました。 （TGT-53654）
* [!UICONTROL Element]件の貢献度レポートエラーにより、**MVT レポートが読み込まれません。** Target UIでMVT アクティビティレポートの読み込みに失敗し、「要素貢献度レポートを取得できませんでした」というエラーが表示される問題を修正しました。 レポートが正しく表示され、要素の貢献度が完全に表示されるようになりました。 （TGT-53691）
* **注文の詳細を[!UICONTROL Experience Targeting] （XT） アクティビティのCSV問題にエクスポートします。** XT アクティビティに[!UICONTROL Export Order Details to CSV] オプションが正しく表示されず、空のファイルが返される問題を修正しました。 このオプションは、AP アクティビティに対してのみ表示されるようになりました。これにより、正確な書き出し機能を確保し、混乱を防ぐことができます。 （TGT-53798）
...
* **[!UICONTROL Delete Modification]ボタンの問題により、アクティビティの変更を削除できませんでした。** [!DNL Target] UIの[!UICONTROL Delete Modification] ボタンが機能せず、ユーザーがアクティビティ内の変更を削除できない問題を解決しました。 ボタンが期待どおりに動作するようになり、変更を遅滞なく確実に削除できるようになりました。 （TGT-53728）
...
* **アクティビティの場所の配置が[!UICONTROL Edit] ページと[!UICONTROL Overview] ページの間で一致しません。** [!UICONTROL Overview] ページのアクティビティの場所の番号が、[!UICONTROL  Edit Experience] ページで行われた更新と一致しない問題を解決しました。 これにより、両方のビューで位置の一貫性が維持され、正確な整列が確保され、位置の抜けや番号の間違いを防ぐことができます。 （TGT-53960 および TGT-53954）
* **更新されたVECで[!UICONTROL Design] モードに切り替えることができません。** 更新されたVEC UIで、[!UICONTROL Browse] モードで新しいページに移動した後、ユーザーが[!UICONTROL Design] モードに戻れなかった問題を解決しました。 [!UICONTROL Design] トグルが正しく機能するようになり、変更をページ間でシームレスに適用できるようになりました。 （TGT-53988 および TGT-53993）
* **クエリパラメーターがアクティビティの概要に表示されません。** 更新されたUIで、アクティビティの[!UICONTROL Overview] ページにクエリパラメーターが表示されず、[!UICONTROL Overview]とページ配信URLの間に不一致が生じる問題を解決しました。 クエリパラメーターが正しく表示されるようになり、アクティビティの場所が完全に表示され、ビュー間で一貫性が保たれます。 （TGT-53701）
...
+++ 詳細を見る
* **更新されたUI**&#x200B;で操作性の問題を解決しました。 [!UICONTROL Observers]は、従来のUIと同様に、[!UICONTROL View Activity] オプションを使用してアクティビティをプレビューできるようになりました。 （TGT-51741）
* **[!UICONTROL Observer]人のユーザーが、更新されたUIでアクティビティコンテンツを表示できるようになりました。** 更新されたアクティビティ UIで、監視者ロールのユーザーの表示を復元しました。 以前は、オブザーバーは変更、オファー、コンテンツの変更を表示できませんでした。これは、従来のUIで使用できる機能です。 （TGT-53785）
* **[!UICONTROL Approver]人のユーザーが、編集者権限エラーなしでアクティビティ目標を編集できるようになりました。** アクティビティ作成UIで、承認者レベルのユーザーが変更を高度な目標設定に保存するのをブロックする権限の問題を解決しました。 影響を受けるユーザーは、十分なアクセス権があるにもかかわらず、エディター権限を必要とする`403 Forbidden.Resource` エラーを受け取りました。 （TGT-53819）
...
* **複数のオーディエンスの選択が「このアクティビティのみ」レポートに復元されました。** アクティビティ作成UIで、[!UICONTROL Goals & Settings]の[!UICONTROL This activity only] セクションでユーザーが複数のオーディエンスを選択できない問題を解決しました。 （TGT-53283）
* **オーディエンスベースのレポートグラフで、コンバージョンデータが正しく表示されるようになりました。** デフォルト以外のオーディエンスを選択する際にグラフが失敗する原因となる[!UICONTROL Reports] タブの問題を解決しました。 データや信頼度の指標は利用可能でしたが、視覚的なグラフには直線しか表示されておらず、分析は困難でした。 （TGT-53769）
* **[!UICONTROL Targeting] UIに、除外されたオーディエンスルールが明確に示されるようになりました。** アクティビティ作成UIの[!UICONTROL Targeting] セクションで、[!UICONTROL Exclude]に設定されたオーディエンスルールが明確に表示されない問題を解決しました。 そのため、ターゲティングロジックを確認する際に、特に特定のURLを除くオーディエンスに対して、混乱が生じました。 （TGT-53809）
* **オーディエンス定義の値が、[!UICONTROL Targeting] タブで選択およびコピーできるようになりました。** アクティビティ作成インターフェイスの問題を解決し、ユーザーが[!UICONTROL Targeting] タブでオーディエンスルール値を選択およびコピーできなくなった。 この機能は従来のUIでは利用できましたが、更新されたUIでは利用できませんでした。 （TGT-53856）
...
* **ページ読み込みイベントの選択は、レコメンデーションの変更でも保持されるようになりました。** レコメンデーションのイベントタイプを[!UICONTROL View]から[!UICONTROL Page Load]に切り替える際に、ユーザーが変更を保存できないアクティビティ作成UIの問題を解決しました。 選択は正常に行われましたが、移動して元に戻り、アクティビティの公開がブロックされました。 （TGT-53957）
...
* **&quot;[!UICONTROL Export order details to CSV]&quot;が完全なデータをダウンロードしました。** 更新された[!UICONTROL Overview] UIで、有効なレポートデータが存在する場合でも「[!UICONTROL Export Order details to CSV]」オプションが空のファイルをダウンロードする問題を解決しました。 （TGT-53787）
...
* **アクティビティ作成UIに保存されたアクティビティの編集が復元されました。** 変更を保存した後、ユーザーがアクティビティを編集できない問題を解決しました。 影響を受けたアクティビティは「[!UICONTROL Applying initial modifications]」で停止したままとなり、さらに更新がブロックされ、[!UICONTROL Cancel] ボタンが非表示になりました。 （TGT-53631）
* **VECが&quot;[!UICONTROL Applying initial modifications].&quot;**&#x200B;で停止しなくなりました 多数の変更を伴うエクスペリエンスを読み込む際に長い遅延が発生するVECのパフォーマンスの問題を解決しました。 影響を受けるユーザーは、特にExperience B シナリオで、UIが「[!UICONTROL Applying initial modifications]」で数分間停止していました。 （TGT-53727）
* **VECは、ルート要素なしで変更を読み込むようになりました。**
明確なルート要素を欠く変更を読み込むと、エクスペリエンスが停止する問題をVECで解決しました。 これらの変更により、UIが「A[!UICONTROL pplying initial modifications]」に無期限にハングしていました。 （TGT-53799）
...
* **VECが&quot;[!UICONTROL Applying initial modifications].&quot;**&#x200B;で断続的に停止しなくなりました エクスペリエンスの読み込みに失敗し、「[!UICONTROL Applying initial modifications]」でスタックしたままになるVECの断続的な問題を解決しました。 この動作は一貫性がなく、リダイレクトループまたは手動キャッシュのクリアが必要な場合がありました。 （TGT-53916）
* **VECの読み込み問題を再現できませんでした。** 既存のアクティビティを編集する際に、VECが「[!UICONTROL Applying initial modifications]」でスタックしたままになる問題を調査しました。 この振る舞いは、親エレメントを欠いたHTMLの内容に関連していると疑われた。 繰り返しを監視し続け、HTML オファーに構造化コンテナを使用して安定性を確保することをお勧めします。 （TGT-53972）
* VECの&#x200B;**[!UICONTROL Design]モードは、断続的に[!UICONTROL Browse] モードのように動作しなくなりました。** [!UICONTROL Design] モードが[!UICONTROL Browse] モードのように断続的に動作し、クリック可能な`<a>` リンクを許可し、要素の選択を妨げるUIの問題を解決しました。 これにより、ホバーボックスが消え、修正ワークフローがブロックされました。 （TGT-53136）
* **ボタンが[!UICONTROL Composer] モードでクリックすると、ページのリダイレクトがトリガーされなくなります。** 更新されたVEC UIで、[!UICONTROL Composer] モードでボタンをクリックすると、ボタンのターゲット URLに予期しないリダイレクトが発生する問題を解決しました。 これにより、call-to-action（CTA）要素を編集できず、オーサリングワークフローが中断しました。 （TGT-53137）
...
* VECの&#x200B;**[!UICONTROL Design]モードで、編集可能なコンポーネントのリンク ナビゲーションがブロックされるようになりました。** 更新されたVECで、[!UICONTROL Design] モードでもボタンやリンクなどのクリック可能な要素がページリダイレクトをトリガーする問題を解決しました。 この動作は[!UICONTROL Browse] モードを模倣し、ユーザーが主要コンポーネントを変更することを妨げました。 （TGT-53696）
* **&quot;[!UICONTROL Clicked an element]&quot;指標が、リダイレクトまたはエラーなしで機能するようになりました。** 「[!UICONTROL Clicked an element]」コンバージョン指標を選択する際に、予期しないリダイレクトと空白の画面が発生する新しい作成UIの問題を解決しました。 セットアップ中にボタンをクリックすると、要素を登録する代わりにナビゲーションがトリガーされ、「[!UICONTROL element not found]」エラーが発生しました。 （TGT-53817）
...
* **「[!UICONTROL Targeting]」セクションは、オーディエンスルールが除外に設定されていることを示していません。** 除外ロジックで設定されたオーディエンスが、アクティビティ作成UIの[!UICONTROL Targeting] セクションに含まれるように誤って表示されました。 バックエンドが除外ルールを正しく適用しましたが、UIは除外ルールを視覚的に表現できず、「除外」ラベルが省略され、キャンペーンの設定中に混乱が生じました。 [!UICONTROL Targeting] セクションに除外ルールが明確に表示されるようになりました。これにより、オーディエンス設定とターゲティングのビジュアライゼーションの一貫性が確保されます。 （TGT-53809）
...
* **既存のアクティビティを変更する際に、条件コンテンツを編集できませんでした。** アクティビティを編集すると、[!UICONTROL Criteria] コンテンツセクションが無効になり、ボタンがグレー表示され、応答が表示されなくなりました。 この問題は、アクティビティの更新中に[!UICONTROL Criteria]設定が完全に編集可能であることを確認することで解決されました。 お客様は、選択を切り替えたり、回避策を使用したりすることなく、[!UICONTROL Criteria] コンテンツを変更できるようになりました。これにより、更新されたアクティビティ作成プロセスの柔軟性と使いやすさが向上しました。 （TGT-53812）
* **アクティビティ内で条件を編集できませんでした。** アクティビティ内から条件にアクセスすると、[!UICONTROL Edit Criteria]および[!UICONTROL Remove Criteria] オプションが無効になりました。 ただし、[!UICONTROL Recommendations] タブを使用して、同じ条件を正常に編集できます。 条件は、アクティビティ編集ワークフローと[!UICONTROL Recommendations] タブの両方から完全に編集可能になり、一貫性のある効率的な編集エクスペリエンスを実現します。 （TGT-53814）

+++

+++[!UICONTROL Reports]

* **A[!UICONTROL utomated Personalization] アクティビティでアドホックオファーを生成すると、レポートの不整合が発生しました。** [!UICONTROL Automated Personalization] （AP）アクティビティで「アドホックオファーを生成」機能を使用すると、レポートが不正確になりました。 具体的には、オファーIDが場所をまたいで再利用されたため、レポートデータが誤って帰属したり上書きされたりしていました。 現在、アドホックオファーは、場所ごとに異なるIDで生成され、設定されたあらゆるエクスペリエンスで正確なトラッキングとレポートを実現しています。 （TGT-53757）
* JavaScript エラーが発生したため、**アクティビティレポートを読み込めませんでした。** 特定のアクティビティの「[!UICONTROL Reports]」タブにアクセスすると、顧客に「問題が発生しました」というメッセージが表示されました。 エラーは、JavaScriptの例外が原因で発生しました：`getAnalyticsReportSummary` GraphQL呼び出し中にトリガーされた、未定義のプロパティを読み取れません（「indexOf」を読み取ります）。 レポートが正しく読み込まれるようになり、更新されたアクティビティ作成ワークフローで同様のエラーが発生するのを防ぐため、エラー処理が改善されました。 （TGT-53797）
* **スクロールバーを操作すると、レポートがクラッシュする。** 「[!UICONTROL Reports]」タブでスクロールバーをクリックすると、ページがクラッシュし、JavaScript エラーが発生しました。
...
* **レポートにプライマリ指標が表示されませんでした。** mboxを使用してコンバージョン指標として設定された主な指標がアクティビティレポートにありませんでした。 メトリック名またはmbox名で検索しても結果が得られず、主要なパフォーマンスデータが表示されません。 プライマリ指標が[!UICONTROL Reports] タブに正しく表示されるようになり、キャンペーンのパフォーマンスの正確な追跡と分析が可能になりました。 （TGT-53773）
* **更新されたUIの[!UICONTROL Reports] タブが、水平スクロールバーを操作するとクラッシュしました。** 水平方向のスクロールバーを使用して指標にアクセスする際に、[!UICONTROL Reports] ビューが断続的にクラッシュし、「問題が発生しました」エラーが発生しました。 スクロールバーが確実に機能するようになり、ズームアウトやShift キーを押しながらスクロールするなどの回避策を必要とせずに、すべての指標を表示および分析できるようになりました。 （TGT-53824）
...
* **モーダル [!UICONTROL Manage Content]の[!UICONTROL Automated Personalization] アクティビティで、アクティビティのみのオーディエンスに対して無効なユーザー入力エラーが表示される問題を修正しました。** AP アクティビティの[!UICONTROL  Manage Content] モーダルでアクティビティのみのオーディエンスを設定する際に、無効なユーザー入力エラーが発生しました。 この問題は、以前はオーディエンスが正常に使用されていたにもかかわらず発生しました。 統合されたオーディエンス設定が、検証エラーをトリガーせずに正しく保存されるようになりました。 （TGT-53749）
...
* **最初のアクティビティ作成中に、オファー決定オプションが一貫して表示されるようになりました。** 更新されたUIで、A/B アクティビティの初回作成フロー中に[!UICONTROL Offer Decision] オプションが表示されない問題を解決しました。特に、Offer Decisionsが有効になっているテナントでシークレットモードでアクセスした場合に発生します。 オプションは、[!UICONTROL Targeting] ステップに移動して[!UICONTROL Experiences]に戻った後にのみ表示されます。 この修正により、[!UICONTROL Offer Decision] オプションが最初のセットアップ中にすぐに使用できるようになり、操作性が向上し、混乱が軽減されます。 （TGT-51888）
...
* **更新されたUIで作成された[!UICONTROL Recommendations]個のアクティビティに対して、クリックの追跡が復元されました。** 更新されたUIで作成された[!UICONTROL Recommendations]個のアクティビティがクリックトラッキングを登録できず、報告されたコンバージョンが0になる問題を解決しました。 従来のUIで構築されたアクティビティでは、クリック数が正しく追跡され、予想どおりにコンバージョンが報告されます。 この修正により、更新されたUIで作成されたRecommendations アクティビティに正しいトラッキング属性が含まれるようになり、コンバージョンレポートとA4T指標との整合性が復元されます。 （TGT-53287）
* **Recommendation アクティビティのクリック追跡が復元されました。** 更新されたUIで作成された[!UICONTROL Recommendations]個のアクティビティがクリックトラッキングを登録できず、報告されたコンバージョンが0になる問題を解決しました。 レガシーUIがトラッキング ID （`at-track-click`）を[!UICONTROL Recommendations] コンテンツに正しく適用しましたが、更新されたUIが誤ってプレースホルダー（`__recsClickTrackIdPlaceholder__`）を挿入したため、バックエンドのトラッキングが妨げられました。 この修正により、[!DNL Recommendations] コンテンツに正しいトラッキング IDが含まれるようになり、コンバージョンレポートとA4T指標との整合性が復元されます。 （TGT-53496）
...
* **カスタム条件コレクションが[!UICONTROL Recommendations] UIに正しく表示されるようになりました。** Recommendations インターフェイスで、カスタム条件を使用して作成されたコレクションで製品結果が表示されない問題を修正しました。 標準の属性ベースのコレクションは期待どおりに機能しますが、カスタムフィルターを使用している場合、有効なカタログが一致しているにもかかわらず、「結果が見つかりません」が返されます。 この修正により、カスタム属性を使用したコレクションが[!UICONTROL Product] タブに正しく入力され、パーソナライズされたレコメンデーションワークフローの全機能が復元されるようになりました。 （TGT-53653）
* **最初に[!UICONTROL Products] ページを開いたときにコレクションに製品が表示されない問題を修正しました。** [!UICONTROL Recommendations] セクション内のコレクションにアクセスする顧客は、[!UICONTROL Products] ページを最初に開いたときに空の製品結果が発生しました。 この問題は、GraphQL クエリのバックエンドエラーが原因で、カスタム条件を持つコレクションの商品データを読み込めませんでした。 問題が解決され、環境の切り替えを必要とせずに製品が正しく表示されるようになりました。 （TGT-53694）
* **フォームベースの[!UICONTROL Recommendations] アクティビティでコレクションを削除できない問題を修正しました。** [!UICONTROL Form-Based Experience Composer]を使用して[!UICONTROL Recommendations] アクティビティを作成しているお客様は、以前に選択したコレクションの選択を解除できませんでした。 UIでは、保存する前にコレクションを選択する必要があり、ユーザーが「すべてのコレクション」に戻せませんでした。 この動作はVEC機能に合わせて更新され、お客様は選択したコレクションなしで保存でき、期待どおりに「すべてのコレクション」にデフォルト設定されています。 （TGT-53708）
...
* **ライブ [!UICONTROL Recommendations] アクティビティにプロモーションを適用するとエラーが発生する問題を修正しました。** 有効な設定の詳細を指定した後でも、フロント プロモーションをライブ [!UICONTROL Recommendations] アクティビティに適用しようとすると、「プロモーションにコレクション IDまたはフィルタリングルール値が見つかりません」というエラーが発生しました。 検証エラーをトリガーすることなく、ライブアクティビティにプロモーションを正常に適用できるようになり、更新されたアクティビティ作成UIでよりスムーズなエクスペリエンスを実現できます。 （TGT-53738）
* **[!UICONTROL Recommendations] UI内のコレクションに製品が表示されない問題を修正しました。** 単一テナントの顧客から、新しいUIの[!UICONTROL Recommendations] セクションで特定のコレクションを表示する際に、製品リストの読み込みに失敗したことが報告されました。 環境を切り替えることで一時的に問題が解決されましたが、この回避策ではユーザーエクスペリエンスが低下しました。 環境の切り替えを必要とせずに、製品エンティティが一貫して読み込まれるようになりました。 （TGT-53783）
...
* **[!UICONTROL Total Revenue]指標が、アクティビティレポートからのCSV書き出しに含まれるようになりました。** 更新された[!UICONTROL Overview] UIで、総収益がアクティビティレポートビューに正しく表示されていたが、CSV エクスポートから$0と表示される問題を解決しました。 この不一致により、ユーザーはオフライン分析とレポート用に書き出されたデータに頼ることができませんでした。 （TGT-53325）
* **[!UICONTROL Total Sales]指標が、アクティビティレポートからのCSV書き出しに含まれるようになりました。** 更新されたUIで、[!UICONTROL Total Sales]指標がアクティビティレポートビューに正しく表示されたが、CSV エクスポートに表示されない問題を解決しました。 この不一致により、ユーザーはダウンロードされたレポートの完全なパフォーマンスデータにアクセスできませんでした。 この修正により、[!UICONTROL Total Sales]個の値がCSV書き出しに正確に含まれるようになり、アプリ内レポートとオフライン分析の一貫性が復元されます。 （TGT-53330）
* **指標が有効になっていない[!UICONTROL Graph View]のエラーメッセージを改善しました。** 関連する[!DNL Analytics] レポートスイートでリクエストされた指標が有効になっていない場合、[!UICONTROL Graph View]が一般的な「問題が発生しました」メッセージを表示するVECの問題を修正しました。 この問題は、バックエンド GraphQLの応答で`not_enabled_metric` エラーがトリガーされました。 この修正により、あいまいなエラーは、ユーザーが[!DNL Analytics]の設定の問題を特定するのに役立つより有益なメッセージに置き換わり、混乱を減らし、不必要なサポートのエスカレーションを減らすことができます。 （TGT-53577）
* **レポート期間がサポートされている90日間の制限を超えた問題を修正しました。** [!UICONTROL Reports] セクションで「[!UICONTROL Last X Days]」フィルターを使用しているお客様は、90日を超える期間を選択できました。これにより、パフォーマンスの問題やデータが不完全になる可能性があります。 フィルターが更新され、最大90日間が適用され、一貫性のある信頼できるレポートが作成されるようになりました。 （TGT-53795）
...
* **Analytics for Target （A4T） レポートで[!UICONTROL Graph View]のデータを表示できなかった問題を修正しました。** A4T統合で[!DNL Target]を使用しているお客様が、A/B テストアクティビティの「レポート」タブでグラフ表示に切り替える際に、「問題が発生しました」エラーが発生しました。 [!UICONTROL Table View]が正しく読み込まれましたが、[!UICONTROL Graph View]は、選択した時間範囲の指標の傾向をレンダリングできませんでした。 [!UICONTROL Graph View]は、サポートされているすべての指標について、想定どおりにパフォーマンスデータを表示するようになり、更新されたUIでの可視性とレポート精度が向上しました。 （TGT-53573）
...
* **パンくずホバーで、VEC内の対応する要素がハイライト表示されるようになりました。** パンくずメニューの項目にカーソルを合わせたときに、エディターで対応する要素を強調表示するように[!UICONTROL Visual Experience Composer]を改善しました。 エレメントの種類（コンテナ、太字テキスト、ボタンなど）も表示されます。 このビヘイビアーは兄弟エレメントに適用され、検証リストに基づいて、SVGなどのサポートされていないタイプを除外します。 （TGT-53411）
...
* **アクティビティ [!UICONTROL Automated Personalization]の[!UICONTROL Manage Content] パネルに一貫性のない場所ラベルが表示される問題を修正しました。** 顧客から、[!UICONTROL Manage Content] パネルに、[!UICONTROL Experiences]と[!UICONTROL Offers] タブで位置番号が一致しないことが報告されました。 （例：エクスペリエンスの場所2と場所4と、オファーの場所1と場所2の場合） 2つの場所のみが設定されている場合でも、 場所ラベルが一貫性を持ち、変更に正確にマッピングされるようになり、アクティビティ作成プロセスにおける明瞭性と使いやすさが向上しました。 （TGT-52934）
...
* **[!UICONTROL AEM Experience Fragment]件のオファーに対するユーザー属性が改善されました。** [!UICONTROL AEM Experience Fragment] （XF）オファーの「[!UICONTROL Last updated]」フィールドに、ユーザー名ではなくコード IDが誤って表示されるVECの問題を解決しました。 この不一致は、更新されたUIでのオファーの選択中に発生し、従来のUIとHTML オファーとの不整合が生じ、最後のエディターの名前が正しく表示されました。 この修正により、オファータイプ間で一貫したユーザーアトリビューションが保証され、透明性が向上し、期待される行動との整合性が高まります。 （TGT-52880 および TGT-52898）
...
* **更新されたレポートの概要UIの「レポート」セクションにダウンロードオプションが復元されました。** 新しい概要UIで「[!UICONTROL Download]」ボタンがレポートセクションに表示されないという問題を解決し、ユーザーが属性重要性スコアを書き出せなくなりました。 このアップデートにより、完全な書き出し機能が復元され、分析とレポート用にダウンロード可能なデータに合理的にアクセスできるようになります。 （TGT-53222）
* **[!UICONTROL Download full CSV report]ボタンが[!UICONTROL Important attributes] ビューで復元されました。** 更新されたアクティビティ作成UIで、レポートビューの[!UICONTROL Important Attributes] セクションに[!UICONTROL Download full CSV report] ボタンが見つからない問題を解決しました。 この修正により、ダウンロード可能なインサイトへのアクセスが復元され、更新されたUIと従来のUIの両方で一貫した機能が保証されます。 （TGT-53238）
* **更新された概要UIの[!UICONTROL Auto Target]のレポートに影響するUIの問題を解決しました。** 更新された概要インターフェイスの複数のUIの問題を修正し、[!UICONTROL Auto Target] アクティビティレポートに影響を与えました。 これらの修正には、次のものが含まれます。

   * サマリーレポートに上昇率と信頼性の指標が欠落している
   * 「構築されたモデル」チェックボックスのカラーインジケーターが正しくない
   * [!DNL Analytics]のデータの相違にもかかわらず、非機能グラフレポート
   * [!UICONTROL Automated Segments]および[!UICONTROL Important Attributes]件のレポートのダウンロードリンクがありません
   * 壊れた[!UICONTROL Automated Segments] レポートの表示

  これらの修正により、予想されるレポート動作が復元され、更新されたUI全体で[!UICONTROL Auto Target] パフォーマンスの可視性が向上します。 （TGT-53484）
...
* 更新されたUIのパラメーターのプレゼンス条件に対する&#x200B;**フォームの検証が修正されました。** 更新されたUIで「[!UICONTROL Custom mbox parameter value is present]」または「[!UICONTROL Parameter is present]」を選択する際に、誤って顧客に値の入力が必要になる問題を解決しました。 この動作は、値に関係なくパラメーターの存在を単純にチェックする意図されたロジックと競合しました。 この修正により、フォームの検証と期待される動作が一致し、よりスムーズなアクティビティの設定が可能になり、更新されたUIの完全な導入がサポートされます。 （TGT-53638）
* **ページ配信内のパラメーター存在ルールに対するフォームロジックが修正されました。&quot;** 更新されたUIで、「[!UICONTROL Parameter is present]」、「[!UICONTROL Parameter is not present]」、「[!UICONTROL Parameter value is present]」、「[!UICONTROL Parameter value is not present]」などのページ配信ルールを選択する際に、ユーザーが誤って追加パラメーター値を入力する必要があった問題を解決しました。 この動作は、従来のUIと矛盾しており、値を指定せずにパラメーターの存在を検出するという意図されたロジックと矛盾していました。 この修正により、期待されるルール設定の動作が復元され、アクティビティの設定が合理化され、ユーザビリティが向上します。 （TGT-53640）
...
* **A/BおよびMVT アクティビティで解決された場所の名前変更の問題。** 更新されたUIで、[!UICONTROL A/B Test]または[!UICONTROL Multivariate Test] （MVT）アクティビティ内の場所の名前を変更しても、場所リスト、ターゲティング、および戻りの間を移動した後に保持されないバグを修正しました。 このアップデートにより、場所の名前の変更が保存され、アクティビティワークフロー全体で一貫して反映されます。 （TGT-52367）
* **更新されたUIでMVTおよびAP アクティビティの場所の名前の永続性が修正されました。** 更新されたUIで、[!UICONTROL Multivariate Test] （MVT）および[!UICONTROL Automated Personalization] （AP）アクティビティで編集された場所名が正しく保存されない問題を解決しました。 場所の名前を変更した後、[!UICONTROL Targeting]や[!UICONTROL Experiences]などのタブ間を移動すると、名前が以前の状態に戻りました。 この修正により、タブ間で一貫した場所の命名が保証され、アクティビティの設定中に信頼性が向上します。 （TGT-52927）
* **MVT アクティビティの管理エクスペリエンス パネルで場所のラベル付けが修正されました。** 更新されたUIで、[!UICONTROL Multivariate Test] （MVT）アクティビティの[!UICONTROL Manage Experiences] パネルに一貫性のない場所の番号付けが表示される問題を解決しました。 この修正により、位置情報ラベルが順次に行われ、ユーザー定義の変更と正しく整合することが保証され、エクスペリエンスのセットアップ時の明瞭性と使いやすさが向上します。 （TGT-52929）
...
* **お客様は、[!UICONTROL Activity Overview]**&#x200B;からアクティビティ名またはドキュメント名をコピーできませんでした。以前は、更新されたアクティビティ作成プロセスで、アクティビティ名または関連するオファー/ドキュメントを[!UICONTROL Activity overview]から直接コピーできませんでした。 この制限は、特に小さな画面での操作性に影響を与えました。 顧客は、回避策なしで、アクティビティ名とドキュメント名の両方を簡単にコピーできるようになりました。 （TGT-51850）
...
* **AP アクティビティは、[!UICONTROL Reports] セクション**&#x200B;でモデル対応のデータを取得しませんでした。[!UICONTROL Reports] セクションでAutomated Personalization （AP） アクティビティを表示しているお客様は、レポートグループとオファーレベルでモデル対応の指標を表示できませんでした。 この問題は、モデル対応データがバックエンドから正しく取得されていなかったために発生しました。 機能が復元され、モデル対応データが期待どおりに表示されるようになりました。 （TGT-53600 および TGT-53601）
* **将来にスケジュールされたアクティビティが、[!UICONTROL Activity]概要**&#x200B;に「[!UICONTROL Live]」ステータスを誤って表示しました。お客様は、将来に開始するようにスケジュールされたアクティビティが、[!UICONTROL Activity]概要で「[!UICONTROL Live]」として誤ってマークされていることに気付きました。 このステータスの不一致は解決され、スケジュールされたアクティビティは、ページの更新を必要とせずに「[!UICONTROL Scheduled]」として正しく表示されるようになりました。 （TGT-52835）
...
* **製品リストが[!UICONTROL View Collection] ダイアログに表示されませんでした：**&#x200B;以前は、お客様は[!UICONTROL Recommendations] タブでコレクションを表示する際に、製品リストを表示できませんでした。 [!UICONTROL View Collection] ダイアログに関連する製品が正しく表示されるようになり、更新された[!UICONTROL Recommendations] UIの透明性と使いやすさが向上しました。 （TGT-50531）
* **[!UICONTROL Product Catalog Search]高度な検索**&#x200B;で大文字と小文字を区別するフィルタリングが発生する問題を修正しました。[!UICONTROL Product Catalog Search] ページの高度な検索フィルタリングでは、バックエンドとGraphQL サービスの両方の動作に合わせて、大文字と小文字が区別されなくなりました。 このアップデートにより、テキストの大文字と小文字を問わず、顧客に対して一貫性のある正確な提案結果が提供されます。 （TGT-53585）
* **更新された[!UICONTROL Product Catalog Search] UIの詳細検索で候補が表示されませんでした**：更新された[!UICONTROL Product Catalog Search] UIの詳細検索機能を使用しているお客様は、候補が表示されないため、正確な値を正しいスペルで入力する必要がありました。 そのため、商品を効率的に探すことが難しくなっていました。 高度な検索入力時に、候補が正常に表示されるようになりました。 （TGT-52008）
* **一部の承認者が[!UICONTROL Product Catalog Search]**&#x200B;で製品を表示できませんでした：[!UICONTROL Approver]権限を持つお客様は、同じ役割を持つ他のユーザーがアクセス権を持っているにもかかわらず、[!UICONTROL Product Catalog Search]で製品を表示できませんでした。 この問題は、カタログの表示に影響を与える権限の不整合が原因で発生しました。 すべての承認者が、期待どおりに[!UICONTROL Recommendations] セクションで製品を表示できるようになりました。 （TGT-53617）
...
* **レポートページでオーディエンスを切り替えると、ターゲット UI**&#x200B;でエラーが発生しました。お客様は、[!UICONTROL Reports] セクションで特定のオーディエンスを選択する際にエラーが発生しました。 この問題は、バックエンドのGraphQL呼び出しで無効なオーディエンス処理が発生し、予期しないエラーやデータ不足が発生したことが原因で発生しました。 この問題は解決され、データがない場合でも、デスクトップオーディエンスがエラーなしで読み込まれるようになりました。 （TGT-53370）
* [!UICONTROL Reports] セクションの&#x200B;**[!UICONTROL Graph view]で[!DNL Analytics]**&#x200B;の値が表示されませんでした：Re ポート セクションの[!UICONTROL Graph view]にアクセスしているお客様で、すべての値がゼロとして表示されるデータが見つかりませんでした。 この問題は、[!UICONTROL Analytics]からの誤ったデータ取得が原因で発生しました。 [!UICONTROL Graph view]が期待どおりに正確な値を表示するようになりました。 （TGT-52792）
+++
...
* **関数が見つからなかったため、[!UICONTROL Enhanced Experience Composer] （EEC）を使用して「Cookieを受け入れる」をクリックできませんでした**：お客様は、EEC経由でCookieを受け入れようとすると、コンソールエラーが発生したと報告しました：`handleclickAcceptAllButton is not defined`。 Cookie受け入れ機能が期待どおりに動作するようになり、更新されたUIでアクティビティ作成中にスムーズなエクスペリエンスが実現します。 （TGT-52794）
...
* **Backspaceを使用してすべての日付値を消去すると、ページがクラッシュする**:Backspaceを使用して「[!UICONTROL Specified Date & Time]」フィールドからすべての値を消去すると、[!UICONTROL Goals & Settings] セクションのスケジュール設定アクティビティがクラッシュする。 この問題は、日付処理ロジックのnull参照エラーが原因で発生しました。 ページがクラッシュすることなく、空の日付入力を適切に処理するようになりました。 （TGT-53624）
* **無効なペイロードが原因で[!UICONTROL Product Catalog Search]に商品が表示されませんでした**: [!UICONTROL Product Catalog Search]の[!UICONTROL Recommendations] セクションにアクセスしているお客様が、無効なGraphQL ペイロードが原因で空の結果が発生しました。 このバックエンドエラーにより、製品データが正しく読み込まれませんでした。 更新されたUIで、製品が期待どおりに表示されるようになりました。 （TGT-53630）
* 更新されたVEC **では、**[!DNL Scene7]&#x200B;個の画像が低解像度で保存されました。更新されたVECでエクスペリエンスを編集しているお客様は、[!UICONTROL Scene7]個の画像URLが解像度パラメーターなしで保存され、低画質の画像が配信されたことに気づきました（意図した800×800ではなく400×400）。 画像URLは、適切な解像度を確保するために正しいパラメーターを保持するようになりました。 （TGT-52631）
...
* **更新されたVEC**&#x200B;の[!UICONTROL Failed]および[!UICONTROL Draft] アクティビティ セクションを廃止しました：[!UICONTROL Failed]および[!UICONTROL Draft] アクティビティ オプションが更新されたVECから削除されました。 新しいUIでは、ドラフト状態がサポートされなくなり、失敗したキャンペーンはバックエンドに保存されません。 これらのオプションは、もはや関連性がありません。 関連するフィルターとバックエンドフィールド （例：`uiSyncFailed`、`errorMessage`）も削除され、アクティビティ管理が効率化されました。 （TGT-53150）
...
* **[!UICONTROL Browse] モードで戻るボタンを2回押すと、VECがクラッシュしました**: VECで[!UICONTROL Browse] モードを進んでいるお客様は、ブラウザーの戻るボタンを2回押すとクラッシュしました。 この問題により、エディターがフリーズし、ページの更新が必要になりました。 エディターがクラッシュすることなく確実に戻るナビゲーションを処理するようになりました。 （GT-53568）
...
* **スケジュールされたアクティビティが、ページの更新**&#x200B;まで[!UICONTROL Live] ステータスを正しく表示しない問題を修正しました。お客様は、アクティビティをスケジュールして将来の日時に公開すると、ステータスが[!UICONTROL Scheduled]ではなく[!UICONTROL Live]に直ちに表示されることを確認しました。 これにより、アクティビティがスケジュールされていることを確認メッセージが正しく示していたにもかかわらず、混乱が生じました。 ページを更新すると、ステータス表示が修正されました。 この問題は解決され、スケジュールされたアクティビティは、更新を必要とせずにスケジュールされたステータスを正しく表示するようになりました。 （TGT-52937）
...
* **お客様がアクティビティ作成プロセス中にレポートスイート名を入力できない問題を修正しました**: アクティビティ作成プロセス中にレポートソースとして[!DNL Adobe Analytics]を使用しているお客様が、特定のレポートスイートを検索するために[!UICONTROL Report Suite] ドロップダウンリストに入力できませんでした。 これにより、レポートスイートが多数ある組織で、手動スクロールで設定が大幅に遅れるワークフローに影響が及びます。 ドロップダウンリストはアルファベット順ではなく、入力された入力に一貫して応答しなかったため、「Office + Store - Web – 製品」などのレポートスイートを見つけるのが困難でした。 この問題は解決され、顧客はレポートスイート名を入力して効率的に検索できるようになりました。 （TGT-53345）
...
* **UIで[!UICONTROL Insert Before]または[!UICONTROL Insert After]を使用してエクスペリエンスフラグメントを挿入できない問題を修正しました**&#x200B;お客様は、更新されたUIで「前に挿入」または「後に挿入」オプションを使用してアクティビティに[!UICONTROL Experience Fragments]を挿入しようとしたときにエラーが発生しました。 表示されたエラーメッセージは、「オファーの内容には、1つのHTML要素が含まれている必要があります」です。 この問題は、更新されたUIに固有のもので、以前のバージョンでは発生しませんでした。 この問題は解決済みです。このエラーが発生することなく、お客様は[!UICONTROL Experience Fragments]を正常に挿入できます。 （TGT-53442）
...
* **保存後に[!UICONTROL Offer Decision] セレクターを変更できず、予期せず変更される問題を修正しました**：更新されたアクティビティ作成プロセスで、お客様は[!UICONTROL Offer Decision] セレクターを意図したとおりに変更できませんでした。 セレクターの変更が失敗し、保存後にセレクターが誤った値に戻りました。 この動作により、決定オファーがVisual Experience Composer （VEC）から消え、さらに編集がブロックされました。 セレクターの変更が正しく保存され、保存後もVECで決定オファーが表示され、編集可能な状態が維持されるようになりました。（TGT-53433）
* **アクティビティの保存後にアクティビティから[!UICONTROL Offer Decisions]が消えた問題を修正しました**: アクティビティ作成プロセス中に追加された[!UICONTROL Offer Decisions]が、アクティビティの保存と再開後に保持されませんでした。 この問題は、動的コンテンツを編集し、特定のセレクターとプロパティを使用して[!UICONTROL Offer Decisions]を挿入する際に発生しました。 [!UICONTROL Offer Decisions]は保存後も正しく保持され、セレクターは維持され、一貫したターゲティングと編集動作が保証されます。 （TGT-53434）
...
* **[!UICONTROL Catalog Search]**&#x200B;での画像の読み込みに一貫性がなかった問題を修正：[!UICONTROL  Catalog Search]のサムネールと画像がアクティビティ作成プロセスで一貫して読み込まれなかった問題を修正しました。 「サムネール URL」列が表示され、一部の製品画像がナビゲーションまたは検索アクションの後に部分的に読み込まれるか、まったく読み込まれない場合を除き、画像が表示されませんでした。 画像の読み込み動作が安定し、列の表示やナビゲーションアクションに関係なく、サムネールが確実に表示されるようになりました。 （TGT-52778）
...
* **完全な製品属性データを[!UICONTROL Recommendation Catalog]が断続的に表示できなかった問題を修正しました**：更新された[!DNL Recommendations] UIで、顧客は、データがフィードに存在していたにもかかわらず、メッセージなどの特定の製品属性が[!UICONTROL Catalog Search]結果に一貫して表示されない問題を経験しました。 この問題では、欠落している値を取得するために、列の表示を手動で再設定する必要がありました。 [!UICONTROL Catalog Search]は、設定されたすべての属性を確実に表示するようになり、手動で列をリセットする必要がなくなりました。 （TGT-52769）
* **ライブアクティビティで[!UICONTROL Front Promotion]を無効にできない問題を修正しました**: ライブアクティビティで[!UICONTROL Front Promotion]を無効にしようとする試みは保存されませんでした。 [!UICONTROL Change Promotion]を選択して無効にした後、アクティビティを再編集してもプロモーションはアクティブのままになり、レコメンデーション設定が更新されません。 プロモーション設定が正しく保存されるようになりました。これにより、お客様はライブアクティビティのプロモーションを期待どおりに無効または変更できるようになりました。 （TGT-53231）
* **データを含まない[!DNL Recommendations] [!UICONTROL Promotion]を有効にすると、不明確なエラーメッセージ**&#x200B;がトリガーされる問題を修正しました。必要な値を指定せずに[!DNL Recommendations] アクティビティで[!UICONTROL Front]または[!UICONTROL Back Promotion]を有効にすると、一般的な「無効な入力エラー」メッセージが表示されます。 根本的な問題は設定フィールドがありませんでしたが、エラーメッセージに原因が明確に示されていないため、トラブルシューティングが困難でした。 アクティビティ作成プロセスで、`collectionId`やルールなどの必須フィールドが見つからない場合に、明確で実用的なエラーメッセージが表示されるようになりました。これにより、お客様は設定の問題をすばやく特定して解決できます。 （TGT-52616）
* **[!UICONTROL Recommendations] タブ内の[!UICONTROL Edit] モーダルに[!UICONTROL Product] リストが表示されない問題を修正しました**：顧客が[!UICONTROL Recommendations] タブで[!UICONTROL collection]または[!UICONTROL exclusion]を編集する際に、フィルターされた製品リストを表示できませんでした。 リストは、適用されたルールに基づいてリアルタイムで更新されることを想定していましたが、意図したとおりに表示されませんでした。 この問題は解決され、ルールが変更されると、製品リストが正しく表示され、動的に更新されるようになりました。 （TGT-53481）
* **更新されたUIの詳細を表示ダイアログのレイアウトに関する問題を修正しました**：更新されたUIの詳細を表示モーダルのレイアウトが変更され、明瞭性と使いやすさが向上しました。 ダイアログには2つのタブが含まれるようになりました。
   * [!UICONTROL Details] タブ：選択した項目のすべての関連情報を表示します。
   * [!UICONTROL Inventory] タブ：現在のコレクションと除外ルールでフィルタリングされたすべての製品を表示します。

  この機能強化により、顧客は、アクティビティ作成プロセス内で、項目固有のデータと在庫のコンテキストをより簡単に操作し、理解できるようになります。 （TGT-53503）

   * **レコメンデーションアクティビティの削除されたプロモーションが保存後に再度表示される問題を修正しました**：顧客は、[!UICONTROL front]または[!UICONTROL back]のプロモーションが[!DNL Recommendations]のアクティビティから削除され、アクティビティが保存されたときに、プロモーションが再開時に引き続き表示されることを報告しました。 この問題は、ステージング環境と実稼動環境の両方で発生し、更新されたアクティビティ作成プロセスに影響を与えました。 問題が解決されました。 アクティビティから削除されたプロモーションが、保存後も正しく保持されるようになりました。 （TGT-53490）
...
* **[!UICONTROL Automated Segments] レポートにnull オーディエンス値が表示される問題を修正しました**：顧客は、アクティビティレポートの[!UICONTROL Automated Segments]でオーディエンスデータにnullが表示されていると報告し、セグメントパフォーマンスの正確な分析が妨げられました。 この問題は、有効なオーディエンスデータが想定されていたにもかかわらず、[!UICONTROL Reports] セクションにアクセスして[!UICONTROL Automated Segments]を選択したときに発生しました。 [!UICONTROL Automated Segments]がオーディエンス値を正しく表示し、信頼性の高いレポートとセグメント化インサイトを確保するようになりました。 （TGT-52793）
...
* **更新されたUIで[!UICONTROL Browse]と[!UICONTROL Design]のモードを切り替えるとSPAの状態がリセットされる問題を修正しました**：更新されたUIで[!UICONTROL Browse]と[!UICONTROL Design]のモードを切り替えると、web エディターがリロードされ、SPAの状態がリセットされると顧客から報告されました。 この問題により、ワークフローが中断され、顧客は情報を再入力する必要がありました。 問題が解決されました。 モードを切り替えると、SPA状態が保持されるようになり、アクティビティの作成中に、よりスムーズで一貫性のあるエクスペリエンスを実現できるようになりました。 （TGT-53074）
...
* **AP アクティビティの[!UICONTROL Targeting] ステップへの進行をブロックするアクティビティ作成プロセスの問題を修正**：顧客が[!UICONTROL Automated Personalization] （AP）アクティビティの[!UICONTROL Targeting] ステップに進めないアクティビティ作成プロセスの問題を修正しました。2つの場所が追加されました。 この動作は、複数のオファーを含む単一の場所で十分だった以前のエクスペリエンスとは異なっていました。 お客様がAP ワークフローの一部として単一の場所の設定を引き続き使用できるようにするための要件が修正されました。 （TGT-53426）
...
* **更新されたUIでレポートスイートを検索できない問題を修正しました**：更新されたUIでは、[!UICONTROL Goals & Settings] セクションの[!UICONTROL Report Suites] ドロップダウンリストで、顧客が入力および検索できず、特定のレポートスイートを見つけることが困難でした。特に、エントリ数が多いテナントの場合は。 以前のUIとは異なり、リストは並べ替えられておらず、入力ベースのフィルタリングが不足していました。 この問題は解決されました。 お客様は、レポートスイートを検索およびフィルタリングして、従来のUIで使用できる機能を復元できるようになりました。 （TGT-53514）
...
* **単一のワークスペースに制限されているお客様が他のワークスペースからアクティビティを表示できる問題を修正しました**: アクセスが1つのワークスペースに制限されているお客様が、アクティビティ作成プロセスで[!UICONTROL All Workspaces]を選択すると、すべてのワークスペースでアクティビティを予期せず表示できるようになりました。 この可視性により、他のワークスペースでのライブアクティビティに意図しない変更が加わるリスクがあり、web サイトのパフォーマンスに影響を与える可能性がありました。 Workspaceのアクセス制御が強化され、割り当てられたワークスペース内のアクティビティのみを表示および操作できるようになりました。 （TGT-53101）
* **お客様がアクセス権を持たずに[!UICONTROL Default Workspace]からアクティビティを表示できる問題を修正しました：** ステージングワークスペースに制限されたアクセス権を持つお客様が、アクティビティ作成プロセスを介して[!UICONTROL Default Workspace]からアクティビティを表示できました。 この動作は、バックエンドの設定とアクセス権が正しいにもかかわらず発生しました。 Workspace アクセス制御が強化され、割り当てられたワークスペース内でのみアクティビティを表示できるようになりました（TGT-53297）。
...
* **更新されたUIでのAP アクティビティのアクティビティ作成プロセスにおけるエクスペリエンス要件の問題を修正**: アクティビティ作成プロセスで、更新されたUIで2つのエクスペリエンスではなく2つの場所が[!UICONTROL Automated Personalization] （AP）アクティビティで必要になる問題を修正しました。 この動作により、顧客が複数のオファーを含む単一の場所を設定するユースケースがブロックされました。これは以前サポートされていました。 元の機能に合わせて要件が修正され、お客様は場所の数に関係なく2つのエクスペリエンスを使用してAP アクティビティを続行できるようになりました。 （TGT-53429）
* **クリックトラックモードでのトラッキングされた要素フィールドの動作を修正して、未保存の入力を防ぎ、明瞭性を向上しました**: アクティビティ作成プロセスで、[!DNL Click Track] モードの[!UICONTROL Tracked Element] フィールドは編集可能でしたが、入力された名前が保持されず、顧客に混乱が生じる問題を修正しました。 未保存の入力を防ぐためにフィールドが無効になり、選択した要素の名前が明確になり、目標設定とトラッキング精度が向上しました**（TGT-53458）。
* **アクティビティ作成プロセスで、[!UICONTROL Click Track] モードで追跡されたコンポーネントの名前付けをブロックする問題を修正**：顧客が[!UICONTROL Click Track] モードで追跡されたコンポーネントに名前を付けることができなかったアクティビティ作成プロセスの問題を修正しました。 名前を入力した後、フィールドは編集可能に見えましたが、入力は保持されず、編集モードで「MY プライマリ GOAL 0」などの汎用ラベルがデフォルトで使用されます。 トラッキングされた要素フィールドが無効になり、目標の設定とトラッキング精度を向上させるために、名前付けの動作が明確になりました。 （TGT-51396）
...
* **高度な検索で提案を提供できなかったカタログ検索UIの問題を修正しました**: [!UICONTROL Advanced Search]機能で提案を提供できなかった新しい[!UICONTROL Catalog Search] UIの問題を修正しました。 ユーザーは、正確なスペルを入力して正確な値を入力する必要があり、検索が煩雑になり、ミスが発生しやすくなりました。 この修正により、[!UICONTROL Advanced Search]は、ユーザーが入力する際に関連する提案を提供するようになり、使いやすさが向上し、製品を探す際の摩擦が軽減されました。 （TGT-52008）
...
* **商品メッセージをレコメンデーションビューに表示できなかったアクティビティ作成プロセスの問題を修正しました**: メッセージが存在していたにもかかわらず、[!DNL Recommendations] ビューの[!UICONTROL Message]列に商品データが表示されなかったアクティビティ作成プロセスの問題を修正しました。 顧客は、コンテンツを一時的に表示するために、列を手動で削除したり再追加したりしなければならず、スクロールや検索時に再度表示されがちでした。 このアップデートにより、製品メッセージの一貫性のある可視性が復元され、カタログのナビゲーションとレビューのワークフローが改善されます。 （TGT-52777）
...
* **[!UICONTROL Last Updated]列による整列の一貫性のない並べ替えを修正：** [!UICONTROL Last updated]列によるカタログの並べ替えで結果に一貫性が生じる、アクティビティ作成プロセスの問題を修正しました。 顧客は、更新タイムスタンプに基づいて商品データを確実に整理できず、カタログのレビューと管理がより困難になりました。 並べ替えが期待どおりに機能するようになり、更新されたUIの精度と使いやすさが向上しました。 （TGT-53116）
...
* **アクティビティの読み込みを修正し、アクティビティ作成プロセスの[!UICONTROL Cancel] ボタンの問題**：特定のアクティビティの読み込みに失敗し、お客様が変更にアクセスできなくなったアクティビティ作成プロセスの問題を修正しました。 さらに、[!UICONTROL Cancel] ボタンが応答しなかったため、お客様は読み込みプロセスを停止したり、編集ビューを終了したりすることができませんでした。 この修正により、アクティビティが確実に読み込まれ、[!UICONTROL Cancel] ボタンが期待どおりに機能するようになり、Visual Experience Composerの全体的な安定性とユーザーエクスペリエンスが向上します。 （VEC）（TGT-53218）
* **更新されたVEC UIでエクスペリエンスの切り替えの問題を修正し、編集がブロックされ、[!UICONTROL Cancel] ボタン**&#x200B;が無効になりました。更新されたVEC UIで、エクスペリエンスのターゲット設定（XT）アクティビティでエクスペリエンスを切り替えるときに変更が読み込まれなかった問題を修正しました。 顧客は、最初に入力したエクスペリエンスを超えてエクスペリエンスを編集できず、[!UICONTROL Cancel] ボタンが見つからないか、応答しませんでした。 この修正により、すべてのエクスペリエンスで変更が正しく読み込まれるようになり、ヘルパー拡張機能がなくても[!UICONTROL Cancel] ボタンが確実に機能するようになり、編集ワークフローが改善され、フラストレーションが軽減されます。 （TGT-53256）
...
* **編集セッション間でカスタムコードの変更が確実に保存されないVECの問題を修正**:Visual Experience Composer （VEC）で行われたカスタムコードの変更が1回の試行で確実に保存されない問題を修正しました。 お客様は、[!UICONTROL Edit Content] ボタンと最後の[!UICONTROL Save] ボタンの両方を使用した後でも、スタイルの更新やHTMLの編集などの変更が、web サイトおよびQA URLから断続的に見つからないと報告しました。 このリグレッションが解決され、すべてのカスタムコードの変更が編集セッション間で期待どおりに保持されるようになりました**（TGT-53418）。
...
* 更新された[!DNL Target] UIで、[!UICONTROL Editor]の役割を持つユーザーがアクセスし、制限する必要があるライブアクティビティの編集を試みることができた問題を修正しました。 アクティビティリストと概要画面に、ライブアクティビティの編集オプションが誤って表示され、意図しない変更が発生する可能性があります。 （TGT-53055）
* [!UICONTROL Advanced Search] UIで「値が存在する」または「値が存在しない」演算子を選択すると、ユーザーにオペランドの入力を求めるメッセージが表示される問題を修正しました。このオペランドは、これらの条件に必須ではありません。 （TGT-51961）
...
* コンテンツ配信で正しく動作しているにもかかわらず、エクスペリエンスフラグメント（XF）コンテンツが[!UICONTROL Visual Experience Composer] （VEC）プレビューでレンダリングできない問題を修正しました。 （TGT-53318）
...
* 更新された[!DNL Target] UIで、[!UICONTROL Targeting] タブから[!UICONTROL Experiences]に戻った後、[!UICONTROL Visual Experience Composer] （VEC）で行われたHTML オファーの変更が保持されない問題を修正しました。 （TGT-52874）
* 更新された[!DNL Target] UIで、同じセレクターの前後に1つのHTML オファーを挿入すると、場所が正しく生成されない問題を修正しました。 顧客が[!UICONTROL Targeting] タブから[!UICONTROL Experience] タブに戻ったとき、1つのセレクターのみが保持され、変更が失われ、コンテンツ配信が壊れました。 この動作は、従来のUIとは異なり、場所IDが異なる両方の変更を正しく保持しました。 （TGT-52891）
* 更新された[!DNL Target] UIで、[!UICONTROL Modifications] パネル内で追加されたオファーをクリックすると、右側の[!UICONTROL Configuration] パネルが断続的に表示および非表示になり、オファー設定を操作するのが困難になる問題を修正しました。 （TGT-53288）
...
* 更新されたVECで、お客様が変更を[!UICONTROL Single Page Application] （SPA）ビューに適用できない問題を修正しました。 古いUIで作成されたアクティビティではビュー固有のターゲティングがサポートされていましたが、新しいUIでは、ビューの切り替え制御が無効になっているので、これらのビューの検出または編集を許可できませんでした。 （TGT-52556）
...
* 更新されたVECで、[!UICONTROL Manage Content] セクションがエクスペリエンスのサムネール用の代替テキストを表示できなかった問題を修正しました。 この問題により、特にファイル名が長い場合や切り捨てられている場合は、ユーザーがドキュメントを識別することが困難でした。 （TGT-52291）
* VEC アクティビティで[!UICONTROL page delivery] ルールを設定する際に、お客様が誤った検証エラーを検出するエラーを修正しました。 具体的には、テキストをサポートする必要がある場合でも、テキスト値を入力する際に「どの演算子にも等しい」や「どの演算子にも等しくない」などの演算子が「演算子タイプに対して無効な入力」をトリガーします。 （TGT-52629）
* 「オファーを選択」ボタンを使用してオファーを選択する際に、更新されたUIの[!UICONTROL Modifications] パネルで一貫性のない動作が発生する問題を修正しました。 既存のオファーを置き換える代わりに、UIは重複を追加し、いずれかのオファーと対話しようとすると、`selectorNotFound`などのコンソールエラーが発生しました。 さらに、一部のオファーでは「オファーを選択」ボタンが表示されず、編集可能なプロパティのみが表示されます。 （TGT-53321）
...
* 更新された[!DNL Target] UIのワークスペース間でアクティビティをコピーする際の複数の問題を修正しました。 ユーザーがソース ワークスペースと宛先ワークスペースの両方で「[!UICONTROL Approver]」役割を持っている場合でも、特にライブアクティビティでのオーディエンスアクセスに関連するエラーと、誤った権限検証が発生しました。 （TGT-53002）
...
* [!UICONTROL Create Criteria] モーダルのローカライズの問題を修正しました。[!UICONTROL Choose Price Rule] ドロップダウンリストで「次の値の間」オプションを選択すると、[!UICONTROL Inclusion Rules] セクションで「to」という文字列がローカライズされませんでした。 （TGT-49754）
* フィード作成ウィザードの[!UICONTROL Environment] ドロップダウンリストにある文字列「[!UICONTROL All host groups]」のローカライズの問題が正しくローカライズされない問題を修正しました。 （TGT-46737）
...
* QA環境でアクティビティを作成できない問題を修正しました。 プロセスは正常に完了する代わりに[!UICONTROL Activities] ページにリダイレクトされました。 （TGT-53328）
...
* 更新されたUIの[!UICONTROL Catalog Search]のフィルター候補が不正確だった問題を修正しました。 （TGT-52007）
...
* ユーザーが[!UICONTROL Manage Content]をクリックし、Automated Personalization （AP） アクティビティの編集中に[!UICONTROL Done]をクリックすると、ページが空白になり、応答しなくなる問題を修正しました。 （TGT-53047 および TGT-52993）
* [!UICONTROL Goals & Settings]の下の[!UICONTROL Viewed an mbox]変換指標を選択すると、ページがクラッシュする問題を修正しました。 （TGT-53346、TGT-53343、TGT-53348）
* [!UICONTROL Redirect to URL]機能が期待どおりに機能しない問題を修正しました。有効なURLでもリダイレクトが発生しません。 （TGT-53307）
...
* ワークスペースが「[!UICONTROL All Workspaces]」に設定されている場合に、既にデフォルトワークスペースに存在するアクティビティをコピーすると、誤ったエラーが発生する問題を修正しました。
...
* 更新された[!DNL Target] UIで、[!UICONTROL Automated Personalization] （AP）アクティビティで削除されたオファーが、元の名前ではなく`Deleted option with ID: X`として表示される問題を修正しました（例：従来のUIに表示される`Offer Name [Deleted]`）。 この修正により、削除されたオファーに対する意味のあるラベル付けが復元され、明確性が向上し、レポートをより正確でユーザーフレンドリーにすることができます。 （TGT-52921）
...
* 変更を[!DNL Page Load]から[!UICONTROL View]に移動すると、変更が重複し、[!UICONTROL Page Load]に残りながら[!UICONTROL View]にも表示される問題を修正しました。 さらに、[!UICONTROL View]から変更を削除すると、[!UICONTROL Page Load]からも誤って削除されます。 （TGT-53270）
...
* [!UICONTROL Automated Personalization] （AP）アクティビティの作成時または編集時に&#x200B;**[!UICONTROL Manage Content]** アイコン （![ コンテンツ管理アイコン ](/help/main/assets/icons/Experience.svg)）をクリックすると、エディターがクラッシュする原因となる[!UICONTROL Form-Based Experience Composer]の問題を修正しました。 （TGT-53047）
...
* リストの下部にスクロールして、従来のUIと一致する動作を復元すると、[!UICONTROL Catalog Search]が追加の結果を読み込めない問題を修正しました。 （TGT-53088）
* [!UICONTROL Criteria Details] ダイアログボックスからのアイテムの削除をブロックする問題を修正しました。 （TGT-53245）
* 名前のない製品を開いたり、操作したりできない問題を修正しました。 この問題は、名前のない結果を返す環境を選択する際に発生し、製品の詳細にアクセスできなくなります。 （TGT-53007）
* 特定の製品を選択すると、[!UICONTROL Catalog Search] ページがクラッシュし、空白の画面が表示される問題を修正しました。 （TGT-53087）
* ユーザーが[!DNL Target] UIでsite_cart_z1 [!DNL Recommendation] アクティビティを編集できない問題を修正しました。 アクティビティを開こうとすると、[!UICONTROL Overview] ページでエラーが発生し、エディターへのアクセスがブロックされました。 （TGT-53221）
...
* VECでイメージオファーを設定する際の[!UICONTROL Insert Before]および[!UICONTROL Insert After] オプションの[!UICONTROL Undo]機能に関する問題を修正しました。

  以前は、画像オファーに対して[!UICONTROL Insert Before]または[!UICONTROL Insert After] アクションを元に戻すと、一貫性のない動作が発生したり、修正を正しく元に戻すことができませんでした。特に、従来の[!DNL Target] UIで作成されたアクティビティでは、この問題が発生していました。 この問題は、元に戻すアクションがこれらの変更に対して確実に機能するように解決されました。 （TGT-52809）
...
* レガシー機能を[!UICONTROL Goals & Settings]の[!UICONTROL Priority] スライダーに復元し、お客様がレガシーUIでサポートされているように、数値を直接入力できるようにしました。 （TGT-53185 および TGT-53219）
...
* 顧客が[!UICONTROL Goals & Settings] ページで特定のアクティビティに関するレポートを表示した際に、[!UICONTROL View in Analytics] リンクが実稼働環境ではなくQA環境を誤ってポイントする問題を修正しました。 （TGT-53163）
...
* アクティビティ [!UICONTROL Overview] ページでメトリック名が見つからず、正しいメトリック名ではなく「オファー」のみが表示される問題を修正しました。 （TGT-53054）
...
* アクティビティの保存を妨げ、「未定義のプロパティを読み取れません（「マップ」を読み取っています）」というエラーメッセージをトリガーする[!UICONTROL Form-Based Experience Composer]の問題を修正しました。 （TGT-53145）
...
* [!UICONTROL Catalog Search]から製品をクリックすると、「製品の詳細を取得できませんでした」というエラーが表示され、閉じるオプションなしでモーダルが開く問題を修正しました。 （TGT-53082）
* コレクションまたはプロモーションを変更する際に、[!UICONTROL A/B Test] アクティビティの[ オファー](/help/main/c-recommendations/recommendations-as-an-offer.md)のレコメンデーションが正しく更新されない問題を修正しました。 （TGT-52884）
...
* 複数のエクスペリエンスで使用されているセレクターが選択されたように常に強調表示されない[!UICONTROL Goals & Settings] ページの問題を解決しました。 （TGT-53062）
...
* ワークスペース間でプロパティを変更する際の問題を修正しました。
   ...
   * [!UICONTROL Properties] リストに警告が表示され（一部のプロパティが互換性がないことを示している可能性があります）、顧客が[!UICONTROL Add]または[!UICONTROL Remove]をクリックしてから[!UICONTROL Save]をクリックすると、宛先ワークスペースにないプロパティがすべて削除されます。 顧客が[!UICONTROL Cancel]をクリックすると、宛先ワークスペースにプロパティが存在しない場合でも、すべてのプロパティが残ります。 （TGT-47094）
   * 同じワークスペースに留まる場合、またはデフォルト以外のワークスペースからデフォルトまたは別のワークスペースに切り替える場合、すべてがそのまま残ります。 （TGT-53078）
* アクティビティの元のワークスペースコンテキストを尊重するように、エンティティ検証ロジックを更新しました。 [!UICONTROL Experience Fragments] （XF）などのエンティティは、アクティビティが最初に作成されたワークスペースに基づいて検証されるようになりました。 例えば、XFがデフォルトのワークスペースに存在し、アクティビティがワークスペース Xからワークスペース Yにコピーされる場合、XFが元の（デフォルトの）ワークスペースで有効である限り、検証は引き続き渡されます。 （TGT-53196）
...
* [!UICONTROL Activity QA] URLに不要なクエリパラメーターが含まれている問題を修正しました：`at_preview_evaluate_as_true_audience_ids`。 （TGT-52907）
...
* トラフィック配分の設定中に[!UICONTROL Auto-Allocate] （AA）が最初に選択された場合に、ユーザーが[!UICONTROL Auto-Target] （AT）アクティビティを作成できない問題を修正しました。 この問題により、バックエンドの検証エラーが発生し、アクティビティが保存されなくなります。 （TGT-53096）
...
* [!UICONTROL Approver]の役割を持つユーザーがアクティビティのみのオーディエンスの絞り込みを追加または保存できない問題を修正しました。 これを試みると、ユーザーがアクティビティの承認と管理に十分な権限を持っていても、「[editor]」権限が必要であると記載された403 Forbidden エラーが発生しました。 （TGT-52984）
* アクティビティ固有のオーディエンスが[!UICONTROL Remove Audience Refinement] オプションを使用して削除されると、同じアクティビティ内で再選択するためにオーディエンスがオーディエンスリストに表示されなくなるという問題を修正しました。 この動作により、最初から再作成しない限り、ユーザーは同じオーディエンスを再追加できません。 （TGT-52979）
...
* アクティビティに[!UICONTROL All Visitors]以外のオーディエンスを割り当てようとした場合の問題を修正しました。 保存すると、次のエラーメッセージが表示されます。「リクエストを完了できません。 問題が解決しない場合は、[!UICONTROL Adobe Client Care]に連絡してください。」 （TGT-53008）
* アクティビティエディター内で新しいオーディエンスを作成して割り当てた後、アクティビティの保存がブロックされる問題を修正しました。 表示されたエラーメッセージは「リクエストを完了できません。 問題が解決しない場合は、[!UICONTROL Adobe Client Care]に連絡してください。」 （TGT-52977）
...
* [!UICONTROL Goals & Settings] ステップでレポートソースとして[!DNL Adobe Analytics] （A4T）を選択すると、顧客がアクティビティの作成または保存をブロックする問題を修正しました。 この問題は、特に[!UICONTROL Custom Event]指標（例：「カスタムイベント 16」）を選択する際に発生し、「無効なユーザー入力」というエラーが発生しました。 （TGT-52910）
* 「[!UICONTROL View in Analytics]」リンクをクリックすると、ユーザーが意図した[!DNL Analytics] ダッシュボードではなくホームページにリダイレクトされる問題を修正しました。 （TGT-53092 および TGT-53093）
...
* 更新された[!UICONTROL Overview] UIで[!DNL Recommendations] アクティビティを表示する際の問題を修正しました。[!DNL Adobe Analytics] （A4T）がレポートソースとして選択されていると、[!UICONTROL Goals & Settings] セクションの読み込みに失敗します。 次のエラーメッセージが表示されました：「問題が発生しました。 ご要望の処理は完了できません。 問題が解決しない場合は、アドビのカスタマーケアにお問い合わせください。」 （TGT-52999）
...
* すべてのコンテンツオプションを削除した後、[!UICONTROL Manage Content]機能が場所の状態を正しく検証しなかった問題を修正しました。 この問題は、アクティビティ設定を保存または続行しようとすると、動作の一貫性が失われたり、エラーが発生したりする可能性があります。 （TGT-52801）
...
* [!UICONTROL Targeting] ステップから[!UICONTROL Experiences]に戻る際に、HTML オファーを使用して行った変更が見つからない可能性があるお客様への対応を明確にしました。 この顧客の場合、影響を受けるweb サイトでは、ページの読み込みごとに変更される複数のDOM セレクターが動的に生成されました。 その結果、最初に修正に使用されたセレクターは、エディターを再度開いたときに見つからず、修正が見つからないか無効に見えます。 このシナリオは計画通りに機能しています。 エディターで変更が引き続き視覚的に表示されるようにするには、クライアントが安定した一貫性のあるセレクターを使用し、ページのリロード時に変更しないことをお勧めします。 （TGT-52874）
...
* 新しい[!DNL Recommendations] フィード [status](/help/main/c-recommendations/c-products/feeds.md#status)を追加しました：[!UICONTROL Partial Import Failed]。 （KB-2215）
* [!DNL Recommendations]を[!UICONTROL promotions]に追加する際のアクティビティ作成ワークフローに影響する問題を修正しました。 ユーザーが「[!UICONTROL Promote by Attribute]」を選択してフィルタリングルール （例：[!UICONTROL Parameter Matching]）を追加した場合、選択したルールタイプとオペランド値は、アクティビティの保存と再編集後に保持されませんでした。 再び開くと、フィルタリングルールタイプが予期せず変更され、オペランド値が欠落します。 （TGT-53059）
* ルールのロジックに関係なく、単一のルールで作成されたプロモーションが誤って解釈され、「アイテムのリスト」プロモーションタイプとして表示される[!DNL Recommendations] UIの問題を修正しました。 （TGT-53063）
* 更新された[!UICONTROL Overview]UIを使用する際に、[!DNL Recommendations]を含む[!UICONTROL Experience Targeting] （XT）アクティビティに「[!UICONTROL Download Recommendations Data]」ボタンが表示されない問題を修正しました。 （TGT-52730 および TGT-52756）
...
* レコメンデーションを使用してフォームベースのA/B アクティビティで&quot;[!UICONTROL Promote by attribute]&quot; プロモーションを設定する際に、顧客がフィルタリングルールを保存できない問題を修正しました。 アクティビティを保存して再度開くと、フィルタリングルールが見つからず、アクティビティを正常に保存できませんでした。 （TGT-53057）
...
* [!UICONTROL Reports] ページから「[!UICONTROL Export order details to CSV]」を選択すると、空のファイルがダウンロードされる問題を修正しました。 この問題は、有効な注文データがアクティビティに存在する場合でも発生しました。 （TGT-52225）
...
* カスタムコードを編集して保存すると、[!UICONTROL Modifications] パネルが応答しなくなる問題を修正しました。 （TGT-53075）
* バリアントエクスペリエンスでカスタムコードに対して行われた変更が、意図せず[!UICONTROL Control] エクスペリエンスに反映される問題を修正しました。 これにより、配信動作が意図しない変更になりました。 [!UICONTROL Control] エクスペリエンスは、他のエクスペリエンスに対して行われたカスタムコード編集から分離されたままになります。 （TGT-52413）
...
* アクティビティ作成の[!UICONTROL Modifications] ステップで行われた変更が一貫して保存されない問題を修正しました。 場合によっては、すべての手順を完了して[!UICONTROL Save]をクリックした後、[!UICONTROL Modifications] セクションに追加されたカスタムスクリプトが、保存処理中に表示されるエラーがないにもかかわらず、ライブサイトに反映されないことがあります。 （TGT-52661）
...
* [!UICONTROL Browse Mode]でユーザーが別のURLを参照できない問題を修正しました。 これにより、テスターとエディターは、同じアクティビティセッション内で代替ページを検証またはプレビューできなくなりました。 （TGT-53052）
* アクティビティの作成中に複数の[!UICONTROL Visual Experience Composer] （VEC） インスタンスが同時に開く問題を修正しました。 この問題は、ユーザーが[!UICONTROL Enhanced Experience Composer] （EEC）を無効にし、[!UICONTROL Page Delivery] ステップでweb サイト URLから末尾のスラッシュを削除した場合に発生しました。 （TGT-52782）
* ユーザーが別の指標を選択した後でも、[!UICONTROL Goals & Settings]手順の[!UICONTROL Revenue]指標ドロップダウンが[!UICONTROL Revenue per Visit] （RPVISIT）に間違ってデフォルト設定される問題を修正しました。  この問題は、指標の設定パネルを折りたたんで再展開すると発生し、以前に選択した値がリセットされます。 （TGT-52811 および TGT-52878）
* [!UICONTROL Automated Personalization] （AP）および[!UICONTROL Multivariate Testing] （MVT）アクティビティでのオファーの命名とコンテンツの翻訳に関連するアクティビティ作成ワークフローのいくつかの問題を修正しました。

  対処された主な課題：

   * 同じ名前で複数のHTML オファーを作成する（例えば、「Experience」）際に「オファー名の重複は許可されていません」というエラーが発生しましたが、UIは、競合の原因となっているオファーを明確に示しませんでした。
   * 右側のパネルを使用してオファーの名前を変更すると、UIの名前が更新されましたが、変更は[!UICONTROL Manage Content] タブまたは[!UICONTROL Offers] タブに反映されず、永続的な検証エラーが発生しました。
...
* [!UICONTROL A/B Test] アクティビティでバリアント エクスペリエンス （エクスペリエンス Bなど）に対して行われた変更が保持されない問題を修正しました。 エクスペリエンスを切り替えると、バリエーションの変更は消えます。 この問題は、コントロールのエクスペリエンスには影響しません。 （TGT-52664）
* 特定の顧客がアクティビティを作成または保存できず、他の顧客が同じアクションを問題なく実行できる問題を修正しました。 問題がアカウント間で一貫性を失いました。（TGT-52842）
* 更新されたVECで、ユーザーが修正を[!UICONTROL Page Load event]に移動できなかった問題を修正しました。これは、従来のUIに存在していた機能です。 （TGT-52617）
* 更新されたUIで、変更を作成する際に[!UICONTROL page load] イベントが[!DNL Target]に表示されない問題を修正しました。更新はビューにのみ適用されます。 （TGT-52604）
...
* 更新されたVECで変更を適用する際に、最初に変更が正しく表示され、予想される[!UICONTROL Experience Fragment]が含まれる問題を修正しました。 ただし、エクスペリエンスを切り替えたり、追加の編集を行ったりすると、セレクターの問題により、一部の変更が適用されない場合があります。 （TGT-52679）
* 既存のアクティビティを複製して新しいアクティビティを作成すると、複製されたアクティビティ内のQA リンクが元のアクティビティのページ URLを誤って保持する問題を修正しました。 （TGT-52775）
* 更新されたVECで[!UICONTROL On-device Decisioning]が利用できなくなるという問題を修正しました。 （TGT-52371）
* 製品[!DNL Recommendations] アクティビティを編集できない問題を修正しました。 Target UIを介してVECにアクセスしようとすると、[!UICONTROL Overview] ページにエラーが表示され、編集が妨げられました。 （TGT-52823）
...
* [!UICONTROL Editor]の役割を持つユーザーが[!DNL Recommendations] アクティビティを編集できない問題を修正しました。 デザインを変更してアクティビティを保存しようとすると、ユーザーが関連するワークスペースに既にその役割を持っていても、「[editor]」権限が必要であると記載された403 Forbidden エラーが発生しました。 （TGT-52836）
...
* 特定のアクティビティに対して[!UICONTROL Automated Segments]と[!UICONTROL Important Attributes]のレポートが生成されない問題を修正しました。 （TGT-52904）
* 更新されたVECで、[!UICONTROL Automated Personalization] （AP）アクティビティのデフォルトのコンテンツ処理が従来のUIと一致しない問題を修正しました。 グループが明示的に追加されていない場合、システムはデフォルトの`optionGroup`を`optionGroupLocalId = 0`に自動的に追加するようになりました。 このグループには、既定のオプション （例：`optionLocalId: 0`）が含まれています。 デフォルトのコンテンツが削除されると、対応するオプショングループも削除されます。 （TGT-52651）
* 以前に削除したエクスペリエンスから`experienceLocalId`を再利用できない[!UICONTROL Multivariate Test] （MVT）アクティビティの問題を修正しました。 （TGT-52672）
...
* [!DNL Recommendations] デザインを作成できない問題を修正しました。 [!UICONTROL Create]をクリックすると、「スクリプト内で少なくとも1つのエンティティ変数を使用する必要があります」というメッセージがトリガーされました。 （TGT-52395 および TGT-52899）
...
* [!UICONTROL Form-Based Experience Composer]の問題を修正しました。[!UICONTROL Location] ドロップダウンの特殊文字を含むmboxにカーソルを合わせると、エディターが空白になり、「要素」で「querySelector」を実行できませんでした。 エラーが表示されます。 （TGT-52717）
...
* [!UICONTROL Page Delivery]の「[!UICONTROL URL - does not contain]」ルールが機能しない問題を修正し、ブロックする必要がある場合でもコンテンツを表示できるようにしました。 （TGT-52754）
* [!UICONTROL Page Delivery]がエラーメッセージを誤って表示する問題を修正しました：「重複したページ URLは許可されていません。 （TGT-52765）
* エクスペリエンスフラグメントを含む[!UICONTROL Page Delivery] URLのオーディエンスが#で誤って追加されて作成される問題を修正しました。 （TGT-52786）
* [!UICONTROL Goals and Settings] ページでアクティビティをコピーして設定を編集すると、[!DNL Target] UIが応答しなくなる問題を修正しました。 （TGT-52797）
* 更新された[!UICONTROL Visual Experience Composer] （VEC）で、[!UICONTROL A/B Test] アクティビティの追加ページを同じURLにリダイレクトすることが誤って許可されていた問題を修正しました。 （TGT-51838）
* アクティビティの編集時に[!UICONTROL Goals and Settings] ページの指標に対する変更が保存されない問題を修正しました。 （TGT-52799）
...
* 更新された[!UICONTROL Visual Experience Composer] （VEC）で、`triggerView()`件の呼び出しが検出されない問題を修正しました。 （TGT-52575）
* 更新された[!UICONTROL Visual Experience Composer] （VEC）で、ユーザーが[!UICONTROL Single Page Application] （SPA）ビューに変更を追加できない問題を修正しました。 （TGT-52556）
* 更新された[!DNL Target] UIで、顧客がオファーの詳細を表示できない問題を修正しました。 （TGT-52607）
* [!UICONTROL Offers Library]でオファーに対して行われた更新が、更新された[!UICONTROL Visual Experience Composer] （VEC）に反映されない問題を修正しました。 （TGT-52637）
...
* [!UICONTROL Activity QA] ボタンを持たないアクティビティで空のオプションセレクターが表示される問題を修正しました。 （TGT-52733）
...
* UIで、環境IDを条件に手動で割り当てることができなかった問題を修正しました。 代わりに、[!UICONTROL Product Catalog Search] ホストグループのIDにデフォルトで設定されます。 この修正により、デフォルトだけでなく、すべての環境で条件変更が適用されるようになりました。 （TGT-52817）
* レコメンデーションを含む[!UICONTROL Experience Targeting] （XT）アクティビティで「[!UICONTROL Download Recommendations data]」オプションが見つからない問題を修正しました。 （TGT-52730 および TGT-52756）
...
* 内部web ページの読み込みに失敗した場合に、[!UICONTROL Visual Experience Composer] （VEC）で変更にアクセスできない問題を修正しました。 （TGT-52488 &amp;TGT-52470）
* VECの小さい画面サイズで[!UICONTROL Modifications] パネルが表示されない問題を修正しました。 （TGT-52470）
* 更新されたVECで、[!UICONTROL Browse] モードから[!UICONTROL Design] モードに切り替えるとコンソールエラーが発生し、それ以上の操作が妨げられる問題を修正しました。 （TGT-52532）
...
* [!UICONTROL Automated Personalization] （AP） アクティビティで顧客がオファーを削除または削除できない問題を修正しました。 （TGT-52690）
* 複数ページのアクティビティでアクティビティの選定の動作に一貫性がない問題を修正しました。 （TGT-52694）
* アクティビティの[!UICONTROL Overview] ページで[!UICONTROL Activity Location]の無効なURLが表示される問題を修正しました。 （TGT-52695）
...
* アクティビティに[!UICONTROL Experience Fragments]またはHTML オファーを追加する際に無効なペイロードエラーが発生する問題を修正しました。 （TGT-52779 および TGT-52773）
* 更新された[!DNL Target] UIで、無効な入力エラーが原因でE[!UICONTROL xperience Fragments]が正しく表示できなかった問題を修正しました。 （TGT-52701）
* 無効なユーザーエラーが原因で、顧客が[!UICONTROL Form-based Experience Composer]のアクティビティを編集できない問題を修正しました。 （TGT-52470）
...
* レコメンデーションを含む [!UICONTROL A/B Test] および [!UICONTROL Experience Targeting]（XT）アクティビティで「500 エラー」が発生する問題を修正しました。 この問題は、[!DNL Target] が [!DNL Target] UI や [!DNL Recommendations] バックエンドから使用されていない条件オブジェクトを削除できなかったことが原因です。 （TGT-44383）
...
* お客様がレコメンデーションを含む [!UICONTROL A/B Test] および [!UICONTROL Experience Targeting]（XT）アクティビティで、[!DNL Recommendations] オブジェクト（デザイン、条件、コレクションなど）の使用状況を適切に表示し、また [!DNL Target] UI および [!DNL Recommendations] バックエンドから使用しなくなった条件オブジェクトを削除できるように問題を解決しました。 （TGT-42331）
...
| 機能 | 詳細 |
| --- | --- |
| [!UICONTROL Analytics for Target]（A4T）でのリダイレクトのサポート | [A4T](/help/main/c-integrating-target-with-mac/a4t/a4t.md).<br>詳細については、[Analytics for [!DNL Target] implementation](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)を参照してください。[!DNL Target] リダイレクトがサポートされるようになりました。 |
...
* 9月15日（PDT）から10月6日（PDT）の9:25午前（PDT）までの[!DNL Adobe Target Premium] ユーザーの[!UICONTROL Auto-Target]のレポートに影響を与えた問題を修正しました。 :30影響を受けたコンバージョン指標のレポート（「[!UICONTROL Viewed a page]」または「[!UICONTROL Clicked on mbox]」オプションを使用して設定）を表示すると、コンバージョン率が誤ってレポートされます。 現時点では、配信に関する既知の問題はありません。
...
| 機能 | 詳細 |
| --- | --- |
| ![Premium バッジ](/help/main/assets/premium.png)自動ターゲットアクティビティに対する Analytics for Target（A4T）のサポート | [!UICONTROL Auto-Target]のアクティビティで、Target](/help/main/c-integrating-target-with-mac/a4t/a4t.md)の[Analyticsがサポートされるようになりました。<br>この統合により、[!UICONTROL Auto-Target]のアンサンブルマシンラーニングアルゴリズムを使用して、訪問者のプロファイル、行動、コンテキストに基づいて、各訪問者に最適なエクスペリエンスを選択できます。<br>A/B テストおよびエクスペリエンスのターゲット設定アクティビティで使用するA4T](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)を既に[実装している場合、設定は完了です！<br>詳細については、「[自動配分と自動ターゲット アクティビティのA4T サポート ](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md)」を参照してください。 |
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| Analytics for Target（A4T）での [!UICONTROL Auto-Allocate] アクティビティのサポート | [!UICONTROL Auto-Allocate]のアクティビティで、Target](/help/main/c-integrating-target-with-mac/a4t/a4t.md)の[Analyticsがサポートされるようになりました。<br>この統合により、[!UICONTROL Auto-Allocate] マルチアームドバンディット機能を使用して、目標指標[!UICONTROL Adobe Analytics]および/または[!UICONTROL Adobe Analytics]のレポートおよび分析機能を使用しながら、勝者エクスペリエンスへのトラフィックを促進できます。<br>A/B テストおよびエクスペリエンスのターゲット設定アクティビティで使用するA4T](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)を既に[実装している場合、設定は完了です！<br>詳細については、「[自動配分と自動ターゲット アクティビティのA4T サポート ](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md)」を参照してください。 |
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| Visual Experience Composer（VEC） | **新しい VEC メニューオプション**：VEC でページ要素をクリックすると、その要素のタイプで利用可能なメニューのオプションが表示されます。<ul><li>[!UICONTROL Styles > Background] オプションを使用して、選択された要素の背景画像および色を変更できるようになりました。 （TGT-15001）</li></ul>[ ビジュアルエクスペリエンスオプション ](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#styles)の&#x200B;*スタイル*&#x200B;を参照してください。<br>**クリックトラッキングの改善**: VECおよびシングルページアプリケーション（SPA） VEC内でクリックトラッキングを設定するプロセスが改善されました。<ul><li>クリックの追跡で使用する要素を選択する際に、すべての使用可能な要素の名前が右側の変更パネルに表示され、すばやく簡単に目的の要素を選択できます。</li><li>3 ステップのガイドによるアクティビティワークフローの [!UICONTROL Goals & Settings] ページに、クリックの追跡用に選択した要素の数が表示されます。 この数にマウスポインターを置くと、すべての選択された要素の名前が表示されます。 （TGT-33878）</li></ul>[クリックの追跡](/help/main/c-activities/r-success-metrics/click-tracking.md)を参照してください。 |
...
* いくつかの場所で、[!DNL Target] ユーザーインターフェイスが更新されました。
   ...
   * レポートとアクティビティで、一部のオプション （[!UICONTROL Edit]、[!UICONTROL Share to Feed]、[!UICONTROL View Experience URLs]など） [!UICONTROL More Options] アイコン （![icon_more_options image](assets/icon_more_options.png)をクリックしてアクセスできるようになりました
