...
* **削除後、プロパティトークンが自動的に再追加されない。** この修正は、ユーザーが選択解除したプロパティトークンが、アクティビティの編集時に自動的に再追加される問題を修正します。 以前は、自動検出されたプロパティを削除し、ステップ間を移動すると、システムはアラートやプロンプトを表示せずに削除されたプロパティを再導入したので、予期しないプロパティでアクティビティが有効になっていました。 この更新により、プロパティトークンが削除された後も、その後のすべての編集およびナビゲーション中に削除されたままになります。 （TGT-54040）
...
* **GraphQLでの createdAt および modifiedAt によるフィルタリングのサポート。** GraphQL API で、`createdAt` および `modifiedAt` 日付フィールドによるアクティビティのフィルタリングサポートが追加されました。 この機能強化により、Target UI での正確なフィルタリングが可能になり、クライアントサイドのデータ処理が減少し、フィルタリングされたアクティビティデータを取得するために複数の API 呼び出しが必要なくなります。 （TGT-54499）
...
* **[!UICONTROL Reports] 矢印が展開可能な列を明確に示さない問題を修正しました。** 更新された UI で、追加の列を展開できることをレポートテーブルが明確に表示しない問題を修正しました。 列ヘッダーの近くの [!UICONTROL Reports] 矢印に、より多くの列が使用可能であることを顧客が理解するのに役立つ、表示されないツールチップが追加されました。 （TGT-53479）
...
* 9 月 15 日午後 2 時（PDT）から 9 月 15 日午前 9[!DNL Adobe Target Premium] （PDT）の間、[!UICONTROL Auto-Target] ユーザーの :30 レポートに影響し :25 問題を修正しました。 影響を受けたコンバージョン指標のレポート（「[!UICONTROL Viewed a page]」または「[!UICONTROL Clicked on mbox]」オプションを使用して設定）を表示すると、コンバージョン率が誤ってレポートされます。現時点では、配信に関する既知の問題はありません。
