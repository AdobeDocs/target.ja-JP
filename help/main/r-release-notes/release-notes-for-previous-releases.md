...
リリースノートは、リリースの年月に従って新しい順に記載されています。
...
* **追加のビューに適用すると、カスタムコードが失われます。** この修正により、カスタムコードを追加のビューに追加および適用する際に、あるビューに適用されたカスタムコードが失われる問題が解決されました。 以前は、ユーザーがアクティビティの複数のビューにカスタムコードを適用すると、以前に適用したカスタムコードが消えていました。 この修正により、追加のビューに変更を適用する際に、すべてのビューでカスタムコードが適切に保持されます。 （TGT-54230）
...
* **プロファイルパラメーターの「中間」ルールが条件に保存されません。** この修正により、プロファイルパラメーターで **In Between** 演算子を使用する条件ルールが正しく保存されない問題が解決されました。 以前は、ユーザーが **範囲内** 演算子と入力された範囲値を使用してルールを設定すると、条件は正常に保存されるように見えましたが、条件を再度開いたり編集したりするとルールが消えていました。 この修正により、条件を再度開いたときに **中間** ルールが適切に保存され、保持されます。 （TGT-54498）
...
* **すべてのアクティビティ後にインサイトページの位置を変更する。** インサイトページ **の位置** 更新され、ナビゲーションの **すべてのアクティビティ** ページの後に表示されるようになりました。 すべてのアクティビティページが、Target アプリケーションのデフォルトページになりました。 （TGT-54672）
...
* **左側のナビゲーションを折りたたみ可能にします。** 左側のナビゲーションが、すべてのタブに対して折りたたみ可能かつ展開可能になりました。 折りたたむと、ツールチップと共にアイコンが表示されます。 展開すると、アイコンとテキストの両方が表示されます。 この機能強化により、ナビゲーションオプションにすばやくアクセスできる一方で、必要に応じてユーザーの画面スペースが広くなります。 （TGT-54608）
...
* バックエンドが null を返した場合、**デフォルトの experienceEditorType は「visual」になります。** この修正は、一部のシナリオで `experienceEditorType` 値が null として返され、UI でエディタータイプが正しく初期化されなかった意図しない動作が発生する問題を修正しました。 （TGT-54690）
...
* **アクティビティに画像を追加できません – 不明なエラー。** この修正により、アクティビティで画像を追加または置換しようとすると、「不明なエラー」が発生する問題が解決されます。 この問題は、Scene7 URL の同期と認証の失敗に関連していました。 この修正により、Visual Experience Composer で画像を正しく追加および置換できるようになります。 （TGT-54137）
...
* **削除後、プロパティトークンが自動的に再追加されない。**&#x200B;この修正により、ユーザーが選択解除したプロパティトークンが、アクティビティの編集時に自動的に再追加される問題が解決されます。 以前は、自動検出されたプロパティを削除してステップ間を移動すると、警告やプロンプトを表示することなく、削除されたプロパティがシステムに再導入され、予期しないプロパティでアクティビティが有効になっていました。 この更新により、プロパティトークンが削除された後も、その後のすべての編集およびナビゲーション中に削除されたままになります。 （TGT-54040）
...
* **許可リストに加えるされた URL 管理機能。** 許可されたリモートオファー URL パターンを管理するための新しい「許可リストに加えるURL」管理機能が [!UICONTROL Administration] の節に追加されました。 この機能には、許可リストに加えるした URL をテーブルに表示する機能、URL を検索およびフィルタリングする機能、モーダルダイアログを使用して新しい URL を追加する機能、確認を使用して URL を削除する機能が含まれます。 この機能は、リモートオファーに使用できる URL を顧客が制御するのに役立ち、セキュリティとコンプライアンスが向上します。 （TGT-54486）
...
* **GraphQLでの createdAt および modifiedAt によるフィルタリングのサポート。** `createdAt`および`modifiedAt`日付フィールドによるアクティビティのフィルタリングのサポートが、GraphQL APIに追加されました。 この機能強化により、ターゲットUIで正確なフィルタリングが可能になり、クライアント側のデータ処理が削減され、フィルタリングされたアクティビティデータを取得するために複数のAPI呼び出しが必要なくなります。 （TGT-54499）
...
* **Webエディターが提供するワークスペース列を有効にします。**&#x200B;この更新プログラムは、Webエディター内の他のワークスペースに表示されている[!UICONTROL Default Workspace]からのオファーによって引き起こされる顧客の混乱に対処します。 この動作は設計どおりに動作しますが、[!UICONTROL Default Workspace]のオファーはすべてのワークスペースにわたって意図的に表示されます。顧客からは、特に「承認者」などのデフォルト以外のワークスペースでアクティビティを作成する場合に、UIによってワークスペースの原点が明確にされなかったことが報告されました。 分かりやすくするために、Webエディターのオファー一覧で[!UICONTROL Workspace]列が有効になりました。これにより、ユーザーは各オファーが属するワークスペースを簡単に識別でき、表示される追加のオファーの誤解を防ぐことができます。 （TGT-54138）
...
* **[!UICONTROL Workspaces]ドロップダウンは、[!UICONTROL Users]セクションでは20に制限されています。**&#x200B;この修正により、ユーザーが多くのワークスペースにアクセスしている場合でも、[!UICONTROL Workspaces] > [!UICONTROL Administration]セクションの[!UICONTROL Users]ドロップダウンに表示されるワークスペースが20個だけという問題が解決されました。 また、`licenseGroups` に対する基になるGraphQL呼び出しも 20 件の結果に制限されているので、組織内のより多くのワークスペースにアクセスできるにもかかわらず、UI に不完全なリストが表示されていました。 更新によってこのハードリミットが削除されるので、使用可能なワークスペースの完全なセットが返され、正しく表示されるようになりました。 （TGT-53820）
...
* **アクティビティ編集で、自動検出されたプロパティが既に削除されている場合は、追加しないでください。**&#x200B;この修正により、アクティビティを編集すると、ユーザーが以前に削除した自動検出プロパティが自動的に再導入される問題が解決されました。 編集用にアクティビティを再度開くと、削除されたプロパティがシステムによって誤って復元され、[!UICONTROL Properties List]の動作が一貫性を失い、混乱が生じました。 このアップデートにより、自動検出されたプロパティが削除されると、その後のすべての編集中に削除されたままになり、ユーザーが明示的に追加し直さない限り、再び表示されなくなります。 （TGT-54182）
...
* **[!UICONTROL Reports] 矢印が展開可能な列を明確に示さない問題を修正しました。**&#x200B;更新されたUIで追加の列が展開される可能性があることをレポートテーブルに明確に示さなかった問題を修正しました。 列ヘッダーの近くの [!UICONTROL Reports] 矢印に、より多くの列が使用可能であることを顧客が理解するのに役立つ、表示されないツールチップが追加されました。 （TGT-53479）
...
* 新しいVEC UI **[!UICONTROL Experience Fragment]で**&#x200B;名が切り詰められました(TGT-54312)
...
+++詳細を表示
* **韓国語と日本語のUIのいくつかのローカリゼーションエラーを修正しました。** (TGT-54003、TGT-54004、TGT-54006、TGT-54007、およびTGT-54018)
...
* **アクティビティ条件を編集すると、更新された UI で空白の画面が表示されることがありました。** 更新された UI で、特定のアクティビティで [!UICONTROL Edit Criteria in Experiences] クリックすると空白の画面が表示されることがあった問題を解決しました。 基準エディターがすべてのアクティビティにわたって確実に読み込まれるようになり、ユーザーは中断することなく編集できるようになりました。 （TGT-53961）
* **更新されたUIでシーケンス条件を編集できません。** [!UICONTROL Sequence Criteria]を編集しようとすると、条件ポップアップが読み込み時に停止し、空白の画面が表示される更新されたUIの問題を解決しました。 基準エディターが正しく読み込まれるようになり、ユーザーは中断することなくシーケンス基準を編集および更新できるようになりました。 （TGT-53985）
...
* **貢献度レポートエラーが原因で MVT レポート [!UICONTROL Element] 読み込まれない。** Target UI で MVT アクティビティレポートを読み込めない問題を修正しました。次のエラーが表示されます。「要素貢献度レポートを取得できませんでした」 レポートが正しく表示されるようになり、要素の関与が完全に表示されるようになりました。 （TGT-53691）
* **[!UICONTROL Experience Targeting] (XT)アクティビティに関する注文の詳細をCSVにエクスポートします。** XT アクティビティで「[!UICONTROL Export Order Details to CSV]」オプションが正しく表示されず、空のファイルが返される問題を修正しました。 このオプションは AP アクティビティに対してのみ表示されるようになり、正確な書き出し機能を確保し、混乱を防ぎます。 （TGT-53798）
...
* **優先セレクターが更新されたUIで認識されません。**&#x200B;更新されたUIで、`data-target-component-id`などの優先セレクターがVEC内のCSSセレクターリストに表示されない問題を解決しました。 動的に生成されるクラス名の代わりに、優先する属性を確実に選択できるようになりました。これにより、SPAページの更新時に安定したターゲティングが可能になります。 （TGT-53908）
...
* **対象ユーザーの除外ルールが[!DNL Target] UIに含まれるものとして正しく表示されませんでした。除外ルールで構成された**&#x200B;人の対象ユーザーが、アクティビティ内のターゲット設定を編集するときに含まれているように見えました。 除外ロジックは実行中に正しく適用されましたが、UIは「excluding」ラベルを省略してルールを正確に反映できませんでした。 [!DNL Target] UIでは、対象ユーザーの構成とターゲットワークフローの両方で除外ルールが正しく表示されるようになりました。これにより、キャンペーン設定が明確になり、一貫性が保たれます。 （TGT-53808）
...
* **エクスペリエンス名が重複しているため、アクティビティを保存できない問題を修正しました。** 条件とデザインの特定の組み合わせを含むアクティビティを保存する際に、無効なユーザー入力エラーが発生しました。 設定が有効に見えた場合でも、エラーは重複したエクスペリエンス名によってトリガーされました。 異なる設定を持つアクティビティを、名前の競合を発生させずに保存できるようになりました。 （TGT-53805）
...
* **ライブ[!UICONTROL Recommendations]アクティビティにプロモーションを適用するとエラーが発生する問題を修正しました。**&#x200B;のお客様において、有効な構成の詳細を入力した後でも、有効な[!UICONTROL Recommendations]アクティビティにフロントプロモーションを適用しようとしたときに、「プロモーションにコレクションIDが指定されていないか、ルール値がフィルター処理されていません」というエラーが発生しました。 検証エラーを発生させることなく、プロモーションをライブアクティビティに正常に適用できるようになりました。これにより、更新されたアクティビティ作成UIでのエクスペリエンスがスムーズになります。 （TGT-53738）
* **[!UICONTROL Recommendations] UI内のコレクションで製品が表示されない問題を修正しました。単一テナントの**&#x200B;のお客様から、新しいUIの[!UICONTROL Recommendations]セクションで特定のコレクションを表示したときに製品リストの読み込みに失敗したことが報告されました。 この問題は一時的に環境を切り替えることで解決されましたが、この回避策によりユーザーエクスペリエンスが低下しました。 環境の切り替えを必要とせずに、製品エンティティを一貫して読み込めるようになりました。 （TGT-53783）
...
* **メトリックが有効になっていない場合の[!UICONTROL Graph View]のエラーメッセージを改善しました。** VEC で、関連する [!UICONTROL Graph View] レポートスイートでリクエストされた指標が有効になっていない場合、[!DNL Analytics] ーザーに一般的な「エラーが発生しました」というメッセージが表示される問題を修正しました。 この問題は、バックエンドのGraphQL応答の`not_enabled_metric`エラーが原因で発生しました。 この修正により、あいまいなエラーが、[!DNL Analytics]の構成の問題をユーザーが特定しやすくするための有益なメッセージに置き換えられ、混乱や不要なサポートエスカレーションが減少します。 （TGT-53577）
...
* **選択した環境ではなくデフォルトの環境を使用して、パフォーマンス CSV レポートが生成される問題を修正しました。** 以前は、顧客がレポート設定で環境を変更し、パフォーマンスレポートを生成すると、結果の CSV に、選択した環境ではなくデフォルト環境のデータが含まれていました。  UIで`environmentId`パラメーターが正しく渡され、選択した環境がレポートに反映されるようになりました。 さらに、エラー処理が改善されました。 CSVの生成中にGraphQLエラーが発生した場合、空のCSVファイルが生成される代わりに、明確なエラーメッセージがUIに表示されるようになりました。 （TGT-53064）
* **Analytics for Target （A4T）レポートで [!UICONTROL Graph View] にデータを表示できない問題を修正しました。A4T統合で**&#x200B;を使用している[!DNL Target]のお客様が、A/Bテスト活動の[レポート]タブでグラフビューに切り替える際に、「問題が発生しました」というエラーが発生しました。 [!UICONTROL Table View] は正しく読み込まれましたが、選択し [!UICONTROL Graph View] 時間範囲での指標の傾向をレンダリングできませんでした。 [!UICONTROL Graph View] では、サポートされているすべての指標のパフォーマンスデータを期待どおりに表示できるようになり、更新された UI での可視性とレポートの精度が向上しました。 （TGT-53573）
...
+++詳細を見る
* **要素メタデータが、ブレッドクラムメニューにカーソルを合わせたときに表示されるようになりました。**&#x200B;項目の上にマウスポインターを置いたときにID、クラス、名前などの追加の要素の詳細を表示するように、VECのブレッドクラムメニューを改善しました。 この機能強化により、ユーザーはアクティビティの設定中に要素をより簡単に識別および区別できます。 （TGT-53409）
* **パンくずリストにカーソルを合わせると、VECの対応する要素がハイライト表示されるようになりました。** [!UICONTROL Visual Experience Composer]を改善し、ブレッドクラムメニューの項目にカーソルを合わせたときに、エディター内の対応する要素をハイライト表示しました。 また、コンテナ、太字テキスト、ボタンなどのエレメントタイプも表示されます。 このビヘイビアーは、兄弟エレメントに適用され、検証リストに基づいて、SVGなどのサポートされていない型を除外します。 （TGT-53411）
...
* **VEC でのページの更新時に、カスタムコードの変更が保持されるようになりました。** Webサイトの更新中にカスタムコードの変更が失われるVECの問題を修正しました。 この問題は複数のリロードを行ったページで発生し、エディターがリセットされて未保存の変更が元に戻ります。 この修正により、編集中にページが再読み込みされた場合でも、カスタムコードの編集が確実にそのまま維持され、誤って進行状況が失われるのを防ぐことができます。 （TTGT-53501）
* **VEC内のサイトアクセスのログインの問題が解決されました。**&#x200B;ユーザーがVEC経由でサイトにアクセスしているときにサイトにログインできない問題を修正しました。 ログインフローでユーザーが繰り返しログインページにリダイレクトされ、アクティビティの設定とプレビューができない。 この修正により、VECがログイン動作を妨げることがなくなり、認証されたユーザーの予想されるアクセスが復元されます。 （TGT-53524）
* **VECヘルパー拡張機能で解決されたCookieの重複の問題。** QA中に[!UICONTROL Adobe Experience Cloud Visual Editing Helper]拡張機能がプレビューリンクを介して`at_qa_mode` Cookieを複製する問題を修正しました。 プレビューインデックスを手動で切り替えると、ドメイン間で値が競合する cookie が複数作成されるので、テスターがバリアントを確実に切り替えることができませんでした。 この動作は、ターゲットUIの外部でも発生し、内部アカウントとクライアントアカウントの両方に影響を与えました。 この修正により、エントリの重複を防ぎ、ドメインスコープを調整して、手動での cookie のクリーンアップを必要とせずにシームレスなバリアント切り替えが可能になり、一貫した cookie 処理が保証されます。 （TGT-53579）
...
* **カーソルを合わせて選択したときに要素タイプを表示することで、VEC での要素選択が強化されました。** アクティビティの作成時の操作性を向上させるために、VEC でHTML要素にカーソルを合わせたり、選択したりすると、要素のタイプがデコレートされるようになりました。 この機能強化により、お客様は正しい要素をより簡単に識別および選択できます。 選択した要素は異なる色でハイライト表示され、ハイライト表示された要素は青色で縁取りされます。 エレメントタイプも表示され、編集中により明確なコンテキストが提供されます。 （TGT-53502）

+++

### データストリームの更新（2025年9月19日）
...
* **ページ配信のパラメータープレゼンスルール用に修正されたフォームロジック。」** 更新された UI で、「[!UICONTROL Parameter is present]」、「[!UICONTROL Parameter is not present]」、「[!UICONTROL Parameter value is present]」、「[!UICONTROL Parameter value is not present]」などのページ配信ルールを選択する際に、ユーザーが追加のパラメーター値を入力する必要が誤って発生していた問題を解決しました。 この動作は従来のUIと一貫性がなく、値を指定せずにパラメータの存在を検出するという意図したロジックと矛盾していました。 この修正により、予期されるルール設定動作が復元され、アクティビティの設定が合理化され、操作性が向上します。 （TGT-53640）
...
+++詳細を見る
* **お客様は、[!UICONTROL Activity Overview]**&#x200B;からアクティビティ名またはドキュメント名をコピーできませんでした。以前は、お客様は、更新されたアクティビティ作成プロセスで、[!UICONTROL Activity overview]からアクティビティ名または関連するオファー/ドキュメントを直接コピーできませんでした。 この制限は、特に小さな画面の使いやすさに影響を与えました。 アクティビティ名と文書名の両方を、回避策なしで簡単にコピーできるようになりました。 （TGT-51850）
...
* **Backspace を使用してすべての日付値を消去すると、ページがクラッシュする原因になりました**：顧客が [!UICONTROL Goals & Settings] セクションでアクティビティをスケジュールしているときに、Backspace を使用して「[!UICONTROL Specified Date & Time]」フィールドからすべての値を消去するとクラッシュが発生しました。 この問題は、日付処理ロジックの null 参照エラーが原因です。 ページは、空の日付入力をクラッシュすることなく優雅に処理できるようになりました。 （TGT-53624）
...
* **[!UICONTROL Browse]モードで戻るボタンを2回押すと、VECがクラッシュしました**: VECで[!UICONTROL Browse]モードを移動しているお客様が、ブラウザーの戻るボタンを2回押すとクラッシュしました。 この問題により、エディターがフリーズし、ページを更新する必要がありました。 エディターは、クラッシュすることなく、確実にバックナビゲーションを処理できるようになりました。 （GT-53568）
* **場所のマッピングが定義されていないため、アクティビティを編集できませんでした**: `LocationMapping.behaviorTargetedActivity`ロジックの場所IDが定義されていないことが原因で、お客様がアクティビティを編集しようとしたときにエラーが発生しました。 この問題により400エラーが発生し、アクティビティの更新がブロックされました。 場所に関連する検証エラーなしで、アクティビティを編集できるようになりました。 （TGT-53607）
* **アクティビティの保存で無効なユーザー入力エラーが発生しました**：更新された VEC で小規模な変更を加えた後にアクティビティを保存しようとすると、無効なユーザー入力エラーが発生しました。 このエラーは、バックエンド検証ロジックの場所マッピングが一致していないことが原因で発生しました。 場所に関連するエラーをトリガーせずに、アクティビティを正常に保存できるようになりました。 （TGT-53603）
...
+++詳細を表示
* **期限切れの「時間枠」オーディエンスが削除後もアクティビティの保存をブロックしていた問題を修正しました**：期限切れの「時間枠」オーディエンスに関連するエラーが発生したため、お客様が更新されたUIでアクティビティを保存できませんでした。 影響を受けるオーディエンスを削除した後でも、「アクティビティに無効な時間枠のオーディエンスが含まれています」というエラーメッセージが表示されます。 この問題が発生したのは、アクティビティのみのオーディエンスが使用されなくなっても、システムがそのオーディエンスの検証を続けたためです。 この挙動は、タイムゾーンの不一致によってさらに複雑になりました。 検証ロジックが修正され、このエラーが発生しなくてもアクティビティを保存できるようになりました。 （TGT-53517）
...
* **ページ切り替え時の無効なオプション参照が原因で、アクティビティ作成プロセスでクラッシュが発生する修正**：編集中にページを 3 回切り替えると、UI がクラッシュするアクティビティ作成プロセスの問題を修正しました。 無効なオプション参照が原因でクラッシュが発生し、「localId &#39;7&#39;のオプションが見つかりません」などのコンソールエラーが発生します。 このアップデートにより、お客様はシステムの障害や中断を発生させることなく、ページを切り替えて変更を適用できるようになりました。 （TGT-53295）
...
+++詳細を表示
* **アクティビティ作成プロセスで、URL の先頭のスペースが原因で QA リンクが壊れる問題を修正しました**：アクティビティ作成プロセスで、保存時にアクティビティ URL の先頭のスペースが正しく削除されなかった問題を修正しました。 この結果、QAリンクが無効になり、バックエンドで書式が正しくなくなりました。 更新後、URL はクリーンに保存されるようになり、リンク切れを防ぎ、顧客に対する QA ワークフローの信頼性を向上させることができます。 （TGT-53427）
...
+++詳細を見る
* **高度な検索で候補を表示できなかった、カタログ検索UIの問題を修正しました。**: [!UICONTROL Catalog Search]機能で候補を表示できなかった、新しい[!UICONTROL Advanced Search] UIの問題を修正しました。 正確なスペルで正確な値を入力する必要があったため、検索が面倒でエラーが発生しやすくなります。 この修正により、[!UICONTROL Advanced Search] はユーザーのタイプに応じて関連性の高い提案を提供するようになり、操作性が向上し、製品を見つける際の摩擦が軽減されます。 （TGT-52008）
...
* **レコメンデーションビューで製品メッセージの表示に失敗した、アクティビティ作成プロセスの問題を修正しました**：メッセージが存在しても、レコメンデーションビューの [!UICONTROL Message] 列に製品データが表示されない [!DNL Recommendations] クティビティ作成プロセスの問題を修正しました。 コンテンツを一時的に表示するには、手動で列を削除して再度追加する必要がありましたが、スクロールや検索を行うと再び表示されなくなることがよくあります。 この更新により、製品メッセージの一貫性のある表示が復元され、カタログのナビゲーションとレビューワークフローが向上します。 （TGT-52777）
...
* **[!UICONTROL Last Updated] 列による一貫性のない並べ替えを修正：** activity-create プロセスで、カタログを [!UICONTROL Last updated] 列で並べ替えると一貫性のない結果が生成される問題を修正しました。 お客様が更新タイムスタンプに基づいて製品データを確実に整理できなかったため、カタログの確認と管理がより困難になりました。 並べ替えが期待通りに機能するようになり、更新されたUIでの精度と使いやすさが向上しました。 （TGT-53116）
...
* 更新された[!DNL Target] UIで、[!UICONTROL Visual Experience Composer]タブから[!UICONTROL Targeting]に戻った後、[!UICONTROL Experiences] (VEC)で行われたHTMLオファーの変更が保持されない問題を修正しました。 （TGT-52874）
...
+++詳細を表示
* QA環境が複数のタブにわたってデータを読み込めず、インターフェイスが使用できない問題を修正しました。 （TGT-53377）
* QA環境でアクティビティを作成できない問題を修正しました。 プロセスは、正常に完了する代わりに[!UICONTROL Activities]ページにリダイレクトされました。 （TGT-53328）
...
* [!UICONTROL Automated Personalization] （AP）アクティビティでの正確な履歴レポートをサポートするために、削除されたオプション永続性レイヤーを Target バックエンドに再実装しました。 以前は、オプションを削除すると、その名前が失われるので、過去のパフォーマンスデータを解釈するのが難しかったです。
  ...
  **主な改善点**:
   ...
   * レポートUIで、元のオプション名（例： `Option Name [Deleted]`）と履歴メトリックを表示して、明瞭度と使いやすさを向上できるようになりました。
...
+++詳細を表示
* デフォルト以外のワークスペースからデフォルトのワークスペースに、またはデフォルト以外のワークスペース間でアクティビティをコピーする際の問題を修正しました。 オファーが複製され、競合を防ぐために追跡と名前付けが強化されました。

  **主な改善点**:
   * オファーは、更新されたIDとメタデータを使用して移行先のワークスペースに再作成されます。
   * コピーされたオファーの名前は、「オファー名のコピー」の形式にランダムな数字またはタイムスタンプを加えた名前に変更され、一意性が保たれます。
   * オファーとアクティビティの状態が更新され、新しいIDが反映されます。
...
+++詳細を見る
* お客様が[!UICONTROL Goals & Settings]ページで特定の活動のレポートを表示したときに、[!UICONTROL View in Analytics]リンクが運用環境ではなくQA環境を正しく示さない問題を修正しました。 （TGT-53163）
...
* 本番環境で、更新されたUIのエンティティをクリックすると「製品の詳細を取得できませんでした。 この問題が解決しない場合は、[!DNL Adobe Client Care] にお問い合わせください。」 （TGT-53071）
...
+++詳細を表示
* 注文の詳細をCSVファイルに保存するとファイルが空になる問題を修正しました。 （TGT-52225）
...
* アクティビティの編集を妨げ、エラーメッセージ「未定義のプロパティを読み取れません（「マップ」を読み取ります）」をトリガーした問題を修正しました。 （TGT-53161）
...
+++詳細を見る
* ワークスペースの切り替え時のアドホックオファーの処理が改善されました。
   * デフォルトのワークスペースからデフォルト以外のワークスペース（またはデフォルト以外のワークスペース）に切り替えると、アドホック・オファーが正しくコピーされるようになりました。 初期化中にワークスペースコンテキストが更新され、新しいIDがオファーに割り当てられて一意性が確保されます。
   * 同じワークスペース内に留まっている場合、変更は行われません。 （TGT-53079）
...
* ワークスペース間でプロパティを変更する際の問題を修正しました。
   ...
   * [!UICONTROL Properties]リストに警告が表示され（一部のプロパティに互換性がない可能性があることを示す可能性があります）、お客様が[!UICONTROL Add]または[!UICONTROL Remove]をクリックして[!UICONTROL Save]をクリックすると、対象のワークスペースにないすべてのプロパティが削除されます。 お客様が[!UICONTROL Cancel]をクリックすると、すべてのプロパティは、対象のワークスペースに存在しない場合でも残ります。 （TGT-47094）
   * 同じワークスペースを使用している場合や、デフォルト以外のワークスペースからデフォルトまたは別のワークスペースに切り替えた場合は、すべてがそのまま残ります。 （TGT-53078）
* アクティビティの元のワークスペースコンテキストを考慮したエンティティ検証ロジックの更新。 [!UICONTROL Experience Fragments] (XF)などのエンティティは、アクティビティが最初に作成されたワークスペースに基づいて検証されるようになりました。 たとえば、XFがデフォルトのワークスペースに存在し、アクティビティがワークスペースXからワークスペースYにコピーされた場合、元の（デフォルト）ワークスペースでXFが有効であれば、検証は合格します。 （TGT-53196）
...
+++詳細を表示
* [!UICONTROL Approver]ロールを持つユーザーが、アクティビティのみのオーディエンスの絞り込みを追加または保存できない問題を修正しました。 これを試みると、ユーザーにアクティビティを承認および管理するための十分な権限があるにもかかわらず、「[エディター]」権限が必要であることを示す403 Forbiddenエラーが発生しました。 （TGT-52984）
* [!UICONTROL Remove Audience Refinement]オプションを使用してアクティビティ固有のオーディエンスを削除すると、オーディエンスが同じアクティビティ内での再選択のためにオーディエンスリストに表示されなくなる問題を修正しました。 このビヘイビアーは、最初から再作成しない限り、ユーザーが同じオーディエンスを再度追加することを防ぎます。 （TGT-52979）
...
* [!DNL Adobe Analytics]ステップでレポートソースとして[!UICONTROL Goals & Settings] (A4T)を選択すると、お客様がアクティビティを作成または保存できない問題を修正しました。 この問題は、特に [!UICONTROL Custom Event] 定の指標（「カスタムイベント 16」など）を選択する際に発生し、「無効なユーザー入力」というエラーが発生していました。 （TGT-52910）
...
* 更新された[!DNL Recommendations] UIで[!UICONTROL Overview]アクティビティを表示しているときに、[!UICONTROL Goals & Settings] (A4T)がレポートソースとして選択されている場合に[!DNL Adobe Analytics]セクションの読み込みに失敗する問題を修正しました。 次のエラーメッセージが表示されました：「エラーが発生しました。 要求を完了できません。 問題が解決しない場合は、アドビのカスタマーケアにお問い合わせください。」 （TGT-52999）
...
* 新しいページを追加し、異なるエクスペリエンス内の特定の要素を削除する際に、「無効な入力」エラーが発生する問題を修正しました。 このエラーは、特にエクスペリエンスを切り替えたり、共有ページ構造を変更したりするときに、要素の操作中に重複`LocalIds`が生成されるとトリガーされます。 （TGT-52720）
...
* [!DNL Recommendations] を使用してアクティビティを追加する場合に、アクティビティ作成ワークフローに影響す [!UICONTROL promotions] 問題を修正しました。 ユーザーが「[!UICONTROL Promote by Attribute]」を選択してフィルタールール（[!UICONTROL Parameter Matching] など）を追加した場合、アクティビティを保存して再編集した後、選択したルールタイプとオペランドの値が保持されませんでした。 再び開くと、フィルタリング規則の種類が予期せず変更され、オペランド値が見つからなくなります。 （TGT-53059）
...
+++詳細を見る
* ビューに変更を適用すると、ビューが複製され、アクティビティが「無効なユーザー入力」エラーを返す問題を解決しました。 この修正により、複製または検証エラーを引き起こすことなく、ビューの変更が正しく適用されるようになります。 （TGT-52886）
...
* バリアントエクスペリエンスでカスタムコードに対して行った変更が、意図せず [!UICONTROL Control] スタムエクスペリエンスに反映される問題を修正しました。 これにより、配信動作に意図しない変更が生じました。 [!UICONTROL Control]エクスペリエンスは、他のエクスペリエンスに対して行われたカスタムコードの編集から分離されたままになります。 （TGT-52413）
...
* [!UICONTROL Browse Mode]でユーザーが別のURLを参照できない問題を修正しました。 これにより、テスターと編集者は、同じアクティビティセッション内で別のページを検証したり、プレビューしたりすることができなくなりました。 （TGT-53052）
* アクティビティの作成中に複数の[!UICONTROL Visual Experience Composer] (VEC)インスタンスが同時に開かれる問題を修正しました。 この問題は、ユーザーが[!UICONTROL Enhanced Experience Composer] (EEC)を無効にし、[!UICONTROL Page Delivery]ステップでWebサイトのURLの末尾のスラッシュを削除したときに発生しました。 （TGT-52782）
...
* 更新されたVECで一部のアクティビティの変更が正しく表示されない問題を修正しました。 （TGT-52818）
...
* [!UICONTROL Editor] の役割を持つユーザーが [!DNL Recommendations] アクティビティを編集できない問題を修正しました。 デザインを変更し、アクティビティを保存しようとした場合、ユーザーが関連するワークスペースで既にその役割を持っていても、「[エディター]」権限が必要であるという内容の403 Forbiddenエラーが発生します。 （TGT-52836）
...
* バックエンドAPIを介した[!DNL A/B Test]アクティビティ更新の検証エラーメッセージが改善されました。 重複する場所の名前が存在する場合、`locations.selectors`に対して「重複する名前は許可されません」というメッセージが明確に表示されるようになりました。 （TGT-52589）
...
* [!UICONTROL Form-Based Experience Composer]の[[!UICONTROL Location]]ドロップダウンで特殊文字を含むmboxにカーソルを合わせたときにエディターが空白になり、「&#39;Element&#39;に対して&#39;querySelector&#39;を実行できませんでした」というエラーが発生する問題を修正しました。 エラーが表示されます。（TGT-52717）
* 新しい「PARTIALLY_IMPORTED」インジケータにより、フィードステータスの精度が向上しました。 以前は、ファイル内のすべての行が読み込まれなくても、フィードが「成功」とマークされていました。これは誤解を招きました。 （TGT-52892）
...
* アクティビティの作成時にデフォルトワークスペースを選択する際の、不要な検証を削除しました。 必須プロパティ検証は、デフォルトのワークスペースには適用されませんが、デフォルト以外のワークスペースには適用されます。 （TGT-52449）
...
* ユーザーがエクスペリエンスフラグメントを追加しようとしたときに「無効な入力」エラーが発生する、更新された[!UICONTROL Visual Experience Composer] (VEC)の問題を修正しました。 （TGT-52701）
* 更新された[!UICONTROL Visual Experience Composer] (VEC)で対象ユーザーのターゲティングを編集する際に、「対象ユーザーの編集」モーダルが空に表示される問題を修正しました。 （TGT-52749）
...
* 9月15日の午後2[!DNL Adobe Target Premium]時(PDT)から10月6日の午前9[!UICONTROL Auto-Target]時(PDT)までの:30人のユーザーの:25のレポートに影響を与えた問題を修正しました。 影響を受けたコンバージョン指標のレポート（「[!UICONTROL Viewed a page]」または「[!UICONTROL Clicked on mbox]」オプションを使用して設定）を表示すると、コンバージョン率が誤ってレポートされます。現時点では、配信に関する既知の問題はありません。
