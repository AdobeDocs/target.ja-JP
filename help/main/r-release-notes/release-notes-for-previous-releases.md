...
* **新しいVisual Experience Composer UIでの認証後の断続的なグレー画面。**&#x200B;新しいVisual Experience Composerにログインした後、ユーザーに一時的にグレーの画面が表示される問題を修正しました。これにより、さらに編集できなくなります。 このアップデートにより、認証後の読み込み信頼性が向上します。 （TGT-54626）
...
### [!DNL Target Standard/Premium] 26.3.1（2026年3月3日（PT））

**アクティビティ**

+++詳細を見る

* **追加のビューに適用すると、カスタムコードが失われます。**&#x200B;この修正プログラムは、追加のビューにカスタムコードを追加および適用する際に、1つのビューに適用されたカスタムコードが失われた問題を解決します。 以前は、ユーザーがアクティビティ内の複数のビューにカスタムコードを適用すると、以前に適用されたカスタムコードは消えていました。 この修正により、追加のビューに変更を適用する際に、すべてのビューでカスタムコードが適切に保持されるようになります。 （TGT-54230）
...
* **プロファイルパラメーター「間の間」ルールが条件に保存されません。** プロファイル パラメーターに&#x200B;**In Between**&#x200B;演算子を使用する条件ルールが正しく保存されなかった問題を解決します。 以前、ユーザーが&#x200B;**In Between**&#x200B;演算子を使用してルールを設定し、範囲値を入力した場合、条件は正常に保存されたように見えましたが、条件を再度開いたり編集したりすると、ルールが消えます。 この修正により、**In Between**&#x200B;規則が正しく保存され、条件が再度開かれたときに保持されるようになります。 （TGT-54498）
...
* **左側のナビゲーションを折りたたみ可能にします。**&#x200B;左側のナビゲーションは、すべてのタブに対して折りたたんで展開できるようになりました。 折りたたむと、ツールヒントと共にアイコンが表示されます。 展開すると、アイコンとテキストの両方が表示されます。 この機能強化により、ナビゲーションオプションへの迅速なアクセスを維持しながら、必要に応じてより多くの画面スペースが確保されます。 （TGT-54608）
...
* **バックエンドがnullを返す場合、デフォルトのexperienceEditorTypeを「visual」に変更します。**&#x200B;この修正では、一部のシナリオで`experienceEditorType`値がnullとして返され、エディタータイプが正しく初期化されなかったUIで意図しない動作が発生する問題に対処します。 （TGT-54690）

+++

### [!DNL Target Standard/Premium] 26.2.2（2026年2月16日（PT））

**[!UICONTROL Visual Experience Composer]（VEC）**

+++詳細を見る

* **アクティビティに画像を追加できません – 不明なエラーです。**&#x200B;この修正プログラムは、ユーザーがアクティビティで画像を追加または置換しようとしたときに「不明なエラー」が発生した問題を解決します。 この問題は、Scene7 URLの同期と認証エラーに関連していました。 この修正により、Visual Experience Composerで画像を正しく追加および置き換えることができるようになりました。 （TGT-54137）

+++

### [!DNL Target Standard/Premium] 26.2.1（2026年2月8日（PT））

**プロパティ**

+++詳細を見る

* **削除後にプロパティトークンが自動的に再追加されません。**&#x200B;この修正プログラムは、ユーザーが選択を解除したプロパティ トークンが、アクティビティの編集時に自動的に再追加される問題に対処します。 以前、ユーザーが自動検出されたプロパティを削除し、ステップ間を移動すると、システムはアラートやプロンプトを表示せずに削除されたプロパティを再導入し、予期しないプロパティでアクティビティが有効になりました。 このアップデートにより、プロパティトークンが削除されると、その後のすべての編集とナビゲーション中に削除されたままになります。 （TGT-54040）

+++

### [!DNL Target Standard/Premium] 26.1.2（2026年1月30日（PT））
...
* **target=&quot;_blank&quot;のリンクが新しいタブで開きます。**&#x200B;この修正により、~ モードでクリックすると、~target=&quot;_blank&quot;[!UICONTROL Browse]のリンクを含む作成されたweb サイトが新しいブラウザータブで開き、エディター内のプレビューエクスペリエンスが中断する問題が解決されました。 この動作は、アンカー要素が変換され、そのターゲットがエディター内でのナビゲーションを維持するために上書きされた従来のUIとは異なり、作成されたページのネイティブリンク属性が拡張機能の挿入されたJavaScriptによってインターセプトされなかったために発生しました。 このアップデートにより、~target=&quot;_blank&quot;~を使用するリンクがWeb エディター内で適切に処理されるようになり、オーサリング中に外部タブが開かなくなります。 （TGT-54134）
* **プロパティの選択を解除します。**&#x200B;このアップデートでは、アクティビティエディターで自動検出されたプロパティの選択を解除する際に、ユーザーに明確な通知を表示する視覚的な警告が導入されました。 以前は、自動検出されたプロパティを削除しても、そのプロパティが完全に削除されることを示すものはありませんでした。そのため、c0はターゲティング設定が誤って失われる可能性があります。 この修正により、従来のUIの動作と一致する警告アイコンが追加され、プロパティの選択を解除するとアクティビティから削除されることをユーザーに通知できます。 （TGT-54121）
* **[!UICONTROL Workspaces]ドロップダウンは、[!UICONTROL Users] セクションで20に制限されています。**&#x200B;この修正により、ユーザーがもっと多くのワークスペースにアクセスできる場合でも、[!UICONTROL Workspaces] > [!UICONTROL Administration] セクションの[!UICONTROL Users] ドロップダウンが20個のワークスペースのみを表示する問題が解決されました。 `licenseGroups`の基礎となるGraphQL呼び出しも20件に制限され、ユーザーが組織内のより多くのワークスペースにアクセスできたにもかかわらず、UIに不完全なリストが表示されました。 この更新により、このハード制限が削除され、使用可能なワークスペースの完全なセットが返され、正しく表示されるようになりました。 （TGT-53820）
* **オファーのモーダルにワークスペース列が表示されない問題を修正しました。**&#x200B;更新されたUIでオファーのモーダルにワークスペース列が表示されない問題を修正しました。 これは、[!UICONTROL Default Workspace]からのオファーが、選択したワークスペースからのオファーと一緒に表示され、その出所を示さなかったため、顧客に混乱をもたらしました。 ワークスペース列が有効になり、顧客は各オファーがどのワークスペースに属しているかを明確に識別できます。 （TGT-52320）
...
* **アクティビティの編集では、既に削除されている場合、自動検出されたプロパティを追加しないでください。**&#x200B;この修正プログラムは、アクティビティを編集すると、ユーザーが以前に削除した自動検出プロパティが自動的に再導入される問題に対処します。 編集のためにアクティビティを再開すると、削除されたプロパティが誤って復元され、[!UICONTROL Properties List]の動作が不安定になり、混乱が生じます。 このアップデートにより、自動検出されたプロパティが削除されると、その後のすべての編集中に削除され、ユーザーが明示的に追加しない限り再び表示されなくなります。 （TGT-54182）
* **既に削除されている場合は、自動検出されたプロパティを追加しないでください。**&#x200B;この修正により、ユーザーが自動検出されたプロパティをアクティビティから手動で削除すると、その後のアクティビティエディター内でのナビゲーション中にシステムが再導入されなくなります。 以前は、ユーザーが自動検出されたプロパティの選択を解除し、[!UICONTROL Targeting] ステップに移動してから[!UICONTROL Experiences]に戻った場合、エディターは、アクティビティエディターの状態スライスに保存されている自動検出リストに基づいて、削除されたプロパティを再入力していました。 更新されたロジックは、自動検出されたプロパティを~ActivityState~ スライスの現在のプロパティと比較し、ユーザーが既に削除した自動検出されたプロパティを再追加しないようにしました。 これにより、ステップをまたいで一貫性のある行動を実現し、ユーザーの意図を尊重できます。 （TGT-54181）
...
* **自動検出された[!UICONTROL Properties]を状態に追加します。**&#x200B;このアップデートにより、~ActivityEditorSlice.ExperienceEditor~状態で、Web エディターからアクティビティ [!UICONTROL Experiences] タブに渡されたすべての自動検出プロパティ IDの最新リストが常に維持されます。 ユーザーが[!UICONTROL Experiences] タブに移動するたびに、状態は新しく検出されたプロパティで更新され、重複を防ぎながら、正確な追跡と信頼性の高い下流の動作が保証されます。 （TGT-54119）
...
* **[!UICONTROL Environment]ドロップダウンには100件のみが表示されます。**&#x200B;この修正は、100を超える環境を持つ顧客が[!UICONTROL Environment]内の[!UICONTROL Recommendations] ドロップダウンの最初の100個のエントリしか表示できない制限に対処します。 基になるGraphQL クエリ （~getEnvironmentsV2~）は、ハードコードされたページサイズ 100でページ分割されたため、追加のページが使用可能な場合でも、UIに部分的なリストのみが表示されます。 環境が100を超えているお客様の場合、この問題により、オプションが欠落し、選択エクスペリエンスが不完全になりました。 このアップデートでは、すべての環境が返されて表示されるように制限が増え、環境数に関係なく完全に表示されます。 （TGT-53903）
...
* **[!UICONTROL Reports]矢印が拡張可能な列を明確に示さない問題を修正しました。**&#x200B;更新されたUIで、レポート テーブルに追加の列を展開できることが明確に表示されない問題を修正しました。 列ヘッダーの近くにある[!UICONTROL Reports]矢印に消えるツールチップが追加され、より多くの列が使用可能であることを顧客が理解しやすくなりました。 （TGT-53479）
...
* **ビューに適用された変更を削除できません。**&#x200B;この修正プログラムは、変更が最初に追加のビューに再適用されない限り、ユーザーがアクティビティ内の変更を削除できなかった問題を解決します。 アクティビティ（アクティビティ ID 302467など）を編集する場合、変更を削除しようとすると何も影響がなく、ユーザーが不要な変更を削除できないようにします。 しかし、「さらにビューに適用」を使用して変更を再適用し、`Page Load` イベントに割り当てると、削除が突然予想通りに機能しました。 （TGT-54088）
...
* 新しいVEC UI **[!UICONTROL Experience Fragment]で**&#x200B;名が切り捨てられました（TGT-54312）
* **指標[!UICONTROL Advanced Settings]に[!UICONTROL Revenue]を使用できません。**&#x200B;この修正により、[!UICONTROL Advanced Settings]の[!UICONTROL Revenue]指標に[!UICONTROL Goals & Settings]を設定する際に、ユーザーが403 「アクセス拒否」エラーを検出した問題が解決されます。 プライマリ目標に関連付けられた依存関係の条件を追加する際に問題が発生しました。バックエンドでは、既にアクティビティの作成と編集に十分な権限を持っているユーザーであっても、誤ってエディター権限が必要でした。 その結果、有効な設定にもかかわらず、アクティビティの保存に失敗しました。 このアップデートでは、権限チェックが修正され、適切なアクセス権を持つユーザーが禁止リソースエラーをトリガーせずに収益メトリックの依存関係を正常に追加できるようになりました。 （TGT-54092）
...
* **URL条件を削除すると、目標指標の設定がリセットされる問題を修正しました。** [!UICONTROL Goal]指標で1つのURL条件を削除すると、更新されたUIで設定全体がリセットされる問題を修正しました。 お客様が[!UICONTROL Conversion] > [!UICONTROL Viewed a Page]の下に保存されたURL条件を削除しようとすると、目標タイプが予期せず[!UICONTROL Viewed an Mbox]に切り替わり、以前に設定されたすべての設定が削除されました。 このアップデートにより、選択したURL条件のみが削除され、残りのすべての目標設定が維持されます。 （TGT-53271）
...
+++詳細を見る
* **更新されたUIで非表示または無効なセレクターを使用して決定を提供できません。**&#x200B;更新されたUIで、要素がVisual Experience Composer （VEC）で表示されない限り、非表示または無効なセレクターに関連付けられたオファーの決定を編集できない問題を解決しました。 編集はパネルから直接サポートされるようになりました。従来のUIで使用できる機能を復元し、セレクターの表示に関係なくオファーの決定を変更できるようになりました。 （TGT-53899）
...
* 更新されたUIで&#x200B;**[!UICONTROL Message]列が製品データを断続的に表示できませんでした。**&#x200B;更新された[!UICONTROL Recommendations] UIで、[!UICONTROL Message]の[!UICONTROL Catalog Search]列がフィードに値が存在していたにもかかわらず、製品データを断続的に表示できなかった問題を解決しました。 列は、すべての製品で正しいメッセージ値を一貫して表示するようになり、列の再設定を手動で必要とせずに信頼性の高い可視性を確保します。 （TGT-52777）
...
* **コレクションから特定の製品を開くと、「リクエストされたリソースが見つかりませんでした」と返され、モーダルには閉じるオプションがありませんでした。**&#x200B;更新されたRecommendations UIで、コレクションから特定の製品を開くと「リクエストされたリソースが見つかりませんでした」というエラーが発生し、閉じるオプションを指定せずに空白モーダルが表示される問題を解決しました。 モーダルで製品の詳細が正しく読み込まれるようになり、クローズオプションが常に使用できるようになりました。 （TGT-53986）
...
+++詳細を見る
* 更新されたUIで&#x200B;**[!UICONTROL Goals & Settings]をレポートソースとして使用すると、[!DNL Adobe Analytics] エラーメッセージが表示されます。**&#x200B;更新された[!UICONTROL Overview] UIで、目標セクションに「問題が発生しました」というエラーが表示される問題を解決しました。 ご要望の処理は完了できません。 [!DNL Adobe Client Care] （A4T）がレポートソースとして選択されたときに問題が解決しない場合は、[!DNL Adobe Analytics]にお問い合わせください。 目標が[!UICONTROL Adobe Analytics]指標で正しく表示されるようになり、レポートソース全体で一貫した可視性が確保されました。 （TGT-54021）
...
+++詳細を見る
* **更新されたUIで複数のレポートオーディエンスを選択できません。**&#x200B;更新されたUIで、ユーザーがアクティビティの編集時に、新しく作成された複数のレポートオーディエンスを同時に選択できない問題を解決しました。 複数のオーディエンスを一度に割り当てることができるため、レポートの設定における柔軟性と効率性が向上しました。 （TGT-53253）
...
+++詳細を見る
* **更新されたUIで決定オファーを編集または置き換えることができません。**&#x200B;更新されたUIで、[!UICONTROL Modifications] パネルを通じてオファーを決定して編集または置き換えることができず、オファー名が空白で表示される問題を解決しました。 決定オファーは完全にアクセス可能で編集可能になり、従来のUIとの整合性が回復され、顧客がアクティビティ内で直接オファーを管理できるようになります。 （TGT-53884）
...
+++詳細を見る
* **エンティティ属性の一致を持つ属性によるプロモーションは、アクティビティの保存後にレコメンデーションキーを読み込めませんでした。** アクティビティを保存した後に編集した場合、ルールタイプ [!UICONTROL Promotion by Attribute]のタイプ [!UICONTROL Entity Attribute Matching]のプロモーションでレコメンデーションキーが読み込まれない問題を修正しました。 この問題は、`customKeyId`がGraphQLを通じてリクエストされていないことが原因で発生しました。 プロモーション編集中にレコメンデーションキーが正しく読み込まれるようになりました。 （TGT-53117）
* **ExpBからExpAに切り替えると、推奨事項が視覚的に保持されます。** エクスペリエンス Bにレコメンデーションを挿入してからエクスペリエンス Aに切り替えると、レコメンデーションオファーボックスが表示される問題を解決しました。 これは視覚的な不整合のみでした。エクスペリエンスを切り替える際に変更が正しくレンダリングされるようになり、正確なUI動作が確保されました。 （TGT-53911）
* **に一致する[!UICONTROL Promotion by Attribute]の[!UICONTROL Entity Attribute]Recommendation キーが読み込まれていません。** アクティビティを保存した後に編集した場合、ルールタイプ [!UICONTROL Promotion by Attribute]のタイプ [!UICONTROL Entity Attribute Matching]のプロモーションでレコメンデーションキーが読み込まれない問題を解決しました。 GraphQLを通じてレコメンデーションキーが正しく取得され、プロモーションが期待どおりに表示され、機能するようになりました。 （TGT-53917）
* **更新されたUIで、非表示のHTML要素に関する推奨事項を編集できません。**&#x200B;非表示のHTML要素に適用されたレコメンデーションアクティビティを編集できない[!UICONTROL New Create]およびVEC UIの問題を解決しました。 この機能が期待どおりに動作するようになり、従来のUIとのパリティを復元し、要素の表示に関係なくレコメンデーションを変更できるようになりました。 （TGT-53953）
* **更新されたUIで、非表示のHTML要素に対するレコメンデーションアクティビティを編集できません。**&#x200B;非表示のHTML要素に適用されたレコメンデーションアクティビティを編集できない、更新されたUIの問題を解決しました。 この機能が期待どおりに動作するようになり、従来のUIとのパリティを復元し、要素の表示に関係なくレコメンデーションを変更できるようになりました。 （TGT-53951）
* **更新されたUIに属性値が断続的に欠落しているレコメンデーションカタログ。**&#x200B;更新された[!UICONTROL Recommendations] UIで、製品フィードに存在する場合でも、カタログ検索リストで特定の属性値（メッセージなど）を断続的に表示できない問題を解決しました。 属性値が、列の再構成を必要とせずに検索結果で一貫して読み込まれるようになり、カタログ管理の信頼性と効率性が向上しました。 （TGT-52769）
* 更新されたUIの&#x200B;**[!UICONTROL Download Recommendations]アクティビティに[!DNL Recommendations] ボタンがありません。**&#x200B;更新された[!DNL Recommendations] UIで、レコメンデーションを使用してA/B アクティビティに[!UICONTROL Download Recommendations] ボタンが表示されない問題を解決しました。 ボタンが正しく表示され、レコメンデーションデータを従来のUIの機能と一致した形式で書き出せるようになりました。 （TGT-53768）
...
* **アクティビティ条件を編集すると、更新されたUIで空白の画面が表示されることがあります。**&#x200B;更新されたUIの問題を解決しました。[!UICONTROL Edit Criteria in Experiences]をクリックすると、特定のアクティビティに対して空白の画面が表示されることがありました。 条件エディターがすべてのアクティビティで確実に読み込まれるようになり、ユーザーが中断することなく編集できるようになりました。 （TGT-53961）
* **更新されたUIでシーケンス条件を編集できません。**&#x200B;更新されたUIで[!UICONTROL Sequence Criteria]を編集しようとすると、読み込み中に条件ポップアップが停止したままになり、空白の画面が表示される問題を解決しました。 条件エディターが正しく読み込まれるようになり、ユーザーは中断することなくシーケンス条件を編集および更新できるようになりました。 （TGT-53985）
...
+++詳細を見る
* **[!UICONTROL Multivariate Test]（MVT）の場所とグラフ レポートの問題により、レポートの生成が妨げられました。** MVT アクティビティがTarget UIで[!UICONTROL Location Contribution]およびグラフ レポートを生成できなかった問題を解決し、「問題が発生しました。 お客様のリクエストを完了できません。」 レポートがUI内で正しく読み込まれるようになり、完全に表示されるようになりました。 （TGT-53654）
* **件の貢献度レポートエラーにより、[!UICONTROL Element]MVT レポートが読み込まれません。** MVT アクティビティレポートをTarget UIに読み込めず、「要素貢献度レポートを取得できませんでした」というエラーが表示される問題を修正しました。 レポートが正しく表示され、要素の貢献度が完全に表示されるようになりました。 （TGT-53691）
...
+++詳細を見る
* **[!UICONTROL Delete Modification]ボタンの問題により、アクティビティの変更を削除できませんでした。** [!UICONTROL Delete Modification] UIの[!DNL Target] ボタンが機能しない問題を解決し、ユーザーがアクティビティ内の変更を削除できないようにしました。 ボタンが期待どおりに動作するようになり、変更を遅滞なく確実に削除できるようになりました。 （TGT-53728）
...
* **更新されたVECで[!UICONTROL Design] モードに切り替えることができません。**&#x200B;更新されたVEC UIで、[!UICONTROL Design] モードで新しいページに移動した後、ユーザーが[!UICONTROL Browse] モードに戻れなかった問題を解決しました。 [!UICONTROL Design] トグルが正しく機能するようになり、変更をページ間でシームレスに適用できるようになりました。 （TGT-53988 および TGT-53993）
* **クエリパラメーターがアクティビティの概要に表示されません。**&#x200B;更新されたUIで、アクティビティの[!UICONTROL Overview] ページにクエリパラメーターが表示されない問題を解決しました。この問題により、[!UICONTROL Overview]とページ配信URLの間に相違が生じます。 クエリパラメーターが正しく表示されるようになり、アクティビティの場所が完全に表示され、ビュー間で一貫性が保たれます。 （TGT-53701）
...
* **[!UICONTROL Observer]人のユーザーが、更新されたUIでアクティビティコンテンツを表示できるようになりました。**&#x200B;更新されたアクティビティ UIの監視者ロール ユーザーの表示を復元しました。 以前は、オブザーバーは変更、オファー、コンテンツの変更を表示できませんでした。これは、従来のUIで使用できる機能です。 （TGT-53785）
* **[!UICONTROL Approver]人のユーザーが、編集者権限エラーなしでアクティビティ目標を編集できるようになりました。**&#x200B;承認者レベルのユーザーが高度な目標設定への変更を保存するのをブロックするアクティビティ作成UIの権限の問題を解決しました。 影響を受けるユーザーは、十分なアクセス権があるにもかかわらず、エディター権限を必要とする`403 Forbidden.Resource` エラーを受け取りました。 （TGT-53819）
...
+++詳細を見る
* **複数のオーディエンスの選択が「このアクティビティのみ」レポートに復元されました。** ユーザーが[!UICONTROL This activity only]の[!UICONTROL Goals & Settings] セクションで複数のオーディエンスを選択できないアクティビティ作成UIの問題を解決しました。 （TGT-53283）
...
* **[!UICONTROL Targeting] UIで、除外されたオーディエンスルールが明確に示されるようになりました。** アクティビティ作成UIの[!UICONTROL Targeting] セクションで、[!UICONTROL Exclude]に設定されたオーディエンスルールが明確に表示されない問題を解決しました。 そのため、ターゲティングロジックを確認する際に、特に特定のURLを除くオーディエンスに対して、混乱が生じました。 （TGT-53809）
* **オーディエンス定義の値が、[!UICONTROL Targeting] タブで選択およびコピーできるようになりました。** ユーザーが[!UICONTROL Targeting] タブでオーディエンスルール値を選択およびコピーできないアクティビティ作成インターフェイスの問題を解決しました。 この機能は従来のUIでは利用できましたが、更新されたUIでは利用できませんでした。 （TGT-53856）
...
+++詳細を見る
* **zh_CN ページエディターコンテキストの「quote」の誤訳を修正しました。** zh_CN ロケールのコンテキスト翻訳エラーを修正しました。「quote」という用語が「报价」として不正確に翻訳され、商用価格見積もりが意味されていました。 タイポグラフィ/見出しスタイル/ブロック引用セクションでは、意図する意味は、価格設定ではなくフォーマット要素である引用ブロックを指します。 （TGT-53841）
* **zh_CN ページエディターのコンテキストで「quote removed」の誤訳を修正しました。** zh_CN ロケールの翻訳エラーを修正しました。「quote removed」が「移除了报价」として不正確にレンダリングされ、商用価格の見積もりを意味します。 タイポグラフィ/見出しスタイル/ブロック引用セクションでは、この用語は、価格設定ではなく書式要素（引用ブロック）を指します。 （TGT-53843）
* **zh_CN ページエディターコンテキストの「quote rearranged」の誤訳を修正しました。** zh_CN ロケールのコンテクスト翻訳エラーを修正しました。「quote rearranged」が「重新排列了报价」として不正確に翻訳され、商用価格見積もりが意味されていました。 タイポグラフィ/見出しスタイル/ブロック引用セクションでは、この用語は、価格設定ではなく書式要素（引用ブロック）を指します。 （TGT-53844）
* **zh_CN ページエディターのコンテキストで「quote changed」の誤訳を修正しました。** 「見積が変更されました」が「更改了报价」として不正確にレンダリングされるzh_CN ロケールの翻訳エラーを修正しました。これは、商用価格の見積もりを示唆しています。 タイポグラフィ/見出しスタイル/ブロック引用セクションでは、この用語は、価格設定ではなく書式要素（引用ブロック）を指します。 （TGT-53845）
...
+++詳細を見る
* レコメンデーションの&#x200B;**CSS セレクターの変更が正しく保存されるようになりました。** ユーザーがレコメンデーションのプレースメント用にCSS セレクターを更新できないアクティビティ作成UIの問題を解決しました。 保存後に変更が元に戻され、ターゲティングコンテナの更新がブロックされます。 （TGT-53835）
...
+++詳細を見る
* 組織をまたいだデータ露出からGQL エンドポイントを保護するために、**IMS プリフィルターが追加されました。** licenseGroupsおよびtargetProperties GraphQL エンドポイントに影響するAdmin タブのセキュリティ上の脆弱性を解決しました。 この問題は、組織間の製品データにアクセスするために悪用される可能性のある管理者クライアントトークンでJIL APIを使用したことに起因しています。 （TGT-53837）
...
+++詳細を見る
* アクティビティ作成UIで&#x200B;**オーサリングの安定性が復元されました。** オーサリングが失敗し、リンクが予期せずクリック可能になり、ユーザーがページから離れるようにリダイレクトされるVEC UIの断続的な問題を解決しました。 （TGT-53153）
* **アクティビティ作成UIで、保存されたアクティビティの編集が復元されました。**&#x200B;変更を保存した後にユーザーがアクティビティを編集できない問題を解決しました。 影響を受けたアクティビティは「[!UICONTROL Applying initial modifications]」で停止したままとなり、さらに更新がブロックされ、[!UICONTROL Cancel] ボタンが非表示になりました。 （TGT-53631）
* **VECが「[!UICONTROL Applying initial modifications]」で停止しなくなりました。**&#x200B;多数の変更を含むエクスペリエンスを読み込む際に長い遅延が発生するVECのパフォーマンスの問題を解決しました。 影響を受けるユーザーは、特にExperience B シナリオで、UIが「[!UICONTROL Applying initial modifications]」で数分間停止していました。 （TGT-53727）
...
* **アクティビティの変更を保存する機能が正常に動作するようになりました。**&#x200B;新しい作成UIで、ユーザーがアクティビティの目標と詳細設定を編集する際に変更を保存できない権限に関連する問題を解決しました。 影響を受けるユーザーは、適切なアクセス権を持っているにもかかわらず、赤いエラーリボンと「Forbidden.Resource」メッセージが表示されました。 （TGT-53816）
* **VEC UIで、複数のビューにわたってエクスペリエンスの変更が保持されるようになりました。** エクスペリエンス開発に影響を与えた更新されたVECの複数の問題を解決しました。 HTML オファーを使用する場合やビューを切り替える場合は、特に、変更内容が正しく保持されませんでした。 （TGT-53825）
* **変更が複数のエクスペリエンスにまたがる場合、すべてのビューが正しく表示されるようになりました。**&#x200B;変更が複数のビューに適用された場合に1つのビューのみが表示されるアクティビティ作成UIの問題を解決しました。 修正が正しく適用されていても、ホバーツールチップで関連するビューをすべて一覧表示できませんでした。 （TGT-53827）
* **VECが「[!UICONTROL Applying initial modifications]」で断続的に停止しなくなりました。** エクスペリエンスの読み込みに失敗し、「[!UICONTROL Applying initial modifications]」で停止したままになるVECの断続的な問題を解決しました。 この動作は一貫性がなく、リダイレクトループまたは手動キャッシュのクリアが必要な場合がありました。 （TGT-53916）
* **VECの読み込み問題を再現できませんでした。**&#x200B;既存のアクティビティを編集する際に、VECが「[!UICONTROL Applying initial modifications]」で停止したままになる問題を調査しました。 この振る舞いは、親エレメントを欠いたHTMLの内容に関連していると疑われた。 繰り返しを監視し続け、HTML オファーに構造化コンテナを使用して安定性を確保することをお勧めします。 （TGT-53972）
...
* **ボタンが[!UICONTROL Composer] モードでクリックすると、ページのリダイレクトがトリガーされなくなります。**&#x200B;更新されたVEC UIで、[!UICONTROL Composer] モードでボタンをクリックすると、ボタンのターゲット URLへの予期しないリダイレクトが発生する問題を解決しました。 これにより、call-to-action（CTA）要素を編集できず、オーサリングワークフローが中断しました。 （TGT-53137）
* **自動パーソナライゼーションアクティビティのコード更新をエラーなく保存できるようになりました。**&#x200B;新しい作成UIの問題を解決しました。この問題は、[!UICONTROL Invalid user input]のアクティビティでオファーコードを更新する際に「[!UICONTROL Automated Personalization]」エラーを引き起こしました。 入力が有効な場合でも、エラーによりユーザーが変更を保存できなくなります。 （TGT-53586）
* VECの&#x200B;**[!UICONTROL Design]モードで、編集可能なコンポーネントのリンク ナビゲーションがブロックされるようになりました。**&#x200B;更新されたVECで、[!UICONTROL Design] モードでもボタンやリンクなどのクリック可能な要素がページのリダイレクトをトリガーする問題を解決しました。 この動作は[!UICONTROL Browse] モードを模倣し、ユーザーが主要コンポーネントを変更することを妨げました。 （TGT-53696）
* **&quot;[!UICONTROL Clicked an element]&quot;指標が、リダイレクトまたはエラーなしで機能するようになりました。**&#x200B;新しい作成UIで「[!UICONTROL Clicked an element]」コンバージョン指標を選択する際に、予期しないリダイレクトと空白の画面が発生する問題を解決しました。 セットアップ中にボタンをクリックすると、要素を登録する代わりにナビゲーションがトリガーされ、「[!UICONTROL element not found]」エラーが発生しました。 （TGT-53817）
...
* **オーディエンス除外ルールが、[!DNL Target] UIに含まれているとして正しく表示されませんでした。アクティビティ内でターゲティングを編集する際に、除外ルールで設定された**&#x200B;個のオーディエンスが含まれるように表示されました。 除外ロジックは実行中に正しく適用されましたが、UIはルールを正確に反映できず、「除外」ラベルが省略されました。 [!DNL Target] UIで、オーディエンス設定とターゲティングワークフローの両方に除外ルールが正しく表示され、キャンペーン設定の明確性と一貫性が確保されるようになりました。 （TGT-53808）
* **「[!UICONTROL Targeting]」セクションは、オーディエンスルールが除外に設定されていることを示していません。除外ロジックで設定された**&#x200B;個のオーディエンスが、アクティビティ作成UIの[!UICONTROL Targeting] セクションに含まれるように誤って表示されました。 バックエンドが除外ルールを正しく適用しましたが、UIは除外ルールを視覚的に表現できず、「除外」ラベルが省略され、キャンペーンの設定中に混乱が生じました。 [!UICONTROL Targeting] セクションに除外ルールが明確に表示されるようになりました。これにより、オーディエンス設定とターゲティングのビジュアライゼーションの一貫性が確保されます。 （TGT-53809）
...
* **レコメンデーションボックスがVEC内で見つけたり選択したりするのが困難でした。** （VEC）でレコメンデーションオファーを追加した後、左側のパネルで修正をクリックしても、ページ上の対応するレコメンデーションボックスがハイライト表示またはスクロールされませんでした。 これにより、特にセレクターの下に隠されている場合や最小限にスタイル設定されている場合は、オファーを見つけたり編集したりすることが困難になりました。 レコメンデーションの修正をクリックすると、関連する要素が正しくハイライト表示され、スクロールされるようになりました。これにより、更新されたアクティビティ作成プロセスでの使いやすさと編集効率が向上しました。 （TGT-52571）
* **アクティビティの保存後にレコメンデーションセレクターが誤って書き換えられました。** VECの要素にレコメンデーションを追加する際、最初はセレクターが正しかったのですが、アクティビティを保存して再度開くと、汎用セレクターに変更されました。 元のセレクターを手動で復元しようとすると、検証エラーが発生しました。 保存後もレコメンデーションセレクターが正確に保持され、更新されたアクティビティ作成プロセスで信頼性の高いターゲティングと編集性が確保されるようになりました。 （TGT-53709）
* **既存のアクティビティを変更する際に、条件コンテンツを編集できませんでした。** アクティビティを編集すると、[!UICONTROL Criteria] コンテンツセクションが無効になり、ボタンがグレー表示されて応答しなくなります。 この問題は、アクティビティの更新中に[!UICONTROL Criteria]設定が完全に編集可能であることを確認することで解決されました。 お客様は、選択を切り替えたり、回避策を使用したりすることなく、[!UICONTROL Criteria] コンテンツを変更できるようになりました。これにより、更新されたアクティビティ作成プロセスの柔軟性と使いやすさが向上しました。 （TGT-53812）
* アクティビティ内で&#x200B;**条件を編集できませんでした。** アクティビティ内から条件にアクセスすると、[!UICONTROL Edit Criteria]および[!UICONTROL Remove Criteria] オプションが無効になりました。 ただし、[!UICONTROL Recommendations] タブを使用して、同じ条件を正常に編集できます。 条件は、アクティビティ編集ワークフローと[!UICONTROL Recommendations] タブの両方から完全に編集可能になり、一貫性のある効率的な編集エクスペリエンスを実現します。 （TGT-53814）
...
* JavaScript エラーが発生したため、**アクティビティレポートを読み込めませんでした。特定のアクティビティの「**」タブにアクセスすると、[!UICONTROL Reports]のお客様に「問題が発生しました」というメッセージが表示されました。 エラーは、JavaScriptの例外が原因で発生しました：`getAnalyticsReportSummary` GraphQL呼び出し中にトリガーされた、未定義のプロパティを読み取れません（「indexOf」を読み取ります）。 レポートが正しく読み込まれるようになり、更新されたアクティビティ作成ワークフローで同様のエラーが発生するのを防ぐため、エラー処理が改善されました。 （TGT-53797）
* **スクロールバーを操作すると、レポートがクラッシュする。** 「[!UICONTROL Reports]」タブのスクロールバーをクリックすると、ページがクラッシュし、JavaScript エラーが発生しました。
...
* **レポートにプライマリ指標が表示されませんでした。** mboxを使用してコンバージョン指標として設定された主な指標が、アクティビティレポートにありませんでした。 メトリック名またはmbox名で検索しても結果が得られず、主要なパフォーマンスデータが表示されません。 プライマリ指標が[!UICONTROL Reports] タブに正しく表示されるようになり、キャンペーンのパフォーマンスの正確な追跡と分析が可能になりました。 （TGT-53773）
* **更新されたUIの[!UICONTROL Reports] タブが、水平スクロールバーを操作するとクラッシュしました。**&#x200B;水平スクロールバーを使用してビュー外の指標にアクセスする際に、「問題が発生しました」というエラーが表示され、[!UICONTROL Reports] ビューが断続的にクラッシュしました。 スクロールバーが確実に機能するようになり、ズームアウトやShift キーを押しながらスクロールするなどの回避策を必要とせずに、すべての指標を表示および分析できるようになりました。 （TGT-53824）
...
* **コンテキスト メニューがVECに断続的に表示されませんでした。**&#x200B;更新されたVEC UIのコンテキストメニューは、要素をクリックしても常に表示されないため、編集オプションにアクセスするのが困難でした。 エレメントの選択時にコンテキストメニューが確実に表示されるようになり、更新されたアクティビティ作成プロセスでの編集ワークフローと全体的な使いやすさが向上しました。 （TGT-53015）
* **VEC内の特定の要素に対して、コンテキストメニューが表示されませんでした。**&#x200B;更新されたVECで特定の要素を選択すると、コンテキストメニューが表示されず、変更を適用するのが困難になりました。 サポートされているすべての要素に対してコンテキストメニューが一貫して表示されるようになり、更新されたアクティビティ作成ワークフローでの編集エクスペリエンスの信頼性と使いやすさが向上しました。 （TGT-53248）
* VECでパンくずリストを使用すると、最初のクリックで&#x200B;**コンテキストメニューが消えました。** VECのパンくずリストを使用して親エレメントを選択すると、コンテキストメニューが簡単に表示され、その後表示されなくなり、編集オプションにアクセスすることが困難になりました。 パンくずリスト内でエレメントを移動する際に、コンテキストメニューが表示され、機能するようになり、更新されたアクティビティ作成プロセスでエレメント選択ワークフローの信頼性が向上しました。 （TGT-53424）
* **VECの最上位の要素にコンテキストメニューが表示されませんでした。** VECのパンくずリストを使用してトップレベルの要素（`<div>`や`<main>` タグなど）を選択すると、コンテキストメニューがトリガーされず、これ以上の編集が実行されませんでした。 トップレベルのコンテナを含め、サポートされているすべての要素にコンテキストメニューが一貫して表示されるようになり、アクティビティ作成ワークフローの柔軟性と使いやすさが向上しました。 （TGT-53770）
* 特定のページの&#x200B;**要素はVECで編集できませんでした。**&#x200B;更新されたVECで、ページ上の特定の要素を選択または編集できませんでした。 この問題はそのページに分離され、同じアカウント内の他のページには影響しませんでした。 ページ上のすべての要素が、必要に応じて選択および編集可能になり、アクティビティ作成ワークフローの機能を完全に復元できるようになりました。 （TGT-53353）
* **VECでのエレメント選択時に子エレメントを表示する際のワークフローを改善しました。** アクティビティの作成時の使いやすさと精度を向上させるために、親HTML要素の上にカーソルを合わせたり選択したりすると、VECに子要素が表示されるようになりました。 この機能強化により、ユーザーはページの構造をより深く理解し、より正確な変更を行うことができ、更新されたUIでの編集ワークフローが合理化されます。 （TGT-53416）
* 既存のアクティビティの&#x200B;**要素を修正バーを使用して編集できませんでした。**&#x200B;以前に作成したアクティビティを編集する際に、ページ上の特定の要素に対して修正バーをアクティブ化できず、更新が妨げられました。 この問題は主に改変活動で観察され、新しく作成された活動で再現することが困難でした。 修正バーが一貫して表示され、サポートされているすべての要素を編集できるようになりました。これにより、更新されたアクティビティ作成ワークフローで信頼性と使いやすさが向上しました。 （TGT-53013）
...
+++詳細を見る
* **無効なオーディエンス IDが原因でアクティビティをコピーできない問題を修正しました。更新されたアクティビティ作成プロセスでアクティビティをコピーしようとしているお客様が、無効なオーディエンス ID （–1752722444307など）が原因でエラーが発生しました。**&#x200B;このバックエンド検証の問題により、同じワークスペース内のアクティビティの重複が防止されました。 この問題は解決され、オーディエンス関連のエラーなしでアクティビティを正常にコピーできるようになりました。 （TGT-53717）
* **モーダル [!UICONTROL Automated Personalization]の[!UICONTROL Manage Content] アクティビティで、アクティビティのみのオーディエンスに対して無効なユーザー入力エラーが表示される問題を修正しました。**&#x200B;のお客様は、AP アクティビティの[!UICONTROL &#x200B; Manage Content] モーダルでアクティビティのみのオーディエンスを設定する際に、無効なユーザー入力エラーが発生しました。 この問題は、以前はオーディエンスが正常に使用されていたにもかかわらず発生しました。 統合されたオーディエンス設定が、検証エラーをトリガーせずに正しく保存されるようになりました。 （TGT-53749）
...
+++詳細を見る
* **リダイレクトオファーが`redirectOptions`時にペイロードに`includeContent=true`を含めなかった問題を修正しました。**&#x200B;を含むリダイレクトオファーを取得する`includeContent=true `のお客様に、応答ペイロードの`redirectOptions` フィールドがありませんでした。 この不整合は、オファーのコピーやアクティビティの作成などのワークフローに影響を与えます。 コンテンツが要求されたときに、リダイレクトオファーに`redirectOptions`が正しく含まれるようになりました。 （TGT-53737）
...
* **VECで復元された選択したコレクションを削除するオプション。** ユーザーが[!UICONTROL Recommendations] アクティビティで選択したコレクションのみを置き換えることができたが、完全に削除できなかったVECの問題を修正しました。 この制限により、代替なしでコレクションをクリーンに削除する必要があるユースケースがブロックされました。 この修正により、選択したコレクションを削除する明確なオプションが導入され、アクティビティの設定と従来のUIの動作との整合性の柔軟性が向上しました。 （TGT-53652）
* **カスタム条件コレクションが[!UICONTROL Recommendations] UIで正しく表示されるようになりました。** Recommendations インターフェイスで、カスタム条件を使用して作成されたコレクションで製品結果が表示されない問題を修正しました。 標準の属性ベースのコレクションは期待どおりに機能しますが、カスタムフィルターを使用している場合、有効なカタログが一致しているにもかかわらず、「結果が見つかりません」が返されます。 この修正により、カスタム属性を使用したコレクションが[!UICONTROL Product] タブに正しく入力され、パーソナライズされたレコメンデーションワークフローの全機能が復元されるようになりました。 （TGT-53653）
* **最初に[!UICONTROL Products] ページを開いたときにコレクションに製品が表示されない問題を修正しました。** ページを初めて開いたときに、[!UICONTROL Recommendations] セクションのコレクションにアクセスする[!UICONTROL Products]人の顧客に空の製品結果が発生しました。 この問題は、GraphQL クエリのバックエンドエラーが原因で、カスタム条件を持つコレクションの商品データを読み込めませんでした。 問題が解決され、環境の切り替えを必要とせずに製品が正しく表示されるようになりました。 （TGT-53694）
* **フォームベースの[!UICONTROL Recommendations] アクティビティでコレクションを削除できない問題を修正しました。**&#x200B;を使用して[!UICONTROL Recommendations] アクティビティを作成している[!UICONTROL Form-Based Experience Composer]人のお客様が、以前に選択したコレクションの選択を解除できませんでした。 UIでは、保存する前にコレクションを選択する必要があり、ユーザーが「すべてのコレクション」に戻せませんでした。 この動作はVEC機能に合わせて更新され、お客様は選択したコレクションなしで保存でき、期待どおりに「すべてのコレクション」にデフォルト設定されています。 （TGT-53708）
* **コレクションまたはフィルタールール値が欠落しているため、属性でプロモーションを設定できない問題を修正しました。** アクティビティ作成プロセスで属性でプロモーションを構成しているお客様で、プロモーションにコレクション IDまたはフィルタールール値が不足しているというエラーが発生しました。 設定が完了したように見えても、この検証の問題により進行がブロックされます。 属性で設定すると、プロモーションを正常に保存できるようになりました。 （TGT-53750）
* **エクスペリエンス名が重複しているため、アクティビティを保存できない問題を修正しました。**&#x200B;のお客様は、条件とデザインの特定の組み合わせを含むアクティビティを保存する際に、無効なユーザー入力エラーが発生しました。 設定が有効と表示された場合でも、エクスペリエンス名が重複していることによってエラーがトリガーされました。 異なる設定を持つアクティビティを、名前の競合なしで保存できるようになりました。 （TGT-53805）
* **属性で設定されたプロモーションの検証が無効のままになる問題を修正しました。**&#x200B;すべての必須フィールドが正しく入力されている場合でも、アクティビティ作成プロセスで属性によるプロモーションを設定する際に、顧客に永続的な検証エラーが発生しました。 この問題は、[!UICONTROL Recommendations] ワークフローの検証ロジックが正しくないことが原因で発生しました。 属性ベースのプロモーションが検証され、期待どおりに保存されるようになりました。 （TGT-53811）
* **ライブ [!UICONTROL Recommendations] アクティビティにプロモーションを適用するとエラーが発生する問題を修正しました。**&#x200B;のお客様は、有効な設定の詳細を指定した後でも、ライブ [!UICONTROL Recommendations] アクティビティにフロントプロモーションを適用しようとすると、「プロモーションにコレクション IDまたはフィルタリングルール値が見つかりません」というエラーが発生しました。 検証エラーをトリガーすることなく、ライブアクティビティにプロモーションを正常に適用できるようになり、更新されたアクティビティ作成UIでよりスムーズなエクスペリエンスを実現できます。 （TGT-53738）
* **[!UICONTROL Recommendations] UI内のコレクションに製品が表示されない問題を修正しました。単一テナントの**&#x200B;のお客様から、新しいUIの[!UICONTROL Recommendations] セクションで特定のコレクションを表示する際に、製品リストの読み込みに失敗したことが報告されました。 環境を切り替えることで一時的に問題が解決されましたが、この回避策ではユーザーエクスペリエンスが低下しました。 環境の切り替えを必要とせずに、製品エンティティが一貫して読み込まれるようになりました。 （TGT-53783）
...
+++詳細を見る
* アクティビティレポートからCSV書き出しに&#x200B;**[!UICONTROL Total Revenue]指標が含まれるようになりました。**&#x200B;更新された[!UICONTROL Overview] UIで、合計収益がアクティビティレポートビューに正しく表示されたが、CSV エクスポートから$0と表示される問題を解決しました。 この不一致により、ユーザーはオフライン分析とレポート用に書き出されたデータに頼ることができませんでした。 （TGT-53325）
* アクティビティレポートからCSV書き出しに&#x200B;**[!UICONTROL Total Sales]指標が含まれるようになりました。**&#x200B;更新されたUIで、[!UICONTROL Total Sales]指標がアクティビティレポートビューに正しく表示されていたが、CSV エクスポートから見つからなかった問題を解決しました。 この不一致により、ユーザーはダウンロードされたレポートの完全なパフォーマンスデータにアクセスできませんでした。 この修正により、[!UICONTROL Total Sales]個の値がCSV書き出しに正確に含まれるようになり、アプリ内レポートとオフライン分析の一貫性が復元されます。 （TGT-53330）
* **指標が有効になっていない場合の[!UICONTROL Graph View]のエラーメッセージを改善しました。**&#x200B;関連する[!UICONTROL Graph View] レポートスイートでリクエストされた指標が有効になっていない場合、[!DNL Analytics]が汎用の「問題が発生しました」メッセージを表示するVECの問題を修正しました。 この問題は、バックエンド GraphQLの応答で`not_enabled_metric` エラーがトリガーされました。 この修正により、あいまいなエラーは、ユーザーが[!DNL Analytics]の設定の問題を特定するのに役立つより有益なメッセージに置き換わり、混乱を減らし、不必要なサポートのエスカレーションを減らすことができます。 （TGT-53577）
* **レポート期間がサポートされている90日間の制限を超えた問題を修正しました。** セクションの「[!UICONTROL Last X Days]」フィルターを使用している[!UICONTROL Reports]人のお客様が90日を超える期間を選択できたため、パフォーマンスの問題やデータが不完全になる可能性があります。 フィルターが更新され、最大90日間が適用され、一貫性のある信頼できるレポートが作成されるようになりました。 （TGT-53795）
* **選択した環境ではなく、デフォルト環境を使用してパフォーマンス CSV レポートが生成される問題を修正しました。**&#x200B;以前、お客様がレポート設定の環境を変更し、パフォーマンスレポートを生成した場合、結果のCSVには、選択した環境ではなく、デフォルト環境のデータが含まれていました。  UIが`environmentId` パラメーターを正しく渡すようになり、選択した環境がレポートに反映されるようになりました。 さらに、エラー処理が改善されました。 CSVの生成中にGraphQL エラーが発生した場合、空のCSV ファイルを生成する代わりに、UIに明確なエラーメッセージが表示されるようになりました。 （TGT-53064）
* **Analytics for Target （A4T）のレポートで[!UICONTROL Graph View]のデータを表示できなかった問題を修正しました。A4T統合で**&#x200B;を使用している[!DNL Target]のお客様で、A/B テストアクティビティの「レポート」タブでグラフ表示に切り替える際に、「問題が発生しました」エラーが発生しました。 [!UICONTROL Table View]が正しく読み込まれましたが、[!UICONTROL Graph View]は、選択した時間範囲の指標の傾向をレンダリングできませんでした。 [!UICONTROL Graph View]は、サポートされているすべての指標について、想定どおりにパフォーマンスデータを表示するようになり、更新されたUIでの可視性とレポート精度が向上しました。 （TGT-53573）
...
+++詳細を見る
* **要素のメタデータが、パンくずメニューのホバーに表示されるようになりました。** VECのパンくずメニューを改善して、項目にカーソルを合わせたときに、ID、クラス、名前などの追加の要素の詳細を表示できるようにしました。 この機能強化により、アクティビティの設定中に、より簡単に要素を識別および区別できるようになります。 （TGT-53409）
* **パンくずホバーで、VEC内の対応する要素がハイライト表示されるようになりました。** パンくずメニューの項目にカーソルを合わせると、エディターで対応する要素がハイライト表示されるように[!UICONTROL Visual Experience Composer]が改善されました。 エレメントの種類（コンテナ、太字テキスト、ボタンなど）も表示されます。 このビヘイビアーは兄弟エレメントに適用され、検証リストに基づいて、SVGなどのサポートされていないタイプを除外します。 （TGT-53411）
* **保存されていない変更アラートがVEC修正ワークフローで復元されました。** カスタムコード変更の未保存の変更についてユーザーに通知されなくなったVECの問題を修正しました。 従来のUIとは異なり、新しいエクスペリエンスでは、パーソナライゼーションエディターから移動したり、閉じたりする際にプロンプトが表示されないため、進行状況が誤って失われていました。 この修正により、カスタムコードを含むすべての変更タイプに対するアラートが復元され、保存せずに終了する前にユーザーに警告が表示されます。 （TGT-53435）。
...
* **VEC内のサイト アクセスに関するログインの問題を解決しました。** ユーザーがVEC経由でサイトにアクセスする際にサイトにログインできない問題を修正しました。 ログインフローは、ユーザーをログインページに繰り返しリダイレクトし、アクティビティの設定とプレビューを妨げました。 この修正により、VECがログイン動作を妨害しなくなり、認証済みユーザーに期待されるアクセスが復元されます。 （TGT-53524）
...
* **特定のホームページの要素をクリックするとメモリリークが発生する問題を修正しました。このホームページでアクティビティを作成しているお客様は、ページ要素を操作する際にメモリリークが発生しました。**&#x200B;この問題は、過剰なコンソール警告と、サブフレームでのメモリ使用量の増加に関連しており、特に不正なsrcset属性に関連していました。 インタラクション中もメモリ使用量が安定したままになります。 （TGT-53761）
* **特定のアクティビティを読み込むと、VECがクラッシュし、空白の画面が表示される問題を修正しました。特定のテナントの**&#x200B;人の顧客から、エディターが特定のアクティビティを読み込めなかったことが報告されました。 VECがクラッシュして空白の画面が表示される前に、「初期変更の適用」で停止したままになりました。 VECは、更新されたアクティビティ作成プロセスでエラーなく、影響を受けるアクティビティを読み込むようになりました。 （TGT-52932）
* **アクティビティ [!UICONTROL Manage Content]の[!UICONTROL Automated Personalization] パネルに一貫性のない場所ラベルが表示される問題を修正しました。**&#x200B;人のお客様から、[!UICONTROL Manage Content]のパネルに、[!UICONTROL Experiences]と[!UICONTROL Offers]のタブで位置番号が一致していないと報告されました。 （例：エクスペリエンスの場所2と場所4と、オファーの場所1と場所2の場合） 2つの場所のみが設定されている場合でも、 場所ラベルが一貫性を持ち、変更に正確にマッピングされるようになり、アクティビティ作成プロセスにおける明瞭性と使いやすさが向上しました。 （TGT-52934）
* **保存後にVECの変更が失われる問題を修正しました。**&#x200B;のお客様から、VECで変更を保存した後、ページが更新され、変更が以前のバージョンに戻ると報告されました。 この問題により、アクティビティ全体がすぐに保存されない限り、最新の更新が失われます。 変更内容は保存後も正しく保持されるようになり、エディターが変更を予期せず元に戻すことがなくなり、アクティビティ作成ワークフローで信頼性の高いエクスペリエンスを確保できるようになりました。 （TGT-53500）
...
このアップデートにより、設定の一貫性が確保され、マルチサンドボックス環境での競合を防ぐことができます。 詳しくは、[Adobe Target Destinations](https://experienceleague.adobe.com/ja/docs/experience-platform/destinations/catalog/personalization/adobe-target-connection){target=_blank} ガイドの&#x200B;*Experience Platform connection*&#x200B;を参照してください。
...
+++詳細を見る
* **[!UICONTROL AEM Experience Fragment]件のオファーのユーザー属性を改善しました。** [!UICONTROL Last updated] （XF）オファーの「[!UICONTROL AEM Experience Fragment]」フィールドに、ユーザー名ではなくコード IDが誤って表示されるVECの問題を解決しました。 この不一致は、更新されたUIでのオファーの選択中に発生し、従来のUIとHTML オファーとの不整合が生じ、最後のエディターの名前が正しく表示されました。 この修正により、オファータイプ間で一貫したユーザーアトリビューションが保証され、透明性が向上し、期待される行動との整合性が高まります。 （TGT-52880 および TGT-52898）
...
* **ODS オファーのレンダリングの問題を解決しました。**&#x200B;更新されたUIでVECを使用してアクティビティを構築した場合、[!DNL Offer Decision Service] （AJO）を介して作成された[!DNL Adobe Journey Optimizer] （ODS）オファーがレンダリングされない問題を解決しました。 従来のUIでも同じ設定が正しく機能するため、混乱が生じ、キャンペーンの実行がブロックされました。 この修正により、両方のバージョンで一貫したオファー配信が可能になり、AJO主導のパーソナライゼーションに期待される動作が復元されます。
...
+++詳細を見る
* **更新されたレポートの概要UIの「レポート」セクションにダウンロードオプションが復元されました。**&#x200B;新しい概要UIで[!UICONTROL Download] ボタンがレポートセクションに見つからないという問題を解決し、ユーザーが属性重要性スコアを書き出せなくなりました。 このアップデートにより、完全な書き出し機能が復元され、分析とレポート用にダウンロード可能なデータに合理的にアクセスできるようになります。 （TGT-53222）
* **[!UICONTROL Download full CSV report]ボタンが[!UICONTROL Important attributes] ビューで復元されました。**&#x200B;更新されたアクティビティ作成UIで、レポートビューの[!UICONTROL Download full CSV report] セクションに[!UICONTROL Important Attributes] ボタンが見つからない問題を解決しました。 この修正により、ダウンロード可能なインサイトへのアクセスが復元され、更新されたUIと従来のUIの両方で一貫した機能が保証されます。 （TGT-53238）
* **更新された概要UIの[!UICONTROL Auto Target]のレポートに影響するUIの問題を解決しました。**&#x200B;更新された概要インターフェイスの複数のUIの問題を修正し、[!UICONTROL Auto Target]のアクティビティレポートに影響を与えました。 これらの修正には、次のものが含まれます。
...
+++詳細を見る
* 更新されたUIのパラメーターのプレゼンス条件に対する&#x200B;**フォームの検証が修正されました。**&#x200B;更新されたUIで「[!UICONTROL Custom mbox parameter value is present]」または「[!UICONTROL Parameter is present]」を選択すると、お客様に値の入力が誤って要求される問題を解決しました。 この動作は、値に関係なくパラメーターの存在を単純にチェックする意図されたロジックと競合しました。 この修正により、フォームの検証と期待される動作が一致し、よりスムーズなアクティビティの設定が可能になり、更新されたUIの完全な導入がサポートされます。 （TGT-53638）
* **ページ配信時のパラメーター存在ルールに対するフォームロジックが修正されました。」**&#x200B;更新されたUIで、「[!UICONTROL Parameter is present]」、「[!UICONTROL Parameter is not present]」、「[!UICONTROL Parameter value is present]」、「[!UICONTROL Parameter value is not present]」などのページ配信ルールを選択する際に、ユーザーが誤って追加パラメーター値を入力する必要があった問題を解決しました。 この動作は、従来のUIと矛盾しており、値を指定せずにパラメーターの存在を検出するという意図されたロジックと矛盾していました。 この修正により、期待されるルール設定の動作が復元され、アクティビティの設定が合理化され、ユーザビリティが向上します。 （TGT-53640）
* **更新されたUIのマルチページ ルール ビルダーの検証ロジックが改善されました。**&#x200B;更新されたUI内のマルチページルールビルダーで、複数の検証の問題を解決しました。 これらの修正には、次のものが含まれます。
...
* **A/BおよびMVT アクティビティで解決された場所の名前変更の問題。**&#x200B;更新されたUIで、[!UICONTROL A/B Test]または[!UICONTROL Multivariate Test] （MVT）アクティビティ内の場所の名前を変更しても、場所リスト、ターゲティング、および戻りに移動した後に保持されないバグを修正しました。 このアップデートにより、場所の名前の変更が保存され、アクティビティワークフロー全体で一貫して反映されます。 （TGT-52367）
* **更新されたUIで、MVTおよびAP アクティビティの場所の名前の永続性が修正されました。**&#x200B;更新されたUIで、[!UICONTROL Multivariate Test] （MVT）および[!UICONTROL Automated Personalization] （AP）アクティビティで編集された場所名が正しく保存されない問題を解決しました。 場所の名前を変更した後、[!UICONTROL Targeting]や[!UICONTROL Experiences]などのタブ間を移動すると、名前が以前の状態に戻りました。 この修正により、タブ間で一貫した場所の命名が保証され、アクティビティの設定中に信頼性が向上します。 （TGT-52927）
* **MVT アクティビティの管理エクスペリエンス パネルで場所のラベル付けが修正されました。**&#x200B;更新されたUIで、[!UICONTROL Manage Experiences] （MVT）アクティビティの[!UICONTROL Multivariate Test] パネルに一貫性のない場所の番号付けが表示される問題を解決しました。 この修正により、位置情報ラベルが順次に行われ、ユーザー定義の変更と正しく整合することが保証され、エクスペリエンスのセットアップ時の明瞭性と使いやすさが向上します。 （TGT-52929）
...
* **将来にスケジュールされたアクティビティが、[!UICONTROL Live]概要[!UICONTROL Activity]に「**」ステータスを誤って表示しました。お客様は、将来に開始するようにスケジュールされたアクティビティが、[!UICONTROL Live]概要で「[!UICONTROL Activity]」として誤ってマークされていることに気付きました。 このステータスの不一致は解決され、スケジュールされたアクティビティは、ページの更新を必要とせずに「[!UICONTROL Scheduled]」として正しく表示されるようになりました。 （TGT-52835）
...
* **[!UICONTROL Graph view]セクションの[!UICONTROL Reports]で[!DNL Analytics]**&#x200B;の値が表示されませんでした：Re[!UICONTROL Graph view] ポート セクションのにアクセスしているお客様で、すべての値がゼロとして表示されるデータが見つかりませんでした。 この問題は、[!UICONTROL Analytics]からの誤ったデータ取得が原因で発生しました。 [!UICONTROL Graph view]が期待どおりに正確な値を表示するようになりました。 （TGT-52792）
+++
...
* **Backspaceを使用してすべての日付値を消去すると、ページがクラッシュする**:Backspaceを使用して「[!UICONTROL Goals & Settings]」フィールドからすべての値を消去すると、[!UICONTROL Specified Date & Time] セクションのスケジュール設定アクティビティがクラッシュする。 この問題は、日付処理ロジックのnull参照エラーが原因で発生しました。 ページがクラッシュすることなく、空の日付入力を適切に処理するようになりました。 （TGT-53624）
* **無効なペイロードが原因で[!UICONTROL Product Catalog Search]に商品が表示されませんでした**: [!UICONTROL Recommendations]の[!UICONTROL Product Catalog Search] セクションにアクセスしているお客様が、無効なGraphQL ペイロードが原因で空の結果が発生しました。 このバックエンドエラーにより、製品データが正しく読み込まれませんでした。 更新されたUIで、製品が期待どおりに表示されるようになりました。 （TGT-53630）
* 更新されたVEC **[!DNL Scene7]では、**&#x200B;個の画像が低解像度で保存されました。更新されたVECでエクスペリエンスを編集しているお客様は、[!UICONTROL Scene7]個の画像URLが解像度パラメーターなしで保存され、低画質の画像が配信されたことに気づきました（意図した800×800ではなく400×400）。 画像URLは、適切な解像度を確保するために正しいパラメーターを保持するようになりました。 （TGT-52631）
...
* **更新されたVEC[!UICONTROL Failed]の[!UICONTROL Draft]および** アクティビティ セクションを廃止しました：[!UICONTROL Failed]および[!UICONTROL Draft] アクティビティ オプションが更新されたVECから削除されました。 新しいUIでは、ドラフト状態がサポートされなくなり、失敗したキャンペーンはバックエンドに保存されません。 これらのオプションは、もはや関連性がありません。 関連するフィルターとバックエンドフィールド （例：`uiSyncFailed`、`errorMessage`）も削除され、アクティビティ管理が効率化されました。 （TGT-53150）
...
* **スケジュールされたアクティビティが、ページの更新[!UICONTROL Live]まで** ステータスを正しく表示しない問題を修正しました。お客様は、アクティビティをスケジュールして将来の日時に公開すると、ステータスが[!UICONTROL Live]ではなく[!UICONTROL Scheduled]に直ちに表示されることを確認しました。 これにより、アクティビティがスケジュールされていることを確認メッセージが正しく示していたにもかかわらず、混乱が生じました。 ページを更新すると、ステータス表示が修正されました。 この問題は解決され、スケジュールされたアクティビティは、更新を必要とせずにスケジュールされたステータスを正しく表示するようになりました。 （TGT-52937）
...
* **保存後に[!UICONTROL Offer Decision] セレクターを変更できず、予期せず変更される問題を修正しました**：更新されたアクティビティ作成プロセスで、お客様は[!UICONTROL Offer Decision] セレクターを意図したとおりに変更できませんでした。 セレクターの変更が失敗し、保存後にセレクターが誤った値に戻りました。 この動作により、決定オファーがVisual Experience Composer （VEC）から消え、さらに編集がブロックされました。 セレクターの変更が正しく保存され、保存後もVECで決定オファーが表示され、編集可能な状態が維持されます。（TGT-53433）
...
* **データを含まない[!DNL Recommendations] [!UICONTROL Promotion]を有効にすると、不明確なエラーメッセージ**&#x200B;がトリガーされる問題を修正しました。必要な値を指定せずに[!UICONTROL Front] アクティビティで[!UICONTROL Back Promotion]または[!DNL Recommendations]を有効にすると、一般的な「無効な入力エラー」メッセージが表示されます。 根本的な問題は設定フィールドがありませんでしたが、エラーメッセージに原因が明確に示されていないため、トラブルシューティングが困難でした。 アクティビティ作成プロセスで、`collectionId`やルールなどの必須フィールドが見つからない場合に、明確で実用的なエラーメッセージが表示されるようになりました。これにより、お客様は設定の問題をすばやく特定して解決できます。 （TGT-52616）
* **[!UICONTROL Product] タブ内の[!UICONTROL Edit] モーダルに[!UICONTROL Recommendations] リストが表示されない問題を修正しました**：顧客が[!UICONTROL collection] タブで[!UICONTROL exclusion]または[!UICONTROL Recommendations]を編集する際に、フィルターされた製品リストを表示できませんでした。 リストは、適用されたルールに基づいてリアルタイムで更新されることを想定していましたが、意図したとおりに表示されませんでした。 この問題は解決され、ルールが変更されると、製品リストが正しく表示され、動的に更新されるようになりました。 （TGT-53481）
...
+++詳細を見る
* **AP アクティビティの[!UICONTROL Targeting] ステップへの進行をブロックするアクティビティ作成プロセスの問題を修正**：顧客が[!UICONTROL Targeting] （AP）アクティビティの[!UICONTROL Automated Personalization] ステップに進めないアクティビティ作成プロセスの問題を修正しました。2つの場所が追加されました。 この動作は、複数のオファーを含む単一の場所で十分だった以前のエクスペリエンスとは異なっていました。 お客様がAP ワークフローの一部として単一の場所の設定を引き続き使用できるようにするための要件が修正されました。 （TGT-53426）
...
* **更新されたUIでレポートスイートを検索できない問題を修正しました**：更新されたUIでは、[!UICONTROL Report Suites] セクションの[!UICONTROL Goals & Settings] ドロップダウンリストで、顧客が入力および検索できず、特定のレポートスイートを見つけることが困難でした。特に、エントリ数が多いテナントの場合は。 以前のUIとは異なり、リストは並べ替えられておらず、入力ベースのフィルタリングが不足していました。 この問題は解決されました。 お客様は、レポートスイートを検索およびフィルタリングして、従来のUIで使用できる機能を復元できるようになりました。 （TGT-53514）
...
* **クリックトラックモードでのトラッキングされた要素フィールドの動作を修正して、未保存の入力を防ぎ、明瞭性を向上しました**: アクティビティ作成プロセスで、[!UICONTROL Tracked Element] モードの[!DNL Click Track] フィールドは編集可能でしたが、入力された名前が保持されず、顧客に混乱が生じる問題を修正しました。 未保存の入力を防ぐためにフィールドが無効になり、選択した要素の命名が明確になり、目標設定とトラッキング精度が向上しました。** （TGT-53458）
...
+++詳細を見る
* **AEMで書き出されたフラグメントの意図しないHTML編集を許可するアクティビティ作成プロセスの問題を修正**：お客様が[!DNL Experience Fragments] （AEM）から[!DNL Adobe Experience Manager]内に書き出された[!DNL Target] （XF）のHTMLを編集できるアクティビティ作成プロセスの問題を修正しました。 AEMから公開されたXFは引き続きロックされ、編集できなくなります。 この修正により、AEMで書き出されたフラグメントで「HTMLを編集」オプションが使用できなくなり、コンテンツの整合性と想定されるガバナンスが維持されます。 （TGT-53286）
...
+++詳細を見る
* **高度な検索で提案を提供できなかったカタログ検索UIの問題を修正しました**: [!UICONTROL Catalog Search]機能で提案を提供できなかった新しい[!UICONTROL Advanced Search] UIの問題を修正しました。 ユーザーは、正確なスペルを入力して正確な値を入力する必要があり、検索が煩雑になり、ミスが発生しやすくなりました。 この修正により、[!UICONTROL Advanced Search]は、ユーザーが入力する際に関連する提案を提供するようになり、使いやすさが向上し、製品を探す際の摩擦が軽減されました。 （TGT-52008）
...
* **商品メッセージをレコメンデーションビューに表示できなかったアクティビティ作成プロセスの問題を修正しました**: メッセージが存在していたにもかかわらず、[!UICONTROL Message] ビューの[!DNL Recommendations]列に商品データが表示されなかったアクティビティ作成プロセスの問題を修正しました。 顧客は、コンテンツを一時的に表示するために、列を手動で削除したり再追加したりしなければならず、スクロールや検索時に再度表示されがちでした。 このアップデートにより、製品メッセージの一貫性のある可視性が復元され、カタログのナビゲーションとレビューのワークフローが改善されます。 （TGT-52777）
...
* 更新された[!DNL Target] UIで、[!UICONTROL Visual Experience Composer] タブから[!UICONTROL Targeting]に戻った後、[!UICONTROL Experiences] （VEC）で行われたHTML オファーの変更が保持されない問題を修正しました。 （TGT-52874）
...
* フィード作成ウィザードの[!UICONTROL All host groups] ドロップダウンリストにある文字列「[!UICONTROL Environment]」のローカライズの問題が正しくローカライズされない問題を修正しました。 （TGT-46737）
...
* [!UICONTROL Viewed an mbox]の下の[!UICONTROL Goals & Settings]変換指標を選択すると、ページがクラッシュする問題を修正しました。 （TGT-53346、TGT-53343、TGT-53348）
...
+++詳細を見る
* ビルダークラスの`buildViews` メソッドが、割り当てられた最も高い`viewMaxLocalId`ではなく、ビューの合計数`viewLocalId`を誤って設定する問題を修正しました。 （TGT-53207）
...
+++詳細を見る
* [!UICONTROL Form-Based Experience Composer] （AP）アクティビティの作成時または編集時に&#x200B;**[!UICONTROL Manage Content]** アイコン （![&#x200B; コンテンツ管理アイコン &#x200B;](/help/main/assets/icons/Experience.svg)）をクリックすると、エディターがクラッシュする原因となる[!UICONTROL Automated Personalization]の問題を修正しました。 （TGT-53047）
...
* ユーザーが[!DNL Recommendation] UIでsite_cart_z1 [!DNL Target] アクティビティを編集できない問題を修正しました。 アクティビティを開こうとすると、[!UICONTROL Overview] ページでエラーが発生し、エディターへのアクセスがブロックされました。 （TGT-53221）
...
* VECでイメージオファーを設定する際の[!UICONTROL Undo]および[!UICONTROL Insert Before] オプションの[!UICONTROL Insert After]機能に関する問題を修正しました。
...
* レガシー機能を[!UICONTROL Priority]の[!UICONTROL Goals & Settings] スライダーに復元し、お客様がレガシーUIでサポートされているように、数値を直接入力できるようにしました。 （TGT-53185 および TGT-53219）
...
* コレクションまたはプロモーションを変更する際に、[&#x200B; アクティビティの](/help/main/c-recommendations/recommendations-as-an-offer.md) オファー[!UICONTROL A/B Test]のレコメンデーションが正しく更新されない問題を修正しました。 （TGT-52884）
...
* トラフィック配分の設定中に[!UICONTROL Auto-Target] （AA）が最初に選択された場合に、ユーザーが[!UICONTROL Auto-Allocate] （AT）アクティビティを作成できない問題を修正しました。 この問題により、バックエンドの検証エラーが発生し、アクティビティが保存されなくなります。 （TGT-53096）
...
+++詳細を見る
* 既存のアクティビティをコピーし、レポートソースを[!DNL Adobe Analytics] （A4T）に変更すると、「無効なユーザー入力」エラーが発生する問題を修正しました。 [!DNL Analytics]、`restart_same_experience`、`restart_random_experience`など、`restart_new_experience`のレポートと互換性のない特定の指標アクションが元のアクティビティから保持されたときに、エラーがトリガーされました。 （TGT-52900）
* [!DNL Adobe Analytics] ステップでレポートソースとして[!UICONTROL Goals & Settings] （A4T）を選択すると、顧客がアクティビティの作成または保存をブロックする問題を修正しました。 この問題は、特に[!UICONTROL Custom Event]指標（例：「カスタムイベント 16」）を選択する際に発生し、「無効なユーザー入力」というエラーが発生しました。 （TGT-52910）
...
* 更新された[!DNL Recommendations] UIで[!UICONTROL Overview] アクティビティを表示する際の問題を修正しました。[!UICONTROL Goals & Settings] （A4T）がレポートソースとして選択されていると、[!DNL Adobe Analytics] セクションの読み込みに失敗します。 次のエラーメッセージが表示されました：「問題が発生しました。 ご要望の処理は完了できません。 問題が解決しない場合は、アドビのカスタマーケアにお問い合わせください。」 （TGT-52999）
...
* 更新された[!UICONTROL Overview]UIを使用する際に、[!UICONTROL Download Recommendations Data]を含む[!UICONTROL Experience Targeting] （XT）アクティビティに「[!DNL Recommendations]」ボタンが表示されない問題を修正しました。 （TGT-52730 および TGT-52756）
...
+++詳細を見る
* [!UICONTROL Export order details to CSV] ページから「[!UICONTROL Reports]」を選択すると、空のファイルがダウンロードされる問題を修正しました。 この問題は、有効な注文データがアクティビティに存在する場合でも発生しました。 （TGT-52225）
...
* ユーザーが別の指標を選択した後でも、[!UICONTROL Revenue]手順の[!UICONTROL Goals & Settings]指標ドロップダウンが[!UICONTROL Revenue per Visit] （RPVISIT）に間違ってデフォルト設定される問題を修正しました。  この問題は、指標の設定パネルを折りたたんで再展開すると発生し、以前に選択した値がリセットされます。 （TGT-52811 および TGT-52878）
...
* 更新された[!UICONTROL Rearrange] （VEC） UIに[!UICONTROL Visual Experience Composer] オプションを追加し、従来のVECで利用できる機能に合わせました。 （TGT-46957 および TGT-52876）
...
* 特定の顧客がアクティビティを作成または保存できず、他の顧客が同じアクションを問題なく実行できる問題を修正しました。 問題はアカウント間で一貫性がなかった。（TGT-52842）
...
* 以前に削除したエクスペリエンスから[!UICONTROL Multivariate Test]を再利用できない`experienceLocalId` （MVT）アクティビティの問題を修正しました。 （TGT-52672）
...
* 更新された[&#x200B; UIと](/help/main/c-intro/updated-ui-faq.md) （VEC）に関する一般的な質問に対応するために、[!DNL Target]新しいFAQ記事[!UICONTROL Visual Experience Composer]を追加しました。
* [!UICONTROL URL - does not contain]の「[!UICONTROL Page Delivery]」ルールが機能しない問題を修正し、ブロックする必要がある場合でもコンテンツを表示できるようにしました。 （TGT-52754）
...
* `optionLocalIds`で参照されているすべての`optionGroups`がoptions配列に存在することを確認するための検証を追加しました。 無効な参照は、アクティビティの作成中に自動的に削除されます。 （TGT-52687）
...
* レコメンデーションを含む[!UICONTROL Download Recommendations data] （XT）アクティビティで「[!UICONTROL Experience Targeting]」オプションが見つからない問題を修正しました。 （TGT-52730 および TGT-52756）
...
* 9月15日（PDT）から10月6日（PDT）の9[!DNL Adobe Target Premium]午前（PDT）までの[!UICONTROL Auto-Target] ユーザーの:30のレポートに影響を与えた問題を修正しました。 :25影響を受けたコンバージョン指標のレポート（「[!UICONTROL Viewed a page]」または「[!UICONTROL Clicked on mbox]」オプションを使用して設定）を表示すると、コンバージョン率が誤ってレポートされます。現時点では、配信に関する既知の問題はありません。
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| ![Premium バッジ](/help/main/assets/premium.png)ユーザーベースのレコメンデーション<br>（2019年10月24日（PT）） | 各訪問者のブラウジング、表示および購入履歴に基づいて項目をレコメンドします。これらの項目は、一般に「あなたへのおすすめ」と呼ばれます。<br>この条件により、新規訪問者と再訪問者の両方に対して、パーソナライズされたコンテンツとエクスペリエンスを提供できます。レコメンデーションのリストは、訪問者の最新のアクティビティに対して重み付けされます。また、セッション内で更新され、訪問者がサイトを閲覧するにつれてより詳細にパーソナライズされます。<br>詳細情報は、[条件／アルゴリズム](/help/main/c-recommendations/c-algorithms/algorithms.md#criteria-algorithms)の「ユーザーベースのレコメンデーション」を参照してください。 |
...
<table id="table_DDC5356FD6B8443EAA6EB81C03ADF73A"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>CSV ダウンロードでは、エンティティレコメンデーションを持たない環境（例：
<code>
       &#x200B;# environment: 1724 
