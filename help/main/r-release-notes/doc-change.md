...
| 日付 | トピック | 変更点 |
| --- | --- | --- |
| 2月28日（PT） | [[!DNL Target] リリースノート（プレリリース）](/help/main/r-release-notes/target-release-notes.md) | [!DNL Target] Standard／Premium 24.3.1 リリース（2024年3月4～6日（PT））に関する情報を追加しました。 |
| 2月26日（PT） | [[!DNL Adobe Target]  のお知らせとイベント](/help/main/r-release-notes/target-announcements.md) | 次回の [!UICONTROL Adobe Target Community] Coffee Break（2024年2月28日（PT）開催）に関する情報を追加しました。 |
| 2月23日（PT） | [ [!DNL Recommendations]  フィード処理サーバーで使用される IP アドレス](/help/main/c-recommendations/c-recommendations-faq/ip-addresses-marketing-cloud.md) | 次の重要な注意事項と、許可リストに登録する必要がある新しい IP アドレスを追加しました。<P>**重要**：[!DNL Target] チームは現在、[!DNL Recommendations] フィードをダウンロードするための NAT ゲートウェイアドレスをアップデートしています。IP 許可リストを実装する場合は、次の新しい AWS ホストを必ず許可リストに登録してください。既存のホストは 2024年6月30日（PT）に廃止される予定です。スムーズな移行を実現するには、9 つのアドレスをすべて許可リストに登録します。既存のアドレスを緊急に削除する必要はありません。 |
...
| 1月22日（PT） | [Target リリースノート（現行）](/help/main/r-release-notes/release-notes.md) | 2024年4月30日（PT）までにお客様側での変更が必要となる、[!UICONTROL Browser] オーディエンス属性からの iPad と iPhone の非推奨（廃止予定）に関する重要な情報を追加しました。 |
...
| 11月29日（PT） | [プロファイル一括更新 API](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/methods/bulk-profile-update-api.html?lang=ja){target=_blank} | [!UICONTROL Bulk Profile Update API] v2 を使用する場合、[!DNL Target] でまだ確認していないユーザーのプロファイルを作成する際に、[!DNL Target] で顧客属性を処理する方法と v1 の違いを明確にしました。 |
...
| 11 月 17 日 | [管理者の最初の手順](/help/main/administrating-target/start-target.md) | 次の重要な注意点を追加しました。<ul><li>[!DNL Adobe Admin Console] の [!UICONTROL Product Admin] 権限または [!UICONTROL System Admin] 権限を持つユーザーは、[!DNL Target] の役割に関係なく、[!DNL Target] の [!UICONTROL Administration] ページのすべての設定を編集または変更できます。[!DNL Adobe Admin Console] の [!UICONTROL Product Admin] 権限または [!UICONTROL System Admin] 権限を持たないユーザーがこれらの変更を行うには、特定の [!DNL Target] の役割が必要です。1</li></ul> |
...
| 10月25日（PT） | [[!DNL Target] リリースノート（最新）](/help/main/r-release-notes/release-notes.md) | [!UICONTROL Activities]ページのユーザーインターフェイスのアップデート（2023年10月25日（PT））に関する情報を追加しました |
...
|  | [ [!DNL Analysis Workspace]  での [!UICONTROL Auto-Allocate] アクティビティ用 A4T レポートの設定](https://experienceleague.adobe.com/docs/target-learn/tutorials/integrations/set-up-a4t-reports-in-analysis-workspace-for-auto-allocate-activities.html?lang=ja){target=_blank} | *[!UICONTROL Adobe Target Tutorials]* ガイドのチュートリアル全体を更新しました。 |
...
| 9月22日（PT） | [[!UICONTROL Enhanced Experience Composer]](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/troubleshooting-issues-related-to-the-enhanced-experience-composer-eec.md#section_D29E96911D5C401889B5EACE267F13CF) に関連する問題のトラブルシューティング | IP アドレスのリストを更新し、許可リストに加えました。 |
...
|  | [オファー](/help/main/c-experiences/c-manage-content/manage-content.md) | 画像オファーが [!UICONTROL Enterprise User Permissions] モデルの一部ではないことを説明するメモを追加しました。 |
