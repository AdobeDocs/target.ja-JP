...
文字制限およびその他の制限(オファーサイズ、オーディエンス、プロファイル、値、パラメーターなど) [!DNL Adobe Target] でのアクティビティやその他の要素に影響する情報を含みます。
...
* 最高のパフォーマンスを得るために推奨される制限は、カタログを環境あたり100万個未満にし、すべての環境で1000万個未満にすることです。
* 上限は、環境あたり1000万個、全環境で1億個です。 1環境あたり100万～1000万個の項目がある場合、[!UICONTROL カタログ検索] UIのパフォーマンスに影響が及びます。 [!DNL Target Recommendations]ただし、引き続きレコメンデーションの作成と配信を行っています。
...
* **アクティブなプロファイルスクリプト（有効なスクリプト）の推奨制限**:300
