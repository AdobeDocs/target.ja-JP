...
description: A/B テストは、ほとんどのデジタルマーケティング最適化プログラムの根幹をなすもので、マーケティング担当者が最適化され、ターゲット設定されたエクスペリエンスを訪問者および顧客に提供するのを支援します。ここでは、Adobe targetでA/Bテストを実行する際に企業が直面する最も重要な問題の9つについて説明します。 また、回避方法についても説明し、会社がテストの取り組みを通じてより優れた ROI を達成し、レポートされた A/B テストの結果の信頼性が高まります。
...
A/B テストは、ほとんどのデジタルマーケティング最適化プログラムの根幹をなすもので、マーケティング担当者が最適化され、ターゲット設定されたエクスペリエンスを訪問者および顧客に提供するのを支援します。ここでは、A/Bテストを実施する際に企業が直面する最も重要な落とし穴の10つについて説明します。 また、回避方法についても説明し、会社がテストの取り組みを通じてより優れた ROI を達成し、レポートされた A/B テストの結果の信頼性が高まります。
...
## 落とし穴5:テストの監視 {#section_EA42F8D5967B439284D863C46706A1BA}
...
## 落とし穴6:テストの途中停止 {#section_DF01A97275E44CA5859D825E0DE2F49F}
...
テスト期間中はトラフィック配分の割合を変更しないことをお勧めします。変更すると、データが標準化されるまでテスト結果が歪曲される可能性があるからです。
例えば、A/Bテストアクティビティがあり、その中でトラフィックの80%がエクスペリエンスA（コントロール）に割り当てられ、トラフィックの20%がエクスペリエンスBに割り当てられているとします。テスト期間中に、各エクスペリエンスの配分を50%に変更します。 数日後に、トラフィックの配分をエクスペリエンスBに100%に変更します。

このシナリオでは、ユーザーはどのようにしてエクスペリエンスに割り当てられますか。
...
割合を変更したり、各エクスペリエンスへの訪問者のフローに大きな影響を与えたりする場合は、新しいアクティビティを作成するか、アクティビティをコピーして、トラフィック配分の割合を編集することをお勧めします。

テスト期間中に異なるエクスペリエンスに対する割合を変更した場合、特に多くの購入者が再訪問者である場合、データが標準化されるまでに数日かかります。
別の例として、A/Bテストのトラフィック配分が50/50に分割され、その後、分割を80/20に変更した場合、変更後の最初の数日間は結果が歪んでいるように見えます。 コンバージョンまでの平均時間が長い場合、つまり、誰かが購入するのに数時間または数日かかる場合は、これらのコンバージョンの遅れがレポートに影響を与える可能性があります。 つまり、最初のエクスペリエンスでは、50%から80%に数が増え、平均コンバージョン時間は2日ですが、テストの初日は訪問者の50%からの訪問者のみがコンバージョンしています。 これにより、コンバージョン率が急落したように見えますが、これらの訪問者の80%がコンバージョンに2日かかった後で、再び標準化されます。
