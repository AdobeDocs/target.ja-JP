...
1. Visual Experience Composer（VEC）を使用するには、「**[!UICONTROL ビジュアル**]」（デフォルト）をクリックします。
   ...
   フォームベースの Experience Composer を使用する場合、「**[!UICONTROL フォーム]**」を選択します。

   この 2 つのコンポーザーについて詳しくは、「[エクスペリエンス](../../c-experiences/experiences.md#concept_A2E10F6AFB3D4AEAB6951EE14688848D)」を参照してください。
...
1. 「[エクスペリエンス](../../c-experiences/experiences.md#concept_A2E10F6AFB3D4AEAB6951EE14688848D)」の説明に従ってページ要素を変更し、「**[!UICONTROL 次へ]**」をクリックします。
...
1. 「**[!UICONTROL コンテンツを管理]**」をクリックして、使用可能な組み合わせを設定します。
   ...
   [!UICONTROL エクスペリエンス]リストには、アクティビティ用に選択されたコンテンツの各部分と、それが割り当てられている場所が表示されます。特定のエクスペリエンスにカーソルを合わせ、除外アイコンをクリックすると、そのエクスペリエンスを除外できます。複数のエクスペリエンスのチェックボックスをオンにしてから、ダイアログボックスの右上隅にある除外アイコンをクリックすることで、エクスペリエンスを一括で除外（または追加）することもできます。

   ![](assets/ap_content_batch_exclude.png)

   このリスト表示では、**ステータス**ドロップダウンリストをクリックすることでフィルターを適用し、除外済みのアクティビティまたは追加済みのアクティビティのみを表示できます。
1. 「**[!UICONTROL オファー]**」をクリックすると、コンテンツの一部を選択して [レポートグループ](../../c-reports/offer-reporting-groups-in-automated-personalization.md#concept_194128C0B56B4B26AAB57DB49892960C)に割り当てられます。
   ...
   特定のオファーにカーソルを合わせてからフォルダーアイコンをクリックすると、そのエクスペリエンスを追加できます。複数のエクスペリエンスのチェックボックスをオンにしてからダイアログボックスの右上隅にある「レポートグループ」フォルダーアイコンボタンをクリックすることで、エクスペリエンスを一括でレポートグループに追加することもできます。
...
1. （条件付き）「**[!UICONTROL オファー]**」をクリックして、コンテンツを選択してレポートグループに割り当てたり、ターゲット設定によって特定のオファーの閲覧を特定の訪問者のみに許可したりします。
   ...
   特定のオーディエンスに対するオファーのターゲット設定について詳しくは、「[Target の Automated Personalization オファー](../../c-activities/t-automated-personalization/ap-target-offers.md#task_F207ED7A41B84FD39BB6FCBFABF4B23E)」を参照してください。
1. （条件付き）「**[!UICONTROL 除外グループ]**」をクリックして、アクティビティから除外する要素の組み合わせを選択します。

   ![ステップの結果](assets/exclusion_groups.png)
...
1. アクティビティのコンテンツの設定を完了後、「**[!UICONTROL 次へ]**」をクリックします。
...
1. 「**[!UICONTROL 続行]**」をクリックし、「**[!UICONTROL 目標と設定]**」ページを表示します。
1. 次の設定を使用してアクティビティを構成し、「**[!UICONTROL 保存して閉じる**]」をクリックします。
   ...
   | コンバージョン指標 | デフォルトでは、コンバージョン指標は最適化目標指標と同じです。ただし、「最適化目標と [!UICONTROL 同じ」] オプションを選択解除して、個別のコンバージョン指標を定義できます。 |
