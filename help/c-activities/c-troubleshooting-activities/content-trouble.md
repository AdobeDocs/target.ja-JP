...
mboxDebug は、特に [!DNL Target] をページ上で設定して、Target リクエストが実行され、cookie が設定されていることを確認します。 ただし、mboxDebug は、コンテンツ配信のデバッグに役立つ詳細までは調べません。ページにアクティビティが表示されない場合や、意図していないコンテンツが表示される場合は、mboxTrace を使用してページの調査とデバッグを詳細におこないます。
...
出力には、コンテンツに関する詳細情報が表示されます。 mboxTrace は、キャンペーンまたはアクティビティとプロファイルに関する詳細を表示します。また、実行前のプロファイルのスナップショットと、実行後の変更内容のスナップショットも提供されます。 さらに、各場所で評価されたキャンペーンやアクティビティも表示します。
...
サイトの通常の機能や外見は、mboxTrace の影響を受けません。訪問者には、通常のRecommendationsデザインが表示されます。
...
詳しくは、 [Adobe Experience Cloud Debugger を使用した at.js のデバッグ](/help/c-implementing-target/c-implementing-target-for-client-side-web/c-target-debugging-atjs/target-debugging-atjs.md).
...
## 要素がAdobe Experience Managerパーソナライゼーションにも含まれていると、Target のコンテンツがちらつく、または表示されない。 {#section_9E1DABEB75AB431FB9F09887E6DD07D3}
...
この状況を修正するには、Target が実行されているページでAEMのパーソナライゼーションを無効にします。
...
## 以下を確認します。 [!DNL Target] アクティビティは、クエリー文字列パラメーターを使用して URL を正しく処理します。 {#query-strings}

この [!UICONTROL アクティビティ URL] は、訪問者をアクティビティの対象に認定し、アクティビティエクスペリエンスをユーザーに対してレンダリングするページを決定します。 アクティビティ作成中にコンテンツの完全な URL を入力すると、特にクエリー文字列パラメーターを含む URL で、そのサイトページにコンテンツが配信されるかどうかを確認するメッセージが表示されない場合があります。

デフォルトでは、 [!UICONTROL Visual Experience Composer] (VEC) [Visual Experience Composer の設定](/help/administrating-target/visual-experience-composer-set-up.md). また、アクティビティの作成中に別のページを指定することもできます。

VEC を開いた後に異なるページを表示するには、 **[!UICONTROL 設定の歯車アイコン]** > 選択 **[!UICONTROL ページ配信]** > をクリックし、目的の URL を「 [!UICONTROL アクティビティ URL] フィールドに入力します。

![ページ配信設定 UI の設定](assets/configure-page-delivery.png)

しかし、URL にクエリー文字列パラメーターが含まれている場合はどうなりますか？ パーソナライズされたコンテンツが機能し、表示されるか。 このシナリオでは、ターゲットオーディエンスに関係なく、ベース URL に加えてテンプレートルールを含め、クエリパラメーターを定義できます。

追加のテンプレートルールを含めるには、次のオプションを使用できます。

### オプション 1:URL をレプリケートし、「次を含む」オプションを使用して、テンプレートルールに保持します。

このオプションを使用すると、この URL がアクティビティの対象となりますが、ベース URL を含む URL への追加レコードによってレポートデータに影響を与える可能性があるコーナーケースが添付されていることに注意してください。

このシナリオでは、URL は `https://shopping.mycart.com?type=Summers%20Offers` と、追加のテンプレートルール「次を含む」同じ URL を OR 演算子で区切って指定します。

![テンプレートルールで URL をレプリケート](assets/option1.png)

### オプション 2:クエリ文字列のみを使用して、URL の「contains」条件を制限します。

前のオプションで説明した隅のケースはこのオプションで適用されますが、ここでは、条件付き設定はクエリ文字列にのみ制限されます。

このシナリオでは、URL は `https://shopping.mycart.com?type=Summers%20Offers` また、追加のテンプレートルール「次を含む」（OR 演算子で区切られたクエリ文字列のみ）も含まれます。

![テンプレートルールにクエリ文字列のみが含まれています](assets/option2.png)

### オプション 3:完全な URL のターゲット設定の代わりに、URL の特定の部分を利用します。

このシナリオでは、URL は `https://shopping.mycart.com?type=Summers%20Offers` および追加のテンプレートルールで、 [!UICONTROL クエリ] と [!UICONTROL type] > [!UICONTROL 等しい（大文字と小文字を区別）] > type=Summers%20Offers（OR 演算子で区切る）:
