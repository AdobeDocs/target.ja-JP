...
seo-description: Adobe Targetの多変量分析テスト（MVT）では、ページ上のエレメントのオファーの組み合わせを比較して、特定のオーディエンスに最適な組み合わせを判断し、どの要素がアクティビティの成功に最も影響するかを特定します。
seo-title: 多変量分析テスト
solution: 'Target '
title: 多変量分析テスト
topic: Premium
...
[!UICONTROL 多変量分析テスト] （MVT）では、ページ上のエレメントのオファーの組み合わせ [!DNL Adobe Target] を比較して、特定のオーディエンスに最適な組み合わせを判断し、どの要素がアクティビティの成功に最も影響するかを特定します。
...
* A/B テストを使用してページレイアウトを最適化し、その後に MVT テストを実行してページの各要素に表示する最適なコンテンツを決定します。を参照してください。

   A/B テストでは、レイアウトについての重要なフィードバックが提供されます。他方、MVT テストは、ページデザインの要素内のコンテンツのテストに優れています。複数のコンテンツオプションをテストする前に、レイアウト上でA/Bテストを実行すると、最適なレイアウトと最も効果的なコンテンツを決定するのに役立ちます。

* MVT テストを使用してどの要素が最も重要かを判断し、その後その要素に絞って A/B テストを実行します。

   異なるエクスペリエンスの数が5を超えていて、2つ以上の要素にまたがる場合は、A/Bテストを実行する前にMVTテストを検討することをお勧めします。MVTテストは、ページ上のどの領域がコンバージョンを向上させる可能性が最も高いかを示します。マーケティング担当者は、これらの要素に重点を置いてテストをおこなうことができます。例えば、MVT テストによって、目標を達成するための最も重要な要素がコールトゥアクションであることがわかります。目標を達成するために最も有用な要素およびコンテンツを特定したら、A/B テストを実行して、さらに結果を絞り込むことができます。例えば、2 つの異なる画像を比較してテストしたり、コールトゥアクションの言葉遣いや色を比較したりできます。MVT テストの後に 1 つ以上の A/B テストをおこなうことで、目的の結果を得るための最良のコンテンツを判断できます。
