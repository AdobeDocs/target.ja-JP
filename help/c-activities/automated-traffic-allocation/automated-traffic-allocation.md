...
* 2つ以上のエクスペリエンスをテストする場合、自動配分は、1つのエクスペリエンスが他のすべてのエクスペリエンスを大幅にアウトパフォーマンスした場合に、固定範囲テスト（サンプルサイズ計算ツールによって提案される時間枠）よりも速く勝者を見つけます。 2つ以上のエクスペリエンスが他のエクスペリエンスと「勝者」であるが、互いに密接に一致する場合、自動配分では、どちらが優れているかを判断するのに余分な時間が必要になる場合があります。 この場合、「勝者」エクスペリエンスがパフォーマンスの低いエクスペリエンスよりも優れていたが、どちらが優れているかは特定していないという結論に達すると、通常、固定範囲テストは終了します。
