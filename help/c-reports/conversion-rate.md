---
keywords: ターゲット設定
description: Adobe [!DNL Target] が、各エクスペリエンスのコンバージョン率、上昇率、信頼性（統計の有意性）および信頼区間を表示および計算する方法を説明します。
title: コンバージョン率、上昇率および信頼水準はどのように表示できますか。
feature: レポート
...
発注(`orderConfirmPage`)mboxを挿入し、コンバージョンmboxとして選択すると、AOV、RPV、販売データが各エクスペリエンスに表示されます。
...
これらの計算指標を計算するには、Targetの[Complete Confidence Calculator](/help/assets/complete_confidence_calculator.xlsx) Excelファイルをダウンロードしてアクティビティの値を入力するか、Targetの](/help/assets/statistical-calculations.pdf)で使用する[統計指標を確認します。
...
## [!DNL Target]がスチューデントのt検定の使用を推奨する理由 {#t-test}

A/Bテストは、対照バリアント（エクスペリエンスとも呼ばれます）内の一部のビジネス指標の平均値と、1つ以上の代替エクスペリエンス内の同じ指標の平均値を比較する実験です。

[!DNL Target] では、z検定などの代替手段よりも前提条件が少なく、対照エクスペリエンスと代替エクスペリエンスとの（定量的な）ビジネス指標の一対比較をおこなうのに適した統計テストなので、スチューデントのt検定例を2つ使用することをお勧めします [](https://en.wikipedia.org/wiki/Student%27s_t-test#:~:text=The%20t%2Dtest%20is%20any,the%20test%20statistic%20were%20known.)。

### 詳細

オンラインA/Bテストを実行する場合、各ユーザー/訪問者が1つのバリアントにランダムに割り当てられます。 その後、関心のあるビジネス指標（コンバージョン、注文、売上高など）を測定します。 各バリアントの訪問者数。 次に使用する統計テストでは、平均ビジネス指標（コンバージョン率、ユーザーあたりの注文件数、ユーザーあたりの売上高など）が は、コントロールと指定された代替バリアントに等しい値です。

ビジネス指標自体は任意の分布に従って分布する場合がありますが、（各バリアント内の）この指標の平均の分布は、[中心限定定理](https://en.wikipedia.org/wiki/Central_limit_theorem)を介して正常な分布に収束する必要があります。 この平均のサンプリング分布がどの程度速く通常に収束するかは保証されませんが、通常、オンラインテストで訪問者の規模を考慮すると、この条件が達成されます。

この平均の正規性を考えると、データからの推定（手段差の標準誤差）に基づくスケーリング項に対する正規分布値（ビジネス指標の手段差）の比であるので、使用する検査統計量がt分布に従って示すことができる。 **スチューデントのt検定**&#x200B;は、t分布に従う検定統計量に従う場合に、適切な仮説検定です。

### 他のテストが使用されない理由

一般的なA/Bテストシナリオでは、テスト統計の分母が既知の分散から派生したものではなく、データから推定する必要があるので、**z検定**&#x200B;は不適切です。

**カイ2乗テスト** は、2つのバリアント間に質的な関係があるかどうかを判断するのに適しているので（つまり、バリアント間に違いがないというヌル仮説）、使用されません。T検定は、_定量的_&#x200B;比較指標のシナリオに適しています。

**Mann-Whitney Uテスト**&#x200B;は、（各バリアントに対する）平均ビジネス指標のサンプリング分布が正常に分布しない場合に適した、非パラメトリックテストです。 しかし、前述したように、オンライン試験に関わるトラフィックの大きさを考えると、中心限定定理は通常適用されるので、t検定は安全に適用できます。

**ANOVA**（t検定を2つ以上のバリアントに一般化する）のようなより複雑なメソッドは、テストにエクスペリエンスが2つ以上ある場合に適用できます（「A/Bnテスト」）。 しかし、ANOVAは、「すべてのバリアントが同じ平均を持つかどうか」という問いに対して答えます。一方、一般的なA/Bnテストでは、_どの特定のバリアント_&#x200B;が最適かに関心があります。 そのため、[!DNL Target]では、各バリアントを1つのコントロールと比較する通常のt検定を適用し、複数の比較を考慮してボンフェローニ補正を適用します。
