...
デフォルト（コントロール）エクスペリエンスを含め、アクティビティのすべてのエクスペリエンスにリダイレクトオファーを使用する場合は、すべてのエクスペリエンスに同じ条件を設定することができます。例えば、デフォルトのエクスペリエンスにリダイレクトオファーがないが、他のエクスペリエンスにリダイレクトオファーがある場合、リダイレクトオファーのないエクスペリエンスの速度は固有のメリットになります。テストなどの一時的なシナリオのみにリダイレクトオファーが推奨されます。パーソナライゼーションなどの永続的なシナリオでは、リダイレクトオファーはお勧めしません。「勝者」を決定したら、リダイレクトを削除して、ページ読み込みパフォーマンスを向上させる必要があります。
