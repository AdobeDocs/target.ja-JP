...
at.jsバージョン1.6.3以降を使用する場合、これは問題ではありません。 この競合状態は、以前のバージョンを使用しているお客様にのみ影響します。 Targetチームは、2つのバージョンのat.jsを管理しています。現在のバージョンと2番目の最新バージョン。 Upgrade at.js as necessary to ensure that you are running a [supported version](/help/c-implementing-target/c-implementing-target-for-client-side-web/target-atjs-versions.md).

at.jsのサポートされていない以前のバージョンを使用している場合は、競合条件が発生し、リダイレクトが最初のページで実行される前にAnalytics呼び出しが実行される可能性があります。 その場合は元のページとリダイレクトページでページビュー数がカウントされます。こうしたケースでは、訪問者が最初のページを実際に閲覧しなくても、そのページでページビューが余分にカウントされます。
...
デフォルトの（コントロール）エクスペリエンスを含む、アクティビティのすべてのエクスペリエンスでリダイレクトオファーを使用する理由の1つは、すべてのエクスペリエンスに同じ条件を設定することです。 例えば、デフォルトのエクスペリエンスにリダイレクトオファーがなく、他のエクスペリエンスにリダイレクトオファーがある場合、リダイレクトオファーを使用しないエクスペリエンスの速度には利点があります。 リダイレクトオファーは、テストなどの一時的なシナリオにのみ推奨されます。 リダイレクトオファーは、パーソナライゼーションなどの恒久的なシナリオには推奨されません。 「推奨結果」を決定したら、リダイレクトを削除してページ読み込みパフォーマンスを向上させる必要があります。
