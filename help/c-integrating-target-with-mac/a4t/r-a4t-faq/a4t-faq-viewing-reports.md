...
訪問者がA4Tアクティビティでコンバージョンに適格な場合、表示コンテンツまたはコンバートを行う場合、はイベントデータをに [!DNL Target] 送信します。これにより、ページ上で発生しているコンバージョンイベントや他のクリックストリームイベント [!DNL Analytics]を、関連する [!DNL Analytics][!DNL Target] アクティビティやエクスペリエンスに属性付けできます。

レポートを表示する際は、次の点に注意してくだ [!DNL Analytics] さい。

* 一般に、レポートーウィンドウは常にアクティビティーの開始日から開始する必要があります。
* レポートのウィンドウの外部でコンバージョンが発生した場合、そのコンバージョンはに表示されません [!DNL Analytics]。
* 例えば、 [!UICONTROL 自動ターゲット][!DNL Target]アクティビティのトラフィックの「ターゲット」部分で、訪問者にはセッション間で異なるエクスペリエンスが表示される場合があります。プロファイルやコンテキストが変更され、の機械学習アルゴリズムによって、新しいエクスペリエンスにコンバージョンする可能性が高くなります。 これは、通常のA/Bテストアクティビティとは異なり、エクスペリエンスは訪問間で1人の訪問者に共通です。
* 訪問者が複数の訪問で複数のエクスペリエンスを閲覧した場合、すべてのコンバージョンは常に訪問者が最後に閲覧したエクスペリエンスに関連付けられます。ただし、訪問回数は、訪問者が閲覧したエクスペリエンスごとに増分されます。 これにより、[!UICONTROL レポートの「ターゲット設定][!DNL Adobe Analytics] 」ディメンションでエクスペリエンスを表示する際に、エクスペリエンスごとのコンバージョン率が人為的に抑制されます。
