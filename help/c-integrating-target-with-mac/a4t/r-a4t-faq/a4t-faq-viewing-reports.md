...
レポートソースとして[!DNL Analytics]を使用するアクティビティ(A4T)の場合、目標指標では常に「[!UICONTROL カウントを増分、アクティビティでユーザーを保持]」および「[!UICONTROL すべてのインプレッション]」の設定が使用されます。 これは&#x200B;*設定できません*。
...
* ユーザーがアクティビティの資格を得て、コンテンツが[!DNL [!DNL Target]]から返された場合、上記の指標トリガーはすべて同じです。 したがって、必ずしもそのユーザーがオファーを見たかどうかはわかりません。アクティビティエクスペリエンスがスクロールしないと見えない場所にある場合、[!DNL Target] によってオファーが配信されていても、ユーザーがページを下にスクロールしなければ、オファーは見られていないことになります。
