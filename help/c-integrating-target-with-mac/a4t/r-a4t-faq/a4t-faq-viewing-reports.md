...
そのアクティビティで分類が行われたが、レポートに「未指定」の行が表示される場合は、ターゲット以外の指標を使用してデータを表示していないことを確認します。 レポートでターゲット固有の指標を使用していない限り、「未指定」の行には、ターゲットに関連付けられていない呼び出しのイベントが含まれます。
それぞれ、その行にはターゲット関連の情報(例：訪問者なし/訪問回数/インプレッション数)が含まれません。
