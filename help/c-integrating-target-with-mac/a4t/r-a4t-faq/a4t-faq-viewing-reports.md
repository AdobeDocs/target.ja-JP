...
## Analysis Workspace で [!DNL Target] アクティビティデータを表示できますか？ {#workspace}
...
詳細と例については、Adobe Experience League が提供する [Analytics &amp; Target：分析のベストプラクティスチュートリアル](https://spark.adobe.com/page/Lo3Spm4oBOvwF/)をご覧ください。
...
詳しくは、「目標指標を設定する際に、詳細設定オプションにアクセスできないのはなぜですか？」（[指標の定義 - A4T FAQ](/help/c-integrating-target-with-mac/a4t/r-a4t-faq/a4t-faq-metric-definition.md)）を参照してください。
...
## Analysis Workspace の「アクティビティのインプレッション」と「アクティビティのコンバージョン」が Reports &amp; Analytics よりも高いのはなぜですか？ {#sametouch}
...
## アクティビティの設定中に Analytics 指標が選択されると、「アクティビティのコンバージョン」はどうなりますか？ {#section_F3EBACF85AF846E9B366A549AAB64356}
...
例として、A4T 変数が 90 日後に期限切れになり、テストが 1月1日から 1月15日までアクティブである場合を見てみましょう。
...
ユーザーが 2月1日にサイトに戻り、さらに 5 ページを表示しました。追加の Target アクティビティは発生しませんでした。元のアクティビティは既にアクティブでなくなっています。アクティビティはアクティブではありませんが、eVar の持続性により、まだユーザーをフォローしています。現在のデータは、以下のようになります。
...
エクスペリエンスは、両方ともコンバージョン前に表示されているので、両方とも注文の「クレジット」を受け取っています。しかし、システムでの実際の注文は 1 回のみなので、合計にはそれが反映されています。[!DNL Target] レポートの場合、[!DNL Target] の 2 つのアクティビティを比較して優劣を判定しようとしているわけではないので、ユーザーに表示されるすべてのアクティビティがクレジットを獲得していても問題ありません。ここで比較しているのは、1 つのアクティビティ内の 2 つの項目の結果です。1 人のユーザーは同じアクティビティの中で異なるエクスペリエンスを得ることはできないので、注文クレジットのクロスコンタミネーション（交差汚染）を心配する必要はありません。
...
## Analytics と Analytics for Adobe Target（A4T）でユニーク訪問者数指標の計算方法が違うのはなぜですか？ {#section_0C3B648AB54041F9A2AA839D51791883}

スチューデントの t 検定（信頼感指標）を使用してテストの勝者を選択する A/B テストを実行する場合、前提の 1 つは、固定された対象期間があるということです。その固定サンプルサイズ（対象期間）を考慮しない限り、テストは統計的に有効になりません。

[!UICONTROL ユニーク訪問者]指標が [!DNL Analytics] と [!DNL Target] で異なるのは、実際のテストよりも短い時間を対象として計算した場合のみです。正しいサンプルサイズ（対象期間）でない場合、テストは信頼できるものにはなりません。詳しくは、[Evan Miller の web サイト](https://www.evanmiller.org/index.html)の [How Not to Run an A/B Test](https://www.evanmiller.org/how-not-to-run-an-ab-test.html)（英語）を参照してください。
...
## Analytics において、同一の訪問者が複数のエクスペリエンスでカウントされる場合があるのはなぜですか？ {#section_1397E972D31C4207A142E4D2D6D794A2}
...
* 訪問者が `mbox3rdPartyId` を使用している場合、その匿名の訪問者がサードパーティの ID プロファイルと統合されると、[!DNL Target] はサードパーティの ID と照合するために、訪問者に別のエクスペリエンスを割り当てることがあります。詳しくは、[mbox3rdPartyID のリアルタイムプロファイル同期](/help/c-target/c-visitor-profile/3rd-party-id.md#concept_BF4113593F614987B1D3E359AE1C5732)を参照してください。
...
仮想レポートスイートは、[!UICONTROL レポートスイート]リストに含まれていません。ただし、[!DNL Analytics] の仮想レポートスイートにリンクされたレポートスイートと共有されている A4T データにはアクセスできます。仮想レポートスイートから作成されたオーディエンスは、[!DNL Target] に戻って共有することはできません。
...
## Analytics では訪問はどのようにカウントされますか？ A4T を使用する自動ターゲットアクティビティでは、コンバージョンクレジットはどのように割り当てられますか？
